...
The unit of work is the feature, the sprint, or the overnight optimization — not a single chat. Runs survive terminal close, context exhaustion, and laptop sleep. Parallel sprints fan out across isolated worktrees and converge without you.

- **`ll-parallel`** — kick off N concurrent feature implementations in isolated worktrees. Walk away; they converge.
- **`--background` + `ll-loop resume`** — runs survive terminal close, sleep, and reboot. Resume picks up mid-trajectory, exactly where it stopped.
- **`harness-optimize`** — score-gated overnight optimization. Accept-or-revert each mutation; if interrupted, resume to the highest-scoring commit.
...
- **36 typed CLI tools** (`ll-issues`, `ll-sprint`, `ll-loop`, `ll-parallel`, `ll-action`, …) — structural work runs as commands, not improvised tool calls.
...
**The harness grades:**
- Thirteen layered gate types (exit code through full agentic simulation), cheapest first — failures route back to execution, not forward.
...
- **81 FSM loops** — recurring automation workflows (backlog triage, sprint building, eval harnesses)
...
- [Command Reference](docs/reference/COMMANDS.md) — every `/ll:*` slash command
- [CLI Reference](docs/reference/CLI.md) — every `ll-*` command-line tool
- [Configuration Reference](docs/reference/CONFIGURATION.md) — all options, variable substitution, overrides
- [Loops Guide](docs/guides/LOOPS_GUIDE.md) — FSM YAML authoring, gate types, loop patterns, practical examples
