...
- **33 typed CLI tools** (`ll-issues`, `ll-sprint`, `ll-loop`, `ll-parallel`, `ll-action`, etc.) — structural work runs as commands, not improvised tool calls
...
- **64 skills** — deterministic harnesses for common workflows (confidence checks, issue wiring, loop creation)
- **33 CLI tools** — `ll-auto`, `ll-parallel`, `ll-sprint`, `ll-loop`, `ll-action`, and more
- **87 FSM loops** — recurring automation workflows (backlog triage, sprint building, eval harnesses)
