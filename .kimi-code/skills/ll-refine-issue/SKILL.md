...
`$TRIAGE` is a three-key object; each value has `covered` (bool), `evidence` (string), and `reason` (string or `null`, ENH-2990 — a discriminating code for *why* an uncovered axis is uncovered; not needed to decide whether to spawn):

```json
{
  "locator":        {"covered": true,  "evidence": "Integration Map → commands/refine-issue.md", "reason": null},
  "analyzer":       {"covered": false, "evidence": "", "reason": "no_qualified_refs"},
  "pattern_finder": {"covered": false, "evidence": "", "reason": "no_qualified_refs"}
}
```

**Spawn exactly one Task per axis whose `covered` is `false`** — Agent 1 for `locator`, Agent 2 for `analyzer`, Agent 3 for `pattern_finder`. Spawn them in a SINGLE message with multiple Task tool calls, each with `run_in_background: false`, and wait for their results in this same turn before proceeding. Do not spawn an agent for a covered axis, and do not substitute your own judgment for the triage verdict: it is computed from resolving file references and their change times, which you cannot check as cheaply or as reliably by reading.
...
(`little_loops.issues.program_design:program_design_gate_active`),
...
3. [Verification — e.g., "`pytest tests/test_parser.py -v` passes"]
```
...
Use the H2 extraction pattern from `little_loops.issue_history.doc_synthesis:_extract_section()`:
```python
...
Adopt the `"critical"/"high"/"medium"/"low"` priority model from `little_loops.issue_history.models:Gap`:
...
**Gap-analysis and max_refine_count**: Gap-analysis runs (`--gap-analysis`) do NOT count against `max_refine_count` — they are additive-only, non-destructive, and designed for repeated iterative use. Only full-rewrite passes (`--full-rewrite` or the default non-flag mode) consume the refinement budget. This exemption is implemented via a discriminated Session Log command string, `GAP_REFINE_COMMAND` (`/ll:refine-issue:gap-analysis`, defined in `little_loops.session_log`) — see Step 6.5.
...
`program_design_gate_active()` (`little_loops.issues.program_design`)
derives arming from the most recent `/ll:refine-issue` Session Log entry
(`issue_design_timestamp()`, same module). Checking the gate before this append
...
On a `--gap-analysis` run, write the discriminated `GAP_REFINE_COMMAND` string
(`/ll:refine-issue:gap-analysis`) so it is exempt from `max_refine_count`; on a
full-rewrite/default run, write the bare `/ll:refine-issue` string:

```bash
if [[ "$GAP_ANALYSIS" == "true" ]]; then
  ll-issues append-log <path-to-issue-file> /ll:refine-issue:gap-analysis
else
  ll-issues append-log <path-to-issue-file> /ll:refine-issue
fi
```

If `ll-issues` is not available, fall back to manually appending with **exactly** this format (backticks required) — use the same `/ll:refine-issue:gap-analysis` vs. `/ll:refine-issue` command string based on `$GAP_ANALYSIS`:
