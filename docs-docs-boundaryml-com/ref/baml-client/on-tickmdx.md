---
url: https://docs.boundaryml.com/ref/baml_client/on-tick.mdx
scraped_at: 2026-03-06T01:00:42.958764
filepath: docs-docs-boundaryml-com/ref/baml-client/on-tickmdx.md
---
...
Track the progress of long-running BAML function calls:```python
    from baml_client import b
    from baml_py import baml_py
...
Extract intermediate "thinking" content from streaming LLM responses:```python
    import json
    from baml_client import b
    from baml_py import baml_py
...
Use onTick for comprehensive debugging and logging:```python
    from baml_client import b
    from baml_py import baml_py
...
OnTick can be used alongside [Collectors](/ref/baml_client/collector) for comprehensive logging:```python
    from baml_client import b
...
OnTick callbacks should handle errors gracefully. If an onTick callback throws an error, the function execution will continue:```python
    from baml_client import b
    from baml_py import baml_py
    ...
    ```## LimitationsKeep these limitations in mind when using onTick:1. **Synchronous Functions**: OnTick is **not supported** for synchronous function calls. Attempting to use onTick with sync functions will throw an error.

2. **Error Isolation**: Errors in onTick callbacks do not stop function execution, but they may not be explicitly surfaced.

## API Reference

### OnTick Callback Signature```python
...
OnTick is passed via the `baml_options` parameter (Python) or options object (TypeScript/Go):```python
    # Async function call
