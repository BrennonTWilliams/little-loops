---
url: https://docs.boundaryml.com/ref/baml_client/collector.mdx
scraped_at: 2026-03-06T01:00:41.099545
filepath: docs-docs-boundaryml-com/ref/baml-client/collectormdx.md
---
...
You can attach custom metadata to function calls using tags. These can come from a parent `trace` context or be specified per call.```python
    from baml_client import b
...
When using streaming, you can access the raw Server-Sent Events (SSE) responses received from the LLM provider. This is useful for debugging, logging, or accessing provider-specific data not exposed in the standard response.```python
    from baml_client import b
    from baml_py import Collector
...
| `json()`        | `object \| null` | Parses and returns the text as JSON. Returns null if the text is not valid JSON. |The SSE event type (e.g., "message\_delta", "content\_block\_delta") and event ID are not currently exposed. Only the data payload is available via `text` and `json()`.## Related Topics
