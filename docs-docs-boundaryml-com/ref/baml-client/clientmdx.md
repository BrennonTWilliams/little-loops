---
url: https://docs.boundaryml.com/ref/baml_client/client.mdx
scraped_at: 2026-03-06T01:00:41.093016
filepath: docs-docs-boundaryml-com/ref/baml-client/clientmdx.md
---
...
your code. Here's an example using the async client:```python
    ...
    ``````rust
    use myproject::baml_client::async_client::B;

    async fn example() -> Result<(), baml_client::Error> {
        // Call your BAML functions.
        let story = B.TellMeAStory.call().await?;
        let poem = B.WriteAPoemAbout.call("Roses").await?;
        Ok(())
    }
    ```The sync client is exactly the same but it doesn't need an async runtime,
instead it just blocks.```python
...
The `.stream` object is used to stream the response from a function.```python
    from baml_client.async_client import b
    ...
    fn example() -> Result<(), baml_client::Error> {
        let mut stream = B.TellMeAStory.stream()?;
    ...
    ```### `.request`This feature was added in: v0.79.0The `.request` object returns the raw HTTP request but it **does not** send it.
...
required format in order to send them to the LLM.```python
    from baml_client.async_client import b
    ...
    ```### `.stream_request`This feature was added in: v0.79.0Same as [`.request`](#request) but sets the streaming options to `true`.```python
    from baml_client.async_client import b
    ...
    ``````rust
    // Rust modular API coming soon!
    ```### `.parse`This feature was added in: v0.79.0The `.parse` object is used to parse the response returned by the LLM after
the function call. Can be used in combination with [`.request`](#request).```python
    import requests
    ...
    ```### `.parse_stream`This feature was added in: v0.79.0Same as [`.parse`](#parse) but for streaming responses. Can be used in
combination with [`.stream_request`](#stream_request).```python
