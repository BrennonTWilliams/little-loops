---
url: https://docs.boundaryml.com/ref/baml_client/config.mdx
scraped_at: 2026-03-06T01:00:41.616409
filepath: docs-docs-boundaryml-com/ref/baml-client/configmdx.md
---
...
Setting can also be modified via functions in `baml_client.config`.```python
    ...
    ### reset\_baml\_env\_vars`reset_baml_env_vars` is deprecated and is safe to remove, since environment variables are now lazily loaded on each function callResets the environment variables to the values in the provided dictionary.
    ...
    ```typescript
    resetBamlEnvVars(env: Record): void;
    ``````rust
