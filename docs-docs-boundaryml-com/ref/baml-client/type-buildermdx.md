---
url: https://docs.boundaryml.com/ref/baml_client/type-builder.mdx
scraped_at: 2026-03-06T01:00:44.239974
filepath: docs-docs-boundaryml-com/ref/baml-client/type-buildermdx.md
---
...
To modify an existing BAML type, mark it with `@@dynamic`:```baml
  class User {
    name string
    age int
...
You can also create entirely new types at runtime:```python
    tb = TypeBuilder()
...
You can add descriptions to properties and enum values to help guide the LLM:```python
    tb = TypeBuilder()
...
1. **Dynamic Categories**: When categories come from a database or external source```python Python
  ...
  ```2. **Form Fields**: When extracting dynamic form fields```python Python
  ...
  ```3. **Optional Properties**: When some fields might not be present```python Python
  tb = TypeBuilder()
  ...
  tb = Baml::TypeBuilder.new
  tb.User.add_property("middle_name", tb.string.optional)
  ```All types added through TypeBuilder must be connected to the return type of your BAML function. Standalone types that aren't referenced won't affect the output schema.## Testing Dynamic Types
