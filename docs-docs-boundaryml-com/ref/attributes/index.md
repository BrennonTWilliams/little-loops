---
url: https://docs.boundaryml.com/ref/attributes/
scraped_at: 2026-03-06T01:00:34.585781
filepath: docs-docs-boundaryml-com/ref/attributes/index.md
---
...
Field-Level AttributesExamples of Field-Level AttributesBlock-Level AttributesExamples of Block-Level AttributesKey Differences

[Field-Level Attributes](#field-level-attributes)
[Examples of Field-Level Attributes](#examples-of-field-level-attributes)
[Block-Level Attributes](#block-level-attributes)
[Examples of Block-Level Attributes](#examples-of-block-level-attributes)
...
@alias: Renames a field for better understanding by the LLM.@description: Provides additional context to a field.@skip: Excludes a field from prompts or parsing.@assert: Applies strict validation to a field.@check: Adds non-exception-raising validation to a field.
...
```
1class MyClass {2property1 string @alias("name") @description("The name of the object")3age int? @check(positive, {{ this > 0 }})4}
```
...
```
1class MyClass {2property1 string3property2 int?45@@dynamic // allows adding fields dynamically at runtime6}
```
...
Scope: Field-level attributes affect individual fields, while block-level attributes affect the entire class or enum.Usage: Field-level attributes are used for specific field modifications, whereas block-level attributes are used for broader modifications affecting the whole block.
