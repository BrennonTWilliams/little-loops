---
url: https://docs.boundaryml.com/guide/framework-integration/react-next-js/quick-start.mdx
scraped_at: 2026-03-06T01:00:29.387161
filepath: docs-docs-boundaryml-com/guide/framework-integration/react-next-js/quick-startmdx.md
---
...
This guide walks you through setting up BAML with React/Next.js, leveraging Server Actions and React Server Components for optimal performance.**Requirements:** This integration requires **Next.js 15 or higher**.## Example Usage

BAML automatically generates a server action and React hook for your BAML functions, with built-in support for both streaming and non-streaming modes. For details on the generated hooks, see [Generated Hooks](/ref/baml_client/react-next-js/use-function-name-hook).```baml title="baml_src/prompt.baml"
  class Story {
  ...
  export function StoryForm() {
    ...
    return (writeMeAStory.mutate("About a cat in a hat")}
          disabled={writeMeAStory.isLoading}>
          {writeMeAStory.isLoading ? 'Generating...' : 'Generate Story'}{writeMeAStory.data?.title}{writeMeAStory.data?.content}{writeMeAStory.error &&Error: {writeMeAStory.error.message}});
  }
  ```## Quick Start

Follow the step-by-step instructions below to set up BAML in a new or existing Next.js project.### Create a New Next.js Project

  First, create a new Next.js project with the App Router:```bash npm
  ...
  * Configure other options as needed for your project
  ...
  Next, install BAML and its dependencies:```bash npm
  ...
  Update your `next.config.mjs`:```typescript title="next.config.ts" {1,8}
  ...
  Create a new BAML project in your Next.js application:```bash npm
    npx baml-cli init
    ```
  ...
  BAML automatically generates type-safe Next.js server actions and React hooks for your BAML functions.```baml title="baml_src/prompt.baml"
    class Story {
      title string @stream.not_null
      content string @stream.not_null
    }
    ...
    export function StoryForm() {
      ...
      return (writeMeAStory.mutate("About a cat in a hat")}>
            {writeMeAStory.isLoading ? 'Generating...' : 'Generate Story'}{writeMeAStory.data && ({writeMeAStory.data.title}{writeMeAStory.data.content})}

          {writeMeAStory.error &&Error: {writeMeAStory.error.message}});
    }
    ```
...
#### {writeMeAStory.data?.title}


{writeMeAStory.data?.content}
