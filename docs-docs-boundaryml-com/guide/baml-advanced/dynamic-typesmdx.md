---
url: https://docs.boundaryml.com/guide/baml-advanced/dynamic-types.mdx
scraped_at: 2026-03-06T01:00:21.208147
filepath: docs-docs-boundaryml-com/guide/baml-advanced/dynamic-typesmdx.md
---
...
2. Import the `TypeBuilder` from baml\_client in your runtime code and modify `Category`. All dynamic types you
   ...
   read from when building the output schema in the prompt.```python
    ...
    [openapi-feedback-github-issue]: https://github.com/BoundaryML/baml/issues/892### Dynamic BAML Classes
...
We can then modify the `User` schema at runtime. Since we marked `User` with `@@dynamic`, it'll be available as a property of `TypeBuilder`.```python
    from baml_client.type_builder import TypeBuilder
    from baml_client import b
...
You can modify the set of tools that can be used in the `ChatResponse` type at runtime like this:```python
    tb = TypeBuilder()
...
Note that you must attach the new types to the existing Return Type of your BAML function(in this case it's `User`).```python Python
    from baml_client.type_builder import TypeBuilder
...
### Adding descriptions to dynamic types```python
    tb = TypeBuilder()
...
The `TypeBuilder` has a higher level API `add_baml` to do this:```python Python
    tb = TypeBuilder()
    tb.add_baml("""
