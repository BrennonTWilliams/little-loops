---
url: https://docs.boundaryml.com/guide/introduction/why-baml.mdx
scraped_at: 2026-03-06T01:00:33.660165
filepath: docs-docs-boundaryml-com/guide/introduction/why-bamlmdx.md
---
...
But first, let's see where we're going with this story:*BAML: What it is and how it helps - see the full developer experience*
...
Real resumes break your extraction. The LLM returns malformed JSON:```json
{
  "name": "John Doe",
...
Look what you get immediately:*BAML playground showing successful resume extraction with clear prompts and structured output*

### 1. **Instant Testing**

Test in VSCode playground without API calls or token costs:* **See the exact prompt** that will be sent to the LLM
* **Test with real data instantly** - no API calls needed
* **Save test cases** for regression testing
* **Visual prompt preview** shows token usage and formatting*Build up a library of test cases that run instantly*
...
Instead of rejecting imperfect outputs, SAP actively transforms them to match your schema using custom edit distance algorithms.**SAP vs Other Approaches:**
    ...
    **Key insight:** SAP + GPT-3.5 turbo beats GPT-4o + structured outputs, saving you money while improving accuracy.**What SAP fixes automatically:**
    ...
    * Handles incomplete JSON sequences**Traditional JSON Schema (verbose):**
    ...
    **80% token reduction** while being clearer to the model!**Traditional approach** - Choose reasoning OR structure:
    ...
    **Result:** Chain-of-thought reasoning AND structured output in a single API call.### 4. **Production Features Built-In**
...
### 6. **Type Safety Everywhere**```python
...
**See how changes instantly update the prompt:***Change your types → Prompt automatically updates → See the difference immediately*
...
* **UI state management** - Know exactly what's loading vs completeYour browser does not support the video tag.*See semantic streaming in action - structured data streaming with loading states*
