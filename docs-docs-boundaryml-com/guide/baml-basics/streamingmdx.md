---
url: https://docs.boundaryml.com/guide/baml-basics/streaming.mdx
scraped_at: 2026-03-06T01:00:23.271482
filepath: docs-docs-boundaryml-com/guide/baml-basics/streamingmdx.md
---
...
it into a series of semantically valid partial objects.You can check out more examples (including streaming in FastAPI and NextJS) in the 

  [BAML Examples]

   repo.[call BAML functions]: /docs/calling-baml/calling-functions

[BAML Examples]: https://github.com/BoundaryML/baml-examples/tree/main

Let's stream the output of this function `function ExtractReceiptInfo(email: string) -> ReceiptInfo` for our example:```rust
  class ReceiptItem {
    name string
    description string?
    quantity int
...
received so far.BAML will generate `b.stream.ExtractReceiptInfo()` for you, which you can use like so:
    ...
    ```When using `baml-cli serve`, streaming is available via `http://localhost:2024/stream/{FunctionName}`.
      ...
      partial type definitions for JSON schema yet.Number fields are always streamed in only when the LLM completes them. E.g. if
  the final number is 129.95, you'll only see null or 129.95 instead of partial
  numbers like 1, 12, 129.9, etc.## Cancelling Streams

You can cancel ongoing streams using abort controllers, which is essential for responsive applications that allow users to stop generation or implement timeouts.```typescript
    import { b } from './baml_client'
...
Allow users to stop streaming generation with a "Stop" button:```tsx
    function StreamingComponent() {
      ...
      return (Start StreamingStop{result})
    }
    ``````python
...
annotating the union is sufficient.You can also achieve the same behavior by adding `@@stream.done` to each
  class in the union. The `(T @stream.done)[]` syntax is more concise when
  the classes are used in other contexts where you don't want `@@stream.done`.### `@stream.not_null`
...
This will generate the following code in the `partial_types` module:```python
    ...
    interface BlogPost {
      title: StreamStatecontent: StreamState}
...
| `T @stream.with_state`            | `StreamState[Partial[T]?]`        | Includes streaming state metadata         |The return type of a function is not affected by streaming attributes!## Putting it all together
