...
completed_at: '2026-08-30T01:41:34Z'
...
- **Correction (remediation pass)**: `search.py:279`'s `refinement_commands` (`search.py:272-278`) is a plain `set` literal (curly braces, no `frozenset(...)` call) — not a `frozenset` as stated elsewhere in this issue's Integration Map, Program Design → Signatures, and Decision Rules. The word "frozenset" does not appear in `search.py`. Membership-test semantics (and this issue's "needs no code change" conclusion for that call site) are unaffected by `set` vs `frozenset` — this is a citation correction only.
- **Correction (remediation pass)**: `refine_status.py`'s `_CANONICAL_CMD_ORDER`/`_CMD_ALIASES` span is `39-59`, not `38-58` as cited elsewhere in this issue. Line 38 is the preceding comment (`# Canonical workflow order for command columns`); `_CANONICAL_CMD_ORDER = [` starts at 39; `_CMD_ALIASES`'s closing `}` is at 59 (58 is its last entry, `"/ll:map-dependencies": "map",`).
- **Correction (remediation pass)**: `recursive-refine.yaml`'s per-run attempts counter increments via `f.write(issue_id + '\n')` at line **202**, not line 201 as cited elsewhere in this issue's Codebase Research Findings. Line 201 is the preceding `with open(attempts_file, 'a') as f:` statement the write executes under. This is a citation correction only — the "confirmed unaffected, needs no code change" conclusion is unchanged.
- **Two more production callers silently burn lifetime budget today, beyond `refine-to-ready-issue.yaml`'s `refine_followup`** — same exposure this bug describes, not currently named anywhere in this issue: `scripts/little_loops/loops/autodev.yaml`'s `run_refine` state (header `:821`, action `:830` — `/ll:refine-issue ${captured.input.output} --auto --gap-analysis`; comment at `:822-828` claims "does not consume refine budget") and `refine_for_design` state (header `:1903`, action `:1914`; comment at `:1908-1910` claims "does not consume max_refine_count"); and `scripts/little_loops/loops/rn-remediate.yaml`'s `refine_followup` state (header `:672`, action `:677`; routed to from `re_assess`'s `on_no: refine_followup` at `:729`, with a second explanatory comment at `:726-728`). All three should be named as callers whose behavior changes and are regression-check candidates once the fix lands, the same way `refine-to-ready-issue.yaml`'s `refine_followup` already is.
- **A second, unconditional call site writes the same undifferentiated Session Log entry** — `commands/refine-issue.md` Step 6.5 ("### 6.5. Append Session Log") has two places that write the literal command string: the primary `ll-issues append-log <path> /ll:refine-issue` bash invocation at line 1003, and a separate manual-fallback block immediately below it (line 1006: "If `ll-issues` is not available, fall back to manually appending with **exactly** this format") that hardcodes `` - `/ll:refine-issue` - YYYY-MM-DDTHH:MM:SS - ... `` at line 1009 with no conditional on `--gap-analysis` mode. Step 6.5 is reached on both the gap-analysis and full-rewrite paths, so this fallback text is a second, reachable call site for the discriminator fix that this issue's Implementation Steps do not currently name.
- **A seventh and eighth contradicted-documentation location exist beyond the six named in Expected Behavior/Implementation Step 7**: `scripts/little_loops/loops/autodev.yaml:1908-1910`'s `refine_for_design` comment states "`--auto --gap-analysis` is additive (never removes content, does not consume `max_refine_count`)" — the identical false exemption claim. `autodev.yaml:822-828`'s `run_refine` comment makes a softer version of the same claim ("does not consume refine budget").
- **Two more existing tests pin the `--gap-analysis` action string at the loop callers found above, untouched by this issue's current test list**: `scripts/tests/test_rn_remediate.py::test_refine_followup_uses_gap_analysis_not_full_rewrite` (lines 602-613, asserts `"--gap-analysis" in rfu["action"]` against `rn-remediate.yaml`'s `refine_followup` state) and `scripts/tests/test_autodev_decision_gate.py::test_refine_for_design_invokes_skill` (lines 685-690, asserts `"--gap-analysis" in action` against `autodev.yaml`'s `refine_for_design` state). Neither test is confirmed unaffected the way `test_loops_recursive_refine.py` is (Implementation Step 8), nor listed as needing extension the way the seven files under Acceptance Criteria 6 are.
...
- **`last_command_timestamp` is absent from Signatures and Decision Rules despite having the same exposure as `_REFINE_ENTRY`**: `last_command_timestamp(content: str, command: str) -> datetime | None` (`scripts/little_loops/session_log.py:116`) is the function `research_triage.py:319` actually calls via `REFINE_COMMAND` for the Step 3.0 per-axis staleness check (the same check Implementation Step 4 references). Its matching is single-string exact-equality at `session_log.py:143` (`if entry.group(1) != command: continue`) — structurally identical to `_REFINE_ENTRY`'s exact-literal match, which this issue's Decision Rules already designs a fix for ("must be broadened to also accept a `:`-suffixed form").

  > **Resolved (pre-implementation review, 2026-08-29)**: keep `last_command_timestamp`'s signature and exact-equality semantics unchanged. At the `research_triage.py:319` call site, call it twice — once with `REFINE_COMMAND` and once with `GAP_REFINE_COMMAND` (see the shared-constant finding below) — and take `max()` of the non-`None` results. Rationale: zero signature churn on a function with other potential callers, and the two-command set is closed (exactly the bare and discriminated forms), so a `Sequence[str]` overload buys nothing. Implementation Step 4 below reflects this shape.

- **Shared constant for the discriminated string (pre-implementation review, 2026-08-29)**: the writer of the discriminated entry is prose (`commands/refine-issue.md` Step 6.5), but two Python readers must recognize the exact literal (`program_design.py`'s `_REFINE_ENTRY`, `research_triage.py`'s staleness check). Define `GAP_REFINE_COMMAND = "/ll:refine-issue:gap-analysis"` in `scripts/little_loops/session_log.py` next to `_COMMAND_RE` as the single source of truth; Python readers import it, and `commands/refine-issue.md` cites it by name so a future rename has one anchor. Covered by Implementation Step 2a below.

- **Two display effects in `ll-issues refine-status`'s human table beyond the un-aliased column already noted (pre-implementation review, 2026-08-29)**: (a) the `total` column (`refine_status.py:475`, `len(issue.session_commands)` — "Number of /ll:* skills applied") counts a gap-analysis entry as one additional distinct skill; (b) the discriminated command gets its own dynamic ✓-style column, its header truncated by `_cmd_label`'s fallback (`refine_status.py:132-138` — verified no crash on unknown keys). Both are cosmetic and do not affect `refine_count`'s JSON value. Chosen polish: add `"/ll:refine-issue:gap-analysis": "gap"` to `_CMD_ALIASES` and append it to `_CANONICAL_CMD_ORDER` (one line each) so the column renders as a clean `gap` ✓-column instead of a truncated raw string; the `total`-column effect is accepted as-is (a gap pass *is* a distinct skill application). Covered by Implementation Step 6a below.
...
2a. The discriminated string has exactly one Python source of truth: `GAP_REFINE_COMMAND = "/ll:refine-issue:gap-analysis"` defined in `scripts/little_loops/session_log.py` next to `_COMMAND_RE`. `program_design.py` and `research_triage.py` import it (or build their regex from it); `commands/refine-issue.md` Step 6.5 cites the constant by name alongside the literal so the prose and code can't silently drift.
...
4. `research_triage.py`'s staleness check (`scripts/little_loops/issues/research_triage.py:319`) counts a `--gap-analysis` pass as recent refine activity: `last_command_timestamp` keeps its exact-equality signature unchanged, and the call site calls it twice — `REFINE_COMMAND` and `GAP_REFINE_COMMAND` — taking `max()` of the non-`None` results (per the resolved design in Program Design → Codebase Research Findings). Verified by `scripts/tests/test_research_triage.py`.
...
6a. `refine_status.py`'s human-facing table renders the discriminated entry cleanly: `"/ll:refine-issue:gap-analysis": "gap"` added to `_CMD_ALIASES` and the key appended to `_CANONICAL_CMD_ORDER` (`refine_status.py:39-59`), so it shows as a `gap` ✓-column rather than a truncated raw header. The `total` column counting a gap pass as one more distinct skill is accepted behavior (see Program Design → Codebase Research Findings), not a defect.
7. The three originally-cited exemption claims (`scripts/little_loops/config-schema.json` `commands.max_refine_count` description, `commands/refine-issue.md`, `scripts/little_loops/loops/refine-to-ready-issue.yaml`'s `refine_followup` comment) plus the two `docs/reference/COMMANDS.md` rows, the `docs/reference/CONFIGURATION.md` row, and the two `autodev.yaml` comments (`:822-828`, `:1908-1910`) all describe the same, now-true behavior — per the selected Decision Rules Option 1.

7a. The corrected wording is pinned by tests, giving AC5 a machine check: `test_max_refine_count_exemption_documented` (`scripts/tests/test_refine_issue_command.py:199-202`) is reconciled to assert the *post-fix* claim (exemption exists and is implemented via the `GAP_REFINE_COMMAND` discriminator), and a companion assertion pins the `config-schema.json` `max_refine_count` description and the two `docs/reference/COMMANDS.md` rows to wording that names the discriminated form — following the existing doc-pin pattern that test already establishes.
...
5. `scripts/little_loops/config-schema.json`'s `commands.max_refine_count` description, `commands/refine-issue.md`, `scripts/little_loops/loops/refine-to-ready-issue.yaml`'s `refine_followup` comment, `docs/reference/COMMANDS.md` (both cited rows), `docs/reference/CONFIGURATION.md`, and `autodev.yaml`'s two exemption comments make no claim that contradicts the shipped behavior — and the schema description plus the two `COMMANDS.md` rows are pinned by test assertions per Implementation Step 7a (so this AC is machine-checked, not prose-audited).
...
- **AC5 has no concrete verification mechanism, unlike AC1-4 and AC6**: AC1-4 each cite an exact CLI invocation, JSON field, or code path to check, and AC6 cites a `pytest` exit code. AC5 ("make no claim that contradicts the shipped behavior") requires a human/LLM read of the now eight prose locations (six originally named plus the two `autodev.yaml` comments found above) rather than running a script or regex/string assertion — no such check is proposed anywhere in this issue. (Note: at the time this gap was first identified, the Decision Rules discriminator-shape choice was also still unresolved, which compounded the problem by leaving even the *target* wording undetermined; a subsequent `/ll:decide-issue` pass has since resolved that choice to Option 1 — colon suffix, `/ll:refine-issue:gap-analysis` — per Program Design → Decision Rationale above. That resolution does not by itself give AC5 a machine-checkable form; the gap described here is independent of it and still holds.)
...
## Session Log
- `/ll:manage-issue` - 2026-08-30T01:41:10 - `bb406fc7-ac3c-47a2-8924-75151d7392f6.jsonl`
