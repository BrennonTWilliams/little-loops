...
**Decisions (2026-09-16 review; revised 2026-09-16 after BUG-3489 landed):**
- **BUG-3489 already shipped the reserved-name foundation — extend it, do not recreate it.** Commit `45ec5e223` added `RESERVED_STATE_NAMES` (`policy_builder_core.mjs:152`), `isReservedOutcomeToken()` (`:168`, which also rejects every underscore-prefixed token, covering `_` / `_error` / `_custom`), a throwing `_assertNoReservedTokens()` guard called at the top of `_serializeDecisionTable()` and `_serializeIssueLifecycle()`, and a try/catch in the template's `updatePreview()` (tmpl 809-826) that renders the throw into the YAML pane. Current sets: decision_table = `score, parse_scores, policy_dispatch, failed, error`; issue_lifecycle = `score, policy_dispatch, done, failed, error`; no `rubric` key (unknown modes fall back to the decision_table set). This issue **extends the existing map in place**:
  - decision_table: add `finished` (the only other name `_doneStateName()` can emit)
  - issue_lifecycle: add `issue_id` (`_serializeIssueLifecycle()` emits an `issue_id:` state)
  - rubric (new key): `score`, `parse_scores`, `route_high`, `route_medium`, `done` (from `_serializeRubric()`)
  - keep BUG-3489's `error` entries and the underscore-prefix rule unchanged
  Consequences: update the exact-set pin in `scripts/tests/js/policy_validator.test.mjs:227-236` (`"RESERVED_STATE_NAMES exposes the runtime sets documented by BUG-3489"`) and extend the existing **Reserved names** paragraph in `docs/guides/POLICY_ROUTER_GUIDE.md:249-256` rather than adding a second one. `validateBuilderModel` becomes the primary, non-throwing surface for reserved-name diagnostics; `_assertNoReservedTokens()` stays in the serializers as defense in depth (BUG-3489's Node tests assert the throw), and the `updatePreview()` try/catch stays.
- **`aggregate` is a reserved *dimension* name, not a state name.** `_RESERVED = {"aggregate"}` in `fsm/validation/reachability.py:190` skips `aggregate` as a predicate LHS (it is the overall rubric score; `docs/guides/POLICY_ROUTER_GUIDE.md:116`). It must **not** go into `RESERVED_STATE_NAMES`. Instead `validateBuilderModel` emits an error diagnostic when any authored dimension normalizes to `aggregate` (a `rubric-dim-aggregate.txt` score file would shadow the `rubric-aggregate.txt` the fragments write).
- **`done` stays a legal decision-table outcome name; `_doneStateName()` stays dynamic; `finished` is reserved.** `seedExample("decision_table")` (mjs ~478) names an outcome `done`, and `_doneStateName()` (mjs 799-805) exists precisely to allow that — reserving `done` would make the default page load with an export-blocking diagnostic. The earlier decision to replace the dynamic `done`→`finished` selection with a constant `finished` is **withdrawn**: its premise ("`sample-decision-table.model.json` has no finish outcome with an action") was wrong — `escalate` is a `prompt` outcome with a `finish` transition, which is why `sample-decision-table.yaml` already emits `next: done` and a `done:` terminal. A constant would churn that fixture, the golden HTML, and the `_doneStateName` tests, and make decision-table the only mode whose loops don't end in `done`. The collision the original decision targeted (`_doneStateName()` never checks whether `finished` is also taken) is closed by adding `finished` to the decision_table reserved set. No golden fixture changes for this bullet.
- **Handler logic moves into the `.mjs` core.** There is no jsdom/playwright and no npm dependency budget, so inline `.html.tmpl` handlers cannot be tested by `node:test`. The field/operator reconciliation, draft-input state, and winner lookup in (b)–(d) are extracted as pure functions in `policy_builder_core.mjs` (e.g. `reconcilePredicateForDim`, `evaluateModel`), leaving the template as thin DOM glue. `opsForType()` (tmpl 291-295) moves into the core alongside them. "Browser-editing coverage" in the ACs means Node tests over those pure functions, not DOM tests.
- **Rubric mode silently drops unknown outcomes — surface it.** `_serializeRubric()` (mjs ~963-985) only honors authored outcomes named `light_repair` / `deep_repair`; any other outcome in rubric mode is ignored without feedback. `validateBuilderModel` emits a warning diagnostic for such outcomes (warning, not error — the emitted YAML is still valid).
...
- **Draft-input representation (defect d):** each predicate in the model carries `value` (last committed, always parser-valid) plus `draft: { text, error } | null`. The DOM input shows `draft.text` when present; `validateBuilderModel` emits an error diagnostic for any predicate with a non-null `draft`, so a rejected edit blocks export instead of silently exporting the previous value. `reconcilePredicateForDim(pred, newDimType)` resets `op` to the new type's default and clears `value`/`draft` when the stored op is not legal for the new type. `draft` is optional (absent ≡ null): `buildModel()` (tmpl 248) passes `r.predicates` by reference, so the serializers and `_serializeRulesText` must ignore the field, and the `sample-*.model.json` fixtures do not gain it.
- **Corpus shape for negative cases.** Both corpora carry only positive cases today (`evaluate_cases`/`shadow_cases` with `expected_target`; frontmatter `cases` with `expected_scores`), and `test_policy_builder_corpus.py` iterates only those keys. Add, rather than overload: (1) a `parse_error_cases` key in `conformance_corpus.json` — `{name, rules, expected_error_substring}` — asserted with `pytest.raises(ValueError)` against `parse_rules()` on the Python side and `assert.throws` against `parseRuleTable()` in Node; (2) a `js_reject_cases` key in `frontmatter_encoding_corpus.json` — `{name, frontmatter_text, expected_error_substring}` — asserted in Node only, because Python's YAML loader legitimately accepts the block lists / nested mappings the mini-parser must reject (no Python parity is possible for those). Existing `cases` (comment stripping, quoted comma, decoupled `priority`/`priority_rank`) stay bilateral.
...
- Coordination: **satisfied** — BUG-3489 landed (`45ec5e223` serializer/guard/fixtures, `400ce555f` runtime/docs; status done). `_serializeDecisionTable()` now emits `on_max_steps: failed`, `on_error: failed` on every pipeline state, `_error: failed`, and a `failed: {terminal: true, failure: true}` state; all three `sample-*.yaml` fixtures and `golden_policy_router_builder.html` were regenerated. This issue regenerates them once more, after its own `.mjs`/`.tmpl` edits.
...
- `scripts/tests/js/policy_validator.test.mjs::"serializeLoopYaml matches golden decision-table fixture"`, `scripts/tests/test_policy_builder_emit.py::test_golden_yaml_validates`, and `scripts/tests/test_policy_builder_node_gate.py::test_round_trip_yaml_validates_for_each_mode[sample-decision-table.model.json]` — all pinned to `fixtures/policy_builder/sample-decision-table.yaml`/`.model.json`. _(The `_error: escalate` → `_error: failed` change already landed with BUG-3489; with `_doneStateName()` kept dynamic, this issue expects **no** change to the emitted YAML for valid models — treat any diff in these fixtures as a regression to explain, not a fixture to regenerate.)_ [Agent 2/3 finding]
- `scripts/tests/js/policy_validator.test.mjs:227-236` — `"RESERVED_STATE_NAMES exposes the runtime sets documented by BUG-3489"` pins the exact per-mode sets with `deepEqual`; must be updated for `finished` / `issue_id` / the new `rubric` key. The adjacent `serializeLoopYaml(...) throws naming a reserved ...` tests must keep passing (serializer throw retained).
...
- **Diagnostic-list shape** — three incompatible precedents exist for "validation result list with severity" in this codebase, none of them a drop-in match: `ValidationError`/`ValidationSeverity` (Enum, `path`+`message`+`severity`, sub-checks each return a list folded via `.extend()` into one flat list at the top level) in `fsm/validation/_base.py:15-41` and `fsm/validation/structural_rules.py` (e.g. `validate_fsm()` at :1097-1136); `FindingDetail`/`CheckResult` (`severity: Literal["error","informational"]`, no locator path, paired with a separate `status` field) in `cli/doctor.py:46-80`; and `action_severity: Literal["auto","mention","route"]` in `link_checker.py:105-127`. Shape A (`fsm/validation/_base.py`) is the only one that lives beside this issue's own Python "mirror" modules (`fsm/policy_rules.py`) and the only one aggregated the way `validateBuilderModel` needs (many independent sub-checks, each returning a list, folded into one).
- **MatchResult identity** — no existing "preserve which of N alternatives matched, by index" precedent anywhere in the repo (searched `MatchResult`/`match_index`/`winning_rule`/`rule_index`, no hits). `evaluateRules`/`evaluate_rules` (`policy_builder_core.mjs:305-311` / `fsm/policy_rules.py:232-254`) return only the winning target string today — the proposed `MatchResult` type is new in kind here, not an extension of an existing shape.
- **Dependent-field reconciliation on type change** — no repo-wide precedent outside this same template file. Two conflicting patterns already coexist in it: `oc.transition = {...}` full-object-replace on kind change (`policy-router-builder.html.tmpl:434-437`, `:540-543`) discards a stale dependent value by replacing the container, vs. `aSel.onchange` leaving `oc.body` stale when `actionType` changes (`policy-router-builder.html.tmpl:413`) — the same defect class the issue names for `p.dim`/`p.op` in `dimSel.onchange` (`policy-router-builder.html.tmpl:604`). `opsForType(type)` (`policy-router-builder.html.tmpl:291-295`) already holds the per-type legal-operator table `reconcilePredicateForDim` would need, but it currently lives in the `.tmpl` file, not `policy_builder_core.mjs`.
- **Exported constants for Node-test pinning** — existing convention is a plain `export const X = [...]` array literal with a provenance comment block (`BUILTIN_FRONTMATTER_DIMENSIONS`, `LIFECYCLE_VERBS` — `policy_builder_core.mjs:85-140`), imported by name directly in `scripts/tests/js/policy_validator.test.mjs:26-27,173-185`. Any new export (`validateBuilderModel`, `evaluateModel`, `reconcilePredicateForDim`, `opsForType`) must also be added to the `window.PolicyBuilderCore` mirror object (`policy_builder_core.mjs:1297-1321`) to be reachable from the rendered page — existing entries there are grouped under a `// FEAT-XXXX`-style comment per feature (`RESERVED_STATE_NAMES`/`isReservedOutcomeToken` are already there under `// BUG-3489`). `RESERVED_STATE_NAMES` (`:152-155`) is itself the per-mode-keyed precedent now; the older reserved-token sets (`fsm/schema.py:33-35` `FAILURE_TERMINAL_NAMES`, `fsm/validation/reachability.py:190` `_RESERVED` — a *dimension*-name set, not a state-name set) are flat.
- **Disable-export-on-invalid UI** — no precedent anywhere in the repo for gating a control on a validation-diagnostic list; `policy-router-builder.html.tmpl` is the only `.html.tmpl` file in the tree. The four existing `.disabled =` assignments in this file (`:374`, `:603`, `:659`, `:664`) all gate on structural state (in-use references, boundary position) paired with a `.title` tooltip explaining why, not on diagnostics; `#copy-btn`/`#download-btn` (`:923-935`) currently have no disabled logic at all.

## Program Design

### Types

Proposed MatchResult carries ruleIndex, target, isFallback, and conditionResults; Diagnostic carries `severity: "error" | "warning"`, field path, and message (shape A from `fsm/validation/_base.py`: sub-checks each return a list, folded flat). Predicate gains an optional `draft: { text: string, error: string } | null` alongside the committed `value` (see Decisions). `RESERVED_STATE_NAMES` already exists (BUG-3489) and gains a `rubric` key plus `finished` / `issue_id` entries.

### Signatures

Proposed new JS core contracts:
- `validateBuilderModel(model) -> Diagnostic[]` (absorbs `detectShadows`, the reserved-name check via the existing `isReservedOutcomeToken`, the reserved `aggregate` dimension check, duplicate outcomes, missing references, unsupported mode/type combinations, incomplete actions, invalid step budgets, inverted rubric thresholds, non-null `draft`, and the rubric unknown-outcome warning)
- `evaluateModel(model, scores) -> MatchResult` (compiles via `parseRuleTable(_serializeRulesText(model))` internally so both Try-it panels share one path)
- `reconcilePredicateForDim(pred, newDimType) -> Predicate`
- `opsForType(type) -> string[]` (moved from the template)
- `RESERVED_STATE_NAMES` extended in place: `{ decision_table: Set, rubric: Set, issue_lifecycle: Set }`; `isReservedOutcomeToken(mode, token)` unchanged

Keep `evaluateRules`' existing target-returning API compatible; keep `_assertNoReservedTokens()` throwing inside the serializers.

### Call Path

Target wiring: `updatePreview` -> `buildModel` -> `validateBuilderModel` -> (if no error diagnostics) `serializeLoopYaml`, else render diagnostics and disable `#copy-btn` / `#download-btn`; the existing try/catch around `serializeLoopYaml` (tmpl 809-826) stays as the last line of defense.
...
- `evaluateRules(rules, scores) -> string | null` (`policy_builder_core.mjs:305-311`) — returns only the matched rule's `target` string today; no rule index or identity is returned. This is the gap the proposed `MatchResult` type closes.
- `parseRuleTable(text, grammar?) -> Array<{predicates, target, isCatchall}>` (`policy_builder_core.mjs:610-643`) — `grammar` is optional, falling back to `DEFAULT_PRED_RE`.
- `isReservedOutcomeToken(mode, token) -> boolean` and `RESERVED_STATE_NAMES` (`policy_builder_core.mjs:152-173`, BUG-3489) — already exported, already in the `window.PolicyBuilderCore` mirror (`:1318-1319`), already pinned by Node tests; this issue reuses them.
...
- Regenerate `fixtures/policy_builder/golden_policy_router_builder.html` after any `.mjs`/`.tmpl` edit — required for `test_enh3035_artifact_template_kit.py` to keep passing. The `sample-*.yaml` fixtures are expected to stay byte-identical (see Decisions: `_doneStateName()` stays dynamic).
- Decision-table failure terminal (`failed`) and its fixture regeneration: **done in BUG-3489** (`45ec5e223`). Nothing left here except adding `finished` / `issue_id` / `rubric` to the existing `RESERVED_STATE_NAMES` map.
- Add new cases to `frontmatter_encoding_corpus.json` (comment stripping, quoted comma in a flow-list, decoupled `priority`/`priority_rank`) and to `conformance_corpus.json` (`parseRuleTable` error cases for nonnumeric ordered comparisons and malformed targets) rather than modifying existing cases. Negative cases go under the new `parse_error_cases` / `js_reject_cases` keys defined in Decisions; extend `test_policy_builder_corpus.py` and `policy_validator.test.mjs` to iterate them.
...
- Extend the existing **Reserved names** paragraph in `docs/guides/POLICY_ROUTER_GUIDE.md:249-256` (added by BUG-3489) with `finished`, `issue_id`, the rubric set, and the reserved `aggregate` dimension; document validation diagnostics and export gating in the Visual Builder section; update `CLI.md` where it describes the builder's validation contract (`CONFIGURATION.md`/`API.md` catalog updates moved to BUG-3490).
...
Per-defect file:line anchors, confirmed by direct code reading (re-anchored 2026-09-16 against `main` after BUG-3489's `45ec5e223`, which shifted `policy_builder_core.mjs` by ~66 lines from `RESERVED_STATE_NAMES` onward and by ~80 from `_serializeDecisionTable` onward; all five defects still reproduce):

- **(a) Decision-table Try-it uses uncompiled boolean ops** — `policy-router-builder.html.tmpl` `updateTryIt()` (lines 718-737) calls `evaluateRules(model.rules, scores)` (line 731) on the **raw** rules, whose boolean predicates still carry literal ops `"==true"`/`"==false"`. In `policy_builder_core.mjs` `evalPredicate()` (lines 259-300), `"==true"`/`"==false"` are absent from `ORDERED_OPS` (line 59), fall to the string-compare branch, and because `op` is literally `"==true"` (not `"=="`) the code takes the `!=` arm — `==true` predicates evaluate as `!=`. Contrast: `updateFrontmatterTryIt()` (tmpl lines 754-778) compiles first via `parseRuleTable(_serializeRulesText(model))` (line 772) before evaluating — the raw-rules path is not used there.
- **(b) Both previews recover winner by target name** — `evaluateRules()` (mjs lines 305-311) returns `string | null` only — no rule index/identity. Both `updateTryIt()` (tmpl line 733) and `updateFrontmatterTryIt()` (tmpl line 775) recover the row via `model.rules.findIndex(r => r.target === winnerTarget)`, which returns the *first* rule with that target, not the rule actually matched. Both also silently no-op (no fallback indicator) when `winnerTarget` doesn't match any `model.rules` entry (catch-all fired).
- **(c) Validation gaps** — Python `_parse_predicate()` (`fsm/policy_rules.py:72-95`) rejects non-numeric values for ordered ops via `float(value)` (line 89); JS `parsePredicate()` (mjs lines 591-604) never performs this check. Python's `_TARGET_PATTERN = re.compile(r"^[\w][\w\-]*$")` (`policy_rules.py:37`, enforced 140-144) rejects malformed targets; JS `parseRuleTable()` (mjs 610-643) only checks for an empty string (623-625). `$("add-outcome").onclick` (tmpl 916-922) has no duplicate/reserved-name check, unlike `$("add-dim").onclick` (tmpl 897-914) which does check duplicates (908). `_doneStateName()` (mjs 799-805) returns `"finished"` if `"done"` is used, but never checks whether `"finished"` is *also* already used — closed by reserving `finished`, not by changing the function. `$("f-maxsteps").oninput` (tmpl 891) only guards falsy values (`Number(...) || 1`), so negative step budgets pass through. Since BUG-3489, reserved outcome/target/fallback names already throw from `_assertNoReservedTokens()` (mjs ~180-202) inside the serializers and surface via `updatePreview()`'s try/catch (tmpl 809-826); duplicate outcome names, missing references, and the rest of the list above are still unchecked anywhere.
- **(d) Editing reconciliation** — `dimSel.onchange` in `renderRules()` (tmpl line 604) sets `p.dim` but never resets/validates `p.op` against the new dimension's type (`opsForType()`, tmpl 291-295, holds the legal-op table). The string-value `oninput` handler in `renderRules()` (tmpl 609-619) returns early on invalid input without resetting `valInput.value`, leaving the DOM and model showing different values. `updateFrontmatterTryIt()`'s call to `parseRuleTable(_serializeRulesText(model))` (tmpl 772) has no try/catch (unlike the `parseFrontmatterBlock` call just above it, tmpl 764-769); a freshly-added predicate has `value: ""` (tmpl line 940 / 637), which fails `_PRED_PATTERN` in `parsePredicate()` (mjs 591-604) and throws uncaught.
- **(e) Frontmatter Try-it parser divergence** — `parseFrontmatterBlock()` (mjs 1175-1229) has no `#`-comment stripping, unlike Python's real `yaml.load`-based `parse_frontmatter()` (`frontmatter.py`). The flow-list branch (mjs 1215-1218) splits on every comma via `.split(",")`, breaking on a quoted element containing a literal comma. `encodeFrontmatterScores()` (mjs 1243-1291) derives `priority_rank` (block at 1280-1290) when a dimension is *named* `"priority"`; Python's `encode_frontmatter_scores()` (`frontmatter_scores.py:106-115`) derives it when a dimension's `raw_key == "priority_rank"` and reads `fm.get("priority")` directly — different trigger keys that only coincide because `BUILTIN_FRONTMATTER_DIMENSIONS` (mjs 85-95) always ships both together.
...
- **(g) Dispatch errors route to first outcome** _(moved to BUG-3489; **fixed** in `45ec5e223`)_ — `_serializeDecisionTable()` (now mjs 807-910) emits `_error: failed` (line 878) and a `failed: {terminal: true, failure: true}` state (905-908), matching `_serializeIssueLifecycle()` (mjs 1052-1120).
