...
- **MatchResult identity** — no existing "preserve which of N alternatives matched, by index" precedent anywhere in the repo (searched `MatchResult`/`match_index`/`winning_rule`/`rule_index`, no hits). `evaluateRules`/`evaluate_rules` (`policy_builder_core.mjs:239-245` / `fsm/policy_rules.py:232-254`) return only the winning target string today — the proposed `MatchResult` type is new in kind here, not an extension of an existing shape.
...
- **Exported constants for Node-test pinning** — existing convention is a plain `export const X = [...]` array literal with a provenance comment block (`BUILTIN_FRONTMATTER_DIMENSIONS`, `LIFECYCLE_VERBS` — `policy_builder_core.mjs:85-140`), imported by name directly in `scripts/tests/js/policy_validator.test.mjs:26-27,173-185`. Any new export (e.g. `RESERVED_STATE_NAMES`) must also be added to the `window.PolicyBuilderCore` mirror object (`policy_builder_core.mjs:1215-1237`) to be reachable from the rendered page — existing entries there are grouped under a `// FEAT-XXXX`-style comment per feature. No existing per-mode-keyed (`{decision_table, rubric, issue_lifecycle}`) reserved-set example exists to compare against; existing reserved-token sets (`fsm/schema.py:33-35` `FAILURE_TERMINAL_NAMES`, `fsm/validation/reachability.py:190` `_RESERVED`) are flat, non-per-mode.
