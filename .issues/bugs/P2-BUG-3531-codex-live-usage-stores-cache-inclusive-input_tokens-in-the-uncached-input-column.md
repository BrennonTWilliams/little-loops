...
- Pre-foundation live Codex rows are not identifiable by host: `turn.completed` has no `model` field (rows store `"unknown"`), and the old writer stored no host. ENH-3538's in-progress writer stamps host but deliberately leaves provenance unknown; that does not certify the intermediate rows' input semantics.
- Container validation is also missing: a nonempty list or string in live `usage` raises `AttributeError`, while an empty object is silently discarded by the truthiness check. Component validation alone does not close this gap.
...
1. **Representation.** `normalize_codex_input(usage) -> CodexInputSplit`, a frozen dataclass `(uncached_input: int | None, cache_read: int | None, cache_write: int | None, consistent: bool)`. A consistent observation gives `uncached_input = input - cache_read - cache_write` only when all three inputs are known nonnegative integers. When valid cache components exceed inclusive input, the result is `consistent=False`, with `uncached_input=None` (unavailable, per ENH-3538's null-not-zero rule) and the valid cache components kept exactly as reported. `consistent` is local to the split; it is not added to `TokenUsage`.
   ...
   - **Containers and empty objects.** Validate mapping types before `.get` calls; do not use truthiness to decide whether an observation is present. For live `turn.completed`, absent or explicit-null `usage` means no observation; a present empty object is an all-missing observation, and a present non-object (`[]`, `[1]`, strings, booleans, numbers) is malformed. Empty/malformed live observations reach the detailed callback/persistence with all components `None` and unknown provenance; they never fire the legacy callback or become zero usage. Reject a malformed container before applying any omitted-cache-write exception: an entirely empty or non-object block cannot fabricate a zero component.
...
2. Add shared normalization, container guards, and per-component validation with focused absent/empty/malformed/partial/zero tests; wire the live parser and per-observation rollout cache-rate reader. Share the container contract with ENH-3532.
...
- **Effort**: Provisional — re-estimate after Decision 6 resolves omitted-field and per-turn/cumulative/resume semantics. Stateful differencing may widen the call path beyond a small parser correction.
...
- `usage_from_event(event: dict[str, Any], *, default_model: str) -> TokenUsage | None` — signature unchanged unless Decision 6 demonstrates a need for stateful cumulative handling; the `turn.completed` branch validates container presence/type before normalization and sets `provenance`/`host`.
