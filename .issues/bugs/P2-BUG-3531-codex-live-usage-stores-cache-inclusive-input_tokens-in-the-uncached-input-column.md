...
- Container validation is also missing: a nonempty list or string in live `usage` raises `AttributeError`, while an empty object is silently discarded by the `if not usage:` truthiness check (`subprocess_utils.py` `usage_from_event`). Component validation alone does not close this gap.
...
- Documentation: fixture README (§ Live `codex exec --json` usage contract) records CLI version, capture procedure, observation scope, and cache-write evidence; CHANGELOG documents new Codex input semantics and public cache-rate exclusion fields. Audit schemas pinning the affected JSON fields.
...
2. Add shared normalization, container guards, per-component validation, and the live all-zero rule with focused absent/empty/malformed/partial/zero tests; wire the live parser (measured provenance) and per-observation rollout cache-rate reader. Drive the live-path tests from `exec-json-turn.jsonl`/`exec-json-resume.jsonl`. Share the container contract with ENH-3532.
