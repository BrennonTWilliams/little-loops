...
# BUG-105: ll_create_sprint references undefined --priority argument

## Summary

The `/ll:ll_create_sprint` command shows an example using `--priority P0` but this argument is not defined in the command's frontmatter arguments section. The feature does not exist.

## Context

Identified during audit of the `/ll:ll_create_sprint` slash command. The examples suggest functionality that was never implemented, misleading users.

## Current Behavior

Line 155 of `.claude/commands/ll_create_sprint.md`:
```bash
