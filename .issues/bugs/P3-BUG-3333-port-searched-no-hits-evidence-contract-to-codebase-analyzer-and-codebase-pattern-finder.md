...
5. **Regenerate the eight host mirrors** — this step is mandatory and unguarded by any test (see the caution below):
   ```bash
   ...
   ```
   `ll-adapt` takes exactly one `--host` per invocation (`ll-adapt --help`), so all four run. This rewrites `.codex/agents/{codebase-analyzer,codebase-pattern-finder}.toml` and `.{gemini,kimi-code,qwen}/agents/{codebase-analyzer,codebase-pattern-finder}.md` — 8 files, all carrying a `# generated by ll-adapt` header. Never hand-edit them. Confirm with `git status` that all 8 show as modified; BUG-3330's commit (`96ec85c54`) regenerated the 4 locator mirrors as part of the same change.
