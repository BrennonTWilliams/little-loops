...
**`ll-harness dsl` consequence (review 2026-09-15, fourth pass):** because `cmd_dsl`'s per-task tally (`harness.py:3414-3423`) already routes rc 2 to `errored_count` and its aggregate exit chain returns 2 whenever `errored_count > 0` (`:3524`), one grader-error task among N now makes the whole DSL run exit 2 where it previously exited 1 as a folded failure. This is the intended widening (the documented DSL contract at `EVALUATION_GUIDE.md:138-141` already reads "≥1 task hit a per-task infra error" → 2; widen its wording to "infra or judge error"). The task also leaves the printed `failures` list, so the summary must name it — see the `cmd_dsl` Acceptance Criterion. **Accepted, pre-existing divergence:** `_update_aggregate()` (`:3469-3479`, `:3518-3521`) persists `exit_code` 0/1 on the aggregate row from `all_graded_passed` alone while the process returns 2; this already holds for runner-timeout tasks and now also for judge-error tasks. Not changed here.
...
- [ ] `_evaluate_and_report()` (n=1) has an explicit ERROR branch: stdout `Result` and `--output json` `"result"` read `ERROR` (not `FAIL`, not `PASS`) and a grader-error detail line is printed. The detail is rendered from whichever keys `eval_result.details` actually carries (`error`, or the raw judge fields when the model omitted `verdict`) with a fallback of `unknown grader error` — never `details["error"]` unguarded. Note (review 2026-09-15, fourth pass): every `raise BlockingJsonError` site in `host_runner.py` (`:2728-2812`) sets an `"error"` key alongside its flag (`timeout`/`missing_dependency`/`api_error`/`empty_output`/`raw_preview`), so there is no flags-only shape to render — do not add a branch for one.
...
- [ ] `cmd_dsl`: a per-task grader error lands in `errored_count` (`harness.py:3416-3417`, no code change) and therefore drops out of the printed `failures` list; the aggregate summary gains an `  errored: N (<task names>)` line whenever `errored_count > 0`, so a run that prints `DSL pass-rate: 9/9` and exits 2 names the task whose judge failed. Test: a 2-task DSL run with one `yes` and one `verdict="error"` prints `DSL pass-rate: 1/1`, the `errored:` line with the second task's filename, and returns 2.
...
- [ ] `harness_eval_pass_rate()` excludes `semantic_verdict='error'` rows from both its `successes` numerator and its `scored` denominator, so pre-fix error rows (`semantic_passed=0`) are re-banded by SQL the same way `_rc_from_event()` re-bands them — the two readers must never disagree on the same row. Test: one `yes` row, one `no` row, one pre-fix `error`/`semantic_passed=0` row → 1/2, not 1/3.
- [ ] `harness_eval_abstention_rate()` excludes `semantic_verdict='error'` rows from its `scored` denominator.
- [ ] `_read_target_history()` (`cli/harness.py:1046-1047`) excludes `semantic_verdict == "error"` rows from its hand-computed `pass_scored` and `judged_scored` counts, so `history_pass_rate_runs`/`history_judged_runs` equal the denominators the two rate functions actually use.
...
- [ ] In-code "`semantic_passed = NULL` ⇒ abstained" comments/docstrings are widened to "abstained or grader error": `harness_eval_pass_rate()` docstring (`history_reader/harness.py:418-424`), `harness_eval_abstention_rate()` docstring (`:459-468`), `session_store/schema.py:1005-1012`, and the `semantic_passed` param blocks in `session_store/writers.py` (`:1116-1117, 1173, 1189-1190, 1242-1243, 1311-1312`).
...
- `scripts/little_loops/cli/harness.py` — `cmd_dsl()` summary block (`:3503-3512`): add the `errored:` line (see the `cmd_dsl` Acceptance Criterion); the per-task tally at `:3414-3423` needs no change.
- **Not modified** (explicit non-goal, see Expected Behavior): `_retry_refusal()` (`cli/harness.py:194-222`), the hardcoded `reason="timeout"` in `_record_harness_event()` (`:305`), and `cmd_dsl`'s `_update_aggregate()` exit-code persistence (`:3469-3479`).
...
- `scripts/little_loops/cli/harness.py:2440-2442` — `_run_sample_loop()`'s per-sample `entry["error"]` is derived only from `result.error`/`result.timed_out`; a grader error yields `"result": "ERROR", "error": null` in the n>1 JSON — the same ambiguity this bug describes, one level down. Populate it from the outcome's grader-error detail.
- `scripts/little_loops/history_reader/harness.py:431-435` — `harness_eval_pass_rate()`'s SQL (`SUM(CASE WHEN semantic_passed = 1 ...)` / `COUNT(semantic_passed)`) must gain a `semantic_verdict IS NOT 'error'` guard on both aggregates (see Acceptance Criteria); this supersedes the earlier "accepted, no backfill" stance, which would have left `baseline_for()` (verdict-first via `_rc_from_event()`) and the pass-rate report disagreeing on the same pre-fix row.
...
- `scripts/little_loops/cli/harness.py:2466` — `_run_sample_loop()` also calls `_band_samples(tally)` directly (a second `_band_samples()` call site beyond `_run_baseline_phase()`'s at `:1991`).
- `scripts/little_loops/cli/harness.py:2323-2360` — `_run_compare_arm()` (ENH-3435) reads `res.tally.passed`/`res.tally.graded` directly (`candidate_rate = res.tally.passed / res.tally.graded if res.tally.graded else None`) to compute `BaselineDelta`; a `SampleTally`-field consumer not previously listed.
...
- `scripts/little_loops/cli/harness.py:2434` — `_run_sample_loop()` calls `_grade()`
- `scripts/little_loops/cli/harness.py:2526` — `_evaluate_and_report()` calls `_grade()`
...
- `scripts/little_loops/cli/harness.py:1051, 1056` — `_read_target_history()` calls `harness_eval_pass_rate(...)`/`harness_eval_abstention_rate(...)`
...
### Tests
- `scripts/tests/test_cli_harness.py::TestGradeEvidenceChannels` (`:4381`) — direct `_grade()` calls
- `scripts/tests/test_cli_harness.py::TestBandSamples` (`:965`) — direct `_band_samples()` unit tests
- `scripts/tests/test_cli_harness.py::TestSampleTallyRecord` (`:1012`) — `SampleTally.record()` per exit code
- `scripts/tests/test_cli_harness.py::TestAbstentionVerdict` (`:870`) — `cmd_cmd()` + `capsys` style
...
- `scripts/tests/test_cli_harness.py:1194` — exercises the `harness_eval_pass_rate()` reporting call site at `harness.py:1051`
...
- New test needed: `_run_sample_loop()` JSON `results[i]["error"]` is a non-null string for a `verdict="error"` sample, and `HarnessEvalOutcome.grader_error_detail` for each of the three `details` shapes (`{"error": ...}` as every `BlockingJsonError` site produces, raw judge fields with no `verdict`, `{}` → `unknown grader error`).
- New test needed (fourth pass): `cmd_dsl` with one `yes` task and one `verdict="error"` task prints `DSL pass-rate: 1/1`, an `errored: 1 (<task file>)` line, and returns 2.
...
- `_grade(runner_label, result, args, *, expected_grade=None, side_effects=None, duration_ms=None) -> tuple[int, HarnessEvalOutcome]` (`harness.py:1331`): unchanged signature. New branch `elif eval_result.verdict == "error": grader_error = True; passed = False` placed after the `is_abstention_verdict()` check and before the `!= "yes"` fall-through. Because `grader_error` and a semantic `no` are mutually exclusive (both come from the one verdict), the only extra state needed is `hard_fail = not passed`, captured immediately before the `if args.semantic is not None:` block (i.e. after exit-code, `expected:`, and side-effect folding). The exit-code chain then reads `hard_fail → 1; grader_error → 2; not passed → 1; abstained → 3; else 0`, with the "fail > grader_error > abstain > pass" comment. No second predicate is needed. The `_grade()` docstring gains one line stating that `grader_error=True` is always paired with `passed=False` — consumers that read only `passed` (`show_output` in `_evaluate_and_report()`, `_run_compare_arm()`'s `tally.passed`) intentionally see a non-pass; do not "fix" that to `passed=True`.
...
- `cmd_dsl(args)` (`harness.py:3232`): unchanged signature and unchanged per-task tally; collect the task filename into a new `errored: list[str]` alongside `errored_count += 1` (`:3416-3417`) and append `f"  errored: {errored_count} ({', '.join(errored)})"` to the summary `lines` (`:3503-3512`) when non-empty.
...
`_run_sample_loop()`/`_evaluate_and_report()` (`harness.py:2403`/`:2504`) -> `_grade()` -> `evaluate_llm_structured()` returns `verdict="error"` -> `is_abstention_verdict()` returns False -> new `grader_error=True` branch -> rc 2 -> `SampleTally.record(2)` lands in `errored` / `_evaluate_and_report()` prints `ERROR` + `outcome.grader_error_detail`; `_run_sample_loop()` stamps the same detail on `entry["error"]` -> DB write uses `outcome.semantic_passed_row` (`None`) -> `history_reader/harness.py:_rc_from_event` (`:314-327`) sees `semantic_verdict="error"` and returns 2, matching the run-time banding; `baseline_for()` tallies it as `errored`; `harness_eval_pass_rate()`/`harness_eval_abstention_rate()` and `_read_target_history()` all exclude the row from their denominators by the same `semantic_verdict IS NOT 'error'` rule.
...
- **Effort**: Small-Medium - one branch in `_grade()`, a field + two properties on `HarnessEvalOutcome`, an ERROR branch in `_evaluate_and_report()`, the per-sample `error` key in `_run_sample_loop()`, an `errored:` summary line in `cmd_dsl`, five call sites moved onto the property, `_rc_from_event()`, `harness_eval_pass_rate()`, `harness_eval_abstention_rate()`, and `_read_target_history()` on the read side, docstring/doc wording, tests in `test_cli_harness.py` and `test_history_reader_harness.py`. No new tally field, exit code, or schema change.
- **Risk**: Medium - changes grading semantics; four independent readers of persisted rows (`_rc_from_event()`, `harness_eval_pass_rate()`, `harness_eval_abstention_rate()`, `_read_target_history()`) must all apply the same verdict-first `IS NOT 'error'` rule so pre-fix `harness_events` rows (`semantic_verdict="error"`, `semantic_passed=0`) are re-banded consistently without a migration. No backfill.
- **Breaking Change**: No
