...
The non-recursive `JS_TEST_DIR.glob("*.test.mjs")` stays unchanged: the `feat3304` JS file stays owned by its own Python gate, which only changes by adopting the shared guard.
...
- `test_round_trip_yaml_validates_for_each_mode` and the `feat3304` dashboard runtime gate behave identically apart from adopting the shared guard
...
- `scripts/tests/test_feat3304_artifact_dashboard.py` — `_node_major` (~L1141) and the silent skip in `TestDashboardNodeRuntimeGate::test_generated_page_runtime_behaviour` (~L1171) are the third Node gate; it adopts the shared guard and drops its private copy [review addition, 2026-09-22]
...
- `scripts/tests/test_decisions_yaml_gate.py` — the `validator` fixture docstring cites `test_policy_builder_node_gate.py:52-57` as "the canonical skip-when-missing idiom"; replace the line-number reference with a file/function reference (`_require_node` in the shared helper) and reword: after this change the Node guard is skip-when-missing only with `LL_REQUIRE_NODE` unset, not an unconditional skip template [review addition, 2026-09-22]
...
- Preserve version-probe diagnostics and reject unsuccessful probes; move `_require_node()` to a shared test helper and consolidate all three Node gates (including the `feat3304` dashboard gate, deleting its private `_node_major`) with the `LL_REQUIRE_NODE=1` fail / unset skip policy. Add the mocked guard matrix in a separate test module; set the env var on the existing `unit-tests` job; do not add it to conftest's `_CMD_RUN_ENV_VARS`
