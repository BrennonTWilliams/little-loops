...
- "CI is green" implies the gate executed. A missing Node toolchain is a loud failure, not a silent skip, for every Node-backed gate in the suite (the two policy-builder gates and the `feat3304` dashboard runtime gate).
...
- All three Node gates (`test_node_conformance_suite_passes`, `test_round_trip_yaml_validates_for_each_mode`, `TestDashboardNodeRuntimeGate::test_generated_page_runtime_behaviour`) use the shared `_require_node()`; the duplicate `_node_major` in `test_feat3304_artifact_dashboard.py` is removed. Mocked tests cover missing Node, Node <22, malformed output, nonzero probe exit, `OSError`, and probe timeout: each fails with `LL_REQUIRE_NODE=1` and skips with the variable unset. Node >=22 with a successful probe is accepted. Diagnostics preserve available stdout/stderr, exit status, or exception details without a second probe; missing Node explicitly reports the missing executable
...
- `_require_node() -> str` — moved to a shared helper module under `scripts/tests/`; used by all three Node gate entry points. Under `LL_REQUIRE_NODE=1` converts unavailable/unusable Node into failure; otherwise skips. Includes probe diagnostics when available, or an explicit executable-not-found message
...
- **Effort**: Small — rewrite one JS helper plus a regression test, remove one marker, extract one shared skip-to-fail guard used by three gates, refresh docs
