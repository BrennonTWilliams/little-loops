...
4. **Make a skip loud — in Python, not in the workflow** (review revision, 2026-09-21). The guard belongs in the test module, keyed on an explicit env var (e.g. `LL_REQUIRE_NODE=1`) that the `unit-tests` job sets: a workflow step cannot see the "node present but < 22" skip path. When the var is set, convert skip → fail for **both** Node gates — `test_node_conformance_suite_passes` *and* `_require_node()` — else a missing Node still silently skips the five `test_round_trip_yaml_validates_for_each_mode` tests and the loud-failure criterion is half-met. The failure message must carry the full `node --version` probe output: `_node_major` currently discards the raw version string, so change it to return both the parsed major and the raw stdout/stderr (a `(major, raw)` tuple or a small dataclass) rather than probing a second time (the earlier "`process.version`" phrasing named a Node-runtime concept the Python side never sees). Today a green job and a skipped gate are indistinguishable, which is how this bug stayed invisible.
   - **Runner-Node contingency** (review addition, 2026-09-22): the "runner ships 24" statement in the Integration Map is unverified — no artifact captured the CI Node version, only the path `/usr/local/bin/node`. If the first `LL_REQUIRE_NODE=1` run fails because the `ubuntu-latest` default `node` is < 22, that failure is the guard working as intended, and the resolution is to add `actions/setup-node` pinned to `22.x` or newer to the `unit-tests` job. That is a free hosted action, not paid CI, so it does not conflict with the Testing & CI Policy; it must not narrow the "Node >= 22" posture the gate enforces.
...
- `_node_major(node) -> tuple[int | None, str]` (or a small dataclass) — same file; returns the parsed major **and** the raw `node --version` stdout/stderr so the loud-failure message carries the probe output without a second subprocess call
...
`test_node_conformance_suite_passes` -> `_node_major` -> `subprocess.run` (`node --test scripts/tests/js/*.test.mjs`, 180s timeout) -> `policy_submission.test.mjs` `reviewed` -> `settleHash` -> `createSubmissionController.onChange` (resolves on the `ready`/`refused` emit) <- `emit` <- `startReview`'s `sha256Hex(...).then(...)` <- `crypto.subtle.digest`
...
- Change `_node_major` to return the raw `node --version` output alongside the parsed major; add the Python-side skip-to-fail guard (`LL_REQUIRE_NODE=1`) covering both the conformance gate and `_require_node()`, printing that raw output; the `unit-tests` job in `.github/workflows/ci.yml` only sets the env var
- If the first `LL_REQUIRE_NODE=1` run fails on the runner's default Node being < 22, add `actions/setup-node` pinned to `22.x`+ to the `unit-tests` job (free action, not paid CI)
