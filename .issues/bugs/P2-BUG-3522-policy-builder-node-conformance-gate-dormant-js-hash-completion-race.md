...
- "CI is green" implies the gate executed. A missing Node toolchain is a loud failure, not a silent skip, for every Node-backed gate in the suite (the two policy-builder gates, the `feat3304` dashboard runtime gate, and the `rlhf` smoke-harness regression gate).
...
3. **Drop `@pytest.mark.no_parallel`** from `test_node_conformance_suite_passes` once (1) lands: with the race gone there is nothing for contention to break, and the gate returns to the default parallel invocation. Pair that with `@pytest.mark.timeout(240)` — strictly above the inner `timeout=180` — following the convention in `scripts/tests/test_verify_evidence.py`. Without it, a hung node process trips the 120-second thread watchdog first, whose `os._exit(1)` kills the worker instead of producing a clean `TimeoutExpired` assertion. Timeout protection stays outside `settleHash()`: the new wait is an unbounded await on the controller's own signal, and no additional budget is added inside the JS. If Node remains running without completing, the 180-second subprocess timeout reports the failure; the 240-second pytest timeout is an outer fallback. **The same inversion exists in the dashboard gate** (review addition, 2026-09-22): `scripts/tests/test_feat3304_artifact_dashboard.py::TestDashboardNodeRuntimeGate::test_generated_page_runtime_behaviour` runs `node --test` with an inner `timeout=180` and no `@pytest.mark.timeout` override, so the 120-second watchdog fires first there too. Add `@pytest.mark.timeout(240)` to it in this issue — a small, justified addition to a file already in scope for the guard consolidation.
4. **Make a skip loud through one shared Python guard** (review revision, 2026-09-22). Move the Node guard out of `scripts/tests/test_policy_builder_node_gate.py` into the existing shared helpers module `scripts/tests/helpers.py` (do not create a second helper file) as a public `require_node(min_major: int | None = 22) -> str`, and route **all four** Node-dependent tests through it: `test_node_conformance_suite_passes`, the five `test_round_trip_yaml_validates_for_each_mode` cases, `scripts/tests/test_feat3304_artifact_dashboard.py::TestDashboardNodeRuntimeGate::test_generated_page_runtime_behaviour` (which today carries its own duplicate `_node_major` at ~L1141 and a silent `pytest.skip` at ~L1171 — the third copy is deleted, not kept), and `scripts/tests/test_rlhf_svg_evaluate_smoke.py::test_smoke_harness_survives_non_string_pageerror_message` (~L101: `shutil.which("node")` plus a silent `pytest.skip`, with no version floor). `require_node()` is the only shared entry point callers need; the version probe can stay an internal detail of `helpers.py` (a public `node_major()` is only warranted if a caller actually needs the major separately — none does today). The three policy-builder/dashboard gates use the default floor, and the `rlhf` smoke gate passes `min_major=None` so it gains required-Node enforcement without acquiring a Node 22 floor it never had. When `LL_REQUIRE_NODE=1`, unavailable or unusable Node must fail; with the variable unset, preserve graceful skips. Set the variable on the existing `unit-tests` job. Probe `node --version` once per guard call and preserve stdout, stderr, exit status, and exception/timeout details alongside the parsed major. A nonzero probe exit is unusable even if stdout resembles a supported version. Missing Node must report that the executable was not found; it cannot provide version output.
   - **Retain the probed Node version on successful runs** (review addition, 2026-09-22). Today the exact runner major is never captured (see `## Notes`), because nothing records it when the gate passes. Requirement: the actual probed `node --version` output must end up in an uploaded `unit-tests` artifact (`pytest.log` or `pytest-junit.xml`) on a **passing** run, and that retention must be verified against the artifact of the first post-merge run. The implementation chooses the smallest reliable mechanism; note the constraints that rule out the obvious ones under the current configuration: passing-test stdout is not written to `pytest.log` (no `-s`/`-rP`), JUnit captured-output logging is off by default, and `record_property` warns under the default `xunit2` family. This is what makes the `actions/setup-node` fallback decision evidence-based instead of inferred.
   - Add mocked guard tests for missing Node, Node <22, malformed version output, nonzero probe exit, `OSError`, and `TimeoutExpired`, with `LL_REQUIRE_NODE=1` and with it unset. Assert fail versus skip and useful diagnostics; also cover successful Node >=22, and the `min_major=None` case (any successfully-probed Node is accepted, including a major below 22; missing or unusable Node still fails/skips by the same policy). Verify all four gate entry points use the shared `require_node`. Keep these guard-policy tests in a separate test module so the existing policy-builder gate module remains six executable cases.
   - **Do not add `LL_REQUIRE_NODE` to `_CMD_RUN_ENV_VARS` in `scripts/tests/conftest.py`** (~L1180). That autouse fixture deletes every listed `LL_*` variable for the duration of each test; listing this one would silently neutralize the guard in CI and turn every fail back into a skip. The variable is not in the list today — keep it that way, and read it with `os.environ.get` inside the guard.
   - **Runner Node is confirmed >=22, exact major not captured.** The gate only runs when `_node_major(node) >= 22`, and run `35490420257` on `ubuntu-latest` executed all 231 node subtests, so Node >=22 was present on the hosted runner at `fdf6773d2`. No `actions/setup-node` step is expected. Fallback only: if the first `LL_REQUIRE_NODE=1` run nonetheless fails the guard (image drift), add `actions/setup-node` with `22.x` or newer to the existing `unit-tests` job — still Node >=22 policy, still one job.

The non-recursive `JS_TEST_DIR.glob("*.test.mjs")` stays unchanged: the `feat3304` JS file stays owned by its own Python gate, which changes only by adopting the shared guard and gaining `@pytest.mark.timeout(240)` (Proposed Solution 3).
...
- A regression test reproduces the race deterministically — deferred digest released past the old 50-turn budget; no wall-clock, no threadpool dependence — and fails against the old spin helper while passing with the exact wait, in `scripts/tests/js/policy_submission.test.mjs`. The old helper is deleted in the same change, so the fails-before/passes-after demonstration lives in the PR description (the recorded output from the implementation phases below); the reviewer checks it there. No deliberately failing revision is committed, and no dead copy of the spin helper is kept in-tree to make the demo self-contained
- Rejected-digest and already-refused-review tests prove `settleHash()` finishes on refusal, releases its subscription when present, and lets `reviewed()` reject through its readiness assertion outside the listener. Delayed success still unsubscribes normally. The listener reads `state.review.status` from its callback argument and resolves on any non-`hashing` value (including `none`); no `settleHash()` call spans `dispose()`
...
- The probed `node --version` output from the guard is present in an uploaded `unit-tests` artifact (`pytest.log` or `pytest-junit.xml`) of that same passing post-merge run, and the closure note quotes it. Mechanism is the implementer's choice; the AC is the retained value, not the method
- `scripts/tests/test_conftest_cap.py::TestNoParallelMarkerRouting` still passes, and no other test's marker semantics change
- All four Node-dependent tests (`test_node_conformance_suite_passes`, `test_round_trip_yaml_validates_for_each_mode`, `TestDashboardNodeRuntimeGate::test_generated_page_runtime_behaviour`, and `test_rlhf_svg_evaluate_smoke.py::test_smoke_harness_survives_non_string_pageerror_message`) use the public `require_node(min_major=...)` from `scripts/tests/helpers.py`; the duplicate `_node_major` in `test_feat3304_artifact_dashboard.py` and the private `shutil.which`/skip in the `rlhf` smoke test are removed. The `rlhf` smoke test passes `min_major=None` and keeps no Node 22 floor. Mocked tests cover missing Node, Node <22, malformed output, nonzero probe exit, `OSError`, and probe timeout: each fails with `LL_REQUIRE_NODE=1` and skips with the variable unset. Node >=22 with a successful probe is accepted; with `min_major=None`, any successfully-probed major is accepted. Diagnostics preserve available stdout/stderr, exit status, or exception details without a second probe; missing Node explicitly reports the missing executable
- `LL_REQUIRE_NODE` is not listed in `_CMD_RUN_ENV_VARS` (`scripts/tests/conftest.py`), so the autouse scrub fixture cannot neutralize it
- `test_round_trip_yaml_validates_for_each_mode` behaves identically apart from adopting the shared guard. The `feat3304` dashboard runtime gate changes in exactly two ways: it adopts the shared guard, and it gains `@pytest.mark.timeout(240)` above its inner 180-second subprocess timeout, closing the same watchdog inversion the policy gate had
...
- **The "Node 20 → 24 migration" framing is unsupported.** The exact Node major was never captured in the failure evidence — the artifacts contain `pytest.log`/`pytest-junit.xml`, and the only version detail visible is the executable path `/usr/local/bin/node`. What *is* established: the gate skips below major 22, and the failing run executed 231 node subtests, so the runner had Node >=22. This checkout's `node --version` is v26.0.0. The evidence establishes a synchronization defect; it does not establish or rule out Node-version influence on its frequency.
...
- **Node provisioning is a fallback, not a plan:** the runner already had Node >=22 when the gate last executed. Only if the first `LL_REQUIRE_NODE=1` run fails the guard does the `actions/setup-node` fallback in Proposed Solution 4 apply. No version pin is needed to explain or fix the hash race itself.
...
- `scripts/tests/test_policy_builder_node_gate.py` — module docstring (L1-17) says "no hosted CI … single enforced location is the local suite" and "this repo's environment ships Node 22"; both are stale (`.github/workflows/ci.yml` exists; this checkout ships 26; the runner is confirmed >=22 but its exact major is not captured) — update alongside any resolution [Agent 1 finding; runner-version claim corrected 2026-09-22]
- `scripts/tests/test_feat3304_artifact_dashboard.py` — `_node_major` (~L1141) and the silent skip in `TestDashboardNodeRuntimeGate::test_generated_page_runtime_behaviour` (~L1171) are the third Node gate; it adopts the shared guard, drops its private copy, and gains `@pytest.mark.timeout(240)` over its inner `timeout=180` `node --test` subprocess (same watchdog inversion as the policy gate) [review addition, 2026-09-22]
- `scripts/tests/helpers.py` — the existing shared test helpers module; home of the public `require_node()` guard. No new helper module is created [review addition, 2026-09-22]
- `scripts/tests/test_rlhf_svg_evaluate_smoke.py` — `test_smoke_harness_survives_non_string_pageerror_message` (~L101) is the fourth Node-dependent test: a private `shutil.which("node")` plus silent `pytest.skip`, no version floor. It adopts the shared guard with `min_major=None` [review addition, 2026-09-22]
- `scripts/tests/conftest.py` — `_CMD_RUN_ENV_VARS` (~L1180) must **not** gain `LL_REQUIRE_NODE`; see Proposed Solution 4 [review addition, 2026-09-22]
- `scripts/tests/test_decisions_yaml_gate.py` — the `validator` fixture docstring cites `test_policy_builder_node_gate.py:52-57` as "the canonical skip-when-missing idiom"; replace the line-number reference with a file/function reference (`require_node` in `scripts/tests/helpers.py`) and reword: after this change the Node guard is skip-when-missing only with `LL_REQUIRE_NODE` unset, not an unconditional skip template [review addition, 2026-09-22]
- `scripts/tests/test_decisions_yaml_pre_commit_gate.py` — the module docstring cites the `test_node_conformance_suite_passes` "template at `scripts/tests/test_policy_builder_node_gate.py:45-71`"; same treatment — file/function reference, and describe the guard as fail-or-skip by `LL_REQUIRE_NODE` rather than a skip template [review addition, 2026-09-22]
...
- `settleHash(env) -> Promise<void>` and `reviewed(env, yaml = YAML) -> Promise<void>` — in `scripts/tests/js/policy_submission.test.mjs`; the bounded turn spin becomes an `onChange` subscription whose listener reads `state.review.status` from its callback argument and resolves on the first emit where it is not `"hashing"` — `ready`, `refused`, or `none` (returning immediately if not hashing), unsubscribes, then `await flush()` — an exact wait on the state the assertion reads; assertions stay in `reviewed()`; the wait must not span `dispose()`
...
- `TestDashboardNodeRuntimeGate.test_generated_page_runtime_behaviour(self, project) -> None` — in `scripts/tests/test_feat3304_artifact_dashboard.py`; calls `require_node()` instead of its private probe and gains `@pytest.mark.timeout(240)`
- `require_node(min_major: int | None = 22) -> str` — public, in `scripts/tests/helpers.py`; the single shared entry point used by all four Node-dependent tests (the `rlhf` smoke gate passes `min_major=None`, accepting any successfully-probed major). Probes `node --version` exactly once per call. Under `LL_REQUIRE_NODE=1` converts unavailable/unusable Node into failure; otherwise skips. Includes probe diagnostics when available (stdout/stderr, exit status, exception/timeout details), or an explicit executable-not-found message. Nonzero exit or a failed probe never yields a usable major. How the probe result is represented internally (tuple, small dataclass, private function) is an implementation detail of `helpers.py`; a public `node_major()` is only added if a caller needs the major on its own, and none does today
- `pytest_collection_modifyitems(config, items) -> None` — in `scripts/tests/conftest.py`; unchanged, only this test's marker changes

### Call Path

`test_node_conformance_suite_passes` -> `require_node` (in `scripts/tests/helpers.py`; single internal `node --version` probe) -> `subprocess.run` (`node --test scripts/tests/js/*.test.mjs`, 180s timeout) -> `policy_submission.test.mjs` `reviewed` -> `settleHash` -> `createSubmissionController.onChange` (resolves on the first non-`hashing` emit: `ready`/`refused` from the digest handlers, or `none` from a `contextChanged` reset) <- `emit` <- `startReview`'s `sha256Hex(...).then(...)` <- `crypto.subtle.digest`
...
_History, in one paragraph: the capture-era theory was a timeout budget mismatch (xdist contention starving the 180s subprocess, or the 120s pytest-timeout watchdog hard-killing the worker), which motivated the `no_parallel` marker and an "add a serial `-n 0` CI step" wiring. The CI artifacts falsify it: no worker kill, 0.24% of the budget used, same assertion every time. The one surviving point — the watchdog/inner-budget inversion is a real latent hazard — is carried by Proposed Solution 3's `timeout(240)` marker. A "Node 20 → 24" attribution was also floated and is unsupported (see Notes)._
