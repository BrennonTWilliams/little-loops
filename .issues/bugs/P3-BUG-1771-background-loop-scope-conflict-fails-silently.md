...
- **Stderr reporting**: Use `logger.error()` for the one-line conflict summary and `logger.info()` for supporting detail lines (pattern from `cmd_run()` lines 316-320). `Logger` is already instantiated in `run_background()` at line 925.
...
1. In `run_background()` (`_helpers.py` line 912), after `logger` initialization (line 925) but before `_make_instance_id()` (line 930), add:
...
2. If conflict found and `--queue` not set: report via `logger.error()` + `logger.info()` detail lines (follow pattern from `run.py` lines 316-320), return 1
