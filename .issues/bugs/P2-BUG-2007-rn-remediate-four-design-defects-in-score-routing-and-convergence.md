...
### Defect 1 — `wire` unconditionally chains to `refine` (line 264)
...
### Defect 2 — `diagnose` uses hardcoded magic thresholds (lines 182–186)
...
### Defect 3 — `check_convergence` compares against stale initial `pre_scores` (lines 314–386)
...
2. Locate the `wire` state — `on_success: refine` is hardcoded (line 264); routing skips re-assessment regardless of outcome
3. Locate the `diagnose` state (lines 182–186) — routing uses literals `15` and `50` not derived from context parameters
4. Locate the `check_convergence` state (lines 314–386) — `pre_scores_$ID.json` is written once and never refreshed; run two remediation passes and observe cumulative vs. pass-over-pass delta discrepancy
5. Trace: `check_readiness` (line 97) fails both-threshold test → `check_outcome` (line 110) passes outcome-only test → `diagnose` first rule checks both thresholds again → `route_d_implement` can never fire from this branch
...
| `diagnose` shell action | 182–186 | Replace hardcoded `15` (ambiguity, complexity, change_surface) and `50` (confidence floor) with `${context.diagnose_*}` references |
| `wire` | 264 | Change `on_success: refine` → `on_success: re_assess`; keep `on_error: refine` |
| `check_outcome` | 128 | Change `on_yes: diagnose` → `on_yes: refine` |
| `check_convergence` shell action | ~383 | Append `cp "${context.run_dir}/post_scores_${ID}.json" "${context.run_dir}/pre_scores_${ID}.json"` before the emit token |
...
2. **Parameterize `diagnose` thresholds (Defect 2)** — In the `diagnose` shell action (lines 182–186), replace literals with context references:
   - Line 182: `"$AMBIGUITY" -ge 15` → `"$AMBIGUITY" -ge "${context.diagnose_ambiguity_threshold}"`
   - Line 184a: `"$COMPLEXITY" -ge 15` → `"$COMPLEXITY" -ge "${context.diagnose_complexity_threshold}"`
   - Line 184b: `"$CONFIDENCE" -lt 50` → `"$CONFIDENCE" -lt "${context.diagnose_confidence_floor}"`
   - Line 186: `"$CHANGE_SURFACE" -ge 15` → `"$CHANGE_SURFACE" -ge "${context.diagnose_change_surface_threshold}"`

3. **Add pass-over-pass `pre_scores` refresh (Defect 3)** — At the end of the `check_convergence` shell action, before the emit token (around line 383), append:
   ```sh
...
4. **Reroute `wire` on success (Defect 1)** — Change `wire.on_success` from `refine` to `re_assess` (line 264):
   ```yaml
   # Before
   on_success: refine
   # After
...
5. **Fix dead path: reroute `check_outcome on_yes` (Defect 4)** — Change `check_outcome.on_yes` from `diagnose` to `refine` (line 128):
   ```yaml
   # Before
   on_yes: diagnose
   # After
