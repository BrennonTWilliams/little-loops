...
**Also explicitly unchanged (within-run cancellations, orchestrator path)**: `_maybe_complete_epic` has a second gate after `all_done` — the partial-failure gate `failed_here = epic_child_ids & (failed_ids | failed_issues | skipped_ids)` (`orchestrator.py:1488`), and `mark_skipped` fires for any `was_blocked` worker result (`orchestrator.py:1275`). A child cancelled *during* the run may therefore land in `skipped_ids` and veto the merge for the rest of that run even after this fix. That is deliberate, pre-existing, conservative same-run behavior and is **out of scope** — this fix makes the gate resolvable for children cancelled *before* the run (the permanent-block class). Tests for this fix must set cancelled statuses on disk before the completion event, not cancel mid-run, or the merge will be "mysteriously" vetoed by the failure gate rather than the condition under test.
...
- The safety exposure the exclusion could have protected against — a child whose commits landed on the epic branch and was *then* cancelled getting its partial work merged to base — is mitigated: (a) in the orchestrator path, child work only reaches the epic branch on successful worker completion (`orchestrator.py:1216,1286`), so a cancelled-before-done child leaves nothing on the branch; (b) the `verify_before_merge` gate, **when enabled**, runs the full test suite against the branch tip before any merge, covering the FSM/manual path where a later-cancelled child's commits could be present. Caveat (2026-08-31 review): `verify_before_merge` defaults to `false` (`config-schema.json:428-431`) and is `false` in this repo's own config, so mitigation (b) is opt-in — on the FSM/manual path with default config, a later-cancelled child's partial commits could merge unverified. This residual risk is accepted (it strengthens the case for the optional three-way `done`+`cancelled`+`blocked` test listed in the wiring pass, and for users enabling `verify_before_merge` on epics with churn).
...
2. Extend the BUG-2637 tip-read union in `merge_epic_branch` for the new terminal status: the YAML's inline Python unions epic-branch-tip statuses into the base scan via `git grep -lE '^status: *done'` only. Once `cancelled` counts as terminal, a child cancelled on the epic branch tip (status not yet on base) would still read as `open` → `held_open`. Widen the pattern to `^status: *(done|cancelled)` — either mapping each matched file to its actual tip status, or running two greps and overriding `iss.status` accordingly (the current code force-sets `iss.status = "done"` for every match, which would be wrong for a cancelled match under a single widened grep).
...
4. Verify existing tests for the fully-`done` and any-`blocked` cases still pass unchanged in both test modules, and audit the other epic-gate tests in `test_orchestrator.py` (lines ~1598-1952) for interaction effects.
