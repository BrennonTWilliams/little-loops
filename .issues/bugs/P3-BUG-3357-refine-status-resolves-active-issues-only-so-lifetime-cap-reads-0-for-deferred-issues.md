...
- Doc-sync requirement promoted to an explicit action (remediation pass, 2026-08-29): none of the five numbered Implementation Steps above, nor the two Wiring Phase bullets, touch `docs/reference/CLI.md` — yet Impact's own Codebase Research Findings correct the Effort estimate to say the fix "therefore touches 4 files total, not 3" specifically because of this doc edit. Required action: after the id-filter branch lands, edit `docs/reference/CLI.md:1465`'s `ISSUE-ID` row (currently "Exits 1 if the issue is not found") so it states the lookup resolves any status and reports the not-found message on stderr. Step 5's `pytest scripts/tests/test_refine_status.py scripts/tests/test_issue_parser.py` run does not exercise `docs/`, so this edit cannot be caught by that verification step and must be done and reviewed as its own action.
- Test-assertion gap closed (remediation pass, 2026-08-29): Step 3's instruction to update `test_single_issue_not_found` must assert the corrected message text verbatim, not just the channel/exit code. The exact target string this pass's own Codebase Research Findings established (pattern-finder confirmation above) is `Error: Issue '{id}' not found.` printed to stderr. The updated test must assert against that literal (e.g. `err == f"Error: Issue '{id}' not found.\n"`), not a loose substring check like `"not found" in out`/`"not found" in err` — the current test's substring form (`test_refine_status.py:2081`) passes whether or not the corrected wording is adopted, so it cannot catch a fix that changes only the channel but leaves the stale "in active issues" phrasing in place.
- Step 3 status-coverage ambiguity resolved (remediation pass, 2026-08-29): the parenthetical "a deferred (or done/cancelled)" is satisfiable by a single test covering only one status. Root Cause (`issue_parser.py:4089-4092`) and Expected Behavior ("resolves any status") both name all three excluded statuses (`done`, `cancelled`, `deferred`). The new coverage Step 3 calls for must exercise all three — either three discrete test functions or one parametrized test (e.g. `@pytest.mark.parametrize("status", ["deferred", "done", "cancelled"])`) — each asserting the resolved numeric `refine_count` (per the assertion-value gap already noted above in this section), not merely that the issue is found. A test for only one status leaves the other two unverified.
- Consolidated current-state checklist (remediation pass, 2026-08-29) — assembled from the five numbered Implementation Steps above, the Wiring Phase bullets, and every correction appended below them in this section; provided as a single reading path, it does not reorder, replace, or remove any entry above:
  1. Resolve the id-filter branch via `resolve_issue_path()`/`IssueParser.parse_file()` (Option A) so it works for any status.
  2. Change only the stdout→stderr channel of the not-found message; exit code stays `1` (per Step 2's correction below it).
  3. Update the not-found message text to `Error: Issue '{id}' not found.` (capitalized "Issue", no "in active issues" qualifier — per the pattern-finder confirmation above).
  4. Update `test_single_issue_not_found` to assert the corrected stderr text verbatim (per the test-assertion gap closed above).
  5. Add new test coverage for all three of `deferred`, `done`, and `cancelled` (parametrized or discrete), each asserting the resolved numeric `refine_count` equals a known prior value (per the status-coverage ambiguity resolved above).
  6. Extend `_make_issue()` (`test_refine_status.py:19-32`) with a `status` parameter — it accepts none today and has no catch-all frontmatter mechanism, so a literal `_make_issue(..., status="deferred")` call raises `TypeError` until this is added.
  7. Re-derive the `_ALLOWLIST` line number in `test_issue_parser.py::TestPriorityRegexCompletenessAllowlist` (currently `533`) and update the stale comment at `test_issue_parser.py:1627`, per the Wiring Phase above.
  8. Update `docs/reference/CLI.md:1465`'s `ISSUE-ID` row per the doc-sync requirement above — untested by Step 5's pytest run, so it needs manual confirmation.
  9. Run `python -m pytest scripts/tests/test_refine_status.py scripts/tests/test_issue_parser.py -v` (Step 5) to confirm items 1-7; it does not cover item 8.
