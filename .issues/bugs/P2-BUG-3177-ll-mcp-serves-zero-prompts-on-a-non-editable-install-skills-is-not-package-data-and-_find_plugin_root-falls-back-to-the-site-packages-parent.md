...
- `scripts/pyproject.toml` — add `[tool.hatch.build.targets.wheel.force-include]` with
...
3. in-package `Path(__file__).parent / "skills"` (the wheel copy A′ ships) if `.is_dir()`
...
2. Ship `skills/` into the wheel via Option A′'s `force-include` (three lines in
   `scripts/pyproject.toml`, no file relocation, `plugin.json` untouched). Add one
   sentinel/glob entry to `ll-verify-package-data` for gate coverage — not 69 entries.
