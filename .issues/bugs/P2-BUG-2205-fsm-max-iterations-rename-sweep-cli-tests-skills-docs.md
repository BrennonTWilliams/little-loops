...
completed_at: '2026-06-17T22:01:22Z'
...
- `scripts/little_loops/fsm/validation.py` — **already complete (done by BUG-2204)**
  - `known_top_level_keys` (`:158-161`): `"max_steps"`, `"on_max_steps"`, `"max_iterations"`, `"on_max_iterations"` already added ✓
  - Numeric range checks: `max_steps > 0` (`:977`) and `max_iterations > 0` (`:984`) already in place ✓
  - `_validate_on_max_steps()` already exists at `:1625` ✓
  - `_validate_on_max_iterations()` already exists at `:1646` ✓
...
- `docs/reference/schemas/max_iterations_reached_summary.json` — new file (auto-generated by BUG-2204 step 6; **confirmed exists on disk** as of BUG-2204 landing) ✓
...
Completed the rename sweep across all CLI argparse, display strings, loop YAMLs, skills,
docs, and tests. Fixed 18 test failures introduced when the 84 built-in loop YAMLs were
renamed from `max_iterations: N` → `max_steps: N`. All acceptance criteria met.

- `helpers.py`: `make_test_fsm(max_iterations=50)` → `make_test_fsm(max_steps=50)`
- `test_builtin_loops.py`: 13 `test_max_iterations_and_timeout[_defined]` → `test_max_steps_*`
- `test_deep_research.py`, `test_deep_research_arxiv.py`, `test_rn_plan.py`, `test_rn_plan_apo.py`,
  `test_loop_composer_adaptive.py`, `test_general_task_loop.py`: updated assertions to `max_steps`
- `test_ll_loop_parsing.py`: updated `_create_run_parser` to use `--max-steps` as `-n`
- `test_review_loop.py`: fixed `max_iterations = 50` variable shadowing → `max_steps = 50`

## Session Log
- `/ll:manage-issue` - 2026-06-17T22:01:22 - `c0e8a127-3a38-4edb-890b-a2b7278dcac0.jsonl`
- `/ll:ready-issue` - 2026-06-17T20:36:25 - `c0e8a127-3a38-4edb-890b-a2b7278dcac0.jsonl`
