...
2. **`scopes: []` bypasses validation** — `scripts/little_loops/fsm/validation/structural_rules.py` guards both the unknown-scope check and the shell-only check with `if state.scopes:` (truthy) instead of `is not None`. The runtime already treats `[]` as a *declaring* state: `fsm/runners.py` (`if scopes is not None`) redirects `GH_CONFIG_DIR` and denies every env var (pinned by `test_shell_declared_empty_scopes_redirects_config_dir_no_token`), and `fsm/executor.py:2567` writes a `credential_scope_events` row for it. Validation is the outlier: `scopes: []` on a **non-shell** state skips the shell-only rejection, so the executor writes an audit row claiming a grant while the prompt/skill state scopes nothing — a misleading audit entry with no error anywhere. No validation-layer test exercises an empty scope list.
...
1. **Unguarded `gh` probe**: On a host without `gh` on `PATH`, run an FSM loop with a shell state declaring `scopes: [github]`. In `scripts/little_loops/fsm/runners.py`, the call to `gh_scope_extra(Path(gh_tmp.name), with_token="github" in scopes)` (line 338) invokes `subprocess.run(["gh", "auth", "token"], ...)` inside `host_runner.py`'s `gh_scope_extra()`, which raises `FileNotFoundError`, not `RuntimeError`. Observe: the exception escapes the `except RuntimeError` handler, aborts the whole loop run instead of failing just that state, and leaves the `ll-gh-*` temp dir behind (`gh_tmp.cleanup()` never runs).
2. **`scopes: []` bypass**: Author an FSM state with an explicit empty scope list (`scopes: []`) on a non-shell state type. Run `ll-loop validate` (or trigger the structural rule directly in `scripts/little_loops/fsm/validation/structural_rules.py`). Observe: `if state.scopes:` at line 499 is falsy for `[]`, so neither the unknown-scope check at line 500 nor the shell-only check at line 511 runs — no validation error. Run the loop: the executor writes a `credential_scope_events` row for the state (it checks `is not None`) even though a non-shell state never consults `scopes`.
...
1. `gh_scope_extra()` bounds the probe with an explicit `timeout=10` (a headless macOS Keychain prompt is the concrete hang case) and converts `FileNotFoundError`/`TimeoutExpired`/`OSError` into the same `RuntimeError` contract the callers already handle; the FSM shell path cleans up the temp dir on every failure path.
2. `scopes: []` is validated with `is not None`, matching the runtime's own semantics: `[]` on a **shell** state is a valid "declare nothing, deny all" grant (already honoured and tested at the execution layer — do not change it); `[]` on a **non-shell** state is rejected by the existing shell-only check with its existing message. The decision is made; the implementer only pins it with validation-layer tests for both cases.
3. The CMD runner in `runner_spec.py` applies `gh_scope_extra()` whenever `spec.scopes is not None` (with `with_token="github" in spec.scopes`, mirroring `fsm/runners.py`) and records the grant via `write_credential_scope()` so both dispatch paths produce identical audit rows. Temp-dir lifecycle: `gh_tmp.cleanup()` runs on the immediate `gh_scope_extra()` failure return **and** in a `finally` that spans the `Popen`, the selector drain loop, and the timeout-kill branch — not just the failure return. A `RuntimeError` from the probe returns `RunnerResult(exit_code=2, error=str(exc))` without spawning, matching the adjacent `ValueError` branch (the CMD runner's infra-failure convention is `exit_code=2`; the FSM path's is `1`).

**Run-id decision (resolved):** add a keyword-only `run_id: str | None = None` parameter to `run_action()` and `_run_cmd()`. `cli/queue.py`'s drain loop passes `entry.id`; `cli/harness.py` and `cli/action.py` omit it and the runner falls back to `spec.name`. The audit row uses `state=spec.name`. Every declaring dispatch writes a row regardless of caller — skipping non-queue callers would recreate the audit blind spot the epic AC forbids. Do **not** add `run_id` to `ActionSpec` itself: it is frozen and round-tripped by `queue_store._serialize_action`/`_deserialize_action`, and an id is a property of the dispatch, not the spec.
...
- `gh_scope_extra(config_dir: Path, *, with_token: bool) -> dict[str, str]` (`scripts/little_loops/host_runner.py:186`) — wrap the `subprocess.run(["gh", "auth", "token"], ...)` probe so `FileNotFoundError`/`TimeoutExpired`/`OSError` raise the same `RuntimeError` the docstring already promises, and add `timeout=10`. Signature unchanged.
...
- `run_action(spec: ActionSpec, *, run_id: str | None = None) -> RunnerResult` (`scripts/little_loops/runner_spec.py:362`) — new keyword-only, optional; forwarded to the handler for `RunnerType.CMD` only.
- `_run_cmd(spec: ActionSpec, *, run_id: str | None = None) -> RunnerResult` (`scripts/little_loops/runner_spec.py:237`) — new keyword-only, optional; `run_id or spec.name` is the audit key.
- `write_credential_scope(db_path: Path | str, *, run_id: str, state: str, scopes: frozenset[str], var_names: frozenset[str], ts: str | None = None) -> bool` (`scripts/little_loops/session_store/writers.py:1945`) — call from the CMD runner path with `run_id=run_id or spec.name`, `state=spec.name`, `db_path=resolve_history_db()`, wrapped in `except Exception: pass` exactly like `fsm/executor.py:2568-2582`.

### Call Path

`cli/queue.py:480` `run_action(entry.action, run_id=entry.id)` (**new** kwarg) -> `scripts/little_loops/runner_spec.py` `_run_cmd(spec, run_id=...)` (~line 251) -> `resolve_scopes(spec.scopes)` (existing) -> **new**: `write_credential_scope(resolve_history_db(), run_id=run_id or spec.name, state=spec.name, ...)` (best-effort, mirroring `fsm/executor.py:2567-2582`) -> **new**: `gh_tmp = TemporaryDirectory(prefix="ll-gh-")` + `gh_scope_extra(Path(gh_tmp.name), with_token="github" in spec.scopes)` (mirroring `fsm/runners.py:334-346`) -> existing `Popen(... env=project_child_env(extra=extra, env_allow=env_allow))` -> **new** outer `finally: gh_tmp.cleanup()` spanning drain + timeout-kill.

### Wiring Phase (added by `/ll:wire-issue`)

_These touchpoints were identified by wiring analysis and must be included in the implementation:_

- Thread the run-identifier per the resolved decision in Expected Behavior 3: `run_action(spec, *, run_id: str | None = None)` and `_run_cmd(spec, *, run_id=None)` (`runner_spec.py:362,237`); `cli/queue.py:480` passes `entry.id`; `cli/harness.py:833,866,909,942` and `cli/action.py:250,304` are left unchanged and get the `spec.name` fallback. The keyword is optional so `test_cli_queue_run.py`/`test_feat_queue_mcp_tools.py`'s existing positional `run_action(spec)` calls keep working.
...
- Add a CMD-runner isolation test in `test_runner_spec.py` mirroring `test_fsm_runners.py::TestDefaultActionRunnerShellPath` (`scopes=["github"]`, `scopes=[]`, RuntimeError-before-`Popen` cleanup, timeout cleanup) per the Tests subsection above. These tests `patch("subprocess.Popen")` and patch `gh_scope_extra` (or set `GH_TOKEN` in the env so no real probe runs); assertions are on the `env=` kwarg captured from the `Popen` call, never on a real `gh` invocation.
...
- [ ] `gh_scope_extra()` with `gh` absent from `PATH` raises `RuntimeError`, not `FileNotFoundError`; a `subprocess.TimeoutExpired` from the probe also surfaces as `RuntimeError`; the probe passes `timeout=10`. Tests patch `little_loops.host_runner.subprocess.run` to raise each of `FileNotFoundError`/`TimeoutExpired`/`OSError`.
- [ ] A shell state declaring `scopes: [github]` on a host without `gh` fails that state with `exit_code=1` (the FSM path's existing scope-failure convention) and the run continues per normal FSM error handling; no `ll-gh-*` temp dir is left behind.
- [ ] Validation: `scopes: []` on a shell state passes `_validate_*` scope rules with no error; `scopes: []` on a non-shell state is rejected by the shell-only check. Two tests in `TestScopesValidation` pin both cases. The execution-layer behaviour of `scopes=[]` (`test_shell_declared_empty_scopes_redirects_config_dir_no_token`) is unchanged.
- [ ] A CMD `ActionSpec` with `scopes: {github}` spawns its child (mock `Popen`) with an `env=` whose `GH_CONFIG_DIR` points at a fresh `ll-gh-*` tempdir and whose `GH_TOKEN` is the value returned by `gh_scope_extra()`; a CMD `ActionSpec` with `scopes: frozenset()` gets the `GH_CONFIG_DIR` redirect and no `GH_TOKEN`. Asserted on the captured `Popen` env, not by running `gh`. (Real-`gh` behaviour on keychain-backed macOS is BUG-3402's scope, not this issue's.)
- [ ] A CMD `ActionSpec` declaring scopes on a host where `gh_scope_extra()` raises `RuntimeError` returns `RunnerResult(exit_code=2, error=...)` with `Popen.assert_not_called()` and no `ll-gh-*` tempdir left behind; a timed-out scoped CMD child also leaves no tempdir behind.
- [ ] A CMD `ActionSpec` with any declared scopes (including `frozenset()`) produces exactly one `credential_scope_events` row: `run_id=entry.id` when dispatched from `ll-queue`'s drain loop, `run_id=spec.name` from any other caller; `state=spec.name`; `scopes_json`/`var_names_json` contain names only. An undeclared spec (`scopes=None`) writes no row.
