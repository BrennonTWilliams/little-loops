...
- The stale-score leak is live in a shipped loop, not just a fragment-level hypothetical: `scripts/little_loops/loops/policy-refine.yaml` routes `light_repair`/`deep_repair`/`rethink` back to `score` via `next: score`, so pass two re-parses into a run dir still holding pass-one `rubric-dim-*.txt`. The same loop's dispatch declares `_: deep_repair` and `_error: done`, the identical success-misroute shape as the generated decision table.
...
- The `policy-router.yaml` header advertises `_error` as an "optional error fallback" on the same state whose contract says `_` is required, so the documented pattern is one whose `_error` is dead by construction.

## Expected Behavior

Fresh scoring cannot use previous-pass evidence: every `policy_parse_scores` pass clears `rubric-dim-*.txt` and `rubric-aggregate.txt` before reading/parsing its input and publishing new scores. Other run artifacts survive. Recognized partial scoring remains valid under the contract below; wholly unparseable output fails. An omitted `AGGREGATE:` line produces no aggregate file, exactly as an omitted dimension produces no dimension file: routing never sees a score the scorer did not emit. Generated decision-table **and issue-lifecycle** loops route scoring, parsing, and dispatch failures to an explicit failure terminal, without executing user outcome actions, even when the normal fallback is a success outcome. The shipped `policy-refine.yaml` receives the same failure routing. `ll-loop validate` warns when a classify state declares both `_` and `_error`, since that `_error` is unreachable for verdict routing.
...
- Emit `failed: {terminal: true, failure: true}` in `_serializeDecisionTable()`. Set `on_error: failed` on `score`, `parse_scores`, and `policy_dispatch`. In the dispatch route table emit both **`error: failed`** and `_error: failed`, retaining `_` for ordinary unmatched verdicts. The explicit `error` entry handles nonzero exits/timeouts converted to evaluator error verdicts; dispatch `on_error` handles action exceptions, signal kills, and non-recoverable failures (the paths in `_run_action_or_route` that consult `on_error` directly, which route tables otherwise bypass). Keep shared executor precedence unchanged in this issue; any general precedence correction requires separate regression coverage and scope.
- **Lifecycle mode gets the same one-line fix:** `_serializeIssueLifecycle()` already emits `failed:` and `on_error: failed`; add `error: failed` to its dispatch route table and mark `failed` with `failure: true`. The mechanism, test shape, and fixture regeneration are identical to the decision-table change, so excluding it would knowingly ship one fixed and one broken generated shape from the same file. Lifecycle outcome-action semantics beyond dispatch routing remain out of scope.
- **Fix the shipped caller, not just the fragment:** `policy-refine.yaml` gains a `failed: {terminal: true, failure: true}` state, `on_error: failed` on `score`/`parse_scores`, and `error: failed` on its dispatch (replacing `_error: done`). Its `light_repair -> score` cycle is the canonical two-pass reproduction; base the two-pass regression on that shape.
- **Validator lint for the dead sentinel:** add a `ll-loop validate` warning when a state with `evaluate: {type: classify}` declares both `_` and `_error`, stating that `_error` is unreachable for verdict routing and recommending an explicit `error:` route. Rewrite the `policy-router.yaml` header's routing-handoff example accordingly (`error: failed` instead of "`_error: done` optional error fallback"). This is a lint, not an executor change.
- **Collision handling lands here:** before serialization, reject authored `error` or `failed` tokens in outcome names, rule targets, or fallback (all sources of generated outcome states/route keys), with an actionable error asking the author to rename the token and its references. Never silently rename or emit duplicate YAML keys. BUG-3486 later integrates these same reserved names into its broader model/browser validation; this issue cannot depend on that later validation to emit safe YAML. Existing drafts using those names require migration before re-export. Note that `error` was never a usable outcome name: the executor already emits an `error` verdict on any classify nonzero exit, so an authored `error` outcome was silently hijacked before this issue. The guard fixes a latent defect rather than introducing a new restriction. `failed` is reserved (rather than dodged the way `_doneStateName()` dodges `done`/`finished`) because it matches `FAILURE_TERMINAL_NAMES`, the lifecycle precedent, and `on_max_steps: failed` conventions; a dodged name would break those.

## Implementation Steps

1. Add behavioral regressions for two-pass scoring and generated failure routing. Model the two-pass test on `policy-refine.yaml`'s `light_repair -> score` cycle. Use a success fallback so `_` shadowing `_error` is exercised; inject scoring/parsing nonzero exits, dispatch nonzero exit/timeout, and a dispatch action exception. Assert the failure result and that no user outcome action runs. Cover both `_serializeDecisionTable()` and `_serializeIssueLifecycle()` output. Add a case asserting an omitted `AGGREGATE:` line leaves no `rubric-aggregate.txt` and that an `aggregate:<N` rule then does not match.
2. Extract the parser module, implement cleanup and the scoring contract above (absent aggregate writes no file), and wire the fragment to invoke it with exit propagation. Add one actual interpolated-fragment execution test as well as pure parser/module tests.
3. Add the serializer collision guards, explicit failure terminal, upstream `on_error` routes, and dispatch `error`/`_error`/`on_error` routes to `_serializeDecisionTable()`; add `error: failed` and `failure: true` to `_serializeIssueLifecycle()`. Verify ordinary successful routing and unmatched-token fallback are preserved in both modes.
4. Update `policy-refine.yaml`: add the `failed` terminal, `on_error: failed` on `score`/`parse_scores`, and `error: failed` on dispatch in place of `_error: done`. Run `ll-loop validate` on it.
5. Add the `ll-loop validate` warning for classify states declaring both `_` and `_error` (`scripts/little_loops/fsm/validation/evaluator_rules.py`, alongside the existing evaluator lints), with a unit test for the warning and a test that the regenerated fixtures and `policy-refine.yaml` do not trigger it.
...
8. Update `docs/guides/POLICY_ROUTER_GUIDE.md` and the `policy-router.yaml` header/fragment contract to explain clean-slate/partial scoring, absent-aggregate semantics, failure routing, the dead-sentinel lint, and reserved-name migration. Replace the header's `_error: done` handoff example with `error: failed`. Document caller-supplied `on_error` on parsing states and explicit `error` routing on dispatch states. Audit any remaining fragment callers for impact from newly rejected wholly unparseable output and absent aggregate; do not claim all handwritten loops gain the builder's failure routes automatically.
...
- **Effort**: Moderate - parser extraction, behavioral execution tests, serializer guards in both modes, one validator lint, caller update, and fixture regeneration
...
- `scripts/little_loops/loops/policy-refine.yaml` — existing parser/dispatch fragment caller and the live reproduction of the two-pass leak (`light_repair`/`deep_repair`/`rethink` -> `next: score`); modified in this issue to gain the `failed` terminal and explicit `error:` routing.
- `scripts/little_loops/fsm/validation/evaluator_rules.py` (the validation package behind `ll-loop validate`; per-evaluator lints live here) — gains the `_`+`_error` dead-sentinel warning for classify states.
...
- `_serializeIssueLifecycle(model) -> string` — unchanged signature; adds `error: failed` to the dispatch route table and `failure: true` to the existing `failed` terminal. No other lifecycle change.
...
- [ ] Generated issue-lifecycle YAML emits `error: failed` in its dispatch route table and `failure: true` on `failed`; an execution test with a success fallback proves a dispatch `error` verdict reaches `failed`.
- [ ] `policy-refine.yaml` declares a `failed` failure terminal, `on_error: failed` on `score`/`parse_scores`, and `error: failed` on dispatch; it passes `ll-loop validate` with no dead-sentinel warning.
- [ ] `ll-loop validate` warns on a classify state that declares both `_` and `_error`; regenerated fixtures and shipped loops under `scripts/little_loops/loops/` do not trigger the warning.
...
Includes clean-slate LLM parsing, the explicit partial/invalid-output contract (absent aggregate is absent), generated decision-table and issue-lifecycle dispatch failure routing, the `policy-refine.yaml` caller fix, the `_`+`_error` dead-sentinel validator warning, and minimal serializer guards required to land safely. BUG-3486 owns broader browser/model validation and editing, consuming the reserved names established here; BUG-3490 owns catalog discovery.

Excludes shared executor precedence changes (the lint warns; it does not alter `_route()`), lifecycle outcome-action failure semantics beyond dispatch routing, arbitrary user outcome action-failure semantics, missing-dimension rule changes, general score-range/format validation, `lib/rubric-router.yaml`'s aggregate-only parser, and a transactional/generalized scoring framework. Handwritten loops other than `policy-refine.yaml` do not automatically receive failure routes; document the fragment caller contract and audit impact. Do not describe the entire lifecycle serializer as a proven failure-routing reference.
...
The LLM score parser lacks clean-slate handling, treats wholly unparseable output as a successful pass, and fabricates an aggregate of `0` when none was emitted. `_serializeDecisionTable()` omits upstream error routes and a dedicated failure terminal, reuses the first outcome token for `_error`, and supplies a normal fallback that takes precedence over that sentinel in the executor. The same `_`-shadows-`_error` pattern is present in `_serializeIssueLifecycle()`, `policy-refine.yaml`, and the fragment header's own handoff example, because nothing in validation flags a route table whose `_error` cannot be reached. Failure semantics must be established across the entire scoring/dispatch chain, not inferred from the presence of a terminal or `_error` field.
