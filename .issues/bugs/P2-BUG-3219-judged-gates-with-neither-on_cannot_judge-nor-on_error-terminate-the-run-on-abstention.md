...
- `check_semantic` in the harness templates: abstention means the judge could not see the evidence, so `execute` is wrong — it redoes work the judge already could not observe. **Route to the existing `failed` terminal**, following the precedent eight lines below the defect in the same file: `check_invariants` carries `on_error: failed  # ENH-2825: an uncheckable invariant is not a pass` (`harness-single-shot.yaml:170`). That is this codebase's established answer for "the gate could not evaluate", it needs no new state, and it keeps the template fix mechanical — which matters, because the templates are the propagation vector and should not wait on novel design.
...
- `check_semantic` in the three `harness-*` templates: route to the loop's failure terminal (`failed` in two of the three; a new one must be added to `harness-plan-research-implement-report.yaml`), per the `check_invariants` / ENH-2825 precedent — *not* `execute`, which redoes work the judge already could not observe, and not `check_semantic` itself, which would stack a self-loop on top of the ENH-3185 hold. The richer "produce the missing evidence and re-judge" route has no existing state to point at and is deliberately deferred to a follow-up (see Expected Behavior).
...
1. The three `harness-*` template `check_semantic` states gain `on_cannot_judge` routes to the loop's failure terminal (adding one to `harness-plan-research-implement-report.yaml`, which has none), and their inline `#` comments document the new route the same way the existing `on_partial` self-hold is commented, citing the ENH-2825 "an uncheckable gate is not a pass" rationale — this closes the propagation vector first and requires no new design.
2. `rn-build.yaml`/`rn-plan.yaml`'s `check_substrate` states gain `on_cannot_judge` routes to a deterministic probe state, modeled on the existing `check_install`/`check_structure` shell+`exit_code` shape rather than another `llm_structured` guess. This is the one sub-task in this issue that introduces a new state; if it turns out to need more design than the route additions, it can land separately from Steps 1/3/4 without blocking them.
3. `loop-specialist-eval.yaml`/`harness-multi-item.yaml`'s `check_skill` states gain an explicit `on_cannot_judge` route so the two-hold agentic re-simulation cost is skipped (`loop-specialist-eval.yaml` has no failure terminal today and needs one added if that is the chosen destination).
