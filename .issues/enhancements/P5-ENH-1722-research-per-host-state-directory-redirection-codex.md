...
- Per-host path branches key off `host`/`state_dir` params passed down from a single env-read boundary, not raw env reads inside leaf functions — evidence: `_config_candidates(project_root, *, host, state_dir)` (`scripts/little_loops/config/core.py:120-154`), called from `resolve_config_path()` (`:157-181`), which is the sole `os.environ.get("LL_HOOK_HOST"/"LL_STATE_DIR")` read for config-file scoping.
