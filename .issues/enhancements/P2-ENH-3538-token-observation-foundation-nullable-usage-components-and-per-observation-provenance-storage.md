...
Foundation slice extracted from ENH-3528: nullable token observations, per-observation provenance/host/scope/observation-time storage, unknown-default writers, and host-preserving replay. Lands independently, before BUG-3531 (Codex live input normalization), ENH-3532 (Codex historical rollout ingestion) and ENH-3528's reporting/export/staleness work. Adds no labels to `ll-ctx-stats` output and no new ingestion path.
...
- `scripts/little_loops/fake_host.py` (~256–276) — the keys it emits are forced through `int(d.args.get(..., 0))`, so it cannot send an explicit `null`. It also **already omits** a key on every event: `result` never emits `cache_creation_input_tokens`, and `turn_completed` never emits `cache_write_input_tokens`. Once the parser stops defaulting omissions to 0, every fake-host-driven test would silently get `cache_creation_tokens=None` → `cost=None`, a changed `action_complete` payload and an unknown `cost_graph` bucket. Fix: `result` emits `cache_creation_input_tokens` (default `0`, arg `create`) so existing fake-host tests stay complete-data; `turn_completed` keeps omitting `cache_write_input_tokens` by default (that is the real Codex shape under test) with an opt-in arg to emit it. Add per-key omit and explicit-`null` options so the partial-event fixtures (Implementation Step 1) can run end to end.
...
- `host_runner.py` `_usage_from_response` (~3092) — Anthropic API / batch path; keeps `provenance='unknown'` in this issue, sets `host`/`scope_kind='request'`. It currently coerces with `getattr(usage, "cache_read_input_tokens", None) or 0` (same for `cache_creation_input_tokens`); see Design → Anthropic SDK usage for the replacement rule.
...
- Schema-version pins (bump 53 → new version together with `SCHEMA_VERSION` and `schema_manifest.json`): `test_assistant_messages.py`, `test_cli_doctor_install_checks.py`, `test_cli_history.py`, `test_enh_2497_agent_type.py`, `test_enh_2511_mcp_telemetry.py`, `test_feat3304_artifact_dashboard.py`, `test_feat3410_workspace_quality.py`, `test_feat3445_workspace_activity.py`, `test_hook_session_start.py`, `test_session_store_lifecycle.py`, `test_session_store_schema.py`, `test_session_store_writers.py`. Re-grep for `SCHEMA_VERSION` / `53` before landing; the list drifts.
...
Land each step as its own commit, with the full suite green at every commit. A split into two issues was considered and rejected: BUG-3531 needs both the metadata columns and the nullable components, so splitting would not unblock it any earlier.

## Impact

- **Priority**: P2 — unblocks BUG-3531 and ENH-3528 reporting.
- **Effort**: Medium.
- **Risk**: Medium — nullable components ripple into cost/aggregation consumers; mitigated by no-missing-case parity tests. The Codex cache-write reversal carries no cost regression (Codex models are unpriced today); the main hidden churn is fake-host-driven tests, neutralized by the `fake_host` default change.
...
- **Nullable components.** `TokenUsage` token components become `int | None`. `TokenUsage` gains `provenance: TokenProvenance = "unknown"`, `host: str | None = None`, `scope_kind: TokenScopeKind = "unknown"`, `observed_at: str | None = None` and `observed_at_basis: ObservedAtBasis | None = None` (use the Literal aliases from Program Design, never bare `str`, so mypy catches a misspelled value) (default-compatible, same pattern as `is_batch`). Parsers keep missing components as `None` except where a verified host contract establishes that omission means zero. Codex's omitted `cache_write_input_tokens` is not established by an omission-only fixture or ENH-3464's assertion: use the matched-capture/producer-contract evidence requirement in BUG-3531. Until verified, preserve omission as `None`; the foundation need not wait for BUG-3531 or add a reverse dependency. Explicit `null` is never an omission-based zero.
  - **Supersedes ENH-3464 Decision 3.** That decision (quoted in the `usage_from_event` docstring) declared an omitted Codex `cache_write_input_tokens` "a real zero, not `None`". This issue reverses it: rewrite the docstring to say omission is unknown until BUG-3531 Decision 6 establishes otherwise, and cite this issue. Record the reversal in the decisions log (`ll-issues decisions`) so ENH-3464's rationale isn't silently contradicted.
  - **Cost impact of the reversal is nil today.** `MODEL_PRICING` has no OpenAI/Codex models, so live Codex rows are already unpriced (`cost_usd=None`). Making the omitted cache-write `None` changes token completeness only, not any reported cost.
  - **Claude `result` contract.** Real Claude Code `result` events always carry all four usage keys (`input_tokens`, `output_tokens`, `cache_read_input_tokens`, `cache_creation_input_tokens`), so dropping the `0` defaults changes nothing for complete events. It matters only for malformed/partial events and for the fake host (see Integration Map → `fake_host.py`).
...
- **Replay.** Add a metadata-aware usage iterator yielding host alongside the raw line; `_backfill_usage_events` uses it. Existing `_iter_events` tuple consumers are unchanged. The JSONL `list[Path]` source has no host, so it yields `host=None`. Transcript rows are written with `provenance='unknown'`, `scope_kind='request'`, `observed_at` = record `timestamp` with basis `'event'` (NULL/`None` basis when the record has no timestamp), and `host` from `raw_events.host`. `_backfill_usage_events` stops pricing with `int(x or 0)`: cost is `None` when a required component is missing.
  - **Known limitation: `raw_events.host` is the ingest-time host, not the transcript's.** `_backfill_raw_events` writes `effective_host = host if host is not None else resolve_host().name` into the column and never consults `handle.host` (`session_store/lifecycle.py` ~795/~808). An ingest that runs under a different configured host (e.g. `LL_HOST_CLI=codex` while Claude Code transcripts are ingested) mislabels those rows, and pre-ENH-3166 rows carry NULL. Replay copies the column faithfully and does not try to correct it; NULL maps to `host=None`, `provider_vendor=None`. Writing `handle.host` at ingest is a separate change to `raw_events` semantics and is out of scope here; capture it as a follow-up if it matters.
- **Anthropic SDK usage (`_usage_from_response`).** Replace the `or 0` coercion with the same rule as the stream parsers: a missing attribute or explicit `None` for `cache_read_input_tokens` / `cache_creation_input_tokens` stays `None`; a reported `0` stays `0`. The SDK types both fields as optional, so this path can now yield unpriced rows; a test covers `None` → `None` and `0` → `0`. `input_tokens` / `output_tokens` pass through unchanged.
...
- [ ] `fake_host` can omit a usage key or send explicit `null`; the partial-event fixtures use it. Its `result` event emits `cache_creation_input_tokens` by default, so existing fake-host tests keep complete-data results (no new `None` costs or unknown `cost_graph` buckets).
- [ ] A complete Claude `result` event (all four keys present) produces the same `TokenUsage` components, `action_complete` payload values, `usage_events` token/cost columns and `cost_graph` output as before this change.
- [ ] `_usage_from_response` keeps a `None`/absent SDK cache component as `None` and a reported `0` as `0`.
- [ ] The `usage_from_event` docstring no longer claims omitted Codex cache-write is a real zero, and the ENH-3464 Decision 3 reversal is recorded in the decisions log.
...
- [ ] Raw-event host survives backfill/rebuild through the new iterator; existing `_iter_events` consumers are unchanged. A NULL `raw_events.host` (and the JSONL `list[Path]` source) replays as `host=None`, `provider_vendor=None`.
...
- **`fake_host` correction:** it already omits `cache_creation_input_tokens` (`result`) and `cache_write_input_tokens` (`turn_completed`), so the parser change would flip every fake-host test to `None` cache-creation and cost. `result` now emits the key by default.
- **Vendor:** `vendor_for_runner(None)` returns `"other"`; `provider_vendor` is `None` when `host` is `None`.
- **Replay host:** `raw_events.host` is the ingest-time host, not `handle.host`; documented as a known limitation (no correction in this issue).
- **`_usage_from_response`:** its `or 0` coercion is replaced with the null-not-zero rule.
- **ENH-3464 Decision 3** reversal made explicit; no cost impact because Codex models are unpriced.
- **Claude `result`** complete-data invariance gets its own acceptance criterion; the schema-version pin list is expanded to the 12 test files that reference it.
