...
- **`refine-to-ready-issue` side-effect to handle**: on entry, `refine-to-ready-issue.yaml:21-26` writes `0` to `.loops/tmp/recursive-refine-broke-down`. If autodev invokes `refine-to-ready-issue` directly, autodev must read this file after the sub-loop returns (as `recursive-refine:check_broke_down` does at `recursive-refine.yaml:221-234`) to decide whether to still run `/ll:issue-size-review` or skip it. **Decision**: after the sub-loop returns, autodev copies the flag to `.loops/tmp/autodev-broke-down` and reads from the copy for the rest of the state machine. Keeps autodev's state machine readable without cross-loop spelunking; aligns with the rest of the `autodev-*` temp-file namespacing. Cross-loop race on the source file is unchanged (see Risks below).
...
- New: `.loops/tmp/autodev-pre-ids.txt`, `.loops/tmp/autodev-post-ids.txt`, `.loops/tmp/autodev-diff-ids.txt`, `.loops/tmp/autodev-new-children.txt`, `.loops/tmp/autodev-passed.txt`, `.loops/tmp/autodev-skipped.txt`, `.loops/tmp/autodev-broke-down` — mirrors of the recursive-refine bookkeeping files, namespaced to autodev so the two loops never collide if both run concurrently.
- Source handshake file: `.loops/tmp/recursive-refine-broke-down` — written by `refine-to-ready-issue:write_broke_down` (line 233) and cleared by `refine-to-ready-issue:resolve_issue` (line 25). After the sub-loop returns, autodev `cp`s this to `.loops/tmp/autodev-broke-down` and reads only the copy for the rest of the state machine (mirrors the role of `recursive-refine:check_broke_down`, lines 221-234). Renaming the source file would require modifying `refine-to-ready-issue.yaml`, which is out of scope.
