...
1. **`skills/create-loop/loop-types.md:603-643` (Step H3)** — Add a 5th multi-select option for stall detection. All H1 choices (named skill + "Custom prompt") produce `action_type: prompt`, so this option should be pre-selected by default. Follow the existing pattern: prose detection instruction before the YAML block (as done for Tool-based gates at line 605) + `# Show only if` inline comment (line 615 pattern). Label: `"Stall detection (Recommended for prompt-based skills)"`.

2. **`skills/create-loop/loop-types.md:682-802`** — Update both Variant A (lines 682–734) and Variant B (lines 736–802) YAML templates to include a `check_stall` state when the stall detection phase is selected. Placement: between `execute` and `check_concrete`, exactly as in the canonical `loops/harness-multi-item.yaml:60-78` and `loops/harness-single-shot.yaml:26-49`. The `action` field should be `"echo 'checking stall'"` with `action_type: shell`. **Routing differs by variant** (confirmed against canonical files): Variant A (single-shot) uses `on_no: done` (no `advance` state); Variant B (multi-item) uses `on_no: advance` (skip stuck item, return to discover). Both use `on_yes: check_concrete` and `on_error: check_concrete`.
...
### Files to Modify
- `skills/create-loop/loop-types.md` — Step H3 (lines 603–643): add stall detection as 5th multi-select option; Variant A template (lines 682–734) and Variant B template (lines 736–802): add `check_stall` state to evaluation chain when phase selected
...
- `skills/create-loop/loop-types.md:605-628` — prose detection instruction + `# Show only if` comment pattern for **config-conditional** phases (Tool-based gates: shown only if `test_cmd`/`lint_cmd`/`type_cmd` configured). **Note**: stall detection is NOT config-conditional — it is always shown. Use a different inline comment: `# Pre-selected by default: all H1 choices produce prompt-based execution` instead of `# Show only if`.
...
1. **`skills/create-loop/loop-types.md` Step H3** — Added "Stall detection (Recommended for prompt-based skills)" as the 2nd multi-select option (after Tool-based gates), with `# Pre-selected by default: all H1 choices produce prompt-based execution` comment. Also added stall detection row to the observability context table.

2. **`skills/create-loop/loop-types.md` Variant A & B templates** — Added `check_stall` state between `execute` and `check_concrete` in both templates. Variant A uses `on_no: done`; Variant B uses `on_no: advance`. Both use `on_yes: check_concrete` and `on_error: check_concrete` (Variant A: `on_error: done`).

3. **`docs/guides/AUTOMATIC_HARNESSING_GUIDE.md`** — Moved `## Stall Detection` into `## Evaluation Phases Explained` as `### Stall Detection (\`check_stall\`)`. Updated ToC, "Full 6-phase ordering" block (added `check_stall` first), Decision guide (added `check_stall (diff_stall)` row), and Step H3 display to show 5 options. Removed old standalone section.
