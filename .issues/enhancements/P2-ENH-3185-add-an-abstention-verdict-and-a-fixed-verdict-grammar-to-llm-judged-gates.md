...
### When the hold applies vs. when the fallback applies (pinned 2026-08-15)

Earlier drafts stated both "an FSM predicate holds, bounded by a cap" and "undeclared `on_cannot_judge` falls back to `on_error`" without saying which governs the undeclared case — two different behaviors (retry-then-error vs. error-immediately) for the same input. Pinned:

- **Declared route** (`on_cannot_judge:` shorthand, or `cannot_judge` as an explicit `route:` key): route immediately. **No hold** — the loop author has said what abstention means here, and retrying first would delay their handler for no benefit.
- **Undeclared**: the hold applies — retry the same state up to the consecutive-abstention cap (default 2), then escalate to `on_error`. The retry exists because a transient abstention (judge missed an artifact on one run) is recoverable, and only the undeclared case has no better answer.
- **Undeclared `on_cannot_judge` AND undeclared `on_error`**: after cap exhaustion, `_route()` returns `None` and the run terminates via `_finish("error", "No valid transition")` — the same hard-but-loud outcome any unroutable verdict gets today. This is accepted: it is loud, it cannot be mistaken for a pass or a fail, and a loop with no error route has already opted out of graceful degradation. AC6's tests must include this neither-declared case so the terminal behavior is pinned, not incidental.
...
- **AC11 (blocker — the prompt contract must be rewritten, in *three* places).** Adding an enum member does not make a judge emit it while the prompt text forbids it. All three of the following instruct missing-evidence→No and must be rewritten together; any one left behind suppresses abstention on the path it governs.
  ...
  3. **`scaffold_verify.py:73`** — the per-criterion eval prompt: *"Answer NO if the criterion is not met **or evidence is missing/ambiguous**."* Baked into every generated `verify-issue-loop`. Because verify-issue-loop is a headline consumer of this feature, leaving this line makes the feature a no-op precisely where it was most wanted. Note this fixes the *generator* only: existing generated loop YAMLs on disk carry the old text verbatim and keep suppressing abstention until regenerated — consistent with ENH-3200's AC7 no-migration stance, but state it in the docs so a stale loop isn't misread as a bug in this feature.
