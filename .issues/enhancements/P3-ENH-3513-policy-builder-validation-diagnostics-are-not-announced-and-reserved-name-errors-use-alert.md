...
1. Add `EXPORT_DISABLED_REASON`; add `lastErrorKey` tracking + announcement to `updatePreview`/`renderMessages`; add pending-suffix composition inside `showLiveStatus` (covers every caller).
