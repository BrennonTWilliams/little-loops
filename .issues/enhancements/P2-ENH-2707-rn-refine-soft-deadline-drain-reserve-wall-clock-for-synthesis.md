...
completed_at: '2026-07-20T15:59:39Z'
...
- No FSM engine changes — this is entirely a `rn-refine.yaml` context/state edit (no `on_timeout` finalizer hook, no engine-level termination semantics touched).
- No changes to other loops that use `dequeue_next`-style patterns (`check_resume`/`route_resume_synth`, `route_decomposed`) beyond the routing idiom being followed, not modified.
- Does not change `max_depth`/`max_node_iters`/`max_nodes` defaults or semantics — the drain is a wall-clock safety valve, not a resizing of the walk's node budget.
- Does not add a general-purpose `on_timeout` finalizer mechanism to the FSM engine for other loops to use — scoped to `rn-refine`'s specific soft-deadline drain.
...
## Session Log
- `/ll:ready-issue` - 2026-07-20T15:46:07 - `2f8bcced-c373-4fd8-9c08-b3a73dd707e4.jsonl`
...
- `/ll:manage-issue` - 2026-07-20T15:59:02Z - `/Users/brennon/.claude/projects/-Users-brennon-AIProjects-brenentech-little-loops/e12921c1-f6bc-4546-8deb-1e1a2eb7f1de.jsonl`
...
### Changes Made
- `scripts/little_loops/loops/rn-refine.yaml`: added `timeout_total`/`synth_reserve` context vars; `dequeue_next` now checks `${loop.elapsed_ms}` against `timeout_total - synth_reserve` before popping and, past the soft deadline with a non-empty queue, parks the remainder in `undrained.txt` and emits `DEADLINE_DRAIN` (widened `dequeue_next` evaluator pattern routes both `QUEUE_EMPTY` and `DEADLINE_DRAIN` to `build_synth`, `on_yes`/`on_no` wiring unchanged); `check_resume` also treats a non-empty `undrained.txt` as a `RESUME_WALK` signal; `resume_reconcile` merges `undrained.txt` back onto the queue (after visited-but-incomplete nodes) and clears it; `assemble` appends a `PARTIAL_DRAIN` marker (reusing the `RECOVERY_NEEDED` contract) naming the undrained nodes and the resume command; `report`'s Step 1 now also surfaces `PARTIAL_DRAIN`.
- `scripts/tests/test_rn_refine.py`: added drain/no-drain dequeue tests, an evaluator-pattern/routing assertion, a `check_resume` undrained-nonempty test, a `resume_reconcile` merge test, `assemble` PARTIAL_DRAIN present/absent tests, and a context-knob-declared test.
- `docs/guides/LOOPS_REFERENCE.md`: documented `timeout_total`/`synth_reserve` in the context-variable table, the `DEADLINE_DRAIN` path in the FSM flow diagram, and a new "Soft-deadline drain" notes bullet; extended the Resume bullet to cover `undrained.txt` reconciliation.
- `scripts/little_loops/loops/README.md`: updated the `rn-refine` table row to mention the soft-deadline drain and `synth_reserve`.

### Verification Results
- Tests: PASS (`python -m pytest scripts/tests/` — 15575 passed, 38 skipped; `scripts/tests/test_rn_refine.py` — 83 passed)
- Lint: PASS (`ruff check scripts/`)
- Types: PASS (pre-existing `ruamel` stub-import noise only, unrelated to this change)
- `ll-loop validate rn-refine`: PASS

### Notes
- No `CHANGELOG.md` entry added — per project convention, changelog entries are generated by `/ll:manage-release` from completed issues at release-cut time, not hand-added per issue commit.
