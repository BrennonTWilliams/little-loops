...
6. The document goes to stdout by default and to `-o <path>` when given; the lossiness note always goes to stderr, so redirecting stdout yields a clean file.
7. A token load that yields nothing (`design_tokens.enabled: false`, missing path/profile, unresolvable `--profile`) exits 1 with a reason — never an empty document.
...
**`spacing:` deliberately does *not* follow the same ruling — and that asymmetry is a decision, not an oversight.** The vendored fixture's `spacing:` block is named t-shirt steps (`base`, `xs`, `sm`, `md`, `lg`, `xl`, `gutter`, `margin`), while every built-in profile's is a numeric scale (`space.0` … `space.24`). Unlike typography — where the profile carries five disconnected axes and the spec wants composites, so *something* has to be synthesized — a numeric spacing scale is already one-level-flat and maps to `spacing:` losslessly. Synthesizing t-shirt names over it would invent a mapping (`space.4` → `md`?) that no profile states and that no round trip could verify. **Decision: emit the numeric scale verbatim as `spacing:` keys.** The cost is that those keys are numeric-looking, which is exactly the int-key hazard AC 12 exists to close.

**Decision: spec shape wins** for typography. Portability-out is the stated motivation, and an AC that rewards emitting a non-portable document is the wrong gate. The exporter synthesizes a role ramp; typography is excluded from AC 1's value-equality comparison and is instead covered by a shape assertion (every emitted role carries `fontFamily` + `fontSize`) plus a stderr note naming the axes that did not survive.
...
- **Emit `name:` — and know that it re-imports as a bogus token.** The spec fixture opens with `name: Paws & Paths` (`scripts/tests/fixtures/design_md/paws_and_paths_DESIGN.md:2`). The export emits the profile name as the frontmatter `name:` key. But `_map_design_md_namespaces` (`design_tokens.py:277-296`) has **no non-token key allowlist**: `name` is not in `_DESIGN_MD_NAMESPACE_MAP`, is not `colors`, and is not a dict, so it falls through to `mapped["name"] = <value>` and lands in `resolved` as a token — which `render_as_css_vars` then emits as `--name: warm-paper`. This is pre-existing ENH-3264 behavior (the vendored fixture already triggers it), but ENH-3268 is the first thing that *generates* the key. Two consequences, both in scope: (a) every round-trip assertion must exclude the `name` key from comparison (AC 1), and (b) fixing the importer to skip non-token top-level keys is **out of scope** — file it separately if wanted; do not change the import side here.
- **Collision guard on the color mapping.** The per-role export rule is generic over arbitrary user profiles, so two distinct semantic leaves can in principle map to the same flat `colors:` name. Verified: no collision occurs on any of the three built-ins (all three carry the identical leaf set — `surface.{primary,secondary,raised}`, `text.{primary,secondary,muted,inverse}`, `border.{subtle,strong}`, `action.{primary,primary-hover,destructive}`). The exporter must nonetheless detect a duplicate export name and fail via `logger.error(...)` → 1 rather than silently overwriting one leaf with another.
- **`load_design_tokens()` returning `None` is an error, not an empty export.** Without `--profile`, the default path is today's loader, which returns `None` when `design_tokens.enabled` is false, when `config.project_root / dt_cfg.path` does not exist, or when the active profile directory is missing (`design_tokens.py:354-372`, `:437-449`). `cmd_design_md_export` treats `None` as a hard failure: `logger.error(...)` with the reason, return 1. Same escape hatch as an unresolvable `--profile` — never a silent empty document on stdout.
- **`-o` is an output *file*, and its default is stdout.** Per the `session.py export` precedent (`:268-274, 817-837`) rather than the `policy-builder` output-*directory* shape: `design-md export` renders one document, so unset `-o` writes the rendered Markdown to stdout. This keeps the stderr note (which is a separate stream) usable for piping the document into another tool.
- **`--profile` overrides the configured source, including a `design_md` one.** If the project config is `source: design_md`, an unqualified export takes the DESIGN.md path (which is what makes AC 7's prose round trip free) and `--theme` is a no-op there — the loader already warns "DESIGN.md ... has no theme mechanism" (`design_tokens.py:438-442`), and the exporter must not add a second, contradictory note. Passing `--profile <name>` bypasses the DESIGN.md path entirely and exports the named profile; in that case `--theme` applies normally.
...
- **Emit quoted YAML scalars and quoted keys — via `yaml.dump`, not a hand-rolled writer.** The house parser's `BaseLoader` makes every value a string on re-import, so nothing in the round trip forces this; a plain `safe_load` consumer is the one that breaks (`radius.none: 0` → int, `spacing:`'s `0`/`1`/`2` → int keys, `fontWeight: 400` → int). Pin the emitter to `yaml.dump(block, default_style='"', sort_keys=False, allow_unicode=True)` — `default_style` applies to keys as well as scalars, so one call closes both halves. A hand-rolled quoted writer is explicitly rejected: the values being quoted include font stacks carrying embedded single quotes *and* commas (`font.family.body` on warm-paper resolves to `'Lora', 'Charter', 'Iowan Old Style', Georgia, serif`) and shadow values carrying `rgba(74, 64, 49, 0.06)`. Getting the escaping right by hand is a bug factory with no upside.
...
- `scripts/little_loops/cli/artifact.py:152` **and its epilog (`:154-162`)** — _review-pass addition; the wiring pass caught only the `cli/__init__.py` copy._ The `ArgumentParser(description=...)` carries the same stale "Generate self-contained human-facing HTML artifacts" string, and the epilog's `Examples:` block lists only `policy-builder` invocations. This is what `ll-artifact --help` actually prints, so it is the more user-visible of the two. Broaden the description and add `design-md export` examples (with and without `--profile`/`-o`).
...
- **A synthetic partial profile fixture is required, or the typography skip path ships untested.** Verified: all three built-ins carry every axis value the pinned role table consumes — `font.size.{xs,sm,base,lg,xl,2xl,3xl,4xl}`, `font.family.{body,heading}`, `font.line-height.{tight,normal,relaxed}`, `font.weight.{normal,medium,semibold,bold}`. So the "role whose `font.size` step is absent is skipped and named in the stderr note" rule is **unreachable on every built-in**, and both the skip branch and its note line would be dead-untested. Add a tmp-dir profile with a truncated size scale (e.g. `xs`/`sm`/`base` only) and assert the skipped roles appear in the note.
- A collision test for the color mapping: a synthetic profile whose leaves map to the same flat export name must exit 1 via `logger.error`, not silently drop a token.
- A `load_design_tokens() is None` test (`design_tokens.enabled: false`, and a missing active profile) asserting exit 1 with a reason on stderr — not an empty document on stdout.
- A quoting test asserting a font stack with embedded single quotes and commas (`warm-paper`'s `font.family.body`) survives `yaml.safe_load` byte-identical (AC 12).
...
- **All three built-ins carry an identical semantic leaf set**, verified: `color.surface.{primary,secondary,raised}`, `color.text.{primary,secondary,muted,inverse}`, `color.border.{subtle,strong}`, `color.action.{primary,primary-hover,destructive}`. Every one was traced through the live `_classify_design_md_color_role` under the per-role export rule and lands in its original role, with no two leaves colliding on an export name (`primary-hover` → `accent-primary-hover` → `action`).
- **The importer has no non-token top-level key allowlist.** `_map_design_md_namespaces` (`:277-296`) special-cases only `components` (skipped) and `colors` (role-mapped); everything else falls to `_DESIGN_MD_NAMESPACE_MAP.get(ns, ns)`, so a scalar `name:` becomes `mapped["name"]` → `resolved["name"]` → `--name` in `render_as_css_vars`. Pre-existing; the fixture triggers it.
- **The spec's `spacing:` is named, ours is numeric.** Fixture: `base`/`xs`/`sm`/`md`/`lg`/`xl`/`gutter`/`margin`. Profiles: `space.0` … `space.24`. The fixture also quotes its `fontWeight` values (`fontWeight: "800"`), independent support for AC 12's quoting rule.
- **The pinned typography role table is unreachable-in-part on every built-in.** All three carry `font.size.{xs,sm,base,lg,xl,2xl,3xl,4xl}`, `font.family.{body,heading}`, `font.line-height.{tight,normal,relaxed}`, and `font.weight.{normal,medium,semibold,bold}` — so every role in the table resolves and the skip-and-report branch never fires without a synthetic fixture. (`editorial-mono` additionally has `font.size.5xl` and `font.weight.black`, which the table does not consume — both belong in the dropped-axes note.)
- **Values needing escaping are real, not hypothetical.** `font.family.body` resolves through `{font.family.serif}` to `'Lora', 'Charter', 'Iowan Old Style', Georgia, serif` on warm-paper (single quotes + commas); `shadow.sm` is `0 1px 2px rgba(74, 64, 49, 0.06)`. This is why the quoting rule pins `yaml.dump` over a hand-rolled writer.
- **`cli/artifact.py:152`'s `description=` and its epilog (`:154-162`) carry the same stale "HTML artifacts" framing as `cli/__init__.py:6`**, and are the copy `ll-artifact --help` actually renders. The wiring pass listed only the latter.
...
3. Add the `ll-artifact design-md export [--profile] [--theme] [-o]` subcommand — a nested `design-md` → `export` subparser pair plus `(command, subcommand)` dispatch, built-in profile resolution via `importlib.resources`, the `_resolve_token_root` profile-name override, `-o`-as-file with stdout default, and the two hard-failure paths (`load_design_tokens()` returned `None`; `--profile` unresolvable).
4. Emit the stderr note: exported theme, dropped theme(s), dropped token groups (`shadow.*`, `border.width.*`), the typography axes/roles lost to the role synthesis, and `components:` for a `design_md` source. Metadata keys are not listed. Do not add a second theme note on the `design_md` path — the loader already emits one.
5. Complete the test set over all three built-in profiles (round-trip minus `name`, role preservation, typography shape, no-primitives, quoting/escaping), plus the synthetic-profile cases that no built-in reaches: truncated size scale (role skip), colliding color names, and `enabled: false` / missing profile.
6. Docs — including the `cli/artifact.py:152` description + epilog and `cli/__init__.py:6`, both of which currently say "HTML artifacts".
...
- **Scope**: `design_tokens.py` (one new renderer), `cli/artifact.py` (one nested subcommand pair plus refreshed `--help` copy), tests, docs. Estimated ~130 LOC plus tests — the review pass added a collision guard, two hard-failure paths, and three synthetic-profile fixtures.
...
- **Teaching the importer to skip non-token top-level frontmatter keys.** `_map_design_md_namespaces` turns `name:` into a `resolved["name"]` token today (the vendored fixture already triggers it, pre-ENH-3268). This export makes the quirk easier to hit, but fixing it is an import-side change; here it is only worked *around*, by excluding `name` from the round-trip comparison.
- Synthesizing t-shirt-sized `spacing:` keys over the profiles' numeric scale. No profile states that mapping and no round trip could verify it; the numeric scale is emitted verbatim.

## Acceptance Criteria

1. **Semantic colors and `space`/`radius` round-trip by value under the composed mapping.** For each of `default`, `warm-paper`, `editorial-mono`: exporting then re-importing yields the same concrete value for every semantic color token and every `space.*`/`radius.*` token. The test composes the exporter's forward mapping with the importer's own re-derivation (`_classify_design_md_color_role` → `color.<role>.<name>`) to compute the expected re-imported key — **keys are not identical across the round trip, only roles and values are** (`color.border.subtle` → `outline-subtle` → `color.border.outline-subtle`). Asserting forward-mapped keys alone will fail; see "The key mapping must be classifier-aware". **The `name` key is excluded from the comparison**: the importer has no non-token key allowlist, so the frontmatter `name:` AC 7 mandates re-imports as a spurious `resolved["name"]` token (see Decision Rules).
...
12. **The export is correctly typed *and* correctly escaped for a non-little-loops consumer.** Parsing the exported frontmatter with `yaml.safe_load` (not the house `BaseLoader` parser, which cannot see this) yields string values and string keys throughout — `radius.none`, `spacing`'s numeric keys, and `fontWeight` do not come back as ints. Additionally, a value carrying embedded single quotes and commas round-trips byte-identically through `yaml.safe_load`: assert on `warm-paper`'s resolved `font.family.body` (`'Lora', 'Charter', 'Iowan Old Style', Georgia, serif`), which is the case a hand-rolled quoted writer gets wrong.
13. **A `None` token load is a reported failure, not an empty export.** With `design_tokens.enabled: false`, a missing token path, or a missing active profile, `ll-artifact design-md export` writes nothing to stdout, writes a reason via `logger.error`, and exits 1. Same for an unresolvable `--profile <name>` (neither the project `profiles_dir` nor the packaged built-ins).
14. **`-o` writes a file; unset writes the document to stdout.** The stderr note is emitted on stderr in both cases, so `ll-artifact design-md export > DESIGN.md` yields a clean document with the lossiness note still visible in the terminal.
15. **Two semantic leaves mapping to the same flat `colors:` name is a hard error.** Exercised with a synthetic profile; the exporter exits 1 via `logger.error` rather than silently letting one leaf overwrite the other. (No built-in profile collides — all three carry the identical semantic leaf set.)
16. **The typography skip path is exercised.** A synthetic profile with a truncated `font.size` scale causes the roles whose step is absent to be omitted from `typography:` and named in the stderr note. No built-in profile reaches this branch, so without a synthetic fixture it ships untested.
...
- CLI: `ll-artifact design-md export [--profile <name>] [--theme <name>] [-o <path>]`
  - `-o <path>` — output **file**; unset writes the document to stdout (the `session.py export` precedent, not `policy-builder`'s output-directory shape). The dropped-groups note always goes to stderr, so redirecting stdout yields a clean document.
  - Exit 1 with a `logger.error` reason when the token load yields `None` (`design_tokens.enabled: false`, missing path, missing active profile) or when `--profile <name>` resolves in neither the project `profiles_dir` nor the packaged built-ins.
