...
### Files to Modify
- `scripts/little_loops/cli/harness.py` — `BaselineDelta` (`:1559-1574`, additive widening), `_run_compare_arm()` (`:1855`, `frozen=` parameter), `_report_samples()` (`:1405`, render the second arm; factor `_baseline_payload()` out of `:1452-1473`), `_add_evaluator_flags` (`:611-649`, add `--pin-baseline`/`--pin-reason`/`--force`), `_baseline_flag_refusal()` (`:1656`, refuse pin+measure/compare/retry combos), the four `cmd_skill`/`cmd_cmd`/`cmd_mcp`/`cmd_prompt` baseline branches (`:2253-2662`, a `pin_baseline` branch alongside `measure_baseline`, plus `read_pin()` + second refusal in the compare branch), `cmd_dsl` (`:2704`, extend the existing measure/compare refusal to `--pin-baseline`), new `BaselinePin`/`read_pin`/`write_pin`/`_arm_verdict`/`_pair_outcome`/`_run_pin_phase`
- `scripts/little_loops/cli/harness.py:1682` `_incumbent_content_hash()` — **fixed by BUG-3479 first** (raw blob bytes, no strip); this issue depends on it but does not change it
...
- `BaselineDelta`: `candidate, baseline, delta, source, head_sha_differs` (`scripts/little_loops/cli/harness.py:1560`) — widened additively with `vs_incumbent: str | None = None`, `frozen: BaselineResult | None = None`, `pin: BaselinePin | None = None`, `vs_pin: str | None = None`, `outcome: str | None = None` (D5/D7; defaults keep every existing constructor call valid).
...
- `_run_compare_arm(...)` (`cli/harness.py:1855`) gains `frozen: BaselineResult | None = None` and `pin: BaselinePin | None = None`; `_report_samples(...)` (`:1405`) renders the extra fields when `delta.frozen` is set, via the new `_baseline_payload(shown: BaselineResult) -> dict[str, Any]` helper (D7).
...
- `_baseline_flag_refusal(...)` (`:1656`) gains the `--pin-baseline` combination rules (D6).
