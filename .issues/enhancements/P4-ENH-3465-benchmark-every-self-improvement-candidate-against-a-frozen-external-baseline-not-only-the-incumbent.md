...
- New `read_pin(runner: str, target: str) -> BaselinePin | None` (list the entry directory, parse each fragment, return the one with the greatest `pinned_at`, filename as tiebreak) / `write_pin(pin: BaselinePin) -> Path` (write one new `<pinned_at>-<uuid4>.json` fragment via `atomic_write_json()`, creating the directory on first write; never touches existing fragments) — `cli/harness.py`. Both resolve `.ll/` from the project root via `resolve_ll_dir()` (D6).
- New `_pin_refusal(pin: BaselinePin, active: BaselinePin | None, key: BaselineKey, conditions: BaselineConditions, *, force: bool) -> str | None` — the D6 gate, evaluated in order: no full-n measurement of the content being pinned → refusal; `active` present and `active.target_content_hash == pin.target_content_hash` → refusal (no-op); `active` present and no full-n measurement of the active content under current conditions → refusal carrying the D8 procedure for the *active* pin; `active` measured and new rate < active rate → refusal; `force` bypasses all four. Pure over at most two `read_baseline()` results.
...
- `_run_compare_arm(...)` (`cli/harness.py:1907`) gains `frozen: BaselineResult | None = None` and `pin: BaselinePin | None = None`; `_report_samples(...)` (`:1455`) renders the extra fields when `delta.frozen` is set, via the new `_baseline_payload(shown: BaselineResult) -> dict[str, Any]` helper (D7).
...
Compare: `cmd_skill` (compare branch, `:2319`) -> `read_pin("skill", args.target)` -> if a pin exists, `_frozen_baseline_refusal()` on `BaselineKey("skill", args.target, current input_hash, pin.target_content_hash)` (fail closed, D8 message) -> `_run_compare_arm(..., frozen=read_baseline(pinned_key, conditions), pin=pin)` (`read_baseline` is the CLI wrapper over `baseline_for()`, D3) -> one `_run_sample_loop()` (unchanged; the candidate is sampled exactly once) -> `_arm_verdict()` ×2, `_pair_outcome()` -> widened `BaselineDelta` -> `_report_samples()` renders `vs_incumbent`/`vs_pin`/`outcome` in text and under the existing `payload["baseline"]` object in JSON. Exit code path untouched. `cmd_cmd`/`cmd_mcp`/`cmd_prompt` compare branches are untouched (no `read_pin()` call; a pin can only exist for `skill`).

Pin: `cmd_skill` (`--pin-baseline` branch, after `_baseline_flag_refusal`, before measure/compare) -> `_incumbent_content_hash(skill_path)` (None → exit 2, same message as the compare branch's incumbent refusal at `:2325-2332`) -> `read_pin()` -> `_pin_refusal()` (at most two `read_baseline()` calls) -> `write_pin()` -> print the entry (text) / `{"pin": {...}}` (JSON) -> exit 0. No subject invocation, no `harness_events` write.
...
1. `ll-harness skill <target> [args] --pin-baseline` writes a pin whose `target_content_hash` equals `_incumbent_content_hash()` of the target at that moment (and equals `_hash_file()` of the unmodified file). `--pin-baseline` on `prompt`/`cmd`/`mcp`/`dsl` exits 2 before any work. A second invocation writes a second fragment file rather than modifying the first; no fragment is ever deleted or rewritten, and `read_pin()` returns the greatest-`pinned_at` entry. Fragments live under the project root's `.ll/harness-pins/skill--<target>/` regardless of cwd.
...
4. With a pin and full-n rows for the pinned content, the report carries `vs_incumbent`, `vs_pin` ∈ `{ahead, behind, inconclusive}` and `outcome` ∈ `{improvement, drift, regression, inconclusive}`, computed per D4/D5 from `proportion_diff_ci()` over `passed`/`graded` (a tally whose `ci_lo`/`ci_hi` are `None` classifies correctly), in both text and JSON. 3/3 vs 0/3 classifies `ahead` (not `inconclusive`). A candidate `behind` the incumbent reports `regression` even when `vs_pin` is `ahead` or `inconclusive`.
...
8. The frozen arm's JSON object (`baseline.frozen`) has the same keys as the incumbent `baseline` object plus `pinned_head_sha`/`pinned_at`/`reason`/`forced`, and no `head_sha_differs`; text mode prints no "(measured at a different HEAD)" note for that arm.
...
- **Effort**: Small-Medium — no new store or reader (D3), no FSM-layer change (D1), one runner only (D6); the work is a pin record + CLI flag, one `stats.py` helper, two pure classification helpers, an additive widening of `BaselineDelta`/report, tests, and docs.
...
- Pinning became a runner-subcommand flag (`--pin-baseline`) instead of a standalone `pin-baseline` verb (D6), scoped like `--compare-baseline` (skill from HEAD, others via `--baseline-of`).
