...
- `ll-harness <runner> --compare-baseline` (`cli/harness.py:1880`, `_run_compare_arm()`) reports exactly one delta, against `baseline_for()` rows keyed on the HEAD content hash (`_incumbent_content_hash()`, `:1707`) or an explicit `--baseline-of` attempt. There is no second arm and no way to name a reference other than "current HEAD" or "one prior attempt".
...
- No comparison result anywhere is three-valued: `BaselineDelta.delta` is a raw rate difference or `None` (`cli/harness.py:1585-1598`), with no notion of an inconclusive result despite both tallies carrying Wilson CIs.

## Expected Behavior

- A target can carry a **pin**: a recorded content hash of that target (plus HEAD SHA, timestamp, reason) that is written only by an explicit `--pin-baseline` invocation of the runner subcommand and never recomputed from HEAD. Pinning is a flag on the existing runner subcommands (`ll-harness skill <target> [args] --pin-baseline`), not a separate verb, so it shares the evaluator flags, `--samples`, and target/input resolution the frozen arm's lookup needs (D6). It is supported where `--compare-baseline`'s HEAD resolution is: `skill` pins the HEAD blob of the target; `prompt`/`cmd`/`mcp` pin the content hash of a `--baseline-of <attempt>` and refuse without one.
- A pin is only written when the pinned content already has a full-n measurement under the current conditions (first pin and re-pin alike; `--force` overrides and is recorded), so a pin is usable by the next compare at least once.
...
- **D6 — Pinning is explicit, gated, monotone, and append-only.** Pinning is a flag on the runner subcommands, `ll-harness <runner> <target> [args] --pin-baseline [--pin-reason …] [--force]`, registered in `_add_evaluator_flags` next to `--measure-baseline`/`--compare-baseline` and refused by `_baseline_flag_refusal()` when combined with either of them, `--retry-of`, or the `dsl` runner. A standalone `pin-baseline` verb was rejected: the D6 monotone check and the frozen lookup both need `BaselineConditions` (n, semantic prompt/model, subject model, timeout, host) and `input_hash`, which would mean re-declaring the evaluator flags, `runner_args`/`--args`, and all four target-resolution paths. `skill` pins `_incumbent_content_hash()` of the target; `prompt`/`cmd`/`mcp` pin the `target_content_hash` of a required `--baseline-of <attempt>` (same rule as `--compare-baseline`). Never automatic (contrast `EvaluateConfig.auto_promote`, whose rolling overwrite is the drift shape this issue guards against). **Every pin write, first or subsequent, is refused unless `baseline_for()` finds a full-n measurement of the content being pinned under the current input and conditions** (an unmeasured pin would make every later compare fail closed under AC5); a re-pin is additionally refused unless that measurement's pass rate is ≥ the active pin's under the same conditions. `--force` overrides both checks and sets `forced: true` on the entry. The pin file holds a `pins: [...]` list; the last entry is active, earlier entries are never deleted. Storage: `.ll/harness-pins/<runner>--<target-slug>.json` under the **project root** (resolved the same way `.ll/history.db` is, via `resolve_history_db(DEFAULT_DB_PATH).parent` / `find_project_root()`, never `Path.cwd()`), committed like `.ll/decisions.d/` — `.gitignore` already un-ignores `/.ll/`, so no gitignore change is needed — not `.loops/` (per-machine).
- **D7 — Pin absent ⇒ single-arm behaviour unchanged.** No pin means today's one-delta report, byte-for-byte for the existing keys; `test_delta_json_payload` etc. keep passing. The dual shape is additive (`baseline.vs_incumbent`, `baseline.frozen`, `baseline.vs_pin`, `baseline.outcome`), never a rename of existing keys. `baseline.frozen` is an object with the same sub-shape as the existing `baseline` payload (`source`, `n`, `conditions`, `conditions_fp`, `head_sha`, `attempt_ids`, `notes`, `baseline_pass_rate`, `baseline_ci`) plus `pinned_head_sha`, `pinned_at`, `reason`, `forced`; the existing inline payload construction in `_report_samples()` (`:1477-1498`) is factored into a `_baseline_payload(shown) -> dict` helper used for both arms. The frozen arm **omits** `head_sha_differs` and the text-mode "(measured at a different HEAD)" note — its rows were measured at a different HEAD by construction, so the note would be noise; the text line prints `pinned_head_sha` instead.
- **D8 — Measuring pinned content after HEAD has moved is a documented manual procedure.** `--measure-baseline` measures the file on disk; once the lineage commits past the pin, the pinned blob is no longer in the tree, and every conditions change (n, judge model, timeout, host) invalidates the cached frozen rows and re-triggers the AC5 refusal. The refusal message and `docs/guides/EVALUATION_GUIDE.md` spell out the exact steps: `git show <pinned_head_sha>:<target-path> > <target-path>` (the path is recorded on the pin), then `ll-harness <runner> <target> [same args] --measure-baseline`, then `git checkout -- <target-path>`. This works because after BUG-3479 the restored file's `_hash_file()` equals the pinned hash. Automating this (`--measure-pin`) is a follow-up.
...
- `scripts/little_loops/cli/harness.py:1707` `_incumbent_content_hash()` — **fixed by BUG-3479 first** (raw blob bytes, no strip); this issue depends on it but does not change it
...
- `BaselineDelta`: `candidate, baseline, delta, source, head_sha_differs` (`scripts/little_loops/cli/harness.py:1586`) — widened additively with `vs_incumbent: str | None = None`, `frozen: BaselineResult | None = None`, `pin: BaselinePin | None = None`, `vs_pin: str | None = None`, `outcome: str | None = None` (D5/D7; defaults keep every existing constructor call valid).
- New `BaselinePin` (frozen dataclass, `cli/harness.py`): `runner, target, target_path, target_content_hash, pinned_head_sha, pinned_at, reason, forced` — the only new record (D2/D3). `target_path` is the repo-relative path (None for non-file-shaped runners) so the D8 procedure can name it. The frozen arm itself is an ordinary `BaselineResult` returned by `baseline_for()`.
...
- `_compare_baseline_refusal(key: BaselineKey, conditions: BaselineConditions) -> str | None` — existing pre-run compare gate (`cli/harness.py:1862`); called a second time with `BaselineKey(..., target_content_hash=pin.target_content_hash)` when a pin exists, so the frozen arm fails closed the same way.
...
- New `read_pin(runner: str, target: str) -> BaselinePin | None` / `write_pin(pin: BaselinePin) -> None` (append to the `pins` list, create the file/dir on first write) — `cli/harness.py`. Both resolve the pin directory from the project root (D6).
- New `_pin_refusal(pin: BaselinePin, active: BaselinePin | None, key: BaselineKey, conditions: BaselineConditions, *, force: bool) -> str | None` — the D6 gate: no full-n measurement of the content being pinned → refusal; `active` present and new rate < active rate → refusal; `force` bypasses both. Pure over two `read_baseline()` results.
- New `_arm_verdict(candidate: SampleTally, arm: SampleTally) -> str` and `_pair_outcome(vs_incumbent: str, vs_pin: str) -> str` — pure functions (D4/D5), unit-testable without a DB. `_arm_verdict` calls `wilson_ci(passed, graded)` for both tallies itself (D4).
- `_run_compare_arm(...)` (`cli/harness.py:1880`) gains `frozen: BaselineResult | None = None` and `pin: BaselinePin | None = None`; `_report_samples(...)` (`:1430`) renders the extra fields when `delta.frozen` is set, via the new `_baseline_payload(shown: BaselineResult) -> dict[str, Any]` helper (D7).
- New `_run_pin_phase(runner: str, target: str, target_path: Path | None, key: BaselineKey, conditions: BaselineConditions, head_sha: str | None, args) -> int` — called from the four `cmd_*` functions when `--pin-baseline` is set, before the measure/compare branches; exit 2 on refusal with zero DB writes and zero subject invocations.
- `_baseline_flag_refusal(...)` (`:1681`) gains the `--pin-baseline` combination rules (D6).

### Call Path

Compare: `cmd_skill`/`cmd_cmd`/`cmd_mcp`/`cmd_prompt` (compare branch) -> `read_pin()` -> if a pin exists, `_compare_baseline_refusal()` on `BaselineKey(runner, target, current input_hash, pin.target_content_hash)` (fail closed, D8 message) -> `_run_compare_arm(..., frozen=read_baseline(pinned_key, conditions), pin=pin)` (`read_baseline` is the CLI wrapper over `baseline_for()`, D3) -> one `_run_sample_loop()` (unchanged; the candidate is sampled exactly once) -> `_arm_verdict()` ×2, `_pair_outcome()` -> widened `BaselineDelta` -> `_report_samples()` renders `vs_incumbent`/`vs_pin`/`outcome` in text and under the existing `payload["baseline"]` object in JSON. Exit code path untouched.

Pin: `cmd_*` (`--pin-baseline` branch, after `_baseline_flag_refusal`, before measure/compare) -> resolve content hash (`_incumbent_content_hash()` for `skill`; `_resolve_baseline_of()` for the others) -> `read_pin()` -> `_pin_refusal()` (two `read_baseline()` calls at most) -> `write_pin()` -> print the entry (text) / `{"pin": {...}}` (JSON) -> exit 0. No subject invocation, no `harness_events` write.
...
1. **Pure verdict helpers first (TDD).** Add `ARM_VERDICTS`/`PAIR_OUTCOMES`, `_arm_verdict()`, `_pair_outcome()` to `cli/harness.py`. `_arm_verdict` computes Wilson bounds from `passed`/`graded` internally (D4); its table-driven tests build `SampleTally(requested, graded, passed)` with `ci_lo`/`ci_hi` left at `None` to prove that: disjoint-above → `ahead`, disjoint-below → `behind`, overlapping → `inconclusive`, zero-graded candidate or arm → `inconclusive`. All 9 `(vs_incumbent, vs_pin)` combinations map to the D5 table (`(behind, ahead)` and `(behind, inconclusive)` are `regression`, not `inconclusive`).
2. **Pin record and flag.** Add `BaselinePin`, `read_pin()`, `write_pin()` (append-only `pins` list under `<project-root>/.ll/harness-pins/`), `_pin_refusal()` (D6: full-n measurement of the pinned content required on every write; re-pin additionally monotone; `--force` bypasses and sets `forced`), and `_run_pin_phase()`. Register `--pin-baseline`, `--pin-reason`, `--force` in `_add_evaluator_flags`; extend `_baseline_flag_refusal()` (pin + measure/compare/`--retry-of` → exit 2; non-`skill` runners without `--baseline-of` → exit 2) and `cmd_dsl`'s baseline refusal. Refusal exits 2 with zero DB writes and zero subject invocations, following the `--retry-of` precedent.
3. **Second arm.** In the four `cmd_*` compare branches, call `read_pin()`; when present, run `_compare_baseline_refusal()` on the pinned key (message carries the D8 procedure with the pin's `pinned_head_sha` and `target_path` substituted), then pass `frozen=read_baseline(pinned_key, conditions), pin=pin` into `_run_compare_arm()`. When the pin's content hash equals the incumbent hash, set `frozen` to the same `BaselineResult` (no second query) and note it in the report.
...
5. **Tests.** Extend `TestBaselineCompare` (`test_cli_harness.py:3081-3239`): no-pin path byte-identical (existing three `test_delta_*` tests untouched and passing); pin present + full-n rows → dual report and JSON keys, `frozen` payload has no `head_sha_differs`; pin present + no rows → exit 2, zero subject invocations, stderr contains `git show <pinned_head_sha>:` and `--measure-baseline`; first pin refused when the content is unmeasured, allowed with `--force` (entry has `forced: true`); re-pin refused when new content measures below the active pin, allowed when ≥; pin history never shrinks; `--pin-baseline` combined with `--measure-baseline`/`--compare-baseline`/`--retry-of` → exit 2; pin on `prompt` without `--baseline-of` → exit 2; pin files resolve under the project root when cwd is a subdirectory.
6. **Docs.** `docs/reference/CLI.md` (`--pin-baseline`/`--pin-reason`/`--force` rows in the shared-flag table next to `:233-239`; `--compare-baseline` row gains the dual-arm sentence and the D8 re-measure procedure), `docs/guides/EVALUATION_GUIDE.md:386,400-404` (companion frozen-arm paragraph including the D8 procedure), `docs/guides/HARNESS_OPTIMIZATION_GUIDE.md:305-314` (cross-link from the ENH-3421 frozen-reference subsection: within-run vs cross-run anchoring), `docs/reference/API.md` (document `BaselineDelta`/`BaselineResult`/`BaselineKey`/`BaselinePin` — none exist there today), `CHANGELOG.md` one-liner in the current release section (never `[Unreleased]`). Register each new doc string in `scripts/tests/test_wiring_reference_docs.py`.
...
1. `ll-harness skill <target> [args] --pin-baseline` writes a pin whose `target_content_hash` equals `_incumbent_content_hash()` of the target at that moment (and equals `_hash_file()` of the unmodified file); `prompt`/`cmd`/`mcp` pin the content hash of `--baseline-of <attempt>` and refuse (exit 2) without it. A second invocation appends rather than replaces, and the file's `pins` list never loses an entry. The pin file lives under the project root's `.ll/harness-pins/` regardless of cwd.
2. Any pin write is refused (exit 2, no write, no subject invocation) when the content being pinned has no full-n measurement under the current input and conditions; a re-pin is additionally refused when the new content's rate is below the active pin's. `--force` allows either and the entry records `forced: true`.
3. With no pin, `--compare-baseline` text and `--json` output are unchanged from ENH-3435 for every existing key and line (the three existing `test_delta_*` tests pass unmodified). `--pin-baseline` combined with `--measure-baseline`, `--compare-baseline`, `--retry-of`, or the `dsl` runner exits 2 before any work.
4. With a pin and full-n rows for the pinned content, the report carries `vs_incumbent`, `vs_pin` ∈ `{ahead, behind, inconclusive}` and `outcome` ∈ `{improvement, drift, regression, inconclusive}`, computed per D4/D5 from Wilson intervals that `_arm_verdict()` derives from `passed`/`graded` (a tally whose `ci_lo`/`ci_hi` are `None` classifies correctly), in both text and JSON. A candidate `behind` the incumbent reports `regression` even when `vs_pin` is `ahead` or `inconclusive`.
5. With a pin but no full-n rows for the pinned content under current conditions, the run refuses before any subject invocation (exit 2, zero candidate rows written) and the message contains the D8 procedure with the pin's `pinned_head_sha` and target path substituted (`git show <sha>:<path> > <path>`, `--measure-baseline`, `git checkout -- <path>`).
