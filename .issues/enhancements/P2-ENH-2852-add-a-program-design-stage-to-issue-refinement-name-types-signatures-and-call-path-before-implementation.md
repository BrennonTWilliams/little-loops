...
- [ ] The intervention cutover point (the SHA/date at which this gate is enabled) is recorded at `.ll/program-design-cutover.json` (`{"sha": ..., "date": ...}`, decided path + schema) before the gate ships, so FEAT-2855 **and FEAT-2867** can retroactively compute the pre-intervention window from immutable git history; both consumers parse the same file at the same path (see Scope Boundary below).
...
- `skills/manage-issue/SKILL.md` — gains the `Deviations`-writing step (decided; see Design Notes). Its "Mismatch Handling Protocol" (~L325-331) is the attach point — today it handles divergence interactively and persists nothing
