...
4. Expose `recurrent_window` in the circuit schema (`config-schema.json`) and document
...
- [ ] `recurrent_window: N` in `circuit.repeated_failure` causes the circuit breaker to
  fire when a `(state, exit_code, verdict)` triple has been seen N times total (not
  necessarily consecutive)
- [ ] Existing loops without `recurrent_window` are unaffected
- [ ] A `circuit_stall` event is emitted on the event bus when the recurrent threshold
  is crossed (matching the existing consecutive-failure event format)
- [ ] `config-schema.json` updated with `recurrent_window` field
- [ ] `docs/reference/loops.md` `circuit` section documents the new key
- [ ] Tests in `scripts/tests/test_builtin_loops.py` cover the recurrent-window trigger
...
- **In scope**: Adding `recurrent_window` to `circuit.repeated_failure`; tracking total occurrences of `(state, exit_code, verdict)` triples per run; routing to `on_repeated_failure` on threshold breach; emitting `circuit_stall` event; updating `config-schema.json` and `docs/reference/loops.md`
...
- `config-schema.json` — add `recurrent_window` integer-or-null field under `circuit.repeated_failure`
...
- `config-schema.json` — needs `recurrent_window` field with `null` default

### Configuration
- `config-schema.json` — `recurrent_window: integer | null` under `circuit.repeated_failure`
