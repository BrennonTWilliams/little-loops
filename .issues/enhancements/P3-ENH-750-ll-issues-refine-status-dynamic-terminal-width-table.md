...
- `refine_status.py`: add `_elide_columns(active_cols, term_cols, …) -> list[str]` helper called before the render loop
- `config-schema.json` / `ll-config.json`: optional `refine_status.elide_order: list[str]` (default: see Proposed Solution)
- No changes required to other CLI commands

## Implementation Steps

1. Define a default elision order constant in `refine_status.py` (e.g. `_ELIDE_ORDER = ["source", "norm", "fmt", "confidence", "ready", "total", ...]`).
2. After computing active columns and command columns, sum their widths (re-use logic near line 260).
3. If total > `term_cols`, remove columns from `_ELIDE_ORDER` one at a time until the table fits or only the minimum set remains.
4. Expose `elide_order` in `config-schema.json` and read it in `refine_status.py` (override default if set).
5. Add unit tests in `scripts/tests/test_refine_status.py` covering narrow (60-col), medium (80-col), and wide (160-col) scenarios.
