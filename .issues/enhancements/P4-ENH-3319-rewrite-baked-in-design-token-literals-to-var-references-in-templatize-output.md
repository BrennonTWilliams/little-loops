...
- **Hard preconditions — one test each, five in total**: no `<head>`/`<style>`; no root `<html>` (a `<style>`-carrying fragment); a `data-theme` disagreeing with the active theme; `active_theme: sepia` (a real third theme file, which `render_as_css_vars_themed` has no scope for); and a config whose `themed_css_vars` returns the degenerate empty-blocks fallback. All five leave the body entirely unlifted with an unchanged manifest and exit code.
...
- New: `verify_lift_renders(tmp_dir: Path, data: dict[str, Any], emitted_var_names: list[str], config: object) -> str | None` — renders the re-serialized `tmp_dir` and returns an error string if any `[[=`/`[[%` survives or any emitted `var(--x)` lacks a `--x:` declaration; `None` on success (§ Decision Rules § Post-lift render verification).
...
- **Post-lift render verification — a runtime check, not just a test (mandatory)**: Impact § Risk 2 (inert stamp point ⇒ colorless artifact) and § Risk 4 (lifted body never re-serialized ⇒ pre-lift template promoted at exit 0) are both **silent in production**: every span-level assertion and `verify_lift_reversible` stay green, the exit code stays 0, and the defect surfaces only in the AC #2 render-through *test*. Guarding the two highest-impact failure modes with test coverage alone is not enough — the same omission in a later refactor ships a broken artifact.
  Rule: under `--lift-tokens`, after step 6 has re-serialized `template.{suffix}.j2` and `manifest.yaml` into `tmp_dir` and **before** `promote()`, `cmd_templatize` re-renders the lifted `tmp_dir` and asserts two properties of the output string:
  1. No residual Jinja delimiters — neither `[[=` nor `[[%` appears (`build_environment`, `artifact_templates.py:259-279`). Catches an unevaluated or escaped stamp point.
  2. For every `var(--x)` reference the lift emitted, the declaration `--x:` appears in the rendered output. Catches a missing/degenerate `ll.theme_css`, a `manifest["theme"]` that never reached disk, and var-name mangling drift in one assertion.
...
- **Risk**: Medium-High. Four failure modes, none of which the reversibility
  ...
  2. Wrong stamp mechanism — an inert placeholder ships a colorless artifact
     ...
     rendered output still carries `[[=` or lacks a declaration for an emitted
     `var()`, plus the render-through-`render_template` test.
