...
- **Ambiguous literal-to-token mapping — alias-preference filter, then fail-closed**: a naive ">1 candidate ⇒ skip" rule blocks **11 of 22 light and 12 of 22 dark colors** on this repo's own profile, and blocks precisely the semantic tokens the feature exists to lift (see Codebase Research Findings above). The cause is structural: every semantic token is declared as an alias (`"{color.paper.0}"`), so it *always* resolves equal to its primitive and *always* collides with it. Rule:
  ...
  **Reading the raw layers correctly**: unlike `.resolved` (a flat dotted-name→value map, `design_tokens.py:35`), `DesignTokens.semantic` and `.theme` are **raw nested dicts** (`:33-34`) straight from the JSON files. The filter must flatten them with the loader's own `_flatten` and apply the **same layer order the loader uses** — `_flatten(semantic)` then `_flatten(theme)` overriding it (`design_tokens.py:373-380`; typography/spacing sit between and carry no colors). Order matters, and not only for tidiness: a theme file may override a semantic alias with a **concrete literal** (or vice versa), which flips that name's alias status and therefore flips which candidate wins. Testing `semantic` alone would pick the pre-override answer. The predicate is: the effective raw value for this name, after layering, is a string of the form `{...}`.
...
- **Hard preconditions — no partial lift**: the body is lifted only if **all five** hold:
  ...
  If any fails, nothing is rewritten, the manifest is untouched, everything is reported as `unlifted` (with the failed precondition as the reason), and the exit code is unchanged — a body with `var()` refs and no declarations must never reach `promote()`.
...
4. Build the candidate map: invert `tokens.resolved` (loaded with an **explicit** `theme=dt_cfg.active_theme`, per Decision Rules § Theme selection), excluding `_`-prefixed names, then apply the alias-preference filter (Decision Rules § Ambiguous literal-to-token mapping) over the **flattened, correctly layered** raw token data — `_flatten(tokens.semantic)` overridden by `_flatten(tokens.theme)`, since both are nested dicts and a theme file can override an alias with a literal — not `resolved`, which has already erased the alias. Emit `var(--{name.replace(".", "-")})`. Test the recorded ambiguity cases from Codebase Research Findings directly (`#e8dcc4` lifts to `color.border.subtle`; `#fdfbf6` light stays unlifted with both semantic candidates reported), plus a case where the theme layer overrides a semantic alias with a concrete literal and the winning candidate changes.
...
8. Extend `_write_unlifted_tokens_report` with a `"lifted"` list alongside `"unlifted"` and rewrite the hardcoded `_comment` (`templatize.py:701-709`), which currently asserts "Report-only — not rewritten." Thread the lift's authoritative `lifted`/`unlifted` split into `_report_unlifted_tokens` as keyword args defaulting to `None` (see § Signatures) rather than letting it rescan — a rescan of the lifted body always yields `"lifted": []`. Leave its bare `except Exception` intact.
...
- **Risk**: Medium-High. Three failure modes, none of which the reversibility
  ...
  4. Lost lift (new) — the lifted body and `theme: design-tokens` manifest
     ...
     reversibility test green; caught only by the render-through and
     promoted-output tests.
