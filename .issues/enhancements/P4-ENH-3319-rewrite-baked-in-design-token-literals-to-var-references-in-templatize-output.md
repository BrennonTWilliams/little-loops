...
- **Hard preconditions — one test each, five in total**: no `<head>`/`<style>`; no root `<html>` (a `<style>`-carrying fragment); a `data-theme` disagreeing with the active theme; `active_theme: sepia` (a real third theme file, which `render_as_css_vars_themed` has no scope for); and a config whose `themed_css_vars` returns the degenerate empty-blocks fallback. All five leave the body entirely unlifted with an unchanged manifest and exit code. **Plus a sixth test on precondition 5's exception path**: a malformed `themes/<active>.json` makes `themed_css_vars` raise a `JSONDecodeError`, and the assertion is that `cmd_templatize` still exits **0** with an unlifted body — not exit 1 via the outer `except Exception`.
...
- **Declaration source, and the render-time availability hole — decided, revisitable**: the declarations come from `[[= ll.theme_css =]]`, i.e. `themed_css_vars(config)` evaluated **at render time, against the rendering project's config** (`artifact_templates.py:311-318`). This is not the same guarantee as the lift-time check. `themed_css_vars` degrades to `":root {\n}\n[data-theme=dark] {\n}"` — two *empty* blocks — whenever the rendering context has no tokens configured (`artifact_template_kit.py:38-49`), and a `.llat` directory is portable: `render.py`, `extract.py`, and `fsm/persistence.py::promote_run_artifact` may re-render it later, potentially in a project with no design tokens. In that case every lifted `var()` resolves to nothing and the artifact renders **colorless**, with no error. AC #12's all-or-nothing guarantee is a *templatize-time* guarantee and cannot cover this.
  Decision for v1: keep `[[= ll.theme_css =]]` (it is the hook ENH-3035 built, and it costs zero render-engine change, preserving AC #7), and close the near half of the hole with a lift-time precondition — **refuse to lift if `themed_css_vars(config)` returns the degenerate empty-blocks fallback at lift time** (see § Hard preconditions). The residual — a lifted `.llat` re-rendered in a *different*, token-less project — is accepted and must be **documented** in both `docs/reference/CLI.md` and `docs/ARCHITECTURE.md` as a stated limitation of `--lift-tokens`, not left implicit.
...
- **Hard preconditions — no partial lift**: the body is lifted only if **all five** hold:
  ...
  5. `themed_css_vars(config)` at lift time returns a **non-degenerate** result — not the empty-blocks fallback `":root {\n}\n[data-theme=dark] {\n}"` (`artifact_template_kit.py:38-49`). Lifting against a token set that cannot emit declarations produces a colorless artifact immediately, not just on a later re-render (§ Declaration source).
...
2. Add the CSS-value-position guard to `find_token_literals` — implement the **two-part scope-then-nearest-delimiter rule** in Decision Rules § CSS-context guard, not the earlier "after a `:` and before a `;`" phrasing that section now documents as wrong. Write the `href="#dedede"` / `#face {`-inside-a-`<style>`-that-also-contains-`a:hover` / `"#c0ffee"`-in-`<script>` rejection tests **and** the `box-shadow: … #e8dcc4, …` / `background: #fdfbf6 url(…)` / `color: #fdfbf6 !important;` acceptance tests first. **Do this before any rewrite code exists** — it is the difference between a false positive being a warning line and being artifact corruption.
...
- **Effort**: Large — ten implementation steps across a CLI flag, a new
  ...
  roughly fifteen new tests. Not the "expose spans and splice" Small the first
  revision implied.
...
3. A matched literal **outside** CSS-value position — `href="#dedede"`, an id selector `#face {` inside a `<style>` that also contains a `:`-bearing rule such as `a:hover`, `"#c0ffee"` inside `<script>` — is never rewritten and is still reported. Tested explicitly with token values chosen to collide with all three. Conversely, a literal **inside** a declaration value is rewritten regardless of what character follows it: `box-shadow: 0 0 2px #e8dcc4, …`, `background: #fdfbf6 url(…)`, and `color: #fdfbf6 !important;` all lift. The guard is the two-part scope-then-nearest-delimiter rule (Decision Rules § CSS-context guard); a rule phrased as "somewhere after a `:` and before a `;`/`}`" fails the `#face` case, and one phrased as an enumerated immediate-terminator set fails the three acceptance cases.
...
12. Lifting is all-or-nothing per body, gated on **all five** hard preconditions (Decision Rules § Hard preconditions): stamp point placeable, root `<html>` present, no disagreeing `data-theme`, `active_theme ∈ {light, dark}` (or a `design_md` source), and a non-degenerate `themed_css_vars(config)` at lift time. If any fails, nothing is rewritten, the manifest is untouched, everything is reported as `unlifted` with the failed precondition named in the payload-level `lift_skipped_reason` key (per AC #6 — **not** a field on `UnliftedToken`), and the exit code is unchanged. Each of the five has its own test. Precondition 5 is evaluated inside a `try`/`except Exception` so that a *raising* `themed_css_vars` (malformed theme JSON reaches `json.load` unguarded at `design_tokens.py:79-83`) is a failed precondition at exit 0, not an exit 1 through `cmd_templatize`'s outer handler; a sixth test covers that path. No body carrying `var()` refs without their declarations can reach `promote()`.
