...
### Changes Made
1. Moved `_format_relative_time()` to `cli/output.py` as `format_relative_time()` (shared utility)
2. Replaced `===` banners with `── Title ─────` style in `_helpers.py` and `show.py`
3. Dynamic title truncation: `max(45, terminal_width() - 30)` instead of hardcoded 45
4. Omit empty description — only print `Description:` when value exists
5. Human-friendly timestamps — `2026-04-02 19:49 UTC (3h ago)` format
6. Composition breakdown — `Composition: 1 BUG, 1 FEAT | P1: 1, P2: 1`
7. Sprint run state — loads `.sprint-state.json` and shows `Last run: ...`
8. Readiness/confidence scores — `[ready: 85, conf: 72]` inline after each issue
9. Issue file paths — shown below each issue entry
10. `--json` flag — structured JSON output for scripting

### Files Modified
- `scripts/little_loops/cli/output.py` — Added `format_relative_time()`
- `scripts/little_loops/cli/loop/lifecycle.py` — Delegated to shared `format_relative_time`
- `scripts/little_loops/cli/sprint/show.py` — Metadata improvements, JSON output, helper functions
- `scripts/little_loops/cli/sprint/_helpers.py` — Score suffix, file paths, separators, dynamic truncation
- `scripts/little_loops/cli/sprint/__init__.py` — Added `--json` argument to show parser
- `scripts/tests/test_sprint.py` — 9 new tests covering all improvements
