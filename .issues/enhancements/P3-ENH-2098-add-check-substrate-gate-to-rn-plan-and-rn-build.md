...
1. Read `harness-plan-research-implement-report.yaml:57-63` to extract canonical `check_substrate` state shape
...
### Files to Modify
- `loops/rn-plan.yaml` — add `check_substrate` state after plan-review
- `loops/rn-build.yaml` — add `check_substrate` state after plan-approval
- `scripts/tests/test_builtin_loops.py` — extend structural test pattern from ENH-2085

### Dependent Files (Callers/Importers)
- `loops/harness-plan-research-implement-report.yaml` — source pattern to copy (read-only reference)

### Similar Patterns
- `loops/harness-plan-research-implement-report.yaml:57-63` — canonical `check_substrate` state shape to replicate
