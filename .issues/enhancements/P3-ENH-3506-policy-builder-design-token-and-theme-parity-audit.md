...
1. **Rename template refs to existing semantic tokens** (CSS name = `--` + dotted key with `.`→`-`): `--color-surface-base` → `--color-surface-primary`; `--color-surface-sunken` → `--color-surface-secondary`; `--color-text-on-sunken` → `--color-text-secondary`; `--color-border-default` → the appropriate `--color-border-{subtle,strong}` for the component; `--typography-font-family-base` → `--font-family-body`; `--typography-font-family-mono` → `--font-family-mono`. Border role rule: `--color-border-strong` for input/control borders (inputs, selects, buttons, textareas), `--color-border-subtle` for dividers and card/container outlines — `border.subtle` measures only 1.19–1.63:1 against `surface.primary` in every profile, so it must not be the sole boundary of an interactive control. Normalize equivalent fallback literals while preserving readable foreground/background pairs. The `pre` mapping intentionally changes the light-theme YAML block from a fixed dark code panel to a light `surface.secondary` panel (measured `text.secondary` on `surface.secondary`: 7.76–11.85:1 across all six profile/theme combinations).
2. **Add paired action and status colors** to `semantic.json` (the light baseline) plus `themes/dark.json` in every packaged profile — not `themes/light.json`, which is a sparse 1–3 key surface override by convention — for (`default`, `editorial-mono`, `warm-paper`) and this checkout's `.ll/design-tokens/` mirror. Keep `--color-action-primary-text` and introduce `color.action.primary-text`, explicitly paired with `color.action.primary`; do not alias it blindly to `color.text.inverse`. Review measured that proposed inverse mapping at 2.38:1 in editorial-mono dark and 4.17:1 in warm-paper dark. Add `color.status.{success,warning,error,info}.{bg,text}` and keep the template's corresponding names. Require at least 4.5:1 contrast for normal text on primary buttons, status messages, YAML output, and winning-rule highlights across all six profile/theme combinations. Check the actual inherited foreground on `.rule-winner`, not just the status token pair. `test_ambient_mirror_matches_packaged_profiles` and `lint_profile()` theme completeness must stay green. Note `lint_profile()` is color-*group*-level: it forces `dark.json` to override the new `status` group, but will not notice a dark theme missing `action.primary-text` (the `action` group already exists, so dark would silently inherit the light value). The contrast test is the only guard for that key.
3. **Define compatibility for existing consumers before changing tokens.** Local mirrors take precedence and are not merged with newly packaged keys. **Decision (2026-09-19): theme-aware template fallback, no migration tool.** The template declares its own defaults for the new action/status tokens in `:root` and `[data-theme=dark]` blocks placed *before* `/*__THEMED_CSS_VARS__*/`; stamped declarations come later at equal specificity and override them when present (stamped blocks are full sets, so a stamped light value never leaks into dark). One mechanism covers old mirrors, partial `DESIGN.md`, and disabled tokens, touches no custom values, and needs no upgrade command. Without it an old mirror in dark mode renders the light `#d4edda` winning-rule background under light text. Record the behavior in Findings and user documentation. Preserve existing custom values. Test a pre-change mirror missing the new keys, plus partial `DESIGN.md` and disabled-token configurations. Scope full declaration parity to complete packaged profiles; fallback configurations must remain readable without pretending to supply a complete token set.
...
5. **Permanent regression coverage:** extract template `var(--x` references, assert each is declared in both rendered theme blocks for every packaged profile, and compare emitted values with `load_design_tokens(config, theme=...).resolved`. Isolate config (`monkeypatch.chdir(tmp_path)` + a local config helper) and exercise both packaged fallback and freshly materialized mirrors. Do not require every light/dark value to differ: fonts and other shared values legitimately match. Test degraded sources separately; branch on the returned `DesignTokens.source`, since `source: auto` can resolve to `design_md`, whose two blocks intentionally match. The parser must read only the stamped blocks, not the template-local default blocks from step 3. Enumerate profiles from an explicit name list (`default`, `editorial-mono`, `warm-paper`), not `iterdir()` (a stray gitignored `.ll/` under the packaged profiles dir broke iteration once; removed 2026-09-19). Add token-derived contrast checks for the paired colors (including `text.primary` on `status.success.bg` for `.rule-winner`) and string assertions on the emitted CSS (`color-scheme`, default blocks). Browser/computed-style checks are **not** pytest: they live in an on-demand Playwright probe loop (`.loops/verify-enh-3506-theme.yaml` + `.loops/probes/enh-3506-theme-probes.mjs`, FEAT-3488 precedent) using `colorScheme` emulation and `getComputedStyle` on `.rule-winner`, buttons, status messages, `pre`, and a `<select>` in both themes with an opposing OS preference.
6. **Isolate the golden test before regenerating its fixture.** Pin render inputs in the test: `tmp_path` cwd with no `.ll/design-tokens/` mirror, packaged `default` profile, `source: profile`, explicit `active_theme`, plus any other environment-dependent input (catalog resolution/config); use the same setup for regeneration. Verify the intended changes before overwriting (BUG-2303 advisory), then regenerate once. Demonstrate that ambient config/mirror changes do not affect the golden comparison. Correct documentation of the resulting theme and fallback behavior.
7. **Record the two verdicts.** Assess worktree-copy necessity with mirror-present and mirror-absent verification after golden isolation; token-name correctness alone does not establish that the copy is redundant. With the golden pinned to a mirror-less packaged profile (step 6), the expected verdict is *redundant*; the mirror-absent run confirms rather than investigates. Record whether `ll-doctor` needs a template-aware companion, but implement any new doctor companion separately. Spacing/radius tokenization remains out of scope.
...
- `.loops/verify-enh-3506-theme.yaml` + `.loops/probes/enh-3506-theme-probes.mjs` (new) — on-demand Playwright probe for computed styles, native controls, and opposing OS `colorScheme`; never a pytest gate
...
- Compatibility implementation: template-local default `:root`/`[data-theme=dark]` blocks ahead of the stamp placeholder (decided; no migration tooling)
...
1. Establish isolated render fixtures for all packaged profiles and both themes; capture a pre-change mirror lacking the new tokens. Compatibility approach is decided: template-local theme-aware defaults (Proposed Solution step 3).
...
4. Verify all three authoring modes in both themes via the `.loops/` Playwright probe, including an OS preference opposite to the selected theme. Check buttons, all status types, winning-rule highlights (including inherited text), native controls, and YAML output.
...
- [ ] Native controls follow the selected `data-theme` even when OS preference differs. Initial theme and toggling retain existing theme precedence; the on-demand Playwright probe (not pytest) covers all three modes and the listed text surfaces in both themes.
