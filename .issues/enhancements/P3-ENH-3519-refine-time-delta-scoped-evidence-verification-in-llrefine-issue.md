...
**JSON contract of the new modes:** `--save-snapshot` emits the normal file-mode payload plus `snapshot_path`. `--delta-from` emits `{"ok", "mode": "delta", "status": "clean"|"new_findings", "new_findings": [{"span", "resolved_artifact", "added_count", "preexisting_count", "candidates": [{"line", "section", "span", "artifact"}]}], "count", "findings": [...]}` — every `findings` entry (here, in plain file mode, and in the snapshot) gains `resolved_artifact` alongside the existing `artifact`; the addition is backward compatible for current consumers. `findings` remains the full current scan so the caller never loses known findings. Every exit 2 raised by the new modes (as opposed to argparse usage errors) prints a JSON body under `--json`: `{"ok": false, "mode", "status": "incomplete", "error": "<reason>", "findings": [...]}`, where `findings` carries the snapshot's findings when the snapshot loaded but the after-scan failed, else `[]`. "A failed after-scan retains known findings" is delivered by this body; a bare stderr message cannot carry them.
