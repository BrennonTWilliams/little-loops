...
`ll-harness` reports one of pass, fail, or abstain per run. Two runners that both satisfy the same semantic criterion are indistinguishable in the verdict, even when one spent several times the tool calls, tokens, and wall-clock of the other. Only wall-clock (`duration_ms`) is captured and persisted today, and only to the `harness_events` row — it does not appear in `--output json`. Tokens and tool calls are present in the host CLI's stdout that the harness already captures (on the skill path: the stream-json `result` event's `usage`, plus `tool_use` content blocks inside `assistant` events; on the prompt path: the single JSON blob's `usage` for claude-code, or the NDJSON `turn.completed` event for codex) but are never parsed, surfaced, or persisted.
...
1. **Capture is post hoc from stdout, not via streaming callbacks.** The two harness paths that invoke an LLM already capture the usage data: `_run_skill()`'s default blocking branch (`runner_spec.py:250-262`) runs `build_streaming()` so stdout is stream-json (a `result` event carrying `usage`, plus one `tool_use` content block per tool call inside `assistant` events — see `subprocess_utils.py:585-603`); `_run_prompt()` (`:422-435`) runs `build_blocking_json()`, whose stdout shape is host-specific: on claude-code it is one JSON object carrying `usage` and `num_turns`; on codex it is NDJSON (`--json`, `host_runner.py:1086`) ending in a `turn.completed` event carrying `usage`. Parse those after `subprocess.run()` returns. The helper (Decision 8) handles both shapes on both paths; hosts with neither shape (gemini/qwen/omp/opencode/pi) yield `None` for every field. Note that on the skill path `RunnerResult.stdout` is *already* the raw stream-json text — it is what `_grade()` hands the judge and what `--output json` prints today (ENH-2668 byte-for-byte contract) — so parsing it post hoc is sound and the stdout contents must not be "cleaned up" as a side effect of this work. Do **not** wire `on_usage`/`on_usage_detailed` into the runners: `_run_cmd()`/`_run_mcp()` never invoke a host CLI, and the skill/prompt paths would need a streaming restructure for no gain. The earlier wiring directive below is superseded by this decision.
2. **Where the numbers are born and how they flow.** New trailing fields `RunnerResult.input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`, `tool_calls` (all `int | None = None`) are populated inside `runner_spec.py` by the runner that knows its stdout format. From there they flow two ways, both reading off the `RunnerResult`: (a) `_grade()` copies them onto `HarnessEvalOutcome` when it constructs the outcome — a field copy, not a gating input; `_grade()`'s pass/fail/abstain logic reads only `timed_out`/`error`/`exit_code` and stays untouched — so `--output json` and the N-sample per-sample entries carry them without per-site stamping; (b) `_record_harness_event()` gains a `result: RunnerResult | None = None` kwarg (or the five explicit kwargs) and each of the **6** `_record(result, duration_ms, outcome)` closures in `cli/harness.py` (`:2139`, `:2273`, `:2393`, `:2512`, `:2676`, `:2772`) forwards them beside `duration_ms`. `duration_ms` itself is also added to `HarnessEvalOutcome` so the surfaced vector has all three named dimensions, not just the two new ones. It cannot be stamped on the outcome *after* the fact by the `cmd_*` caller: on the single-run path `_evaluate_and_report()` (`cli/harness.py:1948`) calls `_grade()` and prints the `--output json` payload before it returns the outcome, and neither function receives `duration_ms` today. So `_grade()` gains a `duration_ms: int | None = None` kwarg (copied onto the outcome, never read by gating) and `_evaluate_and_report()` gains the same kwarg as a pass-through. Every caller already has the value in scope: the four `cmd_*` single-run paths, `cmd_dsl` (`:2738`), and `_run_sample_loop` (`:1913`).
3. **Token fields are split, not summed.** Mirror all four token fields of `TokenUsage` (`subprocess_utils.py:63`): `input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`. A single `tokens` sum is ambiguous (cache-read inclusion) and would not be "named and stable"; dropping `cache_creation_tokens` would leave the cost-analytics consumer unable to price cache writes, which are billed at a different rate. Consumers derive totals. On codex, `cache_creation_tokens` lands as `0`, not `None`: `TokenUsage` fields are plain `int` (never `None`) and codex's `turn.completed` usage block has no `cache_write_input_tokens` key (it reports a single `cached_input_tokens`). Accept `0` — codex genuinely bills no cache writes — and document it on the field rather than widening the helper's return shape. The claude `result` event also carries `total_cost_usd`; it is **not** captured here — it is host-computed, absent on codex, and derivable from the four token fields plus a price table, so it would be a second, host-dependent source of truth.
4. **`tool_calls` on the PROMPT/DSL path is `None`.** claude-code's `--output-format json` emits no `tool_use` blocks (only `num_turns`, which is not a tool-call count), and codex `item.*` events are not tallied. Accept `None` rather than switching `_run_prompt()` to stream-json. On the SKILL path `tool_calls` counts **top-level** `tool_use` blocks only — calls made inside an Agent-tool subagent never appear in the parent stream — matching the existing FEAT-2878 `on_tool_call` tally; document this on the field. CMD/MCP runners leave all five fields `None`; `_STOCHASTIC_RUNNERS` (`runner_spec.py:79`) is the existing predicate for "may be populated" vs "inherently `None`".
5. **Timed-out and errored runs leave the fields `None`.** "Errored" means `RunnerResult.error is not None` (a spawn failure such as `FileNotFoundError`, where stdout is empty anyway). Timed-out is automatic: `subprocess.run()` discards stdout when it raises `TimeoutExpired`, and both runners return `stdout=""`. A run that exits nonzero, or whose `result` event carries `is_error: true`, but still emits a usage block **does** populate the fields — the cost was real and the row should carry it. `duration_ms` stays populated for every row (it is measured by the caller). No partial capture is attempted.
...
8. **One shared parsing helper, in two layers.** `_process_line` (`subprocess_utils.py:561`) is per-event with closure state (`detected_model`, `tool_call_count`), so a lines-aggregating helper does not fit inside it. Add two functions to `subprocess_utils.py`: (a) `usage_from_event(event: dict, *, default_model: str) -> TokenUsage | None` — the single place that knows the Claude `result` usage keys (`cache_read_input_tokens`/`cache_creation_input_tokens`) and the Codex `turn.completed` keys (`cached_input_tokens`/`cache_write_input_tokens`); both existing `_process_line` branches call it and keep only their side effects (the `on_usage` sum of `input + cache_read`, `is_error` stderr line, `result_seen`); (b) `usage_from_stream_lines(stdout: str) -> tuple[TokenUsage | None, int | None]` — tries `json.loads(stdout)` on the whole string first (the claude `--output-format json` blob is not guaranteed to be one line; `run_blocking_json()` at `host_runner.py:2762-2771` already uses this whole-string-then-last-line chain and is the precedent), then falls back to per-line parsing: `usage_from_event()` for the terminal event, plus a tally of `tool_use` blocks in `assistant` events (`None` when no `assistant` event was seen, so the PROMPT path yields `None` rather than `0`). The harness must not become the fourth inline tally (`cli/loop/audit.py`, `subprocess_utils.py`, `fsm/executor.py` each have their own today).
...
- `RunnerResult` (`scripts/little_loops/runner_spec.py:100`): new trailing fields `input_tokens: int | None = None`, `output_tokens: int | None = None`, `cache_read_tokens: int | None = None`, `cache_creation_tokens: int | None = None`, `tool_calls: int | None = None`, populated by `_run_skill()` (default blocking branch, parsed from stream-json stdout) and `_run_prompt()` (parsed from the claude JSON blob's `usage` or the codex `turn.completed` event; `tool_calls` stays `None`). The existing `tool_trace` field is trace-mode-only and unrelated; leave it alone.
- `HarnessEvalOutcome` (`scripts/little_loops/cli/harness.py:863`): same five fields plus `duration_ms: int | None = None`, all trailing, `None` default, so `--output json` / N-sample entries carry the full vector beside `passed`/`verdict`. The five token/tool fields are copied from `result` inside `_grade()`; `duration_ms` is passed into `_grade()` as a kwarg by the caller that measures it (Decision 2) and copied onto the outcome there too.
- `HarnessEvent` (`scripts/little_loops/history_reader/harness.py:54`): already tracks `duration_ms: int | None` (ENH-2741); append the same five fields with an `# ENH-3464` comment, and add the five column names to `_HARNESS_EVENT_COLUMNS` in the same order.
...
- `usage_from_event(event: dict, *, default_model: str) -> TokenUsage | None` (new, `scripts/little_loops/subprocess_utils.py`) — per-event parser for the Claude `result` and Codex `turn.completed` usage shapes; both `_process_line` branches refactored to call it (Decision 8a)
- `usage_from_stream_lines(stdout: str) -> tuple[TokenUsage | None, int | None]` (new, `scripts/little_loops/subprocess_utils.py`) — whole-stdout parser: whole-string `json.loads` first, then per-line; returns `(usage, tool_call_count)` with `tool_call_count=None` when no `assistant` event was seen (Decision 8b); called by `_run_skill()`/`_run_prompt()`
- `_record_harness_event(..., duration_ms: int, ...)` (`scripts/little_loops/cli/harness.py:216`) — gains trailing `input_tokens=None, output_tokens=None, cache_read_tokens=None, cache_creation_tokens=None, tool_calls=None` kwargs, forwarded to `record_attempt()` → `_insert_harness_event()` (`session_store/writers.py:1367`, `:1109`; `record_attempt()` already forwards `**event_fields`, so only `_insert_harness_event()` and `record_harness_event()` need explicit new kwargs)
- `_grade(..., duration_ms: int | None = None)` (`scripts/little_loops/cli/harness.py:1224`) — gating logic **unchanged**; the only changes are the new kwarg and copying the five efficiency fields from `result` plus `duration_ms` onto the `HarnessEvalOutcome` it constructs (including the early-return outcome at `:1247` for timed-out/errored results, where the five fields stay `None` but `duration_ms` is set). `passed`/`verdict`/`abstained` are computed exactly as today.
- `_evaluate_and_report(..., duration_ms: int | None = None)` (`scripts/little_loops/cli/harness.py:1948`) — pass-through to `_grade()`; the two early `_report()` returns at `:1958-1963` also stamp `duration_ms` on the outcome they construct.

### Call Path

`_run_skill()`/`_run_prompt()` (`runner_spec.py`) parse captured stdout via `usage_from_stream_lines()` (which delegates per event to `usage_from_event()`) -> populate `RunnerResult.{input_tokens,output_tokens,cache_read_tokens,cache_creation_tokens,tool_calls}` -> the caller passes its measured `duration_ms` into `_grade()` (directly from `_run_sample_loop()`, or via `_evaluate_and_report()` on the single-run and DSL paths) -> `_grade()` copies the five fields plus `duration_ms` onto `HarnessEvalOutcome` (not read by any gate) -> each `cmd_*` `_record(result, duration_ms, outcome)` closure in `cli/harness.py` forwards them beside `duration_ms` to `_record_harness_event()` (6 call sites) -> persisted as `harness_events` columns via `record_attempt()` -> read back on `HarnessEvent` via `authoritative_attempt()`/`authoritative_attempts()`. `_evaluate_and_report()` renders `outcome.{duration_ms, tokens..., tool_calls}` into `--output json` and the text summary. `_band_samples()` and `evaluate()` never read the fields; `_grade()` only copies them.
...
1. `subprocess_utils.py`: add `usage_from_event()` and `usage_from_stream_lines()` (Decision 8); refactor `_process_line`'s `result`/`turn.completed` branches to call `usage_from_event()`, keeping the `on_usage` sum, `is_error` stderr line, and `result_seen` side effects in place. Unit-test `usage_from_stream_lines()` directly against a Claude stream-json fixture (with two `tool_use` blocks), a Claude `--output-format json` blob fixture (both single-line and pretty-printed multi-line), a Codex `turn.completed` NDJSON fixture (assert `cache_creation_tokens == 0` and `tool_calls is None`), and non-JSON noise (`"ok"`, expect `(None, None)`).
2. `runner_spec.py`: add the five trailing `RunnerResult` fields; populate them in `_run_skill()`'s default blocking branch and in `_run_prompt()` from captured stdout. Leave `_run_cmd()`/`_run_mcp()`/the `stream_callback`/`trace_mode` branches untouched. `test_runner_spec.py::TestRunActionDispatch`'s two dataclass-equality tests use non-JSON stdout fixtures (`"ok"`, `"4"`), so the parser yields `None` and they should not break; verify rather than pre-emptively edit.
3. `session_store/schema.py`: append a migration adding nullable `input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`, `tool_calls` INTEGER columns to `harness_events` (no `DEFAULT`, no backfill, `# ENH-3464` comment). Check the tail of `_MIGRATIONS` for an ENH-3476 entry first (Decision 9). Add the `TestSchemaV50BaselineConditions`-shaped triad.
...
5. `history_reader/harness.py`: append the five fields to `HarnessEvent` and `_HARNESS_EVENT_COLUMNS`; add a round-trip test beside `test_harness_event_carries_id_and_v49_fields`.
6. `cli/harness.py`: add the five fields plus `duration_ms` to `HarnessEvalOutcome`; add a `duration_ms: int | None = None` kwarg to `_grade()` and `_evaluate_and_report()` (pass-through); in `_grade()`, copy the five fields from `result` and `duration_ms` from the kwarg when constructing the outcome — both the main construction and the timed-out/errored early return (no other change there); pass `duration_ms` at all six `_grade()`/`_evaluate_and_report()` call sites (four `cmd_*` single-run paths, `cmd_dsl` at `:2738`, `_run_sample_loop` at `:1913`); extend `_record_harness_event()`'s signature; at each of the 6 `_record(...)` closures (`:2139`, `:2273`, `:2393`, `:2512`, `:2676`, `:2772`) read the values off the `RunnerResult` and pass them alongside `duration_ms`; include `duration_ms` and the five fields in the `--output json` payload, the per-sample entries of the N-sample summary, and the text summary line (e.g. `tokens in/out/cache_r/cache_w=1234/567/8000/200 tool_calls=8 duration_ms=...`, omitting fields that are `None`).
...
- **Effort**: Medium — only wall-clock is captured today. Touches 6 source files (`subprocess_utils.py`, `runner_spec.py`, `cli/harness.py` incl. 6 `_record_harness_event` call sites and 6 `_grade()`/`_evaluate_and_report()` call sites, `session_store/schema.py` + `writers.py`, `history_reader/harness.py`), one migration, 6 test files, and 6 docs. The parsing itself is small; the breadth is in threading and docs.
...
- **In scope**: capturing, naming, surfacing (`--output json`, text summary, N-sample per-sample entries), and persisting `input_tokens`/`output_tokens`/`cache_read_tokens`/`cache_creation_tokens`/`tool_calls` beside the existing `duration_ms` (which is also newly surfaced in `--output json`), attached to the ENH-3397 run model (attempt × task × repetition × subject). Subject-side cost only. Claude-code and codex stdout shapes.
- **Out of scope**: turning efficiency into a pass/fail gate — a run that passes expensively still passes; any cross-run or cross-sample comparison semantics (aggregates, deltas, "cheaper than" verdicts) — this issue records per-run values only; judge-side (evaluator) cost; `total_cost_usd`/`num_turns` from the claude `result` event (Decision 3); parsing gemini/qwen/omp/opencode/pi output shapes; switching `_run_prompt()` to stream-json to obtain `tool_calls` on the PROMPT/DSL path; the downstream cost-trend analytics consumer over `.ll/history.db` (separate work).
...
- manual review pass - 2026-09-14 (pre-implementation) - fixed the `duration_ms` stamping gap (`_grade()`/`_evaluate_and_report()` gain a kwarg; caller-side stamping was too late for the `--output json` payload); split the shared parser into `usage_from_event()` + `usage_from_stream_lines()` with whole-string-then-per-line parsing (per `run_blocking_json()` precedent); clarified Decision 5's "errored", codex `cache_creation_tokens == 0`, top-level-only `tool_calls`; marked AC2 as manual; corrected Impact call-site count (6, not 10)
- manual review pass - 2026-09-14 - corrected `_record_harness_event` call-site count (6, not 10); relaxed Decision 2 so `_grade()` copies fields onto the outcome (gating untouched); added `cache_creation_tokens` as the fifth field; added `duration_ms` to `HarnessEvalOutcome`/`--output json`; scoped prompt-path capture to claude-code/codex stdout shapes; corrected `tool_use` wording (content blocks in `assistant` events)
- `/ll:ready-issue` - 2026-09-15T00:01:54 - `c2216a97-4524-439c-8eb3-537cb9c8e13f.jsonl`
