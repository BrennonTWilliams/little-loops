...
- `_record_harness_event(..., duration_ms: int, ...)` (`scripts/little_loops/cli/harness.py:216`) — gains trailing `input_tokens=None, output_tokens=None, cache_read_tokens=None, cache_creation_tokens=None, tool_calls=None` kwargs, forwarded to `record_attempt()`/`record_harness_event()` (`session_store/writers.py`)
- `_grade(...)` (`scripts/little_loops/cli/harness.py:1224`) — gating logic **unchanged**; the only change is copying the five efficiency fields from `result` onto the `HarnessEvalOutcome` it constructs. `passed`/`verdict`/`abstained` are computed exactly as today.

### Call Path

`_run_skill()`/`_run_prompt()` (`runner_spec.py`) parse captured stdout via `usage_from_stream_lines()` -> populate `RunnerResult.{input_tokens,output_tokens,cache_read_tokens,cache_creation_tokens,tool_calls}` -> `_grade()` copies them onto `HarnessEvalOutcome` (not read by any gate) -> each `cmd_*` `_record(result, duration_ms, outcome)` closure in `cli/harness.py` forwards them beside `duration_ms` to `_record_harness_event()` (6 call sites) -> persisted as `harness_events` columns via `record_attempt()` -> read back on `HarnessEvent` via `authoritative_attempt()`/`authoritative_attempts()`. `_evaluate_and_report()` renders `outcome.{duration_ms, tokens..., tool_calls}` into `--output json` and the text summary. `_band_samples()` and `evaluate()` never read the fields; `_grade()` only copies them.
...
6. `cli/harness.py`: add the five fields plus `duration_ms` to `HarnessEvalOutcome`; in `_grade()`, copy the five fields from `result` when constructing the outcome (no other change there); have each `cmd_*` caller set `outcome.duration_ms` from its measured value; extend `_record_harness_event()`'s signature; at each of the 6 `_record(...)` closures (`:2139`, `:2273`, `:2393`, `:2512`, `:2676`, `:2772`) read the values off the `RunnerResult` and pass them alongside `duration_ms`; include `duration_ms` and the five fields in the `--output json` payload, the per-sample entries of the N-sample summary, and the text summary line (e.g. `tokens in/out/cache_r/cache_w=1234/567/8000/200 tool_calls=8 duration_ms=...`, omitting fields that are `None`).
...
- [ ] On the claude-code host (and codex for tokens), after `ll-harness skill ...` completes normally the recorded `harness_events` row has non-`None` token fields and a non-`None` `tool_calls`; after `ll-harness prompt ...`/`dsl ...`, token fields are non-`None` and `tool_calls` is `None`. Hosts whose stdout matches neither the claude nor codex shape record `None` for all five.
- [ ] CMD and MCP runs, timed-out runs, and errored runs record all five fields as `None` while `duration_ms` is still populated.
