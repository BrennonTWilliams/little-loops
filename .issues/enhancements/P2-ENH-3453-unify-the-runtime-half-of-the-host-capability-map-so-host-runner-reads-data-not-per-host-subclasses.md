...
- **Report rows are data too:** the 3-valued `CapabilityEntry.status` and prose `note` cannot be derived from a boolean (Codex `agent_select` is `False` yet reported `"partial"` with a workaround note). The runtime entry therefore carries the report rows verbatim as a tuple of `CapabilityEntry`, alongside the six flags and the binary name. The flag/row consistency rule enforced is symmetric: a row named after one of the six flags may not be `"full"` while that flag is `False`, and may not be `"unsupported"` while that flag is `True`. `"partial"` in either direction (Codex `agent_select=False`/`"partial"`, Kimi `agent_select=True`/`"partial"`) stays legal. Both directions hold for all 8 runners today (verified 2026-09-12 review), so the rule lands clean.
- **`HOST_BINARY_NAMES` stays registry-derived.** It keeps reading `cls().describe_capabilities().binary` over `_HOST_RUNNER_REGISTRY` (which for the 8 real runners now resolves through the map wrapper). It is **not** rederived from `RUNTIME_HOST_CAPABILITIES.values()`: test-only hosts (FEAT-3454 `fake`, ENH-3459 `fake-minimal`) are exempt from the map, and FEAT-3454's design requires `TEST_ONLY_BINARIES <= HOST_BINARY_NAMES` with `TEST_ONLY_BINARIES` itself derived from `describe_capabilities().binary`. A map-only derivation would drop the fakes' binaries and break both that invariant and the existing registry-derived test at `scripts/tests/test_host_runner.py:2348-2357`. Only the comment block at `host_runner.py:2021-2027` is reworded.
- **Two implementer decisions, decided (2026-09-12 review):** (1) `load_runtime_capabilities` raises `KeyError` naming the missing host — **not** `HostNotConfigured`. A missing map entry is an invariant violation owned by the `ll-verify-host-map` drift check; `HostNotConfigured` already means "binary not wired" (opencode/pi) to every caller that catches it, so reusing it would mask a programming error as a config state. (2) `RuntimeHostEntry`, `RUNTIME_HOST_CAPABILITIES`, `load_runtime_capabilities`, `render_capability_report` are public (no underscore) in `host_runner` and documented in `docs/reference/API.md`, but are **not** re-exported from `scripts/little_loops/__init__.py` — consumers keep going through `describe_capabilities()`.
...
- `scripts/little_loops/__init__.py:33-42` — public export block re-exports `CapabilityEntry`, `CapabilityReport`, `HostInvocation`, `HostRunner`, `apply_host_cli_from_config`. **Unchanged**: the new runtime-map symbols are public in `host_runner` but not re-exported here (decided, see `## Design`).
...
### Files to Modify
- `scripts/little_loops/host_runner.py` — add `RuntimeHostEntry` (frozen dataclass: `host`, `binary`, `flags: HostCapabilities`, `report_rows: tuple[CapabilityEntry, ...]`), the `RUNTIME_HOST_CAPABILITIES` dict, and `load_runtime_capabilities(host_id) -> RuntimeHostEntry` plus `render_capability_report(entry) -> CapabilityReport`, placed after `CapabilityReport` (line ~391) and before `ClaudeCodeRunner`. Collapse the 8 per-runner `describe_capabilities()` overrides at lines 662, 962, 1094, 1170, 1354, 1535, 1741, 1943 (the hand-written `CapabilityEntry` rows move verbatim, comments included, into the map) and the 8 class-level `capabilities = HostCapabilities(...)` literals at lines 506, 757, 1046, 1122, 1211, 1428, 1621, 1827 (each becomes `capabilities = RUNTIME_HOST_CAPABILITIES["<name>"].flags`). Line 490 is the Protocol stub and stays. `HOST_BINARY_NAMES` (2028-2030) **keeps its registry-derived expression** (see `## Design`); only its comment block is reworded. Lines 1995-2004 hold `_HOST_RUNNER_REGISTRY`; the `name` class attribute on each runner is the shared key.
- `scripts/little_loops/host_runner.py:1323, 1435, 1510` — three inline comments read "see describe_capabilities for the note" / "see describe_capabilities() below" (GeminiRunner `build_*` json_schema drop, OmpRunner `capabilities` literal `structured_output=False`, OmpRunner `build_*` json_schema drop). After the collapse the method bodies they point at are one-liners; re-point each at the host's `RUNTIME_HOST_CAPABILITIES` entry (e.g. "see the `omp` entry in `RUNTIME_HOST_CAPABILITIES`"). The 1435 comment sits inside the `capabilities` literal that step 3 replaces, so it moves into the map entry alongside the flag.
...
- `scripts/little_loops/cli/verify_host_map.py` — rewrite `_check_runtime_contradiction` (lines 100-128): its field-agreement loop is a permanent no-op (fields disjoint by design). Replace with (a) key parity `set(RUNTIME_HOST_CAPABILITIES) == set(_HOST_RUNNER_REGISTRY)` (built-in registry only; extension-registered runners, if that ever lands, are exempt), (b) `entry.host == key` and `entry.flags is RunnerClass.capabilities` for each host, (c) flag/row consistency, both directions: no report row named after one of the six flags may be `"full"` while the flag is `False`, nor `"unsupported"` while the flag is `True`. Also rewrite the **module docstring** (lines 1-30, check 2) which still describes the old field-agreement loop.
- `scripts/little_loops/host_runner.py:2021-2027` — the `HOST_BINARY_NAMES` comment block ("derived from every registered runner's describe_capabilities().binary") must be reworded to name the runtime map as the source.
...
- `scripts/little_loops/host_runner.py:2028-2030` — `HOST_BINARY_NAMES = frozenset(cls().describe_capabilities().binary for cls in _HOST_RUNNER_REGISTRY.values())` — **expression unchanged** (registry-derived so test-only hosts' binaries stay in the set; see `## Design`). Note `HostCapabilityEntry` has no `binary` field, which is another reason the runtime entry must be its own dataclass.
...
- **Single-flag consumer at call boundary**: `_structured_output_args` (`host_runner.py:2365-2383`) and `cli/issues/decisions.py:797,802` are the only sites that read `invocation.capabilities.<flag>` at the consumer; the field is read once and routed from there. No consumer reads a per-runner class-level flag literal directly. (`describe_capabilities()` *is* called directly by `cli/doctor.py:1431`, `cli/action.py:342`, `mcp_server/tools.py:224`, `init/cli.py:108`, and `HOST_BINARY_NAMES` at 2028-2030 — those all go through the public method, which the Option 1 wrapper preserves.)
...
| `scripts/little_loops/cli/verify_host_map.py:100-128` (`_check_runtime_contradiction`) | Drift check that compares `HOST_CAPABILITIES[host]` to `_HOST_RUNNER_REGISTRY[host].capabilities` field-by-field (permanent no-op: no shared fields) | CHANGED | Becomes key-parity + identity + two-directional flag/row consistency check between `RUNTIME_HOST_CAPABILITIES` and `_HOST_RUNNER_REGISTRY` |
| `scripts/little_loops/host_runner.py:2028-2030` (`HOST_BINARY_NAMES`) | `frozenset` of `cls().describe_capabilities().binary` over the registry | PRESERVED | Expression unchanged; still registry-derived so test-only hosts (exempt from the map) keep their binaries in the set. Comment block at 2021-2027 reworded only. |
...
- `load_runtime_capabilities(host_id: str) -> RuntimeHostEntry` — dict lookup; raises `KeyError` with a message naming the missing host (not `HostNotConfigured` — see `## Design`)
...
- `HostRunner.describe_capabilities(self) -> CapabilityReport` — unchanged signature; every concrete runner's body becomes `return render_capability_report(load_runtime_capabilities(self.name))`, **duplicated as a one-liner on each of the 8 runners — no shared mixin/base class** (decided 2026-09-12 review: runners intentionally share no base class because `HostRunner` is a structural Protocol, and the FEAT-3456 host-composition spike is reshaping exactly that surface; a mixin introduced here would pre-empt it). Protocol stub at line 490 stays.
...
0. **Capture the baseline before touching anything — as a one-off, not a committed golden file.** On `main`, run a scratch script that snapshots, for every host in `_HOST_RUNNER_REGISTRY`, today's `RunnerClass.capabilities` and `RunnerClass().describe_capabilities()` to a JSON file outside the repo; after the collapse, re-run it and diff. Do **not** check the snapshot in as a test fixture: a golden file duplicating ~45 report rows would have to be regenerated on every legitimate data edit and would guard nothing the permanent checks don't. The permanent guards are the rewritten `_check_runtime_contradiction` (step 4) plus the existing per-runner `TestDescribeCapabilities` / `test_capabilities_flags` assertions in `scripts/tests/test_host_runner.py`, which already pin every row and flag consumers depend on.

1. `RuntimeHostEntry`, `RUNTIME_HOST_CAPABILITIES`, `load_runtime_capabilities(host_id) -> RuntimeHostEntry`, and `render_capability_report(entry) -> CapabilityReport` live in `scripts/little_loops/host_runner.py` between `CapabilityReport` and `ClaudeCodeRunner`. The existing hand-written `CapabilityEntry` rows (with their rationale comments — BUG-2759, ENH-2627, ENH-2714, ENH-1529, ENH-1530, …) move verbatim into each entry's `report_rows`. Verified by the step 0 pre/post diff (one-off) and by the existing per-runner report/flag tests in `scripts/tests/test_host_runner.py` staying green unchanged.
...
4. `_check_runtime_contradiction` in `scripts/little_loops/cli/verify_host_map.py:100-128` is rewritten from the no-op field-agreement loop to: (a) `set(RUNTIME_HOST_CAPABILITIES) == set(_HOST_RUNNER_REGISTRY)`, (b) per host `entry.host == key` and `_HOST_RUNNER_REGISTRY[key].capabilities is entry.flags`, (c) no `report_rows` row named after one of the six flags is `"full"` while that flag is `False`, and none is `"unsupported"` while that flag is `True` (`"partial"` is legal in both directions). Docstring at lines 101-108 rewritten accordingly, **and the module docstring at lines 1-30 (check 2 currently describes the field-agreement loop — "do not contradict each other on any field name the two dataclasses share") rewritten to describe the new key-parity + identity + flag/row rules**, since that docstring is what `ll-verify-host-map --help` readers and `docs/reference/CLI.md:1009` mirror. **Parity scope:** rule (a) is over the *built-in* registry literal. `host_runner.py:1992-1994` says "Extensions may register additional runners" (nothing does today — verified 2026-09-12, no code outside `host_runner.py` mutates `_HOST_RUNNER_REGISTRY`); if extension registration lands later, an extension runner keeps its own `describe_capabilities()` and `capabilities` literal (Protocol is structural) and is exempt from parity — state this in the rewritten docstring so the rule does not become a landmine. **Test-only hosts are also exempt:** FEAT-3454 / ENH-3459 register `fake` / `fake-minimal` in `_HOST_RUNNER_REGISTRY` with no runtime-map entry, so rule (a) must read `set(RUNTIME_HOST_CAPABILITIES) == set(_HOST_RUNNER_REGISTRY) - TEST_ONLY_HOSTS` (import the constant from `host_runner`; if FEAT-3454 has not landed yet, `getattr(host_runner, "TEST_ONLY_HOSTS", frozenset())` so this lands cleanly in either order). Add a fourth injected-map test asserting a registered test-only host without a map entry produces no error. Verified by `ll-verify-host-map` returning `OK` on `main` and by four new tests that inject a bad map via `patch("little_loops.cli.verify_host_map.RUNTIME_HOST_CAPABILITIES", bad_map)` (one per rule, two for rule (c) — one per direction) and assert the named error string.
...
7. `HOST_BINARY_NAMES` (`host_runner.py:2028-2030`) keeps its registry-derived expression (`cls().describe_capabilities().binary` over `_HOST_RUNNER_REGISTRY`); only the comment block at 2021-2027 is reworded to say the real runners' binaries now come from the runtime map via the wrapper while test-only hosts contribute theirs directly. Do **not** rederive it from the map — that drops the FEAT-3454/ENH-3459 fakes' binaries and breaks `TEST_ONLY_BINARIES <= HOST_BINARY_NAMES` plus the registry-derived test at `test_host_runner.py:2348-2357`. Verified by `TestHostBinaryNames` (`test_host_runner.py:2348-2371`) passing unchanged and `conftest.py:251`'s live-spawn guard still seeing the same frozenset.
...
- Add per-host equivalence test for the render path (fixture-free — Step #0's baseline is a one-off diff and is **not** checked in): for every host in `set(_HOST_RUNNER_REGISTRY) - TEST_ONLY_HOSTS`, assert `render_capability_report(load_runtime_capabilities(name)) == RunnerClass().describe_capabilities()` and `load_runtime_capabilities(name).flags is RunnerClass.capabilities`. Tautological on the day it lands; its job is to fail if anyone later reintroduces a hand-written `describe_capabilities()` body or a class-level `HostCapabilities(...)` literal on a real runner. Mirror the precedent shape `scripts/tests/spike/omp_agent_output_frontmatter_passthrough/test_output_passthrough.py:21-80` (input → helper call → assert equivalence). This is Implementation Step #1's permanent verification test; the pre/post transcription check itself is the Step #0 one-off diff.
