...
- Eight independent spawn sites hand-roll the child environment (census in the Integration Map), and two more — `_run_cmd()` and `_run_prompt()` in `runner_spec.py` — pass no `env=` at all and inherit implicitly.
- `_run_cmd()` in particular runs `bash -c <target>` with the operator's complete ambient environment, which is how most FSM loops do their real work.
...
- **`scripts/little_loops/runner_spec.py::_run_cmd()` (line 215) — the largest exposure, and the one place a `host_runner`-only mechanism cannot reach.** It runs `subprocess.Popen(["bash", "-c", spec.target], ...)` with **no `env=` kwarg and no `HostInvocation` at any point** — it never calls `resolve_host()`. FSM shell actions are where most loops do their real work (including every `gh` invocation), so scoping implemented solely inside `build_streaming()`/`build_blocking_json()` leaves `RunnerType.CMD` running with the full ambient environment. Covering this path is mandatory, not optional.
...
**Spawn-site census (8 total, verified 2026-08-15).** The four `os.environ.copy()` sites are `worktree_utils.py:568`, `mcp_call.py:197`, `subprocess_utils.py:450`, `parallel/worker_pool.py:812`; the four `{**os.environ, ...}` sites are `runner_spec.py:206`, `fsm/handoff_handler.py:131`, `learning_tests/extractor.py:135`, `session_store/lifecycle.py:158`. Plus `_run_cmd()`/`_run_prompt()`, which pass no `env=` at all and so inherit implicitly. Every one of these is an independent hand-rolled construction of the child environment; a scoping mechanism that does not centralize them is bypassable by whichever site is added next (see AC6).
...
- **AC6.** No spawn site constructs the child environment by hand. A single shared projection helper is the only place a child env is built, and a test fails on the introduction of a new raw `os.environ.copy()` / `{**os.environ, ...}` / implicit-inherit `subprocess` call in a spawn path. Without this, scoping is silently bypassable — the census above found 8 independent hand-rolled sites, 3 of which were missed by the first survey pass.
...
With AC6 (centralize 8 spawn sites), AC7 (baseline contract), and AC8 (`_run_cmd`), this is materially larger than a single-file enhancement. Reasonable split if it proves unwieldy: (a) shared projection helper + centralization of all spawn sites, no behaviour change; (b) declaration field + registry + AC2/AC3 enforcement; (c) AC4 audit persistence; (d) `gh` scoping, if in scope.
...
- **Effort**: Large — 8 spawn sites to centralize, a declaration field, a capability registry, `_run_cmd` coverage, an audit record with a new schema migration, and cross-runner tests over 8 runner classes. See the Sizing note for a 4-way split.
