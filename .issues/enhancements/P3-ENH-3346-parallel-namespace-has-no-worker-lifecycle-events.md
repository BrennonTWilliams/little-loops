...
2. Add `parallel.worker_blocked`/`parallel.worker_unblocked` emissions in `orchestrator.py` (at the overlap-deferral/requeue points, not a lock wait); add `parallel.worker_started` in `worker_pool.py`'s `_process_issue` immediately after worktree creation (see Program Design timing decision); add `parallel.queue_changed` inside `priority_queue.py`'s six mutators (queue-side — see Program Design) plus the `load_completed`/`load_failed` resume-path bulk loaders (`priority_queue.py:225-241`, one emit per batch); stamp `run_id` on the two existing emitters. Precondition confirmed still unmet in the live tree this pass: widen `IssuePriorityQueue.requeue()` (`:146-170`) from `-> None` to `-> bool` — `return False` on the existing duplicate-guard early return, `return True` on the success path, mirroring `add()`'s identical guard shape (`:50-73`) — and branch `_requeue_deferred_issues` (`orchestrator.py:1312`, currently a bare `self.queue.requeue(issue)` statement that discards any return) on that value so `worker_unblocked` fires only on `True`. Also define `WorkerBlockedReason: Literal["overlap"]` in `parallel/types.py` alongside the existing `MergeStatus`/`WorkerStage` enums, import it into `orchestrator.py`, and re-export it via `parallel/__init__.py`'s `__all__` (mirroring `MergeStatus`/`WorkerStage`'s existing re-export there).
