...
- **Out of scope: persisting `parallel.*` events to `.ll/history.db` via `SQLiteTransport`.** Confirmed live (see Integration Map → Dependent Files finding): `SQLiteTransport.send()` (`session_store/writers.py:2374-2467`) silently drops every `parallel.*` event today (both existing types and this issue's six new ones) via its terminal `else: return` — it recognizes only loop and `issue.*` events. This issue adds `run_id` stamping and six new emit sites but does not add a `parallel.*` branch to `SQLiteTransport.send()`; `ll-session` tooling cannot query worker-lifecycle history from `.ll/history.db` after this issue ships, only live subscribers of the in-process `EventBus` gain visibility. Left for a follow-up issue rather than expanding this one's scope to touch `session_store/writers.py`.
