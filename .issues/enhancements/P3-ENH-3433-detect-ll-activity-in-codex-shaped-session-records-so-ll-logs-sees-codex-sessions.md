...
- `response_item`/`custom_tool_call` with `name == "exec"` → a Claude `assistant` record: `{"type": "assistant", "timestamp": <envelope.timestamp>, "sessionId": <session_id>, "cwd": <cwd>, "message": {"role": "assistant", "content": [{"type": "tool_use", "id": <payload.call_id>, "name": "Bash", "input": {"command": <unescaped cmd>}}]}}`. **If `cmd` cannot be extracted (decided 2026-09-10 review #3), still emit the `assistant` record with `input.command = <raw JS snippet>`** rather than passing the raw envelope through: the `\bll-\w+` regex still fires on the snippet text, and the paired `custom_tool_call_output` (always normalized) keeps a matching `tool_use` instead of becoming an orphan `tool_result`. A missing `call_id` (as in the synthetic session at `test_ll_logs.py::test_discover_unions_claude_and_codex_workspace_without_error`) yields `id = ""` — never raise. The `cwd` stamp is parity with Claude records only: no reader in `cli/logs.py` (which uses `handle.cwd`) or the `rebuild()` derivers consumes it.
...
**ENH-3422 interaction (decided 2026-09-10 review; status refreshed review #3): Codex stops being ingest-only for `tool_events`, no replay shim.** ENH-3422 is now `status: done` (commit `1365365e8`), unreleased (last tag `v1.161.0`). Its `rebuild()` derivers are host-agnostic (`writers.py::_iter_events` has only the shape-keyed qwen legacy shim), so once Codex exec rows are Claude-shaped with `sessionId`, `rebuild()` derives `tool_events` rows from Codex, and `raw_events.event_type` for those lines becomes `assistant`/`user` instead of `response_item`. **`sessions` rows are not new**: `lifecycle.py::_backfill_sessions` already falls back to the `raw_events.session_id` column for Codex sources (ENH-3422 D3), so a Codex `sessions` row exists on rebuild today; only `tool_events` is the payoff. That makes the live "Codex is ingest-only — no derived-table rows" note in `docs/reference/CLI.md` (line ~3970, `--host codex` row of the `ll-session backfill` flags table) and the `_backfill_sessions` docstring sentence "codex/kimi-code payloads carry no `sessionId` field of their own" (`lifecycle.py:724`) stale the moment this lands; amend both. ENH-3422's own issue text is closed history and is **not** edited. Codex rows ingested *before* this issue stay raw under the `(source_path, line_no)` dedup index — a replay shim like qwen's `is_raw_qwen_record` is **not** added, because a raw Codex `raw_events` row stores only the inner payload (`lifecycle.py:794` serializes `event.payload`; no envelope `timestamp`, no `session_meta` context, no cross-row pairing state) and because ENH-3422 is unreleased, so only dev-machine DBs hold such rows. Document the limitation in the `parse_codex_rollout` docstring: pre-ENH-3433 Codex rows still yield a `sessions` row but no `tool_events` rows on rebuild; re-ingesting them requires deleting the host's `raw_events` rows first.

## Scope Boundaries

- **In scope**: `session_store/codex.py::CodexNormalizer` + `_extract_exec_cmd`; wiring it into `parse_codex_rollout` with `session_meta`-derived `sessionId`/`cwd`, envelope `timestamp` stamping, and `CommandExecution`-derived `is_error`; flipping the three pass-through contract statements (parser docstring, `test_parses_interactive_fixture_header_and_unknown_types_pass_through`, `docs/codex/usage.md`) to the new contract; the `(b)`-analogue check on the three user-prompt carriers; documenting the `/ll:` skill-dispatch and queue-operation signals as not applicable to Codex; a `rebuild()` test proving Codex-derived `tool_events` rows now appear, plus amending the live `docs/reference/CLI.md` ingest-only note and the `lifecycle.py::_backfill_sessions` docstring; updating the `event_type` allow-set in `test_session_store_lifecycle.py:179`; documenting `CommandExecution`'s fields in `docs/codex/usage.md` and the fixture README.
...
- `docs/reference/CLI.md` (~3970, `--host codex` row of the `ll-session backfill` flags table; landed with ENH-3422) — "it is ingest-only — no derived-table rows (ENH-3422)" becomes false for the exec subtypes; amend to "Codex exec calls are normalized at the parser; `rebuild()` derives `tool_events` rows from them; rows ingested before ENH-3433 stay raw (`sessions` row only)". ENH-3422's issue file is done/closed and is not edited.
- `scripts/little_loops/session_store/lifecycle.py:724` — `_backfill_sessions` docstring says "codex/kimi-code payloads carry no `sessionId` field of their own"; narrow to "raw codex/kimi-code payloads (non-exec codex records, all kimi-code records)".
- `scripts/tests/test_session_store_lifecycle.py:179` — `test_backfill_ingests_codex_fixtures_via_handles` asserts every `raw_events.event_type` for the interactive fixture is in `{session_meta, response_item, event_msg, turn_context, world_state}`; add `assistant`/`user` to the allowed set (and assert exactly six rows carry them: three `assistant`, three `user`).
...
- `class CodexNormalizer` (`scripts/little_loops/session_store/codex.py`) — `__init__(self, *, session_id: str, cwd: str)`; `__call__(self, envelope: dict) -> dict | None` maps `response_item`/`custom_tool_call` (`name == "exec"`) to a Claude `assistant` record with `tool_use(id=call_id, name="Bash", input={"command": cmd})`, and `response_item`/`custom_tool_call_output` to a `user` record with `tool_result(tool_use_id=call_id, content=[text blocks minus the `Script completed` header], is_error=True)` when the paired `CommandExecution` failed; stamps `timestamp` (envelope), `sessionId`, `cwd` (parity only — no reader). Internal state: `_pending: dict[str, str]` (`call_id → cmd`) and `_status: dict[str, bool]` (`call_id → failed`, OR-accumulated, both popped on the output), fed by `event_msg`/`item_completed`/`CommandExecution`. A `custom_tool_call` whose `cmd` cannot be extracted still normalizes with `input.command = <raw snippet>`; a missing `call_id` becomes `""`. Returns `None` for every other record (including `CommandExecution` itself), which the caller treats as "pass raw envelope through" — not "drop".
...
- **Breaking Change**: No for `cli/logs.py` consumers. **Yes, narrowly, for direct `parse_codex_rollout`/`iter_events` consumers** that expected the two exec subtypes raw — the only such consumers in-tree are the tests at `test_session_discovery.py:486` and `test_session_store_lifecycle.py:179` (the latter via `raw_events.event_type`); `_codex_cache_usage` reads `event_msg` only and `_extract_codex_user_messages` reads `response_item`/`message` and `event_msg`/`user_message` only, so both are unaffected.

## Dependencies

ENH-3430 (the handles-based readers in `cli/logs.py`; the `sessionId`/`handle.session_id` fallback there becomes belt-and-braces for Codex once this issue stamps `sessionId` in the parser) has landed (`status: done`, commit `415d6b1cb`) — no longer a blocker, and `blocked_by` in frontmatter is now historical. Relates to ENH-3422 (its backfill consumes the parser-level normalizer for free; `status: done`, commit `1365365e8`, unreleased as of 2026-09-10 — the "in-flight" wording in the Integration Map re-refine findings is historical) and ENH-3429 (`_codex_cache_usage` pass-through constraint).
...
- `_extract_exec_cmd(input) == item.command[-1]` for all three fixture execs (lines 13/14, 21/22, 25/26); it handles double-quoted, single-quoted, and backtick `cmd:` values and unescapes `\"`/`\n`; an unrecognizable snippet still yields an `assistant` record whose `input.command` is the raw snippet (never `None`/raise), and a `custom_tool_call` with no `call_id` yields `tool_use.id == ""`.
- A `custom_tool_call_output` with no preceding `CommandExecution` (synthetic rollout) yields a `user` record with no `is_error` key; a `CommandExecution` whose `command[-1]` matches no pending cmd falls back to the most recent pending `call_id`; two `CommandExecution` items for one `call_id` where the first fails and the second succeeds still yield `is_error is True` (sticky).
...
- `rebuild()` over a `raw_events` table ingested from the interactive fixture via a Codex handle derives three `Bash` `tool_events` rows for session `01a086ea-c8bc-79f1-9faa-1ce2716aa80f` (the `sessions` row already existed pre-ENH-3433 via the `raw_events.session_id` fallback and is asserted only as unchanged); a second `rebuild()` yields identical counts. `tool_events` has no failure column, so "one failed" is proven at the normalizer level, not on the table.
- `test_ll_logs.py` Claude-shaped tests pass unmodified (including `test_discover_unions_claude_and_codex_workspace_without_error`, whose synthetic `custom_tool_call` has no `call_id`); `test_cli_ctx_stats.py` Codex cache-rate tests (`:981,998,1008,1038`) pass unmodified; exactly two existing tests change: `test_session_discovery.py:486` (its two exec-subtype assertions) and `test_session_store_lifecycle.py:179` (its `event_type` allow-set).
- `docs/reference/HOST_COMPATIBILITY.md` `[^codexsessions]` footnote states the added `ll-logs` support, the `(a)`/`(b)` N/A result (including the outcome of the three-carrier user-prompt check), and names kimi-code as the remaining gap; `docs/codex/usage.md:101`, the fixture README, and the `parse_codex_rollout` docstring no longer promise blanket pass-through and document the `CommandExecution` fields; the `docs/reference/CLI.md` ingest-only note and the `lifecycle.py:724` docstring are amended.
