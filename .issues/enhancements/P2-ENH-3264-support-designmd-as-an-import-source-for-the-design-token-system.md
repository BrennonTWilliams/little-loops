...
Consequences to honor at the injection sites: `render_as_prompt_context()` on an empty `resolved` takes the flat-fallback path and emits an empty token block, so `cli/loop/run.py:249` injects an **empty** `design_tokens_context` — which is what it already does today for a hollow profile, so no consumer changes. `_themed_css_vars()` produces empty `:root {}` / `[data-theme=dark] {}` blocks, structurally identical to its existing `None` fallback at `artifact.py:70-73`. Both paths still emit their stderr warning.
...
### Files to Modify
- `scripts/little_loops/design_tokens.py` — `_find_design_md()` (new, case-exact) and `_load_design_md()` (new), `load_design_tokens()` source branch (`:160`), `DesignTokens` dataclass (`:27`, gaining `guidance` + a source discriminator), `_resolve_token_root()` (`:129`) gaining a `quiet=` flag for the silent `auto` probe, and `render_as_prompt_context()` (`:225`) for **both** the residual bucket and the `tokens.primitives`-gate on the `_PRIMITIVE_COLOR_PREFIXES` suppression at `:289` — note this last pair changes behavior for **profile** sources too (previously-dropped unmatched keys now appear); assert the three built-ins' output is byte-identical, since none of them currently have unmatched keys and all three ship populated primitives.
...
6. Resolve the `render_as_prompt_context()` semantic-role gap — option **(a)** from "Prompt-context quality under a DESIGN.md source". Three parts: (i) map well-known spec color names onto `surface`/`text`/`border`/`action` in the *nested* dict so the gate at `:234-239` fires; (ii) add the residual bucket + its `_emit_group` call at `:310-315` so unmapped names stop being silently dropped; (iii) gate the `_PRIMITIVE_COLOR_PREFIXES` suppression at `:289` on `tokens.primitives` being non-empty, or (ii) still drops DESIGN.md colors named `accent`/`success`/`warning`/`danger`/`brand`/`neutral` (see "The primitive-suppression branch is source-blind"). Do not leave the bare flat-list fallback in place.
...
1. **`render_as_prompt_context()`** — residual bucket + `_emit_group("Other", …)` + the `tokens.primitives` gate on the `_PRIMITIVE_COLOR_PREFIXES` suppression, with the byte-identical assertion over the three built-ins. Touches existing profile-source behavior; lands alone so a prompt-quality regression bisects cleanly to it.
2. **The import core** — `_find_design_md()`, `_load_design_md()`, the `load_design_tokens()` source branch, the silent `auto` probe (`quiet=`), the degraded token-empty-with-`guidance` paths. Self-contained, and where the test density belongs.
...
- **Scope**: `design_tokens.py`, `config-schema.json`, `config/features.py`, `config/core.py`, `cli/artifact.py` (`_themed_css_vars` only), `hooks/session_start.py`, init/configure UX, docs. *(Down from ~9 modify sites to ~7 after the split; the exporter's ~80 LOC and the two loop-runner sites left with ENH-3268 / BUG-3266.)*
  ...
  **Estimated ~400-500 LOC total.** An earlier draft said "~150 LOC for the reader plus tests," which counts only `_load_design_md()` + the loader branch and will mislead sequencing. The realistic breakdown: ~130 in `design_tokens.py` (adapter, mapping table, alias rewrite, list normalization, source branch, residual bucket, primitives gate, `_find_design_md`); ~40 across `features.py` / `core.py` / `config-schema.json`; ~30 in `artifact.py` + `session_start.py`; ~60 across the 5 `init/*.py` files and 2 `skills/configure/*.md` files (or ~15 if Step 5a drops the picker); ~150 of tests (a new `TestLoadDesignTokensDesignMdFormat` class of ~12 cases plus four mirrored config classes from `test_enh1768_profile_system.py`); plus the vendored fixture and 3 doc files.
