...
3. **`cli/logs.py`**: route the 11 call sites through `detect_sessions(..., include_agents=False)`; route the `--all` enumeration in `discover_all_projects` through `list_workspaces(host, existing_only=...)` (the function already has an `existing_only` kwarg, 167). `_collect_sequences`'s direct `host_layout_for(LL_HOOK_HOST)` at 649 uses the same resolved host. **Three `discover_all_projects` semantics to preserve explicitly (review 2026-09-09):**
   ...
   - **Union rule for `host=None`.** `list_workspaces(host: str)` is not Optional; when the resolved host is `None`, iterate `_REGISTERED_HOSTS`, concatenate, and dedupe on the resolved cwd (a workspace with both Claude Code and Codex sessions appears once). Return type stays `list[Path]` of cwds, sorted.
...
- `scripts/tests/test_ll_logs.py::TestDiscover::test_discover_skips_non_ll_project` (204) — asserts the ll-activity filter (a project dir with non-ll records is excluded); this is the regression lock for § Proposed Solution step 3's "filter stays" rule and must pass unmodified. Add a union-dedupe case: same cwd recorded under two hosts appears once. Note `test_ll_logs.py` also patches `little_loops.cli.logs.get_project_folder` directly with a `side_effect` (e.g. ~2083, `TestExtract`) — those patches go dead the same way as the `test_cli.py` ones and must move to `detect_sessions`.
...
- `extract_conversation_turns(handles: list[SessionHandle], since=None, context_window=3, include_agent_sessions=True, reader="auto", *, history_db: Path | None = None) -> list[list[tuple[str, str]]]` — same replacement (`user_messages.py:1028`); the DB branch resolves `history_db` (caller passes `cwd / ".ll" / "history.db"`) instead of deriving it from `project_folder`; JSONL branch Claude-shaped only
- `_compute_cache_rate_from_jsonl(cwd: Path, *, host: str | None = None) -> dict[str, Any] | None` — keeps the existing four keys (`cache_read`, `cache_write`, `uncached`, `hit_rate_pct`) plus additive `host: str`; internally `detect_sessions(cwd, host, include_agents=False, limit=1)` then dispatches on `handle.host` to the Claude reader or `_codex_cache_usage(events) -> dict | None`
...
4. Rewire `cli/ctx_stats.py`; implement `_codex_cache_usage` with the last-`total_token_usage` + inclusive-`input_tokens` semantics; add the additive `host` key; add the Codex test beside the qwen/gemini pair; run `test_cli_ctx_stats.py` unmodified.
5. Rewire `cli/logs.py` (11 call sites + 649 + `discover_all_projects` via `list_workspaces` with union-dedupe and the preserved ll-activity filter + `_cmd_fleet_review`); switch the no-sessions path to exit 0 + `"No sessions found for: <cwd>"` and update `.loops/ll-logs-telemetry-digest.yaml:66`'s grep; run `test_ll_logs.py` (re-patching its `cli.logs.get_project_folder` sites to `detect_sessions`) and `test_bug_3216_telemetry_digest_invocations.py`, add the `FAILURES_NO_DATA` reachability test, then add Codex fixture variants.
...
- `ll-logs`, `ll-messages` (user messages, commands, **and** `--sft-format --reader auto|jsonl` conversation turns), and `ll-ctx-stats` obtain sessions via `detect_sessions`/`iter_events` and never call `get_project_folder`/`get_sessions_folder` for session enumeration; `ll-logs --all` enumerates workspaces via `list_workspaces`, unions hosts when none is resolved (deduped on cwd), and still applies the ll-activity filter (`test_discover_skips_non_ll_project` passes unmodified).
