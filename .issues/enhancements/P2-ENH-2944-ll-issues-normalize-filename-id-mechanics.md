...
- [ ] `commands/normalize-issues.md` is ≤ ~110 lines, **retains Step 7b's document-reference matching**, and `ll-verify-skill-prose` reports **0** findings for it; `BASELINE_COUNT` in `test_verify_skill_prose.py:19` lowered accordingly (23 → 19, re-verified against the tree at land time — the tree currently reports 21, of which 2 are this issue's)
