...
- Reusable statistic: `def wilson_ci(k: int, n: int, z: float = 1.96) -> tuple[float, float]:` (`scripts/little_loops/stats.py:14`) is already imported and used by `cmd_dsl()`'s pass-rate reporting (`cli/harness.py:1374-1375`) for a rate + 95% CI — available to reuse for this issue's pass-rate-over-n rather than a new statistic
