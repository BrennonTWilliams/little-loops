...
The fix is in `_draw_box()` at `layout.py:584-587`. Currently the non-highlighted path writes name row characters as plain text:
```python
else:
...
The highlighted path (lines 577-583) already applies bold via `colorize(line, f"{highlight_color};1")`. The same pattern should be extended to non-highlighted states using `colorize(line, "1")` for the first content row (`i == 0`):
...
**Important**: Bold must NOT be applied to `display_label` before `_draw_box` — `_compute_box_sizes()` at `layout.py:491` uses `len(display_label[s])` for box width, and ANSI escape bytes would corrupt box sizing.

## Integration Map

### Files to Modify
- `scripts/little_loops/cli/loop/layout.py` — primary change: `_draw_box()` at lines 576-587 (the `else` branch for non-highlighted name rows)
...
### Similar Patterns
- `layout.py:578` — existing bold pattern: `colorize(line, f"{highlight_color};1")` for highlighted states
- `layout.py:577-583` — full highlighted-state name row block to mirror for non-highlighted states

### Tests
- `scripts/tests/test_ll_loop_display.py` — `TestRenderFsmDiagram` class (line 638); ANSI color tests (lines 1213–1264); add new test asserting `"\033[1m"` appears in non-highlighted state name rows when `_USE_COLOR=True`
...
1. Modify `_draw_box()` in `layout.py:576-587` — in the `else` branch, split the first row (`i == 0`) to call `colorize(line, "1")` and write it as a single grid cell (same pattern as highlighted path at lines 577-583)
