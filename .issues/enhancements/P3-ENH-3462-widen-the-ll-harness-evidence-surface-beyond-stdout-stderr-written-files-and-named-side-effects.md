...
- **D5 — Side-effect semantics.**
  - **Snapshot placement.** `_evaluate_and_report()` receives an already-executed `RunnerResult`; `run_action()` is called inside each handler's `_invoke` closure (`harness.py:1755`, `:1889`, `:2009`, `:2093`) and via `_run_prompt_action()` for dsl. The pre-run snapshot and post-run check therefore wrap the *invoke*, not the grade: one helper `_invoke_with_side_effects(invoke, args) -> (result, duration_ms, side_effects)` used by `_run_sample_loop()` and by every single-run path (amended 2026-09-14 review; the original step 3 placed it in `_evaluate_and_report()`, which is unreachable-before-run).
  - `--require-artifact PATH`: snapshot pre-run existence + sha256 (same as `--forbid-path`). After the run, `PATH` must exist, be a regular file, **and** have been created or modified by the run (did not exist pre-run, or sha256 differs). A pre-existing, unchanged file → `passed=False`, `note="pre-existing, unchanged"` — otherwise a stale artifact from a previous run (or from sample 1 under `--samples N`) satisfies the check while the runner did nothing, which is exactly the failure this issue exists to close. When present and changed, its content is read (per-channel budget) and appended to the judge evidence when `--semantic` is set. Missing or unreadable → `passed=False`, record `examined=True, content=None, note="missing"` / `note="unreadable: <err>"`.
  - `--forbid-path PATH`: after the run, `PATH` must **not** exist, or, if it existed before the run, must be byte-identical to its pre-run content. Snapshot pre-run existence + sha256 for each declared path. If `PATH` is a directory, the check is existence-only (no sha256; a pre-existing directory passes, a newly created one fails).
  - `--expect-no-git-changes`: means "the run introduced no new changes," **not** "the tree is clean afterward." Run `git status --porcelain -z` before and after (reuse the subprocess call in `_git_dirty()`, `harness.py:90` — `porcelain_paths()` at `git_operations.py:552` only *parses* that output) and parse with `porcelain_paths()`; fail if the post-run set minus the pre-run set is non-empty. **Also** snapshot a sha256 of the working-tree content for each *pre-dirty* path, so a run that further modifies an already-dirty file (same path in both sets) fails too; note the offending paths. Do not use `_git_dirty()`'s boolean for this criterion; it stays as descriptive provenance only.
...
- **D8 — `--samples N` (ENH-3415) runs the side-effect checks per sample**, re-snapshotting before each sample, so each of the n runs yields the widened evidence. With the D5 created-or-modified rule, sample k's artifact snapshot includes sample k-1's output, so each sample must itself write the artifact to pass.
...
- `passed = False` if any `--require-artifact` path is missing/unreadable/pre-existing-and-unchanged, any `--forbid-path` was created or modified, or `--expect-no-git-changes` sees new porcelain paths or a further-modified pre-dirty path (D5). Precedence: fail > abstain > pass, unchanged.
...
- `_snapshot_side_effects(args, cwd) -> SideEffectSnapshot` (pre-run: per-path exists/sha256 for both `--require-artifact` and `--forbid-path`, porcelain path set, sha256 per pre-dirty path) and `_check_side_effects(snapshot, args, cwd) -> list[ChannelRecord]` (post-run). `_invoke_with_side_effects(invoke, args) -> tuple[RunnerResult, int, list[ChannelRecord]]` wraps the two around one runner invocation (D5 placement). `_grade()` gains a keyword-only `side_effects: list[ChannelRecord] | None = None` parameter; `_evaluate_and_report()` and `_run_sample_loop()` pass it through.
...
- Two design-precedent options for representing "channel not examined," both present in the codebase and not obviously reconcilable:
...
- [ ] AC3 — `--require-artifact PATH`: missing after the run → exit 1, `outcome.passed is False`, channel `PATH` recorded `examined=True, content=None, note="missing"`; present before the run and byte-identical after → exit 1, `note="pre-existing, unchanged"`; created or modified by the run → pass. With the path present-and-changed and `--semantic` set, the judge prompt contains `<artifact path="PATH">` with the file's content.
...
- [ ] AC5 — `--expect-no-git-changes`: a repo dirty before the run and unchanged by it passes; a run that adds one untracked or modified path fails with that path named in the channel note; a run that further modifies an already-dirty tracked file fails with that path named.
...
pass, so the issue as it now reads is up to date for the corrected items — this
...
came back clean; graph=`codegraph` freshness=`fresh`.
...
- **In scope**: widening `_grade()`/`_evaluate_and_report()` (`harness.py`) to treat `result.stderr` as a distinct, declarable judge channel; making a declared-written file's content available as judge evidence; making named side effects assertable (`--require-artifact` created-or-modified by the run, `--forbid-path` untouched, `--expect-no-git-changes` no new porcelain paths and no further-modified pre-dirty paths via a pre/post snapshot — **not** `_git_dirty()`'s boolean, which stays descriptive); and an in-memory `channels` record on the outcome that distinguishes unexamined from examined-and-empty (D1–D9).
