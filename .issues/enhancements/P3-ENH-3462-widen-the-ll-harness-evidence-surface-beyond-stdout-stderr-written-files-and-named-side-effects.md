...
- `_compose_judge_evidence(result, args, side_effects) -> str` — tag-wrap, per-channel 4000-char truncation, blank-line join. `_grade()` calls `evaluate_llm_structured(output=_compose_judge_evidence(...), max_output_chars=None)` so the evaluator's own 4000-char default doesn't re-slice the already per-channel-bounded composed string (D3 amendment).
...
- [ ] AC11 — `--require-artifact`/`--forbid-path` are accepted by all five subparsers (`skill`, `cmd`, `mcp`, `prompt`, `dsl`); `--trace-mode`/`--require-order` remain skill-only (they were never on `prompt_p`).
