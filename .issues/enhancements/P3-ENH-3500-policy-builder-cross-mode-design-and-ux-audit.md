...
1. Render the builder offline (`ll-artifact policy-builder`) and connected (via the `ll-artifact serve` policy-builder flag FEAT-3504 adds).
...
4. Run `/ll:confidence-check` (the surface set is now fixed; previously unscored while blocked).
...
Remaining: none from this pass — `blocked_by` cleared and stale prose refreshed on 2026-09-19. Run `/ll:confidence-check` (Implementation Step 4).
