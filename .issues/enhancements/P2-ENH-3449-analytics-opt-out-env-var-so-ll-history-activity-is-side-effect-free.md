...
- `scripts/little_loops/cli/session.py:422` — `main_session` wraps in `cli_event_context(DEFAULT_DB_PATH, "ll-session", sys.argv[1:])`, also omitting `config=`; sibling read CLI on the same db and the likely next polling target — top candidate for the Decision 2 trivial-fix clause [review 2026-09-11, second pass]
...
- `cli_event_context(db_path=DEFAULT_DB_PATH, binary="", args=None, config=None)` — actual parameter names (`binary`/`args`, not `source`/`argv`; `writers.py:484-488`); signature unchanged; gate logic gains the env-var short-circuit before `resolve_history_db`/`connect`; when `config` is provided the gate becomes (`analytics.enabled` not explicitly false) AND (`cli_commands` glob match), with a missing `analytics` key staying permissive (review 2026-09-11)
...
- Gate-placement constraint (review 2026-09-11, second pass): the new `analytics.enabled` check must live inside the same `try` block as the existing `cli_commands` gate (`writers.py:523-533`) — a malformed non-dict `analytics` value makes `config.get("analytics", {}).get(...)` raise `AttributeError`, which the existing `except Exception` (`writers.py:542`) degrades to row-suppressed + one-line warning; keeping the check there avoids opening a new failure path
...
2. **Wire `config=` into `ll-history`'s `cli_event_context` call** using the same project-config loader other CLIs use, so the `analytics.capture.cli_commands` gate actually applies. **Gate semantics (review 2026-09-11):** when `config` is provided, suppress when `analytics.enabled` is present and `false` (the `ll-init` opt-out shape, `init/core.py:365`) **or** when `cli_commands` excludes the binary; a config missing the `analytics` key entirely stays permissive (legacy/ENH-2932 default), as does `analytics` present with no `enabled` key (e.g. a hand-written `{"analytics": {"capture": {...}}}`) — the `enabled` check fires only on present-and-false (review 2026-09-11, second pass). Do NOT gate on bare `feature_enabled(config, "analytics.enabled")` — it returns `False` for a missing key (`config/features.py:39-60`), which would flip capture off for legacy minimal configs. Audit other `cli/*.py` callers that omit `config` and list them in the Session Log; `ll-session` (`cli/session.py:422`, sibling read CLI on the same db and likely next polling target) is the most likely trivial-fix candidate; fix in this issue only if trivial.
...
- When `config` is provided, the `cli_event_context`/`skill_event_context` gate suppresses when `analytics.enabled` is present-and-false or the glob list excludes the binary; a config missing the `analytics` key — or missing the `enabled` key within a present `analytics` dict — stays permissive (review 2026-09-11).
