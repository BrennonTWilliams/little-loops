...
- **Proposed change**: FEAT-1116 absorbs the surviving scope (legacy shell hooks continue with LL_STATE_DIR for existing hooks; new hooks go through the intent abstraction layer); the per-host-path approach is replaced by thin per-host adapters
