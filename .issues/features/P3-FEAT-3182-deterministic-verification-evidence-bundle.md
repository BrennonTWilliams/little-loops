...
3a. Extend `archive_run()` (`fsm/persistence.py:585`) to copy `probe-*.json` and `prepatch_evidence_*.json` from `run_dir` into the archive dir, so the bundle reads only from `.loops/.history/<run_id>-<loop_name>/`. A run archived before this change produces a `missing_probe_files` gap, not a silent zero count. (Rationale corrected: run_dir is per-instance and never overwritten — `cli/loop/run.py:193-200` — but it is keyed by instance id, not `run_id`, is gitignored, and is never archived or pruned; see `## Design Review (second pass)` item 1.)
