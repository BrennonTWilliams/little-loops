...
Known limitations, documented rather than fixed here (Fourth Review #19/#20):
`ll-loop resume` builds its executor with no `inbound` queue and no `--serve`
bridge (`cli/loop/lifecycle.py:724-737`), so a run interrupted mid-wait and
resumed under `eventbus` can only time out (the no-inbound warning fires);
and the `--serve` dashboard has no HITL UI — its only control posts an
`artifact_interaction` note — so out-of-process verdicts come from `curl` or
a relay, not the browser page. Both are follow-up issues.
...
**Goal**: Answer the request from wherever the operator is — a chat relay reading the socket/SSE stream, or a `curl` against the `--serve` interaction endpoint — and have the FSM resume on the chosen route. (A browser-tab answer needs dashboard UI that does not exist yet — Fourth Review #20, follow-up issue; do not build it here.)
...
- `scripts/little_loops/extension.py:100-125` — add `EventBusBoundExtension(Protocol)` with `bind_event_bus(self, bus: EventBus) -> None`, hasattr-detected like the sibling opt-in Protocols (`ActionProviderExtension`, `EvaluatorProviderExtension`, `LLHookIntentExtension`, `CommunicationAdapterExtension`); it is a generic capability, not adapter-specific (Fourth Review #23). Re-export from `scripts/little_loops/__init__.py` and add it to the scaffold doc-comment list in `scripts/little_loops/cli/create_extension.py:70-95`
...
- `scripts/little_loops/fsm/executor.py:1205-1220` — `_execute_sub_loop()`: after constructing `child_executor`, assign the parent's `_contributed_adapters` (and, for consistency, `_contributed_actions`/`_contributed_evaluators`/`_interceptors`) onto it **by reference — the same dict/list objects, not `dict(...)` copies** (Fourth Review #24) — so a `human_approval` state inside a sub-loop resolves the parent's `eventbus` adapter instead of raising `CommunicationAdapterNotFound` (Second Review #6). Sharing the objects is deliberate: the one adapter/observer stays one instance, and a child's lazily seeded `terminal` adapter becomes visible to the parent
- `scripts/little_loops/cli/loop/feed.py:972-984` and `:1146-1156` — add a `human_response` branch next to the `human_approval_requested`/`human_approval_resolved` ones (live feed and history formatter) showing `alert_id` and `verdict`, so `ll-loop run --follow` / `ll-loop history` readers see a verdict arrive even when its `alert_id` matches nothing (Fourth Review #25)
- `scripts/little_loops/observability/schema.py` + `scripts/little_loops/generate_schemas.py` `SCHEMA_DEFINITIONS` — add the `human_response` variant (inbound-origin, documented the way the `artifact_interaction` variant docstring at `schema.py:395` is); bump both `58` counts in `scripts/tests/test_generate_schemas.py` (`:17`, `:123`) to 59 and generate the `human_response` schema JSON under `docs/reference/schemas/` (Fourth Review #21 — decided, no longer optional)
...
- `.issues/features/P3-FEAT-1794-hitl-interrupt-fsm-state-type.md` — the emitter side (`HUMAN_APPROVAL_REQUESTED_EVENT`), landed in `837638dfe` as `FSMExecutor._execute_human_approval_state()` (`executor.py:2722-2881`); the adapter can be exercised end-to-end against it, and unit-tested independently against a bare `EventBus`
...
- `scripts/little_loops/cli/create_extension.py:70-95` (`_render_extension()`) — scaffold generator's doc-comment enumerates the opt-in mixin Protocols (`InterceptorExtension`, `ActionProviderExtension`, `EvaluatorProviderExtension`, `LLHookIntentExtension`, `CommunicationAdapterExtension`); `EventBusAdapterExtension` is a concrete registration, not a generic opt-in mixin, so it does not belong in this list — but the new `EventBusBoundExtension` Protocol (Fourth Review #23) does; add it there [Agent 1/2 finding, revised]
...
- `docs/reference/EVENT-SCHEMA.md` / `scripts/little_loops/observability/schema.py` — add the `human_response` inbound event shape (`alert_id`, `verdict`, `edited_text?`, `reason?`) and note `_drain_inbound()`'s special-casing of it. `human_approval_requested`/`human_approval_resolved` already landed with FEAT-1794. **Decided (Fourth Review #21): add it** to `SCHEMA_DEFINITIONS` and `DES_VARIANTS` (`test_des_schema.py:41-52` requires both), bump the two hardcoded `58` counts in `test_generate_schemas.py` (`:17`, `:123`) to 59, and generate `docs/reference/schemas/human_response.json` (Third Review #15). EVENT-SCHEMA.md is the contract an external relay reads to learn the POST body shape, so this is documentation, not optional
- `docs/reference/CONFIGURATION.md` § `hitl` — also state the two known limitations: `ll-loop resume` has no inbound path (Fourth Review #19) and the `--serve` dashboard has no HITL UI (Fourth Review #20)
...
- `EventBusBoundExtension(Protocol)` in `extension.py` — `bind_event_bus(self, bus: EventBus) -> None`; hasattr-detected by `wire_extensions()` like the other opt-in Protocols (Fourth Review #23).
- `EventBusAdapterExtension.bind_event_bus(self, bus: EventBus) -> None` — stores the bus and constructs the single `EventBusAdapter(bus)` instance (which registers its observer in `__init__`); called by `wire_extensions()` before `provided_adapters()`. `EventBusAdapterExtension.provided_adapters(self) -> dict[str, CommunicationAdapter]` — returns `{"eventbus": self._adapter}`, the same instance on every call (Third Review #12); raises a clear `RuntimeError` if `bind_event_bus` was never called (only possible if a caller bypasses `wire_extensions()`). The class deliberately defines NO `on_event` — `wire_extensions()` registers any extension with `on_event` as a bus observer (`extension.py:246-248`), which would subscribe this registration-only shim to every event on every run (Second Review #7). Nothing runtime-checks the `LLExtension` Protocol (grep confirmed), so omitting it is safe.
...
- `EventBusAdapter.__init__` registers the one `HUMAN_RESPONSE_EVENT`-filtered observer on the bus immediately, and the adapter never calls `EventBus.unregister()` (Fourth Review #22). The adapter lives for one run, and the observer is a no-op dict lookup while nothing is pending, so the register/unregister lifecycle of earlier drafts is unnecessary and Second Review #9's mid-dispatch hazard is moot.
- `EventBusAdapter.send_alert(self, loop_name: str, state_name: str, prompt: str, captured_context: dict) -> str` (matches ABC at `communication_adapter.py:60`) — generates `alert_id`, creates the pending entry, and returns `alert_id`. It does NOT emit `human_approval_requested` — the executor is that event's sole emitter (Pre-implementation Review #2).
...
Same-thread reentrancy: `EventBus.emit()` iterates `self._observers` live (`events.py:124`), and `unregister()` deletes by index, so an observer that unregisters itself from inside its own callback shifts the list and skips the next observer for that event (Second Review #9). Moot under Fourth Review #22 — the adapter never unregisters — but the observer still only writes into `self._pending` and never mutates the bus. Also verified: the only observer-wiping path, the `PersistentExecutor._on_event` setter (`persistence.py:983-988`), is used by `run_foreground` only for bare executors without an `event_bus`, so an observer registered at wire time survives.
...
- Observer lifecycle: registered once in `EventBusAdapter.__init__`, never unregistered; the callback only mutates `self._pending` (Second Review #9, simplified by Fourth Review #22).
...
5d. `cli/loop/feed.py` (`:972-984` live feed, `:1146-1156` history formatter): add a `human_response` branch showing `alert_id` and `verdict` (Fourth Review #25). Test alongside the existing `human_approval_*` feed tests.
5e. Schema (Fourth Review #21, decided): add the `human_response` variant to `observability/schema.py` `DES_VARIANTS` and to `generate_schemas.py` `SCHEMA_DEFINITIONS` (inbound-origin docstring like `artifact_interaction`), bump both `58`s in `test_generate_schemas.py` to 59, regenerate `docs/reference/schemas/` (a new `human_response` JSON file), and add the `### human_response` section to `docs/reference/EVENT-SCHEMA.md` next to `:549`/`:569`.
6. Add `scripts/tests/test_eventbus_adapter.py`, modeled on `test_terminal_adapter.py`'s class-per-concern shape, covering: `send_alert()` returns a hex id and emits nothing; verdict resolution from a matching `human_response` bus event (approve / reject with reason / edit with text); non-matching `alert_id` and unknown `verdict` are ignored; re-entrancy across repeated `await_response()` calls including retained-verdict-across-timeout; `cancel_alert()` withdrawal and a late verdict after cancel being ignored; the observer is registered exactly once in `__init__` and still registered after `cancel_alert()` (Fourth Review #22); `supports_async()` is `True`.
7. Verification: `python -m pytest scripts/tests/test_eventbus_adapter.py scripts/tests/test_communication_adapter.py scripts/tests/test_extension.py scripts/tests/test_des_schema.py scripts/tests/test_generate_schemas.py scripts/tests/test_fsm_executor.py -k "inbound or adapter or extension or sub_loop or schema" -v` passes; then an end-to-end check under `ll-loop run --serve` with a `human_approval` state answered by `curl -X POST <bridge.url>interaction -d '{"event":"human_response","alert_id":"...","verdict":"approve"}'`.
...
- Verification-time contract check (Third Review #15, decided by Fourth Review #21): `test_generate_schemas.py:17` (`test_all_58_event_types_defined`) and `:123` (`test_creates_58_files`) hardcode the schema count — bump both to 59 when adding `human_response` to `SCHEMA_DEFINITIONS`, add the matching `DES_VARIANTS` entry (`test_des_schema.py:41-52` requires `SCHEMA_DEFINITIONS` ⊆ `DES_VARIANTS`), and regenerate `docs/reference/schemas/` (add `human_response.json`)
...
- `docs/reference/CONFIGURATION.md:1583` § `hitl`: document `channel: "eventbus"`, that it is built in (no `extensions:` entry needed), and that a live run needs `ll-loop run --serve` (or an in-process emitter) to receive verdicts; document the `human_response` POST body shape and that only `alert_id`/`verdict`/`edited_text`/`reason` are forwarded; state the two known limitations (no inbound path on `ll-loop resume`, no dashboard HITL UI — Fourth Review #19/#20). `hitl.default_timeout` already landed in that section with FEAT-1794
...
## Fourth Review (2026-09-05, pre-implementation)

_Manual review against HEAD (`271944ccd`). Already folded into the sections above; this is the index. Numbering continues from the Third Review._

19. **`ll-loop resume` has no inbound path.** `cli/loop/lifecycle.py:724-737` builds the resumed `PersistentExecutor` with no `inbound` queue and no `--serve` bridge, so a run interrupted mid-`human_approval` and resumed under `eventbus` can only time out, and the Step 5c warning fires on every such resume. Documented as a known limitation (Expected Behavior, CONFIGURATION.md § `hitl`); `resume --serve` is a follow-up issue, not scope here.
20. **The dashboard cannot answer.** The only interaction control in `templates/dashboard.llat/template.html.j2:380-400` posts an `artifact_interaction` note; nothing renders `human_approval_requested` or posts `human_response`. Use Case rewritten to curl/relay; dashboard HITL UI is a follow-up issue. Do not build it here.
21. **Schema decided: add `human_response`.** The event flows through every transport, the JSONL log, and history.db, and EVENT-SCHEMA.md is what an external relay reads to learn the POST body shape — documentation, not optional. `test_des_schema.py:41-52` requires `SCHEMA_DEFINITIONS` ⊆ `DES_VARIANTS`, so both get the entry; the two `58` counts become 59; the `human_response` schema JSON is generated under `docs/reference/schemas/`. Now Step 5e.
22. **Observer registered once, never unregistered.** The adapter is per-run and the observer is a no-op dict lookup while nothing is pending, so the register/unregister lifecycle of earlier drafts was unnecessary. Registering in `__init__` removes the bookkeeping, makes Second Review #9's mid-dispatch hazard moot, and avoids being the first production caller of `EventBus.unregister()`. `cancel_alert()` only pops the entry. Verified the one observer-wiping path (`PersistentExecutor._on_event` setter, `persistence.py:983-988`) is used by `run_foreground` only for bare executors without an `event_bus`.
23. **`bind_event_bus` gets a Protocol.** Every other opt-in capability has a hasattr-detected Protocol in `extension.py`; this one was designed ad hoc. Added `EventBusBoundExtension(Protocol)`, re-exported from `__init__.py`, listed in the `create_extension.py:70-95` scaffold doc-comment (the earlier "no change needed" note there was about `EventBusAdapterExtension`, which is still correct).
24. **Step 5b shares objects by reference.** "Copy" read as `dict(...)`; the intent is the same dict/list objects so the single adapter/observer stays one instance and a child's lazily seeded `terminal` adapter is visible to the parent.
25. **Feed rendering.** `cli/loop/feed.py:972-984`/`:1146-1156` format the requested/resolved events but a relayed `human_response` fell through unformatted — the only visible signal to a `--follow` operator that a mismatched `alert_id` POST arrived. Now Step 5d.
26. **Stale text.** Call Path and Dependent Files still said FEAT-1794 was unimplemented; Step 7 said "needs FEAT-1794"; the 09-05 Program Design bullet cited `_emit()` at `:3602` (it is at `:3783`); the Wiring Phase said FEAT-1794 "adds" `hitl.default_timeout` (already landed). All corrected.

Verified still correct at `271944ccd`: everything the Third Review listed, plus `_handle_event` → `event_bus.emit()` is synchronous (`persistence.py:1096`), `session_store` writers ignore unknown event types (`writers.py:2430`), and the `--serve` bridge is added as a transport after `wire_extensions()` (`run.py:622-625`), so the adapter's observer is registered before any relay can post.
...
- **Effort**: Small - one new adapter module (~150 lines), a 3-line `wire_extensions()` pass plus a Protocol, a `BUILTIN_EXTENSIONS` hook in `load_all()`, a 5-line `_drain_inbound()` branch, a 4-line sub-loop propagation, a schema variant + regenerated JSON, two feed branches, tests, docs. No executor dispatch work (that is FEAT-1794).
