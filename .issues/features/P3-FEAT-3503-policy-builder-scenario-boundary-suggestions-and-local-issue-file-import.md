...
- **The `blocked_by: FEAT-3488` dependency is code-satisfied but not tracker-satisfied**: `ll-issues show FEAT-3488` still reports `status: open`. Per this repo's Issue File Format, only `done`/`cancelled` resolve `blocked_by` edges — so the frontmatter edge stays technically live until FEAT-3488 is committed and marked `done`, even though the code it gates already exists in this working tree. Because the change is uncommitted, treat it as provisional: verify `git status`/`ll-issues show FEAT-3488` again immediately before starting implementation, in case it's reverted, further changed, or committed under a different shape.
