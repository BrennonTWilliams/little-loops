...
- **Data density rules out change-point detection.** Windows are calendar months; as measured on this repo's `.ll/history.db` on 2026-09-08, `orchestration_runs` spans 2026-08-02..2026-09-08 (two calendar months, Aug/Sep — zero rows in July), and per-orchestrator monthly points are even sparser than a first pass estimated: `ll-auto` has 2 points (Aug, Sep), `ll-sprint` has 1 point (Aug only). These are point-in-time counts that will grow as the DB accumulates more runs; the conclusion is not sensitive to the exact numbers; if anything, sparser-than-first-estimated data argues more strongly for a baseline comparison over a change-point algorithm. 210 of 554 rows have `started_at IS NULL` and would land in an empty-period bucket (this count is exact and confirmed). The detection statistic is therefore a **prior-K-window baseline comparison**, not a change-point algorithm: for each `(metric, orchestrator)` series sorted by period, baseline = mean of the previous `K` windows (default `K=3`, using however many exist, minimum 1) that clear `min_sample`; the current window is flagged when its relative move in the worse direction exceeds `sensitivity` and it also clears `min_sample`. Windows with an empty/NULL period are excluded from the series and counted in a `skipped_null_period` field on the analysis. Window granularity stays monthly in this issue; a finer `--window` is a follow-up.
- **There is no single "model in effect" per window.** As measured on 2026-09-08, the DB runs multiple models concurrently per orchestration-run month (3 distinct models in Aug 2026, 1 in Sep 2026 — this too will shift as more runs accumulate, and the point stands regardless of the exact count). A boundary join keyed on `effective_from` has nothing to join to. Attribution is instead a **composition shift**: for each flagged window, compute the share of runs per model, per host, and per `ll_version` in the window and in its baseline; report the `(dimension, value)` whose share increased most, provided the increase exceeds `ATTRIBUTION_MIN_SHIFT` (default 0.25 absolute share). Below that, "no attributable change".
- **Most of the stamp gap is already closed.** Per-issue model is derivable today via `issue_sessions -> usage_events.model` (the join the cost metric already uses; reaches 623 of 641 `issue_sessions` issues on this repo as measured 2026-09-08). Host is on `raw_events.host` via the same session join. Only the little-loops **version** is unrecorded anywhere. Scope the schema migration to a single `ll_version TEXT` column on `orchestration_runs` (and `loop_runs`), read model/host through the existing joins, and drop the plan to thread `model`/`host` through every `record_orchestration_run` call site. Write-timing: `__version__` is known at dequeue, so `ll_version` follows the `base_sha` COALESCE pattern (`writers.py:1357-1360`).
...
- `scripts/little_loops/cli/artifact/dashboard.py:42` — imports `SCHEMA_VERSION` from `session_store.schema`; unaffected by the value change itself but confirms no hardcoded `47` elsewhere in this file (only the constant is imported). (`cli/doctor.py:502` was cited here previously but does not import `SCHEMA_VERSION` at that line — verified 2026-09-08, removed as a misattribution; `doctor.py` only mentions `SCHEMA_VERSION` in a docstring at line 493.)
...
- `record_loop_run_summary` test call sites: `scripts/tests/test_history_reader_usage.py:79-85`, `scripts/tests/test_cli_ctx_stats.py:584-590`, `scripts/tests/test_history_reader_events.py:453-601`, `scripts/tests/test_session_store_lifecycle.py:698-718`, `scripts/tests/test_issue_history_parsing.py:148-155,694-712`, `scripts/tests/test_feat3182_evidence_bundle.py:374-382`. `scripts/tests/test_feat3182_executor_git_facts.py:142-149` patches the callable rather than constructing a real call — only breaks if `FSMExecutor._finish()` stops passing an argument this test asserts on via `mock_record.call_args`. (Verified 2026-09-08 — three citations previously listed here were misattributions and are removed: `test_history_reader_events.py:255-256` pointed to an unrelated `write_advisor_consult(...)` call, not this writer, though the paired `453-601` citation in the same file is correct and retained; `test_session_store_queries.py:519-532` does not exist — that file is only 258 lines and has no reference to either writer; `test_issue_history_cli.py:1705-1813` does not exist — that file is only 1384 lines, and the intended target, `TestHistoryQualitySubcommand`, is already correctly cited in the Tests section above at `test_cli_history.py:220-271`.)
...
- `ll_version` is readable through the typed reader path — `recent_orchestration_runs()`/`aggregate_orchestration_runs()`/`OrchestrationRun` and their `loop_runs` counterparts — not only via raw SQL, closing the gap the Wiring Phase's `history_reader/runs.py`/`models.py` items call out.
...
**Remediation applied — 2026-09-08 (same session, follow-up pass):**
- Design notes → Design review corrections rewritten to state the measured
  figures (2 months of data not 3, ll-auto=2/ll-sprint=1 points not 3-4, 3/1
  distinct models per month not 5-8, 623/641 issues not 623/639), hedged as
  point-in-time counts since the conclusion doesn't depend on the exact numbers.
- The 4 broken citations removed/corrected: `cli/doctor.py:502` dropped from the
  Files to Modify note (kept `dashboard.py:42`, which is accurate);
  `test_session_store_queries.py:519-532` and `test_issue_history_cli.py:1705-1813`
  removed from the Tests wiring-pass list (both nonexistent; the latter's real
  target was already correctly cited elsewhere); `test_history_reader_events.py`
  narrowed to its correct `453-601` span, dropping the mismatched `255-256`.
- Added a new Acceptance Criterion requiring `ll_version` be readable through the
