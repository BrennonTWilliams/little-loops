...
- **Storage layout and write ordering.** Submission keys: `ll-policy-builder-submission-<workspaceId>-<projectId>-<requestId>`, plus a per-project index `ll-policy-builder-submissions-<workspaceId>-<projectId>` holding `{activeRequestId, recent: [requestId…], updatedAt}` (`updatedAt` comes from injected `now`). Before POST: verify durable document identity, write the complete record, then the index, then persist `outcome_unknown`; all must succeed. If a new record succeeds but its index write fails, send nothing, preserve the old index/active record, and remove only the newly created unindexed record. If rollback fails or the page crashes between writes, boot enumerates this workspace's submission records and indexes to find orphans: an unindexed `prepared` record can be removed because it never passed the POST barrier; an unindexed `outcome_unknown`/`accepted` record must retain its identity and be surfaced for reconciliation, never blindly deleted. Enumeration/read failures stop cleanup and report a storage diagnostic; they must not be interpreted as an empty index. Test repeated index-write failures, rollback failure, and crash recovery between each write.
- **Record compaction and recovery.** Persist `SubmissionRecord` as either a full immutable envelope plus delivery metadata (`prepared`/`outcome_unknown`) or compact identity plus delivery metadata (`accepted`/`rejected`). On acceptance/rejection atomically replace the stored record with the compact form, retaining all bindings, `revisionId`, and YAML byte length; do not mutate an envelope in place or leave a retryable state without YAML. A failed compaction leaves the prior full record available for conservative recovery. Open-project and boot cleanup **preserve full unresolved envelopes in every project**. Returning to a project reconciles unknown outcomes and permits an explicit identical retry after `request_not_found`; prepared requests remain explicitly submit-able. No automatic resend occurs.
...
- `SubmissionRecord` — discriminated union: `{kind: "full", envelope, yamlByteLength, delivery}` for `prepared | outcome_unknown`, or `{kind: "compact", identity, delivery}` for `accepted | rejected`. Compaction replaces the record atomically; only full records can be submitted.
- `SubmissionIndex {activeRequestId, recent, updatedAt}` — workspace/project-scoped discovery and retention metadata.
...
- [ ] Submission storage uses the documented keys and full/compact record union; retryable states always retain exact YAML, including across Open-project and boot cleanup. Accepted/rejected records compact atomically; failed compaction preserves the prior full record.
- [ ] Record→index ordering and the outcome-unknown barrier prevent POST on any persistence failure. Repeated index failures roll back newly created orphans; boot recovers interrupted writes without deleting attempted requests or the prior active record.
