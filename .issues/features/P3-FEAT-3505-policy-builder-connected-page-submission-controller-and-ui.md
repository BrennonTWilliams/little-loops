...
- **Storage layout and retention.** Submission keys: `ll-policy-builder-submission-<workspaceId>-<projectId>-<requestId>` (envelope + delivery), plus one index key `ll-policy-builder-submissions-<workspaceId>-<projectId>` holding `{activeRequestId, recent: [requestId…]}` so reload finds the active record without scanning. Retain the active record plus the 5 most recent terminal/rejected records per `{workspaceId, projectId}`; prune older ones (envelope holds full YAML) only after the new index write succeeds. Never prune an unresolved (`prepared`/`outcome_unknown`/non-terminal `accepted`) record.
...
2. Extract inline draft/meta storage into `createBuilderStorage` test-first under the Node gate; offline keys unchanged, connected keys workspace-namespaced; rebind the `.tmpl`. **Land steps 1–2 as their own commit** — a behavior-neutral refactor with its own regression surface (BUG-3502 vm harness, FEAT-3488 offline probe, golden) — with the offline probe re-run and the suite green before any controller work starts.
...
- Route the once-per-session storage warning (`.tmpl:410-416`, `showLiveStatus`/`_storageWarned`) through an injected warn callback on `createBuilderStorage`
- Update the purity comments in `policy_builder_core.mjs` (`:1054`, `:1173` — "pure… no ID generation, no clock reads, no storage/hashing/fetch") to carve out the DI factories: the module still touches no ambient globals; factories receive them injected
...
- [ ] After a terminal status, Refresh status and reload both re-read the row; a requeued request (`cancelled` → `awaiting_approval`) resumes polling. Readback with non-matching `bindings` is reported as a conflict, not accepted.
- [ ] Submission storage uses the documented key scheme with a per-`{workspaceId, projectId}` index; pruning keeps the active record plus 5 recent resolved ones and never removes an unresolved record.
...
- Verified accurate: wire contract (`policy_builder_routes.py` 1 MiB cap, `created:false` early return without `warnings`, readback `bindings`, `request_not_found`), `window.PolicyBuilderCore` bridge (`:3587`), `--color-status-*` tokens now in `:root`, `test_wiring_reference_docs.py:254` needle. `ll-verify-evidence` clean; no active required decision rules; Proposed Solution checks (exception handling, fixtures, AC↔Integration Map coverage) found no conflicts.
