...
- Discovering which projects on this machine are running ll-loop or ll-commands
- Extracting ll-relevant entries (loop state transitions, skill invocations, command results) from raw JSONL
...
2. Filters sessions that contain ll-loop, `/ll:` skill, or ll-command activity
...
2. **Filter ll-relevant sessions**: Scan JSONL for entries referencing `ll-loop`, `/ll:`, `ll-auto`, `ll-parallel`, or `ll-sprint`
...
- [ ] Entries are correctly attributed to the ll-command or loop that produced them
