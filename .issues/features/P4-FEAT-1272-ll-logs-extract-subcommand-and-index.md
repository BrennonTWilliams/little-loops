...
- `scripts/doc_scraper.py:504-528` — sitemap → markdown (analogous to index.md generation)
...
- **`queue-operation` record type**: no existing detection implementation anywhere in the codebase — must inspect raw JSONL files empirically to determine record schema before implementing criterion (a) of ll-relevance check
...
- **`doc_scraper.py` is at `scripts/doc_scraper.py`** (not inside `little_loops/`) — confirmed correct at lines 504-528
...
### Concerns
- **FEAT-1271 is the critical blocker**: `logs.py`, `discover_all_projects()`, `test_ll_logs.py`, and the `ll-logs` entry point registration are all created by FEAT-1271, which is still open. FEAT-1272 cannot begin until FEAT-1271 is merged.
- **`queue-operation` record schema is unknown**: Flagged in refine-issue research — no existing detection exists in the codebase; schema must be determined by inspecting live JSONL files before implementing criterion (a) of the ll-relevance check. Criteria (b) and (c) are fully specified and unblocked.
