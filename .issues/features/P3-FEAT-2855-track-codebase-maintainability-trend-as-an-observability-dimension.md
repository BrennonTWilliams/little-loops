...
- **Change-set entropy** — how scattered a typical commit's touched paths are across the directory tree. Pinned definition (2026-07-29): per logical change, the Shannon entropy `H = −Σ p_d log₂ p_d` of the distribution of its touched files across depth-2 directories (files at the repo root count as their own bucket); trend the window median of `H`. Depth-2 is the default because depth-1 is too coarse for src-layout repos and full-path entropy is dominated by filename uniqueness; make the depth a constant, not a config knob, until evidence says otherwise.
...
- **Verdict thresholds are uncalibrated for structural signals — treat the verdict as secondary** (added 2026-07-29). The `analyze_rejection_rates()` 0.8/1.2 ratio cutoffs were chosen for agent-outcome rates, which move faster and mean-revert; nothing argues they discriminate coupling/churn/entropy trends, which are noisier and slower-moving. A confident "degrading" verdict on structural noise would poison trust in the whole metric family. Therefore: the raw per-window time series is the primary output and must be fully readable without the verdict; the improving/stable/degrading label is secondary annotation. Emit the thresholds used alongside each verdict so a future calibration pass (a follow-up issue, not this one) can recompute verdicts from recorded series without re-walking git history.
...
- **`git log --numstat`/rename detection does not exist anywhere in the codebase yet.** The only existing `git log` subprocess wrapper is `_backfill_commit_events()` in `scripts/little_loops/session_store/writers.py` (~line 781 — the flat `session_store.py` was split into the `session_store/` package by ENH-2890), which runs `git log --all --name-only --pretty=format:...` (file paths touched only, no line-diff counts, no `-M`/`--follow`). It's the nearest template to copy the subprocess-invocation shape from (timeout, `cwd=repo_root`, `capture_output=True, text=True`, delimiter-based parsing via `\x1e`/`\x1f`), but a new module must add `--numstat` and `-M` itself.
- **Commit → issue attribution already exists** via the `commit_events` table (`session_store/schema.py:405-418`: `commit_sha` UNIQUE, `issue_id`, `files_json`, ...), written by `record_commit_event()`/`_infer_issue_id()` (`session_store/writers.py:728` / `:704`) and read via `history_reader.py::recent_commit_events(branch, issue_id, ...)`. This is the "issue = unit of logical change" join point the Design Notes call for. The SHA→issue reverse lookup is added by **FEAT-2867** (blocking) — reuse it, do not re-add.
...
- `scripts/little_loops/session_store/writers.py:_backfill_commit_events()` (~line 781) — template for the `git log` subprocess invocation shape (must add `--numstat`, `-M` — not present today)
- `scripts/little_loops/session_store/schema.py` `commit_events` table (:405-418) — existing commit ↔ issue attribution to join sampling windows against
