...
3. **`scan-failures` / `sequences` are context, not flagging inputs.** `scan-failures` clusters
   ...
   loops via `session_ids` → run folders would be a new feature. They are rendered as
   appendix tables under the flagged-loop section so the maintainer can eyeball them.
   **Both appendices are capped** — measured 2026-09-02 on the live fleet, the collectors at
   their parser defaults (`--top None`, `--limit 0` = unlimited) return **106,925** sequence
   chains and **790** failure clusters, which rendered verbatim is a multi-megabyte report.
   `fleet-review` takes `--appendix-top N` (default 20) and passes it as `top=` to
   `_collect_sequences` and `limit=` to `_collect_failure_clusters`; `0` means unlimited.
   Measured cost per run: `loop-fleet` ~3s, `scan-failures` ~12s, `sequences` ~30s (~45s
   total), so `--no-appendices` skips both collectors for a fast RE-MEASURE run (the
   sidecar/delta never depend on them).
...
4. **Baseline is a JSON sidecar, machine-local.**
   ...
   **full `Counter`** of `_derive_loop_outcome` values, not just the top one — it is free to
   ...
   `scripts/little_loops/cli/verify_private_refs.py:446-474`, keyed on loop name. **The
   comparability warning covers the run population, not just the window**: the Summary warns
   when the prior sidecar's `window_days`/`since`/`until` **or `excluded_projects` /
   `projects_scanned`** differ from this run's — `--exclude-project .` removes 1103 of 2248
   built-in runs on this machine, so a delta between an excluded and a non-excluded run is
   meaningless and must not render silently. `.loops/` is
...
7. **Pre-step: extract the collectors `fleet-review` consumes.** Three extractions, each a
   ...
   - `_aggregate_fleet_runs(runs: list[_LoopRunRecord]) -> list[_LoopFleetAggregate]` from
     ...
     make the baseline and tests unstable. **Group by `(loop_name, attribution)`, not
     `loop_name`**: the current table branch labels each row with `runs[0].attribution`
     (`logs.py:2209`), so once `shadowed` exists (Decisions #10) a loop with real built-in runs
     in one project and a modified copy in another would collapse into one row with an
     arbitrary Type and a merged `success_pct`. One aggregate per pair means the `loop-fleet`
     table shows a `builtin` row and a `shadowed` row for such a loop; existing
     `test_loop_fleet_*` fixtures have no mixed attribution, so they are unaffected. Adding
     `--aggregate` to `loop-fleet -j` is optional.
...
8. **`validate` is called as a library, not via `cmd_validate`.** `cmd_validate`
   ...
   `load_and_validate(path, raise_on_error=False, orchestration_request_path=...)` directly
   (`little_loops.fsm.validation`, mirroring `cmd_validate`) and renders the returned
   `list[ValidationError]` itself. **`BRConfig(Path.cwd()).orchestration.request_path` is
   resolved once in `_cmd_fleet_review` and passed into `_validate_builtin_loop` as a
   keyword**, not loaded per flagged loop inside the wrapper; this also keeps the wrapper
   testable without a cwd config. **Do not resolve `path` via `resolve_loop_path`** — it
...
9. **The source repo dominates the fleet — expose it, allow excluding it.** Measured at review
   ...
   `fleet-review` gains a repeatable `--exclude-project DIR` flag (compared with **both sides**
   `resolve()`d — `discover_all_projects()` returns `cwd` values from session JSONL, which on
   macOS can be `/private/tmp/...` where the user typed `/tmp/...`) applied after
   `discover_all_projects()`. No default exclusion — the maintainer
   decides per run; the runbook's HARVEST step recommends `--exclude-project .` when reviewing
   from this repo, **and `--existing-only`** — without it `discover_all_projects()` logs
   hundreds of "Decoded path does not exist" lines to stderr for stale pytest temp projects.
   (`add_corpus_target_args()` has no exclusion primitive today; `--exclude-project` is the
   only new argparse surface beyond `--threshold`/`--min-runs`/`--appendix-top`/
   `--no-appendices`.)
10. **Shadowed built-ins are a third attribution value, never flagged.** `_collect_loop_runs`
    ...
    `issue-refinement` (203, 1). A sixth is **this repo itself**: `.loops/rl-rlhf.yaml` is
    git-tracked here (`bc331070b`) and differs from the built-in, so every self-run of
    `rl-rlhf` is `shadowed` — the 2026-09-02 dry run of the flagging rule flags `rl-rlhf`
    (3 runs, 33%) purely on those runs, which is exactly the misattribution this decision
    prevents. Those 111+ runs executed a different loop and would be flagged
    ...
    `project_path / ".loops" / f"{loop_name}.yaml"` **or `f"{loop_name}.fsm.yaml"`** exists
    (checked once per project via a set of stems built before the `iterdir()` loop, not per
    run), emit `attribution="shadowed"`. The `.fsm.yaml` form counts because
    `resolve_loop_path` prefers `<loops_dir>/<name>.fsm.yaml` over `<name>.yaml`
    (`fsm/loop_paths.py:41-48`), so a compiled copy shadows too. **Apply the check at both
    attribution sites** — the flat-layout branch (`logs.py:2091`) and the legacy nested-layout
    branch (`:2127`); patching only the flat branch leaves legacy runs as `builtin`. `_flag_loops` already requires
...
- `scripts/little_loops/cli/logs.py` — **decided** (see Decisions #1): add a `fleet-review` subcommand via `subparsers.add_parser()` next to `loop-fleet`, with `_cmd_fleet_review(args, logger) -> int`, **and the dispatch branch in `main_logs()`** (`if args.command == "fleet-review": return _cmd_fleet_review(args, logger)`, beside the existing `loop-fleet` branch); extract `_aggregate_fleet_runs()` from `_cmd_loop_fleet` first (Decisions #7). No `Makefile`/`justfile`/shell wrapper/skill (the alternatives considered: `scripts/verify_learning_citations.sh` is the only standalone-script precedent; `skills/audit-loop-run/SKILL.md` the only "chain `ll-*` calls in prose" precedent; neither is chosen).
...
- **Validate wrapper test**: `_validate_builtin_loop("nonexistent-loop", orchestration_request_path=None)` returns `(False, [one error violation])` rather than raising; a real built-in name returns `valid=True` with the same violation list `load_and_validate` yields; **a nested built-in (`code-run-gate`, under `oracles/`) resolves and validates** — this is the regression test for Decisions #8's nested-lookup fix. Plus `_builtin_loop_paths()` keys equal `_get_builtin_loop_names()` and include `code-run-gate`.
- **Shadowed attribution tests** (Decisions #10): a fixture project with `.loops/general-task.yaml` and a `.loops/.history/<id>-general-task/` run → the record's `attribution == "shadowed"`; the same with `.loops/general-task.fsm.yaml` instead of `.yaml`; the same with the run under the legacy nested layout `.loops/.history/general-task/<id>/`; **mixed attribution**: one project with `builtin` runs and another with a shadowed copy of the same loop → `_aggregate_fleet_runs` returns two aggregates (one per attribution) and the `loop-fleet` table shows two rows; it is absent from `_flag_loops` output even with `runs >= min_runs` and `success_pct == 0`; `general-task` still appears under zero-run built-ins when no other project ran it; the report's "Shadowed built-ins" section lists the loop, absolute project path, and run count. A project with only `.loops/.history/` (no copy) still yields `"builtin"`, so existing `test_loop_fleet_*` expectations hold.
...
- **Appendix cap tests**: with fixture sessions producing more than N clusters/chains, the rendered appendices contain exactly N rows each at `--appendix-top N`; `--appendix-top 0` renders all; `--no-appendices` omits both appendix headings and never calls `_collect_failure_clusters` / `_collect_sequences` (patch them to raise) while the sidecar is still written.
- **Population-mismatch test**: a prior sidecar with a different `excluded_projects` list (same window) → the Summary carries the comparability warning naming the differing field.
- **`--exclude-project` test**: two fixture projects, exclude one — its runs vanish from aggregates, its path appears in `excluded_projects`, and a loop that ran only there is now zero-run. Also pass the exclusion via a symlink to the project dir and assert it still matches (both sides `resolve()`d, Decisions #9).
- ~~Conditional shell-script pytest gate~~ — does not apply (Decisions #1).

### Documentation
- `docs/reference/CLI.md` — `ll-logs` subcommand table (~line 3505), flags (`loop-fleet` flags at ~3609), examples (~3651): add a `fleet-review` row, its `--threshold`/`--min-runs`/`--exclude-project`/`--appendix-top`/`--no-appendices` flags, and one example; note the new `shadowed` value in `loop-fleet`'s Type column (Decisions #10). `ll-loop validate` (~904), `diagnose-evaluators` (~1228), `calibrate-budget` (~1247) are already documented and match the implementation; the runbook links them rather than re-documenting.
...
- Emit `attribution="shadowed"` in `_collect_loop_runs` when the project has its own `.loops/<name>.yaml` or `.loops/<name>.fsm.yaml`, at both the flat and legacy-nested attribution sites (Decisions #10); render a "Shadowed built-ins" section; document the value in `docs/reference/CLI.md`.
- `_aggregate_fleet_runs` groups by `(loop_name, attribution)` (Decisions #7) so the `loop-fleet` table never merges `builtin` and `shadowed` runs of the same loop into one row.
- Resolve `orchestration_request_path` once in `_cmd_fleet_review` and pass it to `_validate_builtin_loop` (Decisions #8).
...
- Add `--appendix-top N` (default 20, `0` = unlimited) and `--no-appendices`; pass `appendix_top` as `limit=`/`top=` to the two collectors, never their parser defaults (Decisions #3).
- Extend the baseline comparability warning to `excluded_projects`/`projects_scanned`, not just the window fields (Decisions #4).
- Add the `fleet-review` dispatch branch in `main_logs()`.
- The runbook's HARVEST command is `ll-logs fleet-review --all --existing-only --exclude-project .` when run from this repo (Decisions #9).
...
### Types
- `_LoopRunRecord.attribution` (existing, `:1167`) — value set widens from `"builtin" | "custom"` to `"builtin" | "custom" | "shadowed"` (Decisions #10). The comment already names `shadowed` (landed with the partial session); the `loop-fleet` JSON row and Type column carry the new value through. Drive-by (still open as of 2026-09-02): the `outcome` field comment at `:1166` omits `signal`; add it while there.
...
- `_validate_builtin_loop(name: str, *, orchestration_request_path: str | None) -> tuple[bool, list[ValidationError]]` — new thin wrapper over `_builtin_loop_paths()[name]` + `load_and_validate(path, raise_on_error=False, orchestration_request_path=orchestration_request_path)` (`little_loops.fsm.validation`; Decisions #8). The parameter is **`str | None`, not `Path`**: `load_and_validate` declares `orchestration_request_path: str | None` (`fsm/validation/structural_rules.py:1758`) and `OrchestrationConfig.request_path` is `str = "cli"` (`config/orchestration.py:91`). The caller (`_cmd_fleet_review`) resolves `BRConfig(Path.cwd()).orchestration.request_path` once and passes it in; an unknown name, `ValueError`, `yaml.YAMLError`, or `OSError` becomes a single synthetic error `ValidationError`, mirroring `cmd_validate`'s `--json` branch. `valid` is `not any(v.severity is ValidationSeverity.ERROR ...)`.
...
- `_cmd_fleet_review(args: argparse.Namespace, logger: Logger) -> int` — new `ll-logs fleet-review [--project DIR | --all] [--exclude-project DIR ...] [--window-days D | --since DATE] [--until DATE] [--threshold N=50] [--min-runs N=3] [--appendix-top N=20] [--no-appendices] [--existing-only] [-j|--json]`; dispatched from `main_logs()` like every other subcommand; `--json` prints the sidecar dict to stdout and **writes no files** (Decisions #4). Output stamp is `datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")`.
...
### Call Path
`ll-logs fleet-review --all [--exclude-project DIR] --window-days N` -> `_cmd_fleet_review` -> `discover_all_projects()` minus `--exclude-project` paths (Decisions #9) -> per project `_collect_loop_runs(proj, builtin_names)` **unwindowed** (`logs.py:2015`; emits `shadowed` where the project has its own copy, Decisions #10) -> zero-run set = `_get_builtin_loop_names()` − names seen in the unwindowed **`builtin`** records -> window filter applied in memory via `_in_window()` -> `_aggregate_fleet_runs()` over `builtin` records; `shadowed` records tallied separately per (loop, project) -> `_flag_loops()` (Decisions #2) -> per flagged loop: `_validate_builtin_loop(name)` (Decisions #8) -> `_collect_failure_clusters(projects=...)` / `_collect_sequences(projects=...)` run once over the same post-exclusion list for the appendices, capped at `--appendix-top`, skipped under `--no-appendices` (Decisions #3, #7, #11) -> `_load_prior_baseline(exclude=this run's sidecar path)` -> render markdown + `_write_baseline()` (skipped entirely under `--json`). The runbook (not the command) then drives DIAGNOSE: `cd <project> && ll-loop diagnose-evaluators <loop>` / `calibrate-budget <loop>` (Decisions #5) -> fix via `loop-specialist` -> next `fleet-review` run shows the delta.

### Decision Rules
- **Flag** iff `attribution == "builtin"` AND `runs >= min_runs` AND (`success_pct < threshold` OR `top_outcome in _FLAG_OUTCOMES`), where `_FLAG_OUTCOMES = {"error", "max-steps", "stalled", "failed"}`. Defaults `threshold=50`, `min_runs=3`. `success_pct` is `round(converged / runs * 100)` (unchanged from `loop-fleet`; `interrupted`/`signal` count against it). `top_outcome` is the outcome with the highest count, ties broken by outcome name ascending (deterministic). Note the `top_outcome` clause is *nearly* redundant at `threshold=50` — a non-converged plurality usually implies `success_pct < 50`, but `int(round(99 / 200 * 100))` is `50` under banker's rounding, so a loop with a `failed` plurality can sit at exactly 50% and only the outcome clause catches it. It also lets lowering `--threshold` still catch loops where a failure mode is the plurality. Keep it. (Decisions #2, #7)
- **Not a flagging input**: `scan-failures` clusters, `sequences`, and anything from `dead-skills`. Appendices show at most `--appendix-top` rows each (default 20) and are skipped entirely under `--no-appendices`; neither affects the sidecar. (Decisions #3)
- **Delta**: for each loop in the current aggregates, the **primary** column is `Δsuccess_pct = success_pct_now − success_pct_prior`, shown with `Δruns` beside it (sliding windows compare different run populations, so a raw `Δconverged` alone is misleading); also `Δconverged` and `Δ<outcome>` for each key in `_FLAG_OUTCOMES`. Loops with no prior entry are marked `new`; loops in prior but absent now are listed under "dropped out of window". The Summary emits a comparability warning when the prior sidecar's `window_days`/`since`/`until` **or** `excluded_projects`/`projects_scanned` differ from this run's. No prior sidecar → "no prior baseline" line, no table. The sidecar this run writes is never its own baseline. (Decisions #4)
...
- **Shadowed**: a run is `shadowed` iff `project_path / ".loops" / f"{loop_name}.yaml"` or `f"{loop_name}.fsm.yaml"` exists at harvest time, in either history layout (flat or legacy nested). Shadowed runs are never flagged, never count as zero-run evidence, and never enter `loops` in the sidecar; they are listed in the report and stored under `shadowed`. (Decisions #10)
...
- `_aggregate_fleet_runs()`, `_collect_failure_clusters()`, and `_collect_sequences()` are
  ...
  still pass). `top_outcome` tie-break is deterministic. Aggregates are keyed on
  `(loop_name, attribution)`; a mixed-attribution fixture yields two rows.
...
- Runs from a project that carries its own `.loops/<name>.yaml` or `.loops/<name>.fsm.yaml`
  copy of a built-in are attributed `shadowed` in both history layouts, never flagged, and
  listed in a "Shadowed built-ins" section with the absolute project path (tested with a
  fixture project).
...
- Two fixture sidecars in a test prove the delta table shows a positive Δsuccess_pct for the
  ...
  comparability warning on window **and** on `excluded_projects` mismatch, and a "no prior
  baseline" line on an empty dir; a same-day second run baselines against the first, not
  itself.
- Appendices are capped at `--appendix-top` (default 20) rows each and skipped under
  `--no-appendices`; a test proves the cap and that `--no-appendices` never invokes the two
  collectors yet still writes the sidecar.
