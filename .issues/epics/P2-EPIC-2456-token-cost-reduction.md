...
- **ENH-2720** — Default-flip tranche: flip `orchestration.request_path` default `"cli"` → `"sdk"` (plus per-loop `"batch"` for latency-insensitive automation) once ENH-2719's runs demonstrate parity and realized savings; caching/Batches/warming/deferred-tools are all dormant under the current default. Depends on ENH-2719. *(filed 2026-07-21, P2)* — **`status: done`, decomposed 2026-07-22 into ENH-2737 and ENH-2738**; ENH-2738 carries the live default-flip work and is blocked on ENH-2719.
- **ENH-2745** — Tier 0 before/after cost gate blocked on model-pricing drift (`MODEL_PRICING` missing `claude-sonnet-5`/`claude-opus-4-8`). Filed from ENH-2719's verification pass. *(filed 2026-07-23, P2)*
- **ENH-2746** — F3 compaction shrink ratio measured outside the 50–70% gate band (structural-eviction-only proxy). Filed from ENH-2719's verification pass. *(filed 2026-07-23, P3)*
...
- **Tier 0**: before/after cost delta on a locked 3–5 trace set (measured via host CLI `usage` block, since Tier 1 telemetry isn't online yet) plus a P1-hook regression test; JSON output helpers deliver 20–40% output-token reduction on FSM verdict strings. **[ENH-2719, 2026-07-22] BLOCKED**: `MODEL_PRICING` has no entries for the fleet's current default models (`claude-sonnet-5`/`claude-opus-4-8`), and no post-FEAT-2470 `general-task` trace uses the locked baseline's `claude-sonnet-4-6` — see `docs/observability/realized-savings-verification.md`. Follow-up: ENH-2745.
- **F1**: `cache_read_input_tokens` populated for >50% of FSM iterations in `general-task` runs; cache-marking oracle **never logs a 1.25× write on a block that wasn't reused within K subsequent calls**; fragment_store hit rate ≥80% across the locked Tier 0 trace set; deferred tool loading preserves the cache breakpoint across a 5-skill catalog churn (regression test). **[ENH-2719, 2026-07-22] PASS on paper, sample negligible**: 6/6 `usage_events` rows with `state` populated also have `cache_read_input_tokens > 0` (100%), but the denominator is 6 rows total (out of 183,524) — the live writer path is barely exercised under the default `request_path: "cli"`. See `docs/observability/realized-savings-verification.md`.
- **F3**: Compaction triggers at the configured soft threshold (default 7,500 tokens); eviction preserves system/CLAUDE.md blocks (regression test); eviction+heuristic combo reduces context size to the **50–70% range** on the locked trace set (the 88% cookbook figure is the semantic-summarization upper bound, reserved for a future upgrade) without measurable quality regression on a held-out eval set. **[ENH-2719, 2026-07-22] FAIL (2 of 3 traces over-shrink)**: `evict_sink_and_window()` measured 89.6%/73.8%/61.3% (mean 74.9%) on 3 real sessions — structural eviction alone, not the combined eviction+summarization pipeline. Follow-up: ENH-2746. **[ENH-2746, 2026-07-23] FAIL (worse than the structural-only proxy)**: the combined eviction+`summarize_6_section()` pipeline (the gate's actual scope, per FEAT-2598's own "eviction+summarization together" wording) measured 99.3%/98.9%/98.7% (mean 99.0%) on the same 3 sessions — the 6-section LLM summary compresses far below the 50-70% band, not above it. Gate band reconciliation still needed; see `docs/observability/realized-savings-verification.md`.
- **F6**: Cost table in `ll-loop run` output breaks out `cache_read` / `cache_creation` and is stable across versions (lock the JSON schema in tests). **[ENH-2719, 2026-07-22] PASS (test-enforced)**: `test_fsm_cost_graph.py`, `test_tier0_traces.py`.
- **F5**: `gen_ai.usage.*` attributes parse cleanly under `phoenix serve` (verify in CI via a fixture run; Phoenix install is optional); DES schema accepts 100% of currently-emitted events (F5.1 audit gate). Per-CLI-invocation `gen_ai.invocation.id` UUID unique across all FSM iterations; `GROUP BY gen_ai.invocation.id` rollup returns one row per invocation with token sums matching raw `result`-event `usage` totals. Streaming parity: `cache_read_input_tokens` matches between `client.messages.create()` and `client.messages.stream()` within 0.1%. **[ENH-2719, 2026-07-22] PASS (test-enforced)**: `test_otel_attributes.py`, `test_streaming_cache_parity.py`.
...
- **F10**: Cache hit rate on warmed long-running skills >80% (vs. ~0% without warming) on prompts >50K tokens. **[ENH-2719, 2026-07-22] BLOCKED (structurally dormant)**: unreachable under the default `orchestration.request_path: "cli"`; no opt-in `sdk`/`batch` run set exists to measure. See `docs/observability/realized-savings-verification.md`.
- **F4-gated**: Heuristic compressor hits the **3–6× range** on the locked 10-trace `general-task` set; gate flips to LLMLingua below 0.5× LLMLingua's measured ratio. **[ENH-2719, 2026-07-22] PASS (test-enforced)**: `test_heuristic_compression.py::TestReductionBand` already asserts every trace + the mean fall in 3.0–6.0×.
...
3. ~~**F4 benchmark definition** — lock a 10-trace `general-task` set; heuristic target 3–6× prompt-token reduction; gate flips below 0.5× LLMLingua ratio.~~ **Resolved 2026-07-22 (ENH-2719)**: already locked by FEAT-2675 (`scripts/tests/fixtures/heuristic_traces/manifest.json`, 10 traces) and gated by `test_heuristic_compression.py::TestReductionBand`, which already passes.
...
## Session Log
- `/ll:manage-issue` - 2026-07-23 - **ENH-2719 closed**: realized-savings verification report written (`docs/observability/realized-savings-verification.md`). Success Metrics bullets annotated per-gate (Tier 0 BLOCKED on model-pricing drift; F1 pass-on-negligible-sample; F3 FAIL, 2/3 traces over-shrink; F10 BLOCKED, structurally dormant; F4/F5/F6 PASS, test-enforced). Struck Open Question #3 (F4 10-trace set — already locked by FEAT-2675). Corrected stale `ENH-2720` decomposition status (done → ENH-2737/ENH-2738). Filed follow-ups ENH-2745 (Tier 0 pricing drift) and ENH-2746 (F3 gate-band reconciliation), both parented here. Per ENH-2719's own closure criteria, EPIC-2456 closure is conditioned on this report's existence, not 100% gate pass.
