...
2. The merge coordinator reads that field and chooses the merge target — base branch when `epic_branch is None`, the EPIC integration branch otherwise. An EPIC may declare a `base_branch:` (alias `target_branch:`) frontmatter field to fork/merge its integration branch from a ref other than `parallel.base_branch`; `ll-sprint` dispatch hard-stops if that declared base does not resolve locally or on remote (FEAT-2652).
...
5. When `epic_branches.verify_before_merge` is `true` (default `false`), step 3's merge/PR-open is gated on `ParallelOrchestrator._verify_epic_branch_before_merge`: the EPIC branch tip is checked out in a scratch worktree and `project.test_cmd`/`project.lint_cmd` are run against it. The verify subprocess prepends the worktree's `project.src_dir` onto `PYTHONPATH` so branch-only modules resolve to the worktree, defeating editable-install `.pth` shadowing that would otherwise resolve `import little_loops.<new_module>` to the main checkout and false-fail collection (BUG-2629). A failure blocks the merge/PR-open, leaves the branch un-merged (retried on the next completion event rather than permanently silenced), and is recorded in `ParallelOrchestrator.epic_branch_verify_failures` for `_report_results()` to surface in the run summary (ENH-2603).

BUG-2614: the merge/verify/PR logic underlying steps 3 and 5 is implemented as stateless free functions in `little_loops.worktree_utils` (`merge_epic_branch_to_base`, `verify_epic_branch_before_merge`, `open_pr_for_epic_branch`); `ParallelOrchestrator`'s methods above are thin wrappers around them. The `auto-refine-and-implement` FSM loop's `merge_epic_branch` state calls the same free functions directly, so an EPIC worked via that loop (rather than `ll-parallel`) gets the same merge-back behavior instead of the config flag going silently unhonored.

ENH-2643: on the FSM loop path a merge *failure* is self-diagnosing. `merge_epic_branch_to_base` takes an optional `run_dir` kwarg (threaded through only from the FSM `merge_epic_branch` state; the `ll-parallel` wrapper has no per-run dir and omits it). On a merge conflict — before `git merge --abort` discards the conflict state — it persists `merge-detail.txt` (the bounded `stderr + stdout` tail), `merge-returncode.txt`, and `merge-conflicts.txt` (the `git diff --name-only --diff-filter=U` path list) under `$RUN_DIR`, mirroring the verify gate's `verify-detail.txt` pair so `merge_failed` no longer requires re-running `git merge` by hand to learn the cause. A clean merge writes none of them.

ENH-2630: on the FSM loop path the redundant second verify run is elided. The loop's `verify` state already runs `verify_epic_branch_before_merge` against the epic tip and records its verdict (`$RUN_DIR/verify-verdict.txt`) plus the tip SHA (`$RUN_DIR/verify-sha.txt`). `merge_epic_branch` reuses that verdict — skipping its own `verify_epic_branch_before_merge` call — when the verdict is `passed` and the recorded SHA matches the current tip (the two states run back-to-back, so it normally matches), so the full suite is not run twice per finalize. The binding gate is still invoked as a fallback when the verdict is missing, non-`passed`, or the SHA is stale, so a failing tip can never be merged.

BUG-3364: `verify-verdict.txt` and `epic-merge-verdict.txt` can now also carry the value `error` — the heredoc's own python3 process crashed (e.g. an uncaught `ModuleNotFoundError`, distinct from a completed run classifying a real test/lint/merge failure) before it could write a real verdict. `merge_epic_branch`'s reuse guard above is unaffected: it only fires on `prior_verdict == "passed"`, so an `error` (or any other non-`passed`) prior verdict already falls through to a fresh `verify_epic_branch_before_merge` call, same as `failed`/`collection_error`/`config_error`.
