...
5. When `epic_branches.verify_before_merge` is `true` (default `false`), step 3's merge/PR-open is gated on `ParallelOrchestrator._verify_epic_branch_before_merge`: the EPIC branch tip is checked out in a scratch worktree and `project.test_cmd`/`project.lint_cmd` are run against it. A failure blocks the merge/PR-open, leaves the branch un-merged (retried on the next completion event rather than permanently silenced), and is recorded in `ParallelOrchestrator.epic_branch_verify_failures` for `_report_results()` to surface in the run summary (ENH-2603).
