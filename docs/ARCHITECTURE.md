...
`add_entry`/`list_entries`/`get_entry`/`resolve_entry`/`remove_entry` back `ll-queue`'s `add`/`list`/`status`/`remove` commands (`cli/queue.py`). `update_entry_result` backs `ll-queue run` (FEAT-2683), the serial dequeue-and-execute worker loop that dispatches each `pending` entry through `run_action()` (`runner_spec.py`) in priority/FIFO order and writes back the real `status`/`result`. `run_action()` explicitly excludes `RunnerType.LOOP`, so `.ll/queue.db` cannot carry FSM loop work — the two queue surfaces stay non-overlapping by construction rather than by migration. `ll-loop queue`'s PID-liveness JSON markers under `.loops/.queue/*.json` (`cli/loop/queue.py`) remain a separate, unchanged surface for FSM lock contention, preserved by FEAT-2684 as a compat shim rather than migrated into this store.
...
| `CapabilityReport` (frozen dataclass) | Structured preflight report returned by `describe_capabilities()` — holds `host`, `binary`, `version`, and `capabilities`; consumed by `ll-doctor` and `ll-action` |
