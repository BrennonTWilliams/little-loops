...
`add_entry`/`list_entries`/`get_entry`/`resolve_entry`/`remove_entry` back `ll-queue`'s `add`/`list`/`status`/`remove` commands (`cli/queue.py`). `update_entry_result` backs `ll-queue run` (FEAT-2683), the serial dequeue-and-execute worker loop that dispatches each `pending` entry through `run_action()` (`runner_spec.py`) in priority/FIFO order and writes back the real `status`/`result`. This mechanism supersedes `ll-loop queue`'s PID-liveness JSON markers under `.loops/.queue/*.json` (`cli/loop/queue.py`); FEAT-2684 migrates that compat path separately, using a distinct id space so the two don't collide during the transition.
