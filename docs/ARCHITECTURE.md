...
| `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` | `host_runner.py` | **Hard-disable tool-level background tasks in automation (FEAT-3078/FEAT-3060).** Shares `PruningProfileConfig`'s `automation_profile is not None` gate but is sourced from a distinct, global origin — `orchestration.disable_background_tasks` config (default `true`), not a per-loop/per-state `pruning_profile:`. When enabled, `ClaudeCodeRunner.build_streaming()` injects `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1`, preventing a Claude Code child from silently discarding completed work via `Bash run_in_background: true` whose result the parent never retrieves. Claude-Code-only; the other five runners accept and ignore the parameter. `automation_profile is None` explicitly neutralizes the var to `""` rather than omitting it, same leak-prevention pattern as `LL_AUTOMATION` above. |
