...
- [`scripts/little_loops/loops/hitl-md.yaml`](../../scripts/little_loops/loops/hitl-md.yaml) — Human-in-the-loop single-document review harness: GP-TSM segment → GAN-style generate/evaluate/score pipeline producing an interactive HTML page with per-segment color coding, edit affordances, and "Copy AI prompt" / "Copy updated markdown" controls
