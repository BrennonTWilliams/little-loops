...
- [`skills/create-loop/loop-types.md`](../../skills/create-loop/loop-types.md) — Wizard implementation: the `## Harness Questions` section
...
- [`scripts/little_loops/loops/workflow-generator.yaml`](../../scripts/little_loops/loops/workflow-generator.yaml) — Real-world meta-loop harness (compiler-lowering, not generator-evaluator): lowers a prose brief into a validated FSM-loop YAML artifact via six sequential passes, each LLM pass paired with a non-LLM `exit_code` gate
- [EVALUATION_GUIDE.md](EVALUATION_GUIDE.md) — the sibling pattern in the other direction: measuring whether something works, rather than harnessing a skill to get work done
...
- `ll-loop scaffold-eval` / `ll-loop scaffold-verify` (FEAT-2948, see [`docs/reference/CLI.md`](../reference/CLI.md)) — Python-templating alternative to `/ll:create-eval-from-issues` / `/ll:verify-issue-loop`: generates the same class of harness YAML directly from a Python API rather than through the LLM-driven skill
