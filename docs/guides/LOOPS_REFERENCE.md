...
| `oracles/resolve-decision` | Oracle sub-loop extracted from `autodev`'s inline decision cluster (BUG-3065) — probes whether `decision_needed` is decidable, deposits competing options via `/ll:refine-issue --auto` if not, runs `/ll:decide-issue --auto`, and re-verifies the flag cleared; returns `done`/`failed`. Invoked by `refine-to-ready-issue`'s three decision gates via `loop: oracles/resolve-decision` with `with: {issue_id}` |
