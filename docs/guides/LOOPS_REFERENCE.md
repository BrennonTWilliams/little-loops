...
**Claim-verification gate chain (ENH-3031)**: Before `confidence_check` runs, `refine-to-ready-issue` passes through `verify_issue` (`/ll:verify-issues --check`, non-fatal) → `check_verify_verdict` → `check_hedges` (`ll-issues check-open-questions`, widened to scan `Integration Map` / `Codebase Research Findings` / `Suggested Fix Direction` / `Program Design` in addition to the original three sections) → `check_ac_automatable` (`ll-issues check-acceptance-criteria`). All three gates route `on_no`/exit-1 to `check_refine_limit` **independently of the confidence scores** — a perfect first-pass self-grade no longer buys the shortest path to `done`. Every mid-chain route that used to skip straight to `confidence_check` (after `wire_issue`, `mark_wire_done`, `check_decision_mid_wire`, and a second-pass `check_wire_done`) now retargets to `verify_issue`, so a gate-forced `refine_followup` cycle always re-enters through the gates, not around them.
