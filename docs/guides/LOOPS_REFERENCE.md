...
| `general-task` | Definition-of-done driven task loop — define verifiable criteria first, then execute plan steps in batched passes (all remaining steps per pass by default) and verify until all criteria pass |
| `stepwise-task` | One-step-at-a-time variant of `general-task` — thin entry that binds `steps_per_pass: 1` and delegates to `general-task`, so each step is implemented and delta-verified before the next is selected |
...
| `refine-to-ready-issue` | Single-issue refinement pipeline — refine → wire → verify → confidence-check until the issue reaches ready status |
...
| `oracles/resolve-decision` | Oracle sub-loop extracted from `autodev`'s inline decision cluster (BUG-3065) — probes whether `decision_needed` is decidable, deposits competing options via `/ll:refine-issue --auto` if not, runs `/ll:decide-issue --auto`, and re-verifies the flag cleared; returns `done`/`failed`. Invoked by `refine-to-ready-issue`'s three decision gates, `autodev`'s five decision-gate entry points (ENH-3075), and `rn-remediate`'s two entry points (ENH-3090), all via `loop: oracles/resolve-decision` with `with: {issue_id}` (`skip_probe: "true"` for direct entries that already know a decision is needed) |
...
0. **Pre-flight baseline** — `check_baseline_tests` (shell) resolves the whole-suite test command exactly once (BUG-3269 §3c): `${context.test_cmd}` override, else `ll-config get project.test_cmd` (honors `.ll/ll.local.md`; a present-and-null `test_cmd` opts out — no test gate — rather than guessing `pytest`), writing the resolved command to `${context.run_dir}/resolved-test-cmd.txt`. It then runs the project test suite once before any work begins, writing output to `${context.run_dir}/baseline-test-output.txt` and the exit code to `${context.run_dir}/baseline-exit.txt`. If the command is unresolvable (opt-out) or unrunnable (exit 127, or a missing `ll-config` binary), `baseline-exit.txt` is written as the sentinel `SKIP` instead of a numeric code — a baseline that never ran must never be compared against. This snapshot lets the loop distinguish pre-existing failures from regressions introduced during the task. The state always routes to `define_done` regardless of outcome (a red baseline is recorded, not a blocker).
1. **Define Done** — writes verifiable acceptance criteria to `${context.run_dir}/dod.md`. When the task has a runtime surface (running code, executing tests, installing a service, producing output at runtime), the DoD must include runtime-behavior criteria — static file/import checks alone are insufficient. If the task names an existing exemplar artifact (a path, filename, or "already completed for X" / "like Y" / "matching Z" phrasing), it must locate and read that exemplar before authoring any format-specific criterion, deriving those criteria from what was actually read rather than the artifact-type name; if no exemplar is named, this read is skipped entirely (ENH-3272).
2. **Plan** — decomposes the task into discrete steps in `${context.run_dir}/plan.md`. Applies the same conditional exemplar-read as Define Done, and when it reads one, must carry the observed concrete format forward into the step text itself (e.g. "...as a `sequenceDiagram`, matching `<exemplar path>`"), since `do_work` only ever sees the step text (ENH-3272).
3. **Execute** — five sub-states handle crash recovery, step selection, implementation, verification, and marking:
   ...
   - `select_step` (shell): finds the first unchecked plan step and runs it through the attempt-cap / abandonment / blocker-halt checks, then extends the selection with further unchecked, non-exhausted steps up to `context.steps_per_pass` (default `0` = all remaining; `1` = one step per pass, which is what `stepwise-task` binds). The effective pass size is persisted to `${context.run_dir}/steps-per-pass.txt` on first selection (and halved by `capture_work_exit` on a multi-step timeout). It writes the selected lines, one per line, to `${context.run_dir}/current-step.txt`, clears the per-pass ledger `completed-steps.txt`, touches `pass-started.txt`, writes a crash-recovery checkpoint to `${context.run_dir}/checkpoint.json` (JSON with `in_flight_step` = first line, `in_flight_count`, `timestamp`, and `task_hash` fields), and emits `SELECTED_COUNT: <n>` plus `SELECTED_STEP: <first line>`. Routes to `do_work` on success, or to `spin_gate` if no unchecked steps remain. On a real selection it also clears `${context.run_dir}/continue-work-spin-counter.txt`; on `NO_UNCHECKED_STEPS` it increments that counter (not reset by the per-step abandonment branch — an abandoned step is itself a no-progress signal).
   ...
   - `do_work` (prompt): reads the selected step lines from `current-step.txt`, implements **only** those steps in order (must not modify the plan or DoD files), and keeps a per-step progress ledger: after each finished step it appends that step's exact line to `${context.run_dir}/completed-steps.txt` and rewrites `LAST_FILES: <paths>` (the union so far) in `${context.run_dir}/last-files.txt`. A pass is **context-bounded, not session-bounded**: when the context monitor reports the handoff threshold, the worker stops starting new steps, brings the ledger up to date, and runs `/ll:handoff`, whose `CONTEXT_HANDOFF` marker makes the executor (`on_handoff: spawn`) launch a fresh session that resumes at `do_work`; the re-entered session skips every line already in the ledger and runs `/ll:resume` if `.ll/ll-continue-prompt.md` is newer than `pass-started.txt`. Has a 3600s timeout per session attempt; exit code 124 signals timeout (see **Continue** below). Errors route straight to `capture_work_exit` (no self-retry). Captured as `work_result`.
   ...
   - `mark_done` (shell): marks the pass's steps `[x]` in the plan file using a cross-platform `awk` exact-line pattern, one line at a time. The lines credited come from `completed-steps.txt` when that ledger is non-empty (a pass that stopped early), otherwise every line of `${context.run_dir}/current-step.txt` (bound to selection, not to plan order, so the two states can never desync — BUG-1766). Selected-but-uncompleted lines stay `[ ]` for the next pass and have one attempt refunded from `step-attempts.txt`. It then clears the ledger and removes the in-flight checkpoint if present. The current-step marker is **preserved** so downstream `check_done` and `continue_work` can still read it as their `LAST_STEP` bounded marker (BUG-2538 fixed the lifecycle mismatch from ENH-2486); the next `select_step` overwrites it in place. Routes to `check_done`.
4. **Verify** — reads both the DoD and the plan, then applies a three-step verification policy:
   ...
   2. *Delta-scoped criterion verification* — `do_work` captures `LAST_FILES` (every file created or modified) in `captured.work_result.output` and writes it to `${context.run_dir}/last-files.txt`; `select_step` captures the step text in `captured.selected_step.output`. `check_done` reads these and only re-verifies criteria that are plausibly affected by the delta; previously-`[x]` criteria outside the delta are kept without re-running their commands, bounding per-iteration cost to the slice of criteria the latest step could have touched. This step also carries a **harness-workaround flag** (ENH-2859): if the delta touches only test files and a previously-failing verification now passes, or the diff adds an autouse fixture resetting global/module-level state, the criterion cannot be marked `[x]` without a one-line justification — and a justification that describes production behavior rather than test isolation is itself treated as a failed criterion (a defect signal, not a passing test utility).
   3. *Sample re-verification* — picks exactly **one** already-`[x]` criterion (preferring one not in the previous sample, so successive passes rotate through the checked set) and independently re-verifies it, then **replaces** the `## Sample Verification` section in the DoD (removing the prior section if one exists, creating it if not). Exactly one section survives per iteration — a transient spot-check, not a cumulative audit trail. The sample used to be `min(3, total_checked)`; it was the dominant per-pass wall-clock cost, and `final_verify` re-verifies every criterion before terminal done anyway.
...
5. **Continue** — `continue_work` handles three cases based on `${captured.work_result.exit_code}`:
   - **Exit 124 (timeout)**: the previous `do_work` pass did not finish within 3600s. Before `continue_work` runs, the shell state `capture_work_exit` records the exit code and, when `current-step.txt` holds more than one uncompleted line, mechanically halves the persisted pass size in `steps-per-pass.txt` (refunding one attempt per uncompleted line), so an oversized pass degrades toward stepwise without any LLM judgment; if the worker's `completed-steps.txt` ledger is non-empty it emits `PARTIAL_CREDIT` and routes through `verify_step` → `mark_done` first so finished steps are credited. `continue_work` then reads `current-step.txt`: for a multi-line pass it prints `PASS_SHRUNK` and stops; for a single step it splits that step into 2–3 smaller independently-completable sub-steps and inserts them (as unchecked `- [ ]` lines) before the next unchecked step in `plan.md`. It never appends a DoD-remediation step for a timeout — the work was sound but oversized.
   - **Exit -9 (OOM/SIGKILL)** (ENH-2293): the worker was killed by the operating system due to memory pressure, not a task or code failure. `continue_work` writes an OOM diagnostic to `${context.run_dir}/summary.md` and outputs `DIAGNOSE_OOM`, routing toward `diagnose` via retry exhaustion so the operator receives an actionable post-mortem rather than a silent hard exit. Note: `do_work` has no retry budget — every error routes directly to `capture_work_exit`.
...
To apply project-wide defaults, set `commands.confidence_gate.readiness_threshold` / `outcome_threshold` in `ll-config.json`. For `autodev`, `recursive-refine`, and `eval-driven-development` this takes effect directly — the runner seeds those context variables from config at launch (BUG-2767), with `--context` still winning. `refine-to-ready-issue` likewise omits threshold literals from its `context:` block (BUG-2767), so config defaults apply directly there too.

**Claim-verification gate chain (ENH-3031)**: Before `confidence_check` runs, `refine-to-ready-issue` passes through `verify_issue` (`/ll:verify-issues --check`, non-fatal) → `check_verify_verdict` → `check_hedges` (`ll-issues check-open-questions`, widened to scan `Integration Map` / `Codebase Research Findings` / `Suggested Fix Direction` / `Program Design` in addition to the original three sections) → `check_ac_automatable` (`ll-issues check-acceptance-criteria`) → `check_design` (BUG-3249: `ll-issues check-design`, the Program Design gate). `check_verify_verdict`, `check_ac_automatable`, and `check_design` route `on_no`/exit-1 to `check_refine_limit` **independently of the confidence scores** — a perfect first-pass self-grade no longer buys the shortest path to `done`. This describes only the genuine-failure case; BUG-3294 gives an unresolvable issue ID its own exit code (2) so it takes the (different) `on_error` path instead — `check_verify_verdict.on_error: check_hedges`, `check_ac_automatable.on_error: check_design`, `check_design.on_error: confidence_check` — failing open into the next gate (or, at the end of the chain, `confidence_check`) rather than running remediation against an issue that does not exist. `check_hedges` routes `on_no` to `check_hedge_attempts` instead (BUG-3170): the hedge scan is an absolute-zero probe over vocabulary with no answer/hedge distinction, so a residual count is the steady state for a well-refined issue, not a signal that refinement is incomplete. `check_hedge_attempts` bounds the gate to at most one forced refine per run — its first red reading routes on to `check_refine_limit` as normal, but a second red reading proceeds directly to `check_ac_automatable` rather than spending the shared refine budget and falling through to `breakdown_issue`. Every mid-chain route that used to skip straight to `confidence_check` (after `wire_issue`, `mark_wire_done`, `check_decision_mid_wire`, and a second-pass `check_wire_done`) now retargets to `normalize_structure` → `verify_issue` (ENH-3248), so a gate-forced `refine_followup` cycle always re-enters through the gates, not around them.

**Decision resolution (BUG-3065)**: The loop's three `decision_needed` gates — `check_decision_mid_refine` (after refine), `check_decision_mid_wire` (after wire), and `check_decision_needed` (after a failing outcome score) — no longer exit via `done` on `on_yes`. Each routes to its own `loop: oracles/resolve-decision` call state (`resolve_decision_mid_refine`, `resolve_decision_mid_wire`, `resolve_decision_pre_breakdown`), which deposits competing options if needed, runs `/ll:decide-issue --auto`, and re-verifies the flag cleared. `on_success` resumes the chain at the same target the gate's `on_no` already used (`check_wire_done`, `normalize_structure`) — except `resolve_decision_pre_breakdown`, which re-enters `confidence_check` so the outcome score is recomputed against the resolved decision rather than the ambiguity that caused the failure. `on_failure`/`on_error` route through `check_decide_rate_limited`, which discriminates a 429-exhausted oracle (per-issue `decide-rate-limited-<ID>` marker present → `mark_rate_limit_infra` writes terminal class `infra` and exits via `failed` without deferring the issue) from a genuine unresolved decision, which falls through to `record_decision_unresolved` (mirroring `autodev`'s state of the same name): defer with `--reason decision_unresolved` and exit via `failed`. A single `ll-loop run refine-to-ready-issue <ID>` thus drives a `decision_needed` issue to ready without operator intervention, instead of silently dead-ending after four states.
...
**Technique**: Accepts a natural language task description, generates an initial plan outline and a 9-dimension rubric (breadth, depth, complexity, clarity, consistency, logic_strategy, feasibility, testability, risk_mitigation), then iterates: classify the most needed research type (NEEDS_FILES or NEEDS_WEB) → research → synthesize findings into the plan → score all 9 dimensions → loop until all dimensions reach VERY-HIGH or `max_steps` is exhausted.
...
Phase 1.5 — Decision Resolution (ENH-3090 — shared oracles/resolve-decision sub-loop):
  ...
  (formerly inline check_decision_decidable / deposit_options /
  record_options_deposited / check_open_question_progress states, ENH-2443/ENH-2446)
  live in `oracles/resolve-decision.yaml`, the same sub-loop `autodev` adopted in
  ENH-3075. rn-remediate reaches it via two `loop: oracles/resolve-decision` call
  states:
  resolve_decision (from check_decision_needed.on_yes — full probe/deposit/decide
    chain, skip_probe: "false" default)
    on_success → re_assess | on_failure/on_error → check_decide_rate_limited
  resolve_decision_direct (from check_decision_needed_post.on_yes and diagnose's
    DECIDE route — skip_probe: "true", bypassing the probe since the caller
    already confirmed a decision is needed)
    on_success → re_assess | on_failure/on_error → check_decide_rate_limited
  check_decide_rate_limited (shell_exit: checks `decide-rate-limited-<ID>`,
    the sub-loop's caller-side handshake — a `loop:` call state can never observe
    the sub-loop's rate-limit-exhaustion terminal directly)
    on_yes → rate_limit_diagnostic (429 budget exhausted — terminate gracefully)
    on_no/on_error → emit_needs_manual_review (decision genuinely unresolved)
  See `oracles/resolve-decision` in the loop reference table above for the
  sub-loop's own internal contract (decidability probe, deposit-options marker,
  stall gate, decide, and the BUG-2595 post-decide assert).
...
Phase 3 — Remediation Actions:
  ...
  (decide is no longer an rn-remediate state — /ll:decide-issue now runs inside
  oracles/resolve-decision.yaml's run_decide state, reached via resolve_decision /
  resolve_decision_direct above. emit_needs_manual_review — reached from
  check_decide_rate_limited.on_no/on_error and from check_convergence's
  NEEDS_MANUAL_REVIEW route — still writes MANUAL_REVIEW_RECOMMENDED instead of
  MANUAL_REVIEW_NEEDED when the sub-loop's decide-options-deposited-<ID> marker
  is present (ENH-2443), and still writes the per-issue manual_review_handoff_<ID>.md
  diagnostic to the run directory (ENH-2530) — unchanged by ENH-3090 apart from
  reading the sub-loop's own marker literal directly instead of writing its own.)
...
**FSM flow**:
```
init (snapshot baseline: both the .issues/completed/ set and the live status: done set)
  → resolve_set (scope → SprintManager; else ll-issues next-issues, capped at max_issues)
      ...
      │         ├─ on_success → recheck_set (ENH-2615: EPIC scopes re-resolve
      │         │     the descendant set and re-dispatch any not-yet-dispatched
      │         │     descendants back to delegate, capped at 5 cycles;
      │         │     non-EPIC scopes fall through immediately)
      ...
      │         │     → finalize
      │         ├─ on_failure → delegate_failed (ENH-3366: reads
      │         │     ${captured.delegate.terminated_by} to distinguish
      │         │     autodev's genuine `failed` terminal from budget/signal
      │         │     classes — no longer laundered into recheck_set's
      │         │     clean-pass route)
      │         │     terminal → record autodev_failed → finalize
      │         │     other (timeout/max_steps/interrupted/signal) → recheck_set
      │         │           (preserves ENH-2686's mid-drain residual fold-back)
...
**Closure accounting**: closure is counted over a **union of two paths** (BUG-2403). Leaf issues complete *in place* — `status: done` in frontmatter, never moving directories — while decomposed parents are still git-mv'd into `.issues/completed/`. So `init` snapshots both the `completed/` set and the live done-set, and `finalize` diffs both against their post-run states; a closure counted by either path counts once. `NOT_CLOSED` / `SKIPPED` are read from `autodev`'s `autodev-passed.txt` / `autodev-skipped.txt` under the shared `run_dir`; `ERRORED` is recorded by `record_error` on an `autodev` infrastructure crash. The verdict (`success` / `partial` / `partial-with-errors` / `phantom` / `no-op`) reflects real terminal state, not an exit-code proxy. `summary.json` additionally reports `verify_verdict` (`passed` / `failed` / `collection_error` / `config_error` / `error` / `skipped` / `not_run`, ENH-2601/ENH-2631/ENH-2742/BUG-3364) — advisory only, not folded into `verdict`. ENH-2631: a pytest collection/usage error (exit 2, a harness/env problem) surfaces as the distinct `collection_error` class rather than a plain `failed`, and the failing exit code is carried in the `verify_returncode` field (JSON number, or `null` when verify passed/was skipped/never ran); the full failure detail snippet is persisted to `verify-detail.txt` in the run_dir (kept out of the JSON to avoid escaping arbitrary pytest output). ENH-2742: a missing/misconfigured npm script (stderr containing "missing script", e.g. `test_cmd` pointed at the wrong directory) surfaces as `config_error` instead of `failed` — a harness/config problem, not a code defect. BUG-3364: `error` is distinct from all of the above — it means the heredoc's own python3 process crashed (e.g. an uncaught `ModuleNotFoundError`) before it could classify anything, as opposed to a completed run that classified a real test/lint failure; `merge_epic_branch`'s own artifact, `epic_merge_verdict`, shares the same crash-signaling `error` token alongside its normal `merged` / `pr_opened` / `held_open` / `verify_failed` / `merge_failed` / `skipped` / `not_run` vocabulary. Both states capture the heredoc's own exit code (`$?`, distinct from the *inner* test_cmd/verify-gate result already flowing through `classify()`/`emit()`) immediately after the heredoc and only classify `error` when that exit code is non-zero **and** no verdict reached the artifact file — a legitimately printed verdict is never overridden. `finalize`'s two verdict-file reads additionally default a present-but-empty file to `not_run`, not the empty string that used to reach `summary.json` unexplained. ENH-2743: `summary.json` also reports `closed_via_recovery` — the count of `autodev-skipped.txt` IDs that nonetheless reached `status: done` by the time `finalize` ran (e.g. `ll-auto` re-implemented a parked issue after the sprint queue released the lock), computed via `comm -12` against the done-now snapshot. It is a visibility signal only, folded into neither `closed` nor `parked_rate`, so a reviewer can distinguish permanently-parked skips from ones that self-resolved within the same run.

**Notes**: The backlog set is resolved once upfront (not re-polled per issue); decomposition children created mid-run are still processed depth-first by `autodev`, but brand-new unrelated issues created during the run are not picked up — a deliberate, deterministic semantic. **Exception (ENH-2615)**: when `scope` is an EPIC-NNN id, `recheck_set` re-resolves the EPIC's descendant set after each successful `delegate` pass (the resolution walks `parent:` chains transitively) and re-dispatches any not-yet-dispatched descendants — capped at 5 re-dispatch cycles — so children decomposed mid-run land on the epic branch instead of bypassing it; non-EPIC scopes keep the single upfront resolution. **ENH-3366**: a failed `delegate` pass no longer reaches `recheck_set` directly — it routes through `delegate_failed`, which reads `${captured.delegate.terminated_by}` to tell autodev's genuine `failed` terminal (recorded, then routed to `finalize` so the run's ledger-based verdict still stands) from a budget-exhaustion or signal class (still routed to `recheck_set`, preserving the mid-drain residual fold-back). The loop uses `on_handoff: spawn` and an 8-hour timeout to continue across session boundaries. Auth failures during implementation fast-fail to `ENV_NOT_READY` (inherited from `autodev`'s `check_impl_auth` guard, ENH-2353). Use `ll-loop install auto-refine-and-implement` to copy the YAML to `.loops/` and customize.
...
**Technique**: Accepts a single issue ID or a comma-separated list. Drives a **single unified queue** through an interleaved refine-then-implement loop: delegates per-issue `format → refine → wire → verify → confidence-check` to the `refine-to-ready-issue` sub-loop, and on threshold pass immediately runs `ll-auto --only` against that issue before dequeuing the next one. When `/ll:issue-size-review` decomposes an issue, the new children are prepended depth-first to the same queue and each child is refined-and-implemented before the next sibling. First implementation runs as soon as the first leaf passes refinement — no "refine-all-then-implement-all" gap. Terminates when the queue drains.
                                                                         ...
                                                                         └─ NO  → check_decision_before_size_review → [decision_needed?]
                                                                                                                        ...
                                                                                                                        └─ NO  → run_size_review → enqueue_or_skip → [children found?]
                                                                                                                         ...
                                                                                                                         └─ NO  → check_spike_needed_before_skip → [spike_needed AND NOT spike_attempted?]  (BUG-2654)
                                                                                                                                    ...
                                                                                                                                    └─ NO  → check_reconcile_needed → [(pre-spike Readiness == post-spike AND NOT reconcile_attempted) OR superseded_marker_count > 0?]  (ENH-2689, ENH-2992)
...
**Diagram omissions**: For brevity every `[decision_needed?]` branch above is drawn as a direct `YES → run_decide` hop. In the actual YAML, all five decision-gate entry points — `check_decision_at_dequeue` before `refine_current`, `check_decision_after_refine` after `copy_broke_down`, `decide_current`, `check_decision_before_size_review`, and `triage_outcome_failure` — route first through the shared `oracles/resolve-decision` sub-loop (`resolve_decision` / `resolve_decision_direct` call states); only when it resolves does the flow reach `run_decide`. See the **Decidability gate parity** paragraph below (ENH-2443, BUG-2605, ENH-3075) for the sub-loop's own `check-decidable`/`deposit_options` retry mechanics. Also omitted: `implement_current`'s success edge is drawn as a direct hop to `dequeue_next`; the real routing is `implement_current.on_yes → verify_impl_closed` (BUG-3390), which re-reads the issue's status before dequeuing — see the **Post-implementation closure check** paragraph below. Also omitted: after `recheck_after_decide`'s threshold gate, an `assert_decision_cleared` state re-verifies `decision_needed` was actually resolved before `implement_current` — if not, it routes to `record_decision_unresolved → dequeue_next` instead of implementing. Also omitted (ENH-2717): `run_decide.on_error` (e.g. `/ll:decide-issue --auto` killed mid-turn by the subprocess watchdog, BUG-2718) routes to `check_decision_after_decide_error` rather than falling straight through to `recheck_after_decide` as drawn — if `decision_needed` is still `true`, this state short-circuits directly to `record_decision_unresolved`, skipping the redundant `snap_and_size_review → run_size_review` call that would otherwise re-discover the same unresolved flag; only when `decision_needed` was somehow cleared despite the error does it fall through to `recheck_after_decide`. A fourth `decision_unresolved`-writing route (ENH-2936): `recheck_after_size_review`'s own deferral cascade (design-gate → decision-check → stagnation → low-readiness, see the Ready-but-atomic paragraph below) re-checks `decision_needed` directly — via a `ll-issues show --json` field read, not a separate `check-decidable` call — after the design-gate branch and before the `readiness_stagnated` backstop, deferring `decision_unresolved` when the flag is still armed on the score-failing path (previously masked as `readiness_stagnated`/`low_readiness`, the ENH-2866 postmortem gap). Finally, `implement_current`'s failure path is abbreviated to `dequeue_next`; the real routing is `implement_current.on_no → check_impl_reached` (ENH-2989: a Phase 1 rejection that never reached implementation — `PHASE1_NOT_STARTED` marker — routes to `mark_not_started → dequeue_next`, distinct from a real implementation failure) `→ check_learning_gate` (blocked issues → `mark_gate_blocked → dequeue_next`) `→ check_impl_auth` (auth failures → `abort_env_not_ready → done`, else → `dequeue_next`). **Pre-flight dequeue gates (ENH-2868, ENH-2909, ENH-3148)**: the diagram's `dequeue_next → check_status_at_dequeue → check_decision_at_dequeue → check_blockers_at_dequeue → check_gate_at_dequeue` chain runs before every `refine_current` dispatch. `check_status_at_dequeue` skips already-`done`/`cancelled`/`deferred` IDs (`already_<status>` ledger stem, `skip_already_resolved`). `check_blockers_at_dequeue` — ported from `rn-implement.yaml`'s `check_blocked_by` (ENH-2008) — parses the dequeued issue's own frontmatter directly (not `ll-issues show --json`) and diffs its `blocked_by` list against `ll-issues list --json --status done`; per BUG-2897 a `deferred` blocker is non-terminal and does **not** resolve the edge, so it still classifies BLOCKED. Unlike `mark_gate_blocked`/rn-implement's `mark_deferred`, `skip_blocked` does not set `status: deferred` — it only ledgers `"ID  blocked_by_unmet"` to `autodev-skipped.txt` and clears `autodev-inflight`, so the issue remains `open` and is retried on its next dequeue. `check_gate_at_dequeue` (ENH-3148) recognizes an issue explicitly gated by policy — reusing `recheck_after_size_review`'s `GATE_MARKER` prose-phrase regex verbatim (BUG-3147's inline-matcher precedent) plus a placeholder-Acceptance-Criteria check (no checkbox items, deferral-note prose) — and routes to `defer_gated`, which *does* set `status: deferred --reason blocked_by_gate` (mirroring `mark_gate_blocked`'s BUG-2729 status-recheck guard), before any `refine_current`/`wire`/confidence-check cycle is spent on an issue no amount of refinement can unblock. All four pre-flight gates fail open (`on_error` falls through to the next gate / `refine_current`) so a gate error never blocks the queue. `finalize_done` surfaces `Blocked-by-unmet (%d)` as its own summary bucket alongside `Already-resolved`/`Gate-blocked`/`Decision-unresolved`, excluded from the generic `Skipped` bucket — like those, it is not folded into `summary.json`'s `verdict`.

**Notes**: The loop runs up to 500 iterations with an 8-hour timeout and uses `on_handoff: spawn` to continue across session boundaries. Both `refine_current` (sub-loop) and `implement_current` (shell `ll-auto`) use the `with_rate_limit_handling` fragment (3 retries, 30s base backoff); `refine_current` on rate-limit exhaustion dequeues and continues, while `implement_current` on exhaustion terminates the loop via `done`. The broke-down handshake flag (written by `refine-to-ready-issue` to `${context.run_dir}/recursive-refine-broke-down`) is copied into `${context.run_dir}/autodev-broke-down` only on the `on_success` path (via `copy_broke_down`), so the rest of autodev's state machine reads only the `autodev-*` namespace. When `refine_current` exits via `on_failure` or `on_error`, the sub-loop's `failed` terminal or a signal/crash routes to `skip_inflight` instead — the issue is recorded in `${context.run_dir}/autodev-skipped.txt` and the queue advances without passing an unrefined issue to `implement_current` (ENH-1679). This interleaved design also means partial forward progress is preserved if the run is interrupted — any leaves that already passed refinement have already been implemented. **Infra vs. quality skip classification (ENH-2727)**: `refine-to-ready-issue`'s `diagnose` state (reached via `on_error`/`on_failure` from 8+ states) routes to a `classify_terminal` state that emits a machine-readable termination-class sentinel — `infra` when the failing state was killed externally (captured exit code 143/137/124 = SIGTERM/SIGKILL/timeout, or a signal) **or when its captured `failure_type` is `transient`, `non_recoverable` or `infra_retry`** (BUG-2826), else `quality`. `failure_type` is the executor's own `issue_lifecycle.classify_failure()` verdict, exposed per captured state alongside `output`/`stderr`/`exit_code`; it is what lets an API-layer failure — 404 on an unresolvable model, 401 auth, 429 after retry exhaustion, a network error, all of which collapse to exit 1 — be recognized as infra instead of being blamed on the dequeued issue. Only `real` (an evidenced quality verdict) stays `quality`. Because a model-layer failure kills the prompt-based `diagnose` state the same way it killed the state being diagnosed, `diagnose`'s `next` **and** `on_error` both route through `write_failure_evidence` (BUG-2826), a non-LLM shell state that writes the failing state name, all captured exit codes, all `failure_type` verdicts and stderr tails to `${context.run_dir}/refine-failure-evidence.txt` — so the run is always self-describing without reading the events JSONL. `autodev`'s `skip_inflight` reads that sentinel: a `quality` classification ledgers the issue as `refine_failed` (a genuine refine defect needing attention); an `infra` classification instead routes to `skip_inflight_infra`, ledgering `refine_failed_infra` — a re-runnable transient, distinct from `refine_failed` in `autodev-skipped.txt` and broken out as a dedicated "Infra-skipped" line in the `done` summary rather than folded into the generic Skipped bucket. `refine_current.on_error` (a hard crash, not a diagnosed failure) routes straight to `skip_inflight_infra` since a crash is definitionally infra. `audit-loop-run` treats `refine_failed_infra` as a re-runnable bucket when reading `skipped_breakdown`. **Auth fast-fail (ENH-2353)**: `implement_current` failures are screened for host auth signatures (HTTP 401/403 or "Could not resolve authentication") via the shared `check_impl_auth` state; on match the loop aborts to `abort_env_not_ready` (producing an `ENV_NOT_READY` outcome) rather than recording `IMPLEMENT_FAILED`.
...
**Outcome failure triage** (BUG-1277, ENH-1291, ENH-1415): When `check_passed` fails (confidence thresholds not met), the loop enters `triage_outcome_failure` rather than immediately routing to size-review. This state reads `score_ambiguity` from the issue frontmatter and branches: if `score_ambiguity ≤ 10`, the issue is well-scoped but has an unresolved design decision causing low outcome confidence — the loop routes to `run_decide` (invoking `/ll:decide-issue --auto`) → `mark_decide_ran` (sets `${context.run_dir}/autodev-decide-ran` so decide does not re-fire later in the same iteration) → `rerun_confidence_after_decide` (invoking `/ll:confidence-check` to refresh stale pre-decision scores, BUG-1378) → `recheck_after_decide` (threshold gate). On gate pass, the loop proceeds to `implement_current` without decomposition. On gate fail (ENH-1415), the loop routes to `snap_and_size_review` (refreshes the pre-ids baseline) → `run_size_review` rather than dropping the issue, since the only outcome dimensions that can still drag the score below threshold after decide are Complexity and Change Surface — both decomposable. The decide-ran flag means that if size-review fails to decompose and `recheck_after_size_review` re-enters `decide_current`, that state short-circuits to `implement_current` rather than firing decide a second time. On parse error, the loop falls back safely to `detect_children`. Otherwise (ENH-2640), the loop enters `check_spike_needed`, which reads the `spike_needed` and `spike_attempted` frontmatter flags via `ll-issues show --json`: if `spike_needed` is `true` AND `spike_attempted` is not `true`, the low outcome confidence stems from an unproven **internal** mechanism (`spike_needed` set by `ll-issues set-flags`, ENH-2946), so the loop routes to `run_spike` (invoking `/ll:spike --auto`) → `rerun_confidence_after_spike` (invoking `/ll:confidence-check` to refresh stale pre-spike scores) → `enqueue_or_skip`. The `spike_attempted` guard makes this a one-shot — a completed-or-attempted spike never re-runs. On no match (and on parse error), the loop falls through to `check_missing_artifacts`, which reads the `missing_artifacts` frontmatter flag (set by `ll-issues set-flags` when Outcome Risk Factors mention absent files or unwired components, ENH-2946): if `true`, the loop routes to `run_wire` (invoking `/ll:wire-issue --auto`) → `run_refine` (invoking `/ll:refine-issue --auto`) → `rerun_confidence_after_wire` (invoking `/ll:confidence-check` to refresh stale pre-repair scores, BUG-1491) → `enqueue_or_skip`; if `false`, the loop falls through to `detect_children → size_review`. This four-branch triage prevents incorrect decomposition of issues whose low outcome confidence stems from an unresolved design decision, an unproven internal mechanism, or a wiring gap rather than excessive scope. **Decide-path spike parity (BUG-2654)**: the ENH-2640 spike gate above sits only on `triage_outcome_failure.on_no` (the no-decision branch). An issue routed down the decide path (`recheck_after_decide.on_no → snap_and_size_review → run_size_review → enqueue_or_skip`) — or the no-decide size-review path — funnels through `enqueue_or_skip` before any skip and never visited that gate, so a `spike_needed: true` issue was skipped as `low_readiness` with its spike remedy structurally bypassed. `enqueue_or_skip.on_no` now routes through `check_spike_needed_before_skip` (the same `spike_needed AND NOT spike_attempted` predicate), which gives a pending spike its one shot at `run_spike` and, on no match, falls through to `recheck_after_size_review` — preserving the BUG-1230 leaf-skip semantics. The `spike_attempted` one-shot guard prevents a double-fire across the triage and decide paths and lets the post-spike re-entry (`rerun_confidence_after_spike.next → enqueue_or_skip`) fall through cleanly to the skip write. **Post-spike reconcile plateau (ENH-2689)**: a spike can prove the mechanism yet leave Readiness pinned, because `/ll:refine-issue` only *appends* new "Codebase Research Findings" and never rewrites the issue's own Implementation Steps / Acceptance Criteria / Files to Modify — so `/ll:confidence-check` re-flags the same Concern every pass. To catch this, `check_spike_needed`/`check_spike_needed_before_skip` snapshot the pre-spike Readiness score to `${context.run_dir}/autodev-pre-spike-readiness.txt` on the spike branch, and `check_spike_needed_before_skip.on_no` now routes to `check_reconcile_needed` instead of straight to `recheck_after_size_review`. When the snapshot is bit-identical to the post-spike Readiness (a spike-ran-but-nothing-moved plateau) AND `reconcile_attempted` is not set, the loop routes to `reconcile_current` (invoking `/ll:reconcile-issue`, a targeted in-place rewrite of those three directive sections plus, conditionally, a contradicted `## Scope Boundaries` claim (ENH-2937), from the accumulated findings) → `rerun_confidence_after_reconcile` (one more `/ll:confidence-check`) → `recheck_after_size_review`. The `reconcile_attempted` flag (written by `/ll:reconcile-issue`) makes this a one-shot; on no plateau, missing snapshot, or any error, `check_reconcile_needed` falls through to `check_size_review_ran_this_pass` (BUG-2744, see below) so non-plateau issues behave exactly as before. **Contradiction branch (ENH-2992)**: the plateau and fresh-below terms above are both readiness-shaped, so a directive section contradicted by the issue's own findings only ever reached reconcile when it happened to *also* stall the score — measured at capture, 1,703 issues had been refined and 19 reconciled. `check_reconcile_needed` therefore OR's in a third `contradiction` term: `ll-issues format-check [ID] --format json` reports `superseded_marker_count` (public helper `issue_parser.superseded_marker_count`, counting ENH-2995's `⚠ Superseded` markers inside the three directive sections), and a non-zero count routes to `reconcile_current` regardless of what the readiness score did. The payload is captured into a shell variable and passed through the environment so the state keeps a single inline `python3 -c` predicate — deterministic Python, no LLM in the routing chain (MR-1). This term is deliberately **not** gated by `reconcile_attempted`: that flag is permanent frontmatter no state ever clears, so gating on it would make a second reconcile structurally impossible. It is bounded two ways instead — *structurally*, by `/ll:reconcile-issue` clearing the marker on every directive line it evaluates (including on its no-op branch, which otherwise makes no edits at all), so a consumed marker cannot re-arm the gate; and *numerically*, by a reconcile-scoped per-issue budget of 2 (`autodev-contradiction-reconcile-count.txt`, incremented in `count_repair_cycle_reconcile` via a consume-once `autodev-contradiction-reconcile-armed` handshake the predicate writes only on a contradiction-*only* fire, and reset per-issue at `dequeue_next`). Note the shared FEAT-2751 `autodev-repair-cycle-count.txt` / `CYCLE_COUNT >= 2` ceiling does **not** bound this branch: it is conditioned on `CUR <= PRE` (a contradiction fire that raises the score is never capped by it), it is a deferral-*reason* branch rather than a gate, and it is shared across all six repair classes. One consequence has its own carve-out: `/ll:reconcile-issue` stamps `reconcile_attempted: true` unconditionally, and `recheck_after_size_review`'s pre-deferral remedy dispatcher reads that flag as "readiness remedy already spent" and returns *no* remedy. Since a contradiction fire has no relationship to readiness, `count_repair_cycle_reconcile` stamps a per-issue `autodev-contradiction-reconcile-<ID>` marker on such fires, and the dispatcher dispatches `spike` (rather than nothing) when it sees one — otherwise a perfectly healthy issue that hit one contradiction would silently lose its spike remedy forever. Plateau-driven reconciles carry no such stamp and are unaffected. **Stale cross-issue guard-2 capture guard (BUG-2744, re-cut by BUG-3390)**: `self.captured` (`executor.py:227`) is a single dict scoped to the whole autodev run, not to the current issue — `run_size_review` is the only state that populates `captured.size_review_output` (see below), so `check_guard2_verdict` only makes sense when `run_size_review` actually executed for the CURRENT issue this pass. The original gate keyed on a NEGATIVE marker written only by `check_broke_down`'s `on_no` shortcut, so provenances that skip both `run_size_review` and `check_broke_down` — `rerun_confidence_after_wire`/`rerun_confidence_after_spike`, whose `next` is `enqueue_or_skip` — could reach `check_guard2_verdict` on a multi-issue run with a PRIOR issue's stale capture. The interposed `check_size_review_ran_this_pass` state now requires a POSITIVE marker instead: `count_repair_cycle_size_review` (`run_size_review`'s only successor) writes `${context.run_dir}/autodev-size-review-ran-this-pass`, cleared per-issue at `dequeue_next`; if absent, the issue bypasses `check_guard2_verdict` entirely and routes straight to `recheck_after_size_review`. Unlike the old negative-marker check, this now **fails closed**: a marker-check error also routes to `recheck_after_size_review` rather than falling through to `check_guard2_verdict` — unverifiable provenance must never evaluate possibly-stale text. **Ready-but-atomic earn-the-pass remediation (BUG-2734)**: `run_size_review` captures its status-line output (`capture: size_review_output`). `check_guard2_verdict` checks that captured text for the guard-2 "declined to decompose" verdict — `[ID] skipped: score X (ambiguous)` with `X` in 8-11 (Very Large) — via `evaluate: {type: output_contains, source: "${captured.size_review_output.output}"}` (never interpolated into a shell action, per BUG-2594). This is distinct from guard-1's qualitative-skip line (`"structural score N ... qualitative"`), which still falls straight through to `recheck_after_size_review` unchanged. On a guard-2 match, `check_readiness_for_atomic_remediation` checks Readiness alone (ignoring Outcome) by reading `confidence_score` directly from frontmatter; if it passes, `remediate_oversized_atomic` runs `/ll:wire-issue --auto` (to qualify the issue for Criterion D Pattern B scoring) → `rerun_confidence_after_atomic_remediation` (`/ll:confidence-check`) → `regate_after_atomic_remediation`, a one-shot re-check of the full readiness+outcome gate (honoring a per-issue `outcome_gate_waived: true` frontmatter flag): pass routes to `decide_current`/`implement_current`; a still-failing outcome defers via `set-status ... --reason oversized_atomic` — never `low_readiness`, since readiness already passed to reach this branch. `recheck_after_size_review` also now honors `outcome_gate_waived` on its own low_readiness write. **Automated go-no-go escalation (BUG-3390)**: `regate_after_atomic_remediation`'s FAIL branch (once past the dedicated design-gate remedy) now routes through `check_go_no_go_eligible → run_go_no_go → check_go_no_go_waiver → reopen_waived` before advancing the queue — a one-shot-per-issue-per-run automated counterpart to the manual `outcome_gate_waived` review. `check_go_no_go_eligible` re-reads the issue's frontmatter to confirm it was just deferred `oversized_atomic` (guarded by a `autodev-go-no-go-attempted-<ID>` marker, never cleared at `dequeue_next`) before invoking `/ll:go-no-go <ID> --auto`; a **GO** verdict stamps `outcome_gate_waived: true`, which `check_go_no_go_waiver` reads back from frontmatter (never parsing the skill's stdout, MR-1) and routes to `reopen_waived` — `set-status open`, re-arming `autodev-inflight`, and re-entering the implement chain at `decide_current`. A **NO-GO** verdict, an ineligible issue, or any error falls through to `dequeue_next`, leaving the deferral in place for a manual waiver. **Decision re-check before stagnation (ENH-2936)**: between the design-gate branch above and the `readiness_stagnated` backstop, `recheck_after_size_review` re-checks `decision_needed` and defers `decision_unresolved` when it is still armed — see the Diagram omissions paragraph above for the full rationale.

**Decidability gate parity (ENH-2443, BUG-2605, ENH-3075)**: all five `decision_needed: true` entry points — `check_decision_at_dequeue`, `check_decision_after_refine`, `decide_current`, `check_decision_before_size_review`, and `triage_outcome_failure` — route into the shared `oracles/resolve-decision` sub-loop (`resolve_decision` / `resolve_decision_direct` call states, ENH-3075), which owns the same `ll-issues check-decidable <ID>` deterministic pre-check formerly inline here (and still separately adopted by `rn-remediate`'s own `resolve_decision`/`resolve_decision_direct` call states, ENH-3090). Zero enumerable options routes to the sub-loop's own `deposit_options` (`/ll:refine-issue --auto`) → `record_options_deposited` (writes `${context.run_dir}/decide-options-deposited-<ID>`) → back to `check_decision_decidable`, which short-circuits straight to `run_decide` on the second pass. The marker bounds the detour to one deposit attempt per issue per iteration regardless of which entry point first observes `decision_needed: true`. `autodev` additionally clears the per-issue `decide-options-deposited-<ID>` marker at every `dequeue_next` (using the bareword `$CURRENT`, not `${captured.input.output}`, which still holds the previous iteration's ID at that point) since it runs a long-lived multi-issue queue; `rn-remediate` needs no equivalent clear since it is invoked fresh per issue by its caller (ENH-3090, AC4).

### `scan-and-implement` — Discover, Triage, then Implement Net-New Issues

**Technique**: Full discovery-to-implementation pipeline composed from two existing sub-loops. Before discovery, snapshots the IDs of all currently-active issues to `.loops/tmp/scan-and-implement-pre-ids.txt`. Runs `issue-discovery-triage` as a sub-loop. After discovery, snapshots the post-discovery active-issue IDs and computes `comm -13` against the pre-snapshot — yielding only issues that are **net-new and still active** (i.e., they were created during scan **and** survived triage; issues that were created and then closed by tradeoff-review leave the active set — whether by reaching `status: done` in place or by being moved to `.issues/completed/` — and so naturally drop out of the diff). Passes the resulting ID list as `input` to the `autodev` sub-loop, which then refines and implements each one.
...
| `human-approval-example` | Live, runnable `action_type: human_approval` demo (FEAT-1794) — approve/reject/edit/timeout routing with a bounded, unattended-safe wait |
...
### `workflow-generator` — Reusable FSM-Loop Generator

**Technique**: Meta-loop that lowers a prose brief into a validated, minimal, reusable FSM-loop YAML artifact via six sequential "compiler lowering" passes, each an LLM generation paired with a non-LLM `shell`/`exit_code` gate (MR-1 satisfied by architecture, not by a suppression flag): **intent capture** (brief → `intent.yaml`) → **state-graph sketch** (`intent.yaml` → `graph-sketch.yaml`) → **evaluator attachment** (attaches a non-LLM evaluator per state, chosen from a vocabulary generated at run time from `NON_LLM_EVALUATOR_TYPES`/`EVALUATOR_REQUIRED_FIELDS` (ENH-3355) — intentionally narrower than the full set `ll-loop validate`'s MR-1 accepts, excluding two types that need pipeline infrastructure this loop's generically-sketched states don't produce) → **routing-table resolution** (adds transitions, checks reachability) → **artifact emission** (assembles the final runnable `workflow.yaml`) → optional **adversarial minimum-coupling shrink**. Each validation gate loops back to its generation pass on failure; `emit_artifact`'s gate is `ll-loop validate` itself, re-validating the emitted artifact as an external structural check, with a bounded retry counter (`count_emit_retry`) before falling back to `diagnose`.

**When to use**: You just finished a tedious, repeatable multi-step task and want to turn a one-paragraph description of it into a re-runnable FSM loop, without hand-authoring YAML (the `/ll:create-loop` wizard's autonomous, artifact-emitting counterpart).
...
- Trajectories are written per-state to `${context.run_dir}/states/<state-name>/trajectory.jsonl` (whole-file mode, which has no state queue, writes instead to the `.ll/runs/harness-optimize-<unix-ts>/states/whole-file/` path `init_run` mints)
...
| `ll_auto_learning_gate_check` | Shell state that greps a run-dir file (`${context.run_dir}/ll_auto_last.txt`) for the learning-gate markers and emits a three-way signal: `GATE_INFRA` when the infra-failure marker (`GATE_INFRA_FAILED`, ENH-3084: gate could not run — scope-lock contention, missing binary, wedged child) is present, `GATE_BLOCKED` when the `LEARNING_GATE_BLOCKED` marker (ENH-2319 gate block) is present, `OK` otherwise. The evaluate pattern is `GATE_BLOCKED`, so `GATE_INFRA` falls to `on_no` — pair this with `ll_auto_learning_gate_infra_check` after it to discriminate. Pair before `ll_auto_auth_check` so a gate block/infra failure isn't misattributed as an auth failure. | `action_type: shell` + `evaluate.type: output_contains` (`pattern: "GATE_BLOCKED"`) | `on_yes` (gate blocked), `on_no` (continue → discriminator), `on_error` |
| `ll_auto_learning_gate_infra_check` | Shell state that greps a run-dir file (`${context.run_dir}/ll_auto_last.txt`) specifically for the infra-failure marker (`GATE_INFRA_FAILED`, ENH-3084) and emits `GATE_INFRA` or `OK`. Follows `ll_auto_learning_gate_check` as a discriminator so a gate that could not run is routed to retry/skip rather than the `/ll:explore-api` block remedy. | `action_type: shell` + `evaluate.type: output_contains` (`pattern: "GATE_INFRA"`) | `on_yes` (infra failure → distinct routing), `on_no` (continue), `on_error` |
| `ll_auto_not_started_check` | Shell state that greps a run-dir file (`${context.run_dir}/ll_auto_last.txt`) for the `PHASE1_NOT_STARTED` marker (ENH-2989: every Phase-1 early return in `process_issue_inplace`) and emits `NOT_STARTED` or `OK`. Pair before `ll_auto_learning_gate_check`/`ll_auto_auth_check` so a Phase 1 rejection isn't misattributed as a real implementation failure. | `action_type: shell` + `evaluate.type: output_contains` (`pattern: "NOT_STARTED"`) | `on_yes` (Phase 1 never reached), `on_no` (continue), `on_error` |
