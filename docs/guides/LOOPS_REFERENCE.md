...
**Technique**: Meta-loop that lowers a prose brief into a validated, minimal, reusable FSM-loop YAML artifact via six sequential "compiler lowering" passes, each an LLM generation paired with a non-LLM `shell`/`exit_code` gate (MR-1 satisfied by architecture, not by a suppression flag): **intent capture** (brief → `intent.yaml`) → **state-graph sketch** (`intent.yaml` → `graph-sketch.yaml`) → **evaluator attachment** (attaches a non-LLM evaluator per state, restricted to the same vocabulary `ll-loop validate`'s MR-1 accepts) → **routing-table resolution** (adds transitions, checks reachability) → **artifact emission** (assembles the final runnable `workflow.yaml`) → optional **adversarial minimum-coupling shrink**. Each validation gate loops back to its generation pass on failure; `emit_artifact`'s gate is `ll-loop validate` itself, re-validating the emitted artifact as an external structural check, with a bounded retry counter (`count_emit_retry`) before falling back to `diagnose`.
