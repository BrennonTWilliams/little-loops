...
3. **Execute** — four granular sub-states replace the old monolithic execute:
   - `select_step` (shell): finds the first unchecked plan step, writes it to `.loops/tmp/general-task-current-step.txt`, emits `SELECTED_STEP: <text>`. Routes to `do_work` on success, or directly to `check_done` if no unchecked steps remain.
   ...
   - `mark_done` (shell): marks the first unchecked step `[x]` in the plan file using a cross-platform `awk` pattern, then removes the current-step temp file. Routes to `check_done`.
...
The loop runs up to 200 iterations and uses `on_handoff: spawn` to continue across session boundaries. Each plan step now consumes approximately 6 iterations minimum (`select_step` + `do_work` + `verify_step` + `mark_done` + `check_done` + `count_done`), supporting ~33 plan steps before the cap fires.
