...
4. **Verify** — reads both the DoD and the plan, then applies a three-step verification policy:
   ...
   After writing the verification report, `check_done` routes unconditionally to a `count_done` shell state. `count_done` parses both files and emits a JSON object `{"unchecked_dod": N, "unchecked_plan": N, "failed_samples": N, "total": N}`, then uses an `output_json` evaluator to route deterministically: `total == 0` → `done`, `total > 0` → `continue_work`, missing file → `diagnose`. This replaces the previous `llm_structured` gate, removing LLM judgment from the termination decision and making the success contract machine-readable.
