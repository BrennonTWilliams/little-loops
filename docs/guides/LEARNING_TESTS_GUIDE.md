...
Auto-prove (ENH-2431) lets the gate resolve the block itself: an unproven target gets one `ll-learning-tests prove <target>` attempt before the gate gives up — if proving succeeds, the issue proceeds to `check_depth` in the same run instead of dead-ending. A target that fails proving still routes to `mark_learning_blocked`, which distinguishes an attempted-and-still-unproven block (`LEARNING_GATE_BLOCKED_PRE_DEQUEUE_ATTEMPTED`) from a never-attempted one (`LEARNING_GATE_BLOCKED_PRE_DEQUEUE`) in `failures.txt`.
