...
`rn-implement`'s `check_learning_ready` state (ENH-2406) is a fourth gate call site, chained onto its post-dequeue router before `run_remediation`. It does **not** use the shared `resolve_learning_targets()` helper — it shells out directly to `ll-learning-tests check <target> --stale-aware` once per registered `learning_tests_required` entry, mirroring `check_blocked_by`'s direct-frontmatter-parsing convention rather than the LLM-extraction JIT path. It only catches issues with explicitly registered targets; an issue with no `learning_tests_required` field falls through unaffected, same as today, and still gets checked by the in-`ll-auto` gate further downstream.

By default the gate is check-only, same as before. When the opt-in `auto_prove_learning_gate` context flag is set (ENH-2431), an unproven target gets one `ll-learning-tests prove <target>` attempt before the gate gives up — if proving succeeds, the issue proceeds to `check_depth` in the same run instead of dead-ending. A target that fails proving (or when the flag is unset) still routes to `mark_learning_blocked`, which distinguishes an attempted-and-still-unproven block (`LEARNING_GATE_BLOCKED_PRE_DEQUEUE_ATTEMPTED`) from a never-attempted one (`LEARNING_GATE_BLOCKED_PRE_DEQUEUE`) in `failures.txt`.
