...
| `epic_branches.refresh_on_reuse` | `"merge"` | Staleness guard on a reused EPIC branch relative to its fork base (ENH-3302). `"merge"` (default) merges the base in via a scratch worktree before dispatch; a conflict degrades to `warn` without failing the run. `"warn"` measures and logs only; `"off"` disables the check. See [Configuration Reference](../reference/CONFIGURATION.md#parallel). |
