...
The `ll-logs sequences` primitive extracts n-grams of `ll-*` tool invocations from your session logs and ranks them by frequency. This gives a complementary view to message-history analysis: instead of reading what you *asked for*, it reads what tools were actually *invoked* and in what order.

Feed this directly into `/ll:loop-suggester` for telemetry-driven automation proposals:

```bash
# One-shot: run ll-logs sequences and pipe suggestions to loop-suggester
/ll:loop-suggester --from-sequences

# Inspect the raw n-grams first, then feed a saved file
ll-logs sequences --json --min-count 2 > .ll/sequences.json
/ll:loop-suggester --from-sequences .ll/sequences.json
```

**How it differs from message-history mode:**

| Aspect | Message-history mode | Sequences mode |
|--------|----------------------|----------------|
| Input | What you typed (user messages) | What tools were actually called |
| Source | `ll-messages` JSONL | `ll-logs sequences --json` |
| Pattern signal | Natural language phrases | Tool invocation n-grams |
| Best for | Discovering intent-based workflows | Discovering habitual tool chains |
| Works without message history | No | Yes (reads tool logs directly) |

**Graceful fallback**: If `ll-logs sequences` returns no results (e.g., no session logs exist yet), `/ll:loop-suggester --from-sequences` automatically falls back to the standard message-history path with a notice.

**Output metadata**: Sequences-derived suggestions use `source: "sequences"` in `analysis_metadata` — distinct from `"commands-catalog"` (from-commands mode) and the message-history default. This lets you track which analysis path produced each suggestion file.
