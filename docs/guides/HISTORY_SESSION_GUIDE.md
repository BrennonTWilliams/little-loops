...
| v50 | ENH-3435 | `harness_events` baseline-condition columns (`timeout_s`, `host_cli`, `subject_model`, `input_hash`, `conditions_fp`) + `idx_harness_baseline` composite index, backing `--measure-baseline`/`--compare-baseline` matching |
| v51 | ENH-3464 | `harness_events` efficiency-vector columns (`input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`, `tool_calls`) — reporting only, never read by a pass/fail gate |
| v52 | ENH-3476 | `harness_events.channels_json` — one nullable column persisting ENH-3462's widened evidence (stdout/stderr/declared-artifact/git-clean channels, plus per-side-effect pass/fail) as a JSON array; `NULL` means no graded channel evidence |
...
| `harness_events` | `ll-harness` / eval-run outcome telemetry, including `semantic_*` scoring columns and a `parent_id` linking each DSL per-task row to its parent run (ENH-2740). v39 added `target_content_hash`, `target_path`, and `dirty`, which content-pin a run to the artifact it evaluated. v49 added the run-model columns `cell_key`, `repetition`, `attempt_kind` (`repetition`/`infra_retry`, CHECK-enforced), `continuations` (reserved, unpopulated), and `superseded_by`, plus a partial UNIQUE index on `(cell_key, repetition) WHERE attempt_kind = 'repetition'` guarding against duplicate repetition samples (ENH-3406) — schema-only, no producer populates these yet (ENH-3407). v50 added the baseline-condition columns `timeout_s`/`host_cli`/`subject_model`/`input_hash`/`conditions_fp` (ENH-3435). v51 added the efficiency-vector columns `input_tokens`/`output_tokens`/`cache_read_tokens`/`cache_creation_tokens`/`tool_calls`, parsed post hoc from captured stdout on `skill`/`prompt` runs, `NULL` on `cmd`/`mcp` and on timed-out/errored rows — reporting only, never gates pass/fail (ENH-3464). v52 added `channels_json`, a JSON array of `{name, examined, chars, note, passed}` objects persisting ENH-3462's widened evidence (stdout/stderr/declared-artifact/git-clean channels, plus per-side-effect pass/fail); `NULL` means no graded channel evidence — pre-migration rows, timed-out/errored rows, and the DSL aggregate/malformed-task rows (ENH-3476). Live-write-only — excluded from `rebuild()`. Queryable via `ll-session recent --kind harness` (ENH-2739, v31). |
...
| `history.db_path` | `null` | Override the default `.ll/history.db` location; relative paths resolve against the *resolved* project root (`resolve_ll_dir()`'s upward walk from cwd, ENH-2927) — not the bare working directory, so a command run from a subdirectory still anchors at the project's `.ll/`. The `LL_HISTORY_DB` env var takes precedence over this |
