...
**Flags:** `--auto` — non-interactive mode for FSM loop automation. Auto-decomposes only issues scoring >=8, with a qualitative-skip guard: if `score_ambiguity`, `score_complexity`, and `outcome_confidence` are all present and non-zero, and `score_ambiguity ≥ 18` and `score_complexity ≥ 18`, the issue is skipped rather than decomposed (low confidence is qualitative, not a scope problem — run `/ll:refine-issue` or `/ll:wire-issue` instead). If any of the three fields is absent, the guard is skipped entirely and normal auto-decomposition behavior applies. `--sprint <name>` — scope to issues in the named sprint only.
