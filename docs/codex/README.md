...
- **`--agent` (persona selection, ll-orchestrated)**: When ll's orchestration layer (`ll-auto`, `ll-parallel`, `ll-loop`) requests a persona via `--agent`, `CodexRunner` emits `CapabilityNotSupported` and drops the flag. For interactive Codex TUI sessions, agent selection works via `.codex/agents/*.toml` files generated by `ll-adapt-agents-for-codex --apply`. See [Usage → Current Limitations](usage.md#current-limitations).
