...
| `History` | `## Session Log` body section | Distinct `/ll:*` commands with occurrence counts; omitted if absent. Built on `parse_session_log()`/`count_session_commands()`, which read `session_log_body()` — BUG-3424 made this the *union* of every non-fenced `## Session Log` block (previously only the last), so a not-yet-normalized file with a duplicate heading now shows its complete history instead of silently under-reporting |
