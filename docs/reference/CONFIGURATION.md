...
| `epic_branches.enabled` | `false` | **FEAT-2447, child 1/4 of FEAT-2339.** When `true`, all children of a single EPIC share one integration branch (`epic/<EPIC-ID>-<slug>`) for both fork point AND merge target (per Decision ARCHITECTURE-096). Standalone (parentless) issues keep today's per-worker behavior unchanged. Worker-pool / merge-coordinator / orchestrator wiring lands in FEAT-2448 / FEAT-2449 / FEAT-2450 — until those ship this flag is parsed and stamped into generated configs but the orchestrator path is not active. See `.ll/decisions.yaml#ARCHITECTURE-096`. |
