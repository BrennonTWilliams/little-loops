...
Beyond the host-capability table, `ll-doctor` always runs 5 default install-surface checks (FEAT-2793/FEAT-2794): **Entry Points** (every `[project.scripts]` entry in `pyproject.toml` is importable/callable), **Skills & Commands** (discoverability count via the tool catalog), **Decisions Store** (`.ll/decisions.yaml` + `.ll/decisions.d/*.json` presence), **History DB** (`.ll/history.db` presence/readability), and **FSM Loop Validity** (aggregated `fsm.validation` results over builtin + project-local loop YAMLs).
...
Also reports `stale_symbol_ref` and `stale_cli_flag` (FEAT-3048), extending
the same drift-detection architecture the prose-dependency gap kinds use to
two more claim classes issue bodies assert in backticks. `stale_symbol_ref`
fires when a backticked symbol is attributed to a cited file that itself
resolves (e.g. `` `extract_prose_deps` `` in `` `prose_deps.py` ``, or the
dotted `` `prose_deps.extract_prose_deps` ``/explicit `` `prose_deps.py:extract_prose_deps` ``
forms) but the symbol does not resolve as a function/class def-site or
module-level constant in that file. `stale_cli_flag` fires when a backticked
`` `ll-<tool> <subcommand> [--flag ...]` `` invocation names a subcommand or
long flag the tool's argparse parser does not accept, resolved via a
`--help`-scraped surface index (short flags are ignored — ambiguous in
prose). A bare backticked identifier with no file attribution is never a
symbol claim. Both kinds respect the `<!-- ll-prose-ok: ... -->` suppression
comment on the line before a claim (the same convention
`cli/verify_skill_prose.py` uses). Both ship **report-only** first (the
repo-wide pytest sweep prints but does not fail the suite) until measured
precision on a sampled backlog slice clears the bar.
