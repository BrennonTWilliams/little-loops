...
6. **Summary** panel → `Apply this configuration?` → writes (including automatic skill/command/agent mirroring for gemini/kimi-code/qwen — ENH-3389), dependency validation, and tailored **next steps** (first issue to capture, `/ll:scan-codebase`, the code-graph command if skipped, `git init` when not a repo, `ll-adapt --host codex --apply` for codex, `ll-doctor`, `/ll:help`).
...
| `--baseline` | | Run a blind A/B comparison: executes primary skill with full evaluation gates (harness arm) and creates a matching ungated invocation (baseline arm) in parallel, then feeds both outputs into a blind LLM judge. Writes `ab.json` to the run directory and prints a terminal summary with pass-rate delta, Wilson 95% CI bounds `[lo, hi]` for each arm, and token/duration ratios. The `Verdict:` line runs a paired sign test on the discordant items (`b` = harness-only-pass, `c` = baseline-only-pass) rather than the raw delta sign; it names a winner only when `wilson_ci(b, b+c)` excludes 0.5, otherwise it reports `inconclusive at n=<n> (<b+c> discordant pairs)` (ENH-3298). **Cannot be combined with `--worktree`** — passing both exits with an error. |
...
| `--cross-host` | | Re-run the loop on a second available host CLI and append a cross-host comparison table to the baseline report. Requires `--baseline`. The comparison runs the execute state on the alternate host, then feeds both outputs into the same blind LLM judge. The `⚠ Ordering reversal` warning fires only when both runs independently establish a direction via the same paired sign test as `--baseline`; if either run is inconclusive, a softer `Note:` line is printed instead (ENH-3298). (ENH-2086) |
...
- **MR-2 (WARNING)**: A meta-loop should reference a captured baseline value in a later evaluator (`evaluate.previous`, `evaluate.target`, `evaluate.source`, or `evaluate.reference`). This ensures a measure→propose→apply→re-measure spine is present. Does not block validation.
...
- **Parameter default type mismatch (ERROR)**: A `parameters.<name>` entry whose `default:` value does not match its declared `type` (e.g. `type: string` with `default: 20`). `parameters.<name>.default` is seeded into context on every launch path — standalone `ll-loop run`/`resume`/`simulate`, sub-loop `with:`, and `context_passthrough` — not just `with:` bindings, so a mismatched default silently lands the wrong-typed value in context everywhere the loop runs. Blocks validation. (BUG-3425)
...
| `--host HOST` | Filter to a single host source: `claude-code`, `codex`, `opencode`, `pi`, `kimi-code`, `qwen`, `gemini`, or `omp`. `--host codex` discovers via `detect_sessions()` rather than globbing a project folder (Codex has none) and is cwd-scoped; Codex exec calls are normalized at the parser (`CodexNormalizer`, ENH-3433), so `rebuild()` derives `tool_events` rows from them — rows ingested before ENH-3433 stay raw (`sessions` row only via the `raw_events.session_id` fallback, ENH-3422 D3); re-deriving those requires deleting and re-ingesting the source rows |
...
Persisted work-item queue, backed by a dedicated `.ll/queue.db` (FEAT-2682) — distinct from [`ll-loop queue`](#queue-entries-loopsqueue)'s PID-liveness marker mechanism, which FEAT-2684 preserves unchanged as a compat shim rather than migrating. Schema: `{id, action, enqueuedAt, priority, status, result, claimedAt, ownerPid, attempt, nextAttemptAt}`, ordered by priority tier then FIFO within tier. Builder-origin run requests (FEAT-3498) add an `awaiting_approval` status and request/workspace/project/revision/issue-binding and approval columns (schema v4); see **Host approval** below. Status is one of `pending`, `running`, `done`, `failed`, `dead_letter`, `cancelled` (ENH-3416), `awaiting_approval` (FEAT-3498) — the first two, plus `awaiting_approval`, are non-terminal; the rest are all `requeue`-able. An `awaiting_approval` entry is a request submitted by a policy-builder-origin caller bound to a specific project, issue, and immutable YAML revision; no `--watch` drainer or one-shot `run` ever claims it — only `ll-queue run --id ID --approve` can accept it.
...
Emit `policy-router-builder.html` — a single self-contained page for visually authoring Decision Table, Rubric, and Issue Lifecycle loop YAML. The page inlines three project-derived blobs: design-token CSS variables (light + dark, from `load_design_tokens` / `render_as_css_vars_themed`), the canonical predicate grammar (`policy_rules.grammar_spec()`), and the skill/command catalog (from `skills/*/SKILL.md` + `commands/*.md`), each catalog row carrying a description and (when the skill declares one) an argument hint shown alongside its entry in every skill picker. Decision Table mode presents an ordered, numbered rule list with ↑/↓ reorder controls (on-screen order is precedence order), a "Try it" panel that highlights the first-matching rule for sample values, and a pinned, non-deletable "Otherwise →" fallback picker (a structured dropdown over existing outcomes, not free text). Issue Lifecycle mode (FEAT-3474) is the same decision-table shell aimed at a single Issue file's YAML frontmatter: dimensions are little-loops' own built-in frontmatter fields (locked, pre-typed) plus arbitrary user-declared custom fields, and outcomes are five fixed lifecycle verbs (prepare/refine/gate/implement/verify) that can't be added to or deleted, each pre-bound to a default skill + args and overridable from the same skill-catalog dropdown. A rule, the fallback, or a verb's follow-up may also route straight to one of three built-in terminal destinations — `stopped`/`skipped` (success) and `needs_attention` (failure) — instead of continuing through the verb chain, and hitting the step budget (`on_max_steps`) now lands on `needs_attention` rather than a bare failure (ENH-3492); see the [Policy Router Guide § Issue Lifecycle Mode](../guides/POLICY_ROUTER_GUIDE.md#issue-lifecycle-mode) for the full Built-in Dimension Table, Verb Table, destination semantics, and encoding rules. Rules and "Try it" come before action bindings and the max-steps budget, which are collapsed by default behind an "Advanced" disclosure (it opens automatically if a diagnostic points at an outcome or the step budget, and is never auto-closed). Task presets (document improvement, condition-based routing, preparation, implementation, and implementation with verification) seed the whole form — including a mode switch when the preset's mode differs from the active one — through the same undo-tracked commit path as every other edit; the implementation-with-verification preset routes `implement` to a `verify` step that runs a user-supplied shell acceptance-check command (issue ID first, then any configured args) and treats its exit code as pass/fail, not `/ll:verify-issues`'s issue-file validation. The generated YAML is demoted behind a collapsed "View generated file" `<details>` disclosure; the default view is a plain one-line summary (plus, in Issue Lifecycle mode, a transition summary distinguishing reachable verbs, verification status, and attempts vs. raw step count, and a structural transition graph — FEAT-3501) plus Copy/Download, shadow / unreachable-outcome / unknown-action validation hints, and a light/dark theme toggle that honors the project's configured `active_theme` before falling back to OS preference. The structural transition graph (Issue Lifecycle mode only) is a text/list rendering of a shared `analyzeTransitions(model)` contract — dispatch/goto/rescore/terminal edges, one line per node — that also drives the transition summary, so the two can never disagree; it flags `goto` cycles and outcomes that are referenced but unreachable as warnings, and `rescore` feedback as informational (the normal shape of every preset). It represents *authored* transitions only — the implicit `on_max_steps` → `needs_attention` route and every state's `on_error: failed` route are never shown — so a cycle or unreachable-outcome warning is never a claim about what a run actually does or whether it terminates; see the [Policy Router Guide § Issue Lifecycle Mode](../guides/POLICY_ROUTER_GUIDE.md#issue-lifecycle-mode) for the full contract and its limits. A pure `validateBuilderModel(model)` pass (reserved/duplicate/missing-reference/incomplete-action/step-budget/threshold/draft diagnostics, plus shadow detection) runs on every edit; while any `error`-severity diagnostic exists, Copy/Download are disabled but the YAML preview keeps rendering so you can see what would be emitted. "Try it" evaluates the same compiled rule table the emitted YAML uses (via `evaluateModel`), so it never disagrees with execution over boolean-operator handling or which rule actually won.

Authoring work persists automatically: each mode's edits are saved to `localStorage` as you go (per-mode drafts, restored on reload and when you switch modes back and forth — "Start blank" is the only way to discard a mode's draft, and even that pushes an Undo entry), and Undo/Redo buttons (or Ctrl/Cmd+Z / Ctrl/Cmd+Shift+Z, ignored while typing in a field) step through the whole session's edit history. "Save project" / "Open project" export and re-import a versioned JSON envelope covering every mode's draft — the portable format for resuming work later or handing it to someone else — independent of the generated YAML, which remains the only thing `ll-loop run`/`ll-loop validate` consume. A live status region announces Copy/paste and save/storage outcomes (success or failure) for screen readers and sighted users alike. Theming: both palettes are embedded, native controls follow the page's selected theme (`color-scheme`), and the page ships template-local light/dark defaults for its action/status colors (`color.action.primary-text`, `color.status.{success,warning,error,info}.{bg,text}`) that your token profile overrides when it defines them — so a profile lacking those keys, a partial `DESIGN.md`, or disabled tokens still render readable.

A **Scenarios** panel (FEAT-3488) below "Try it" lets you save named test cases per mode — mode-specific input plus an optional expected target, expected rule index, or "expect the derived fallback" — and re-run the whole saved suite offline via **Run all**, with no skill/shell/LLM/issue mutation ever executed. Each case reports pass / fail / unasserted / unasserted (needs review, when a pinned rule index has gone stale after a reorder or edit) / error (invalid input, or an expected target no longer reachable), plus a suite-wide total and routing-coverage summary (winning vs. merely-evaluated rule indexes, uncovered rules, fallback coverage, or high/medium/low rubric-branch coverage). Editing the policy or a case invalidates displayed results until you re-run. Suites are part of their mode's draft — they survive Save/Open/reload/undo the same way, and "Start blank"/a preset clears only the destination mode's suite (one Undo restores both). **Suggest cases** (FEAT-3503) adds deterministic, deduplicated, unasserted boundary cases (satisfying, `t-1`/`t`/`t+1`, missing-field, and fallback cases per rule; integer threshold neighbours in rubric mode) as one Undo. In Issue Lifecycle mode, **Import issue file** adds one unasserted case from a local issue `.md` file's opening frontmatter block; the body is never read, multi-line values under non-routing keys are ignored, and a read that outlives an edit, undo/redo, mode switch, or Open is discarded (Open project has the same guard). See [Policy Router Guide § Scenario Suites (offline)](../guides/POLICY_ROUTER_GUIDE.md#scenario-suites-offline) for the full contract.
