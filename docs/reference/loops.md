...
>
> In contrast, `best_effort.json` (ENH-3473) **is** archived: `PersistentExecutor.run()` writes it into `<run_dir>/` whenever the run ends on a no-acceptance termination — `max_steps`, `max_iterations_reached`, `timeout`, `stall_detected`, or `cycle_detected` (the `_NO_ACCEPTANCE_TERMINATIONS` set in `fsm/persistence.py`) — holding the run's last attempt (`metadata.best_effort: true`, `terminated_by`, `final_state`, `captured`, and the other `ExecutionResult` fields; `error` is always present, `None` when absent). It is an artifact only — the run is still classified as `max_steps` (etc.), with no new `terminated_by` value or exit code. On every other termination no file is written, and any pre-existing `best_effort.json` is removed — this matters after `ll-loop resume`, which reuses the same `run_dir`: a resumed segment that finishes `terminal` would otherwise leave a stale checkpoint from the earlier segment behind. Loops that configure `on_max_steps`/`on_max_iterations` do **not** reliably avoid the checkpoint: per BUG-158/BUG-2204 (`fsm/executor.py:822-835,973-980`), once the cap-summary handler has run, the executor keeps reporting `terminated_by="max_steps"` even after routing onward to a distinct terminal state, so the checkpoint still fires for that case today. Nested sub-loops (`_execute_sub_loop()`'s child `FSMExecutor`) and the signal-driven force-exit path (`archive_run_only()`) never write one — only the outermost `PersistentExecutor.run()` does.
