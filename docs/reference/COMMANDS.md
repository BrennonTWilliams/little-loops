...
### `/ll:wire-issue`
Post-refinement wiring pass that completes an issue's **Integration Map** — the structured record of every file that must change when the issue is implemented. Where `/ll:refine-issue` fills in the _what_ and _why_, `wire-issue` traces the _where_: every caller, importer, config entry, doc section, test file, replaced-artifact behavior, and side-effect file that the implementation will touch. It may additionally **annotate** a directive line that its own wiring additions contradict with an inline `⚠ Superseded` marker (ENH-2995, ENH-3049) — never a rewrite, and wire may only insert markers, never delete one.
...
- **`sites_to_add`** (ENH-3480): additional call sites, test functions, or doc sections *inside* a file the issue already names — a whole-file exclusion previously made these unreportable until a second pass
...
**Flow**: locate issue (`ll-issues path`) → extract risks from `## Confidence Check Notes` → `### Outcome Risk Factors` → resolve the spike directory from `project.test_dir` (guarded to `tests` when unset) as `<test_dir>/spike/<slug>/` → write a plan doc to `${context.run_dir}` (inside an FSM loop) or `.ll/spikes/` (interactive) in the shape of `skills/spike/plan-template.md` → implement under the resolved spike directory → verify with the plan's `pytest` commands → write back → recommend the next step. Spike code lives **only** under `<test_dir>/spike/`; `allowed-tools` pre-approves writes only under `**/spike/**` (plus `.ll/spikes/**` for the plan doc and `.issues/**` for write-back) — it does not make production files read-only, isolation is enforced by convention and the plan's mandatory regression-guard test. Python/pytest projects only.
...
- `loop_complete.terminated_by == "error"` (ENH-3471: attempt-batch — the action crashed) AND no `evaluate.verdict == "error"` in run (FATAL_ERROR catch-all) → BUG P2; the crash reason is available directly on `loop_complete.error` (primary) — no forensics required
- Last `evaluate` before `loop_complete` has `verdict == "error"` (single occurrence — no threshold) → `BUG — Evaluate error terminated the loop` P2; uses `error` field (falls back to `reason`) from the failing evaluate event; de-duplicates against FATAL_ERROR when both hold for `loop_complete.terminated_by == "no_route"` (ENH-3471: decision-step — no valid route for the errored verdict; this signal supersedes)
...
| `verify-issue-loop`^ | Generate verification loop YAML from a single issue's acceptance criteria (`--mode criteria\|adversarial`) |
