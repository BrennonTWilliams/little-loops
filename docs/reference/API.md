...
Reports twenty gap classes on the returned `FormatGaps` dataclass (`missing`, `renamed`, `empty`, `boilerplate`, `malformed_id`, `prose_dep_drift`, `stale_prose_dep`, `program_design_nonspecific`, `deprecated_key`, `multi_frontmatter`, `testable`, `stale_file_ref`, `unmarked_superseded_directive`, `duplicate_findings_block`, `ambiguous_file_ref`, `missing_behavior_parity`, `soft_dep_hard_edge`, `malformed_dep_id`, `stale_symbol_ref`, `stale_cli_flag` — each a `list[str]`, plus a derived `has_gaps` property and a `to_dict()` for JSON output; re-derive this count from `dataclasses.fields(FormatGaps)` rather than trusting the number written here):
...
- `symbol_index` - Optional `little_loops.issues.symbol_claims.SymbolIndex` (built once per invocation via `build_symbol_index()`) used to resolve symbol claims. When `None` (default), no `stale_symbol_ref` gaps are reported.
