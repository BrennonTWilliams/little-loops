...
Offline replay engine for testing `LLExtension` implementations without a running `ll-loop` or live `EventBus`. Load a JSONL events file recorded during a real run, register your extension, and call `replay()` to drive `on_event` with the recorded events.
...
Create from a pre-parsed list of `LLEvent` objects.
...
Load events from a JSONL file (one JSON object per line). Each line is parsed into an `LLEvent`. Raises `FileNotFoundError` if the path does not exist.
...
Register an extension to receive events during `replay()`. Accepts any object implementing the `LLExtension` protocol. May be called multiple times to register multiple extensions.

```python
def replay(self) -> None
```

Deliver all loaded events to every registered extension in order, calling `on_event(event)` on each extension for each event. Exceptions from individual extensions are not caught — the test sees the raw error.
