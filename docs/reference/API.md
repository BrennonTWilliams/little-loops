...
- **unmarked_superseded_directive** (ENH-2995) — an issue's `### Codebase Research Findings` block contains a correction phrase from a closed detection list while none of `## Implementation Steps`/`### Files to Modify`/`## Acceptance Criteria` carries a `⚠ Superseded` marker. A phrase inside a fenced code block, an inline backtick span, or a double-quoted run (ASCII `"` or curly `“`/`”`) is excluded from the scan (BUG-3447) — a quoted warning string, error message, or test name is not an authored correction.
- **duplicate_findings_block** (ENH-2993) — an H2 section carries more than one `### Codebase Research Findings` block; entries are `"<H2> (N)"`.
- **missing_behavior_parity** (ENH-3045) — a file ref in `## Summary`, `## Proposed Solution`, or `### Files to Modify` resolves (`classify_file_ref()`) and shares a clause with a replacement keyword (`delete`, `remove`, `replace`, `rewrite`, `supersede`, `delegate`, and their inflections — same clause as the ref, split on `.`/`;` + whitespace-or-EOL or a spaced em dash, BUG-3494; no multi-line proximity window), while no `### Behavior Parity` subsection exists under `## Integration Map` (`_heading_bodies()`). Suppressed unconditionally by `behavior_parity_not_applicable: true` in frontmatter — a human decision mirroring `program_design_not_applicable`; `/ll:refine-issue` and `/ll:wire-issue` must never set it themselves. Only reported when `ref_index` is given.
...
| `aggregate_workspace_activity(members, *, since, until=None)` | Sibling to `aggregate_history_dbs()` for cross-repo activity counts rather than quality analysis: per `WorkspaceMember`, read-only via the same schema-skew gate, counts `loops_run`/`loops_completed`/`issues_completed`/`issues_deferred` in `[since, until]` (each bound an optional ISO-8601 timestamp; naive values treated as UTC). Returns a `WorkspaceActivityResult(since, until, per_repo, totals)`; each `RepoActivity` carries a `MemberActivityStatus` (`ok`/`db_missing`/`schema_skew`/`unreadable`), `None` (never `0`) counts when not `ok`, and `has_history: bool | None` (ENH-3450): any-rows-ever over `issue_events` OR `loop_runs` irrespective of the window — `False` for an empty-but-schema'd db and for `db_missing`, `None` for `schema_skew`/`unreadable`, `True` with zero counts for a quiet window (windowless and analytics-table-free by design). Unlike quality's ATTACH/union machinery, every metric here is per-repo sum-decomposable, so `totals: WorkspaceTotals` is a plain Python sum over `ok` members — no ATTACH (FEAT-3445); `totals.has_history_members`/`has_history` count/any over members whose `has_history` is `True` (ENH-3450) |
...
| `check-readiness` | Exit 0 if `confidence_score` and `outcome_confidence` meet thresholds; explicit `--readiness`/`--outcome` flags win over `ll-config.json`; `--honor-waiver` lets `outcome_gate_waived: true` satisfy the outcome half (BUG-3390) |
...
as a harness or judge error — a runner infra error/timeout, or a
grader-internal error such as a crashed/unparseable judge (`cli/harness.py`,
BUG-3477). There is no single global exit-code vocabulary across tools —
read each tool's own contract.
...
print(result.terminated_by)   # "terminal", "max_steps", "max_iterations_reached", "timeout", "interrupted", "cycle_detected", "stall_detected", "workdir_vanished", or "error"
```
...
quote/backslash is a Python syntax break or injection. MR-11
(`little_loops.fsm.validation`) delegates to this module for that
Python-literal-position half (ENH-3342); MR-11 also independently scans the
plain bash-token position (this module deliberately does not — see
`scan_action`'s docstring), so the two are complementary halves of one rule,
not disjoint hazards.
...
Read-side API for `harness_events` rows (ENH-2739's schema, written by `record_harness_event()`) — one row per `ll-harness` / eval run outcome. `recent_harness_events()` returns rows newest first, optionally filtered by exact `runner`/`target` and/or a `since` lower bound on `ts`; returns `[]` on a missing/unreadable DB. `harness_eval_pass_rate()` rolls up the `semantic_passed` tri-state column into a pass fraction for *target*, counting only **authoritative** rows (`superseded_by IS NULL`, ENH-3408) and ignoring `semantic_passed IS NULL` rows (abstentions) and `semantic_verdict = 'error'` rows (grader-internal errors, BUG-3477); `ll-harness` sets `semantic_passed` on every non-abstained, non-grader-error run regardless of whether `--semantic` was supplied, so the denominator is *all non-abstained, non-grader-error authoritative runs* for *target*, not only the `check_semantic`-judged ones. A `--retry-of` chain therefore contributes exactly one row — its surviving attempt — not one per attempt. Returns `None` when there are zero scored rows. Named `harness_eval_pass_rate` (not `harness_pass_rate`) to avoid colliding with the unrelated `ab_writer.ABResults.harness_pass_rate` (an in-memory A/B-comparator field). `harness_eval_abstention_rate()` is its sibling (ENH-3185): same `target`/`since`/`db` signature, returns `{"abstentions": int, "scored": int, "abstention_rate": float} | None`, where `scored` here counts every **authoritative** semantically-judged row (pass+fail+abstain), excluding `semantic_verdict = 'error'` rows (BUG-3477) — a deliberately different, and generally smaller, population than `harness_eval_pass_rate()`'s `scored`, and (post-ENH-3408) the same authoritative-row filtering applies to both.
...
Capture suppression (ENH-3449): when `config` is provided, the row is suppressed if `analytics.enabled` is present-and-false (the `ll-init` opt-out shape) or the `analytics.capture.cli_commands` glob list excludes `binary`; a config missing the `analytics` key — or a present `analytics` dict with no `enabled` key — stays permissive. Independently, setting `LL_ANALYTICS_CAPTURE` to `0`/`false`/`off` (case-insensitive after strip; empty string = unset) suppresses the row *before* `resolve_history_db` — nothing touches the filesystem and `LL_HISTORY_DB` is never consulted, so the kill switch wins over it. One-way kill switch: no truthy value force-enables capture past a config-gate exclusion. `main_history()` and `main_session()` pass their project config (`ll-history`, `ll-session`).
...
- Runtime callers: `runner_spec.py::_run_cmd()` (ENH-3234) and the FSM `StateConfig`/loop-YAML declaration surface (ENH-3235, `fsm/runners.py`'s `DefaultActionRunner` shell branch) both resolve their `scopes` (when declared) into `project_child_env`'s `env_allow` parameter and both apply the same `gh_scope_extra()`/`write_credential_scope()` wiring (BUG-3400 wired the CMD-runner path to match the FSM path).
...
- Always redirects `GH_CONFIG_DIR` to `str(config_dir)` — unconditional, so a declaring-but-non-`github` state still gets the ambient `~/.config/gh` login hidden from `gh auth status`/`gh api`, not just an un-injected token.
- `with_token=False` returns `{"GH_CONFIG_DIR": str(config_dir), "GH_TOKEN": GH_SCOPED_NO_TOKEN}` — `GH_SCOPED_NO_TOKEN` (`"ll-scoped-no-github-token"`) is a module-level, non-empty, obviously-invalid sentinel (BUG-3402). Injecting it (rather than leaving `GH_TOKEN` unset) defeats a keychain-backed macOS `gh`'s ability to re-mint the operator's real token via `gh auth token` — an empty/unset `GH_TOKEN` alone does not, since `gh` treats an empty env var as unset and falls through to the Keychain.
- `with_token=True` additionally resolves `GH_TOKEN`: the `GH_TOKEN`/`GITHUB_TOKEN` env vars first (a value equal to `GH_SCOPED_NO_TOKEN` — e.g. inherited from an outer non-`github` declaring state — is treated as unset), else a `gh auth token` probe run via `project_child_env()` with no `env_allow` (full inherit, deliberately *not* redirected, so it can still see the operator's own session); a probe stdout equal to `GH_SCOPED_NO_TOKEN` is likewise treated as no token.
- No token obtainable (both env vars absent/sentinel and the probe fails/exits non-zero/returns the sentinel): raises `RuntimeError` naming the failure. There is no fallback path that leaves a declaring `github` state un-redirected or silently un-scoped. This is what makes the non-escalation guarantee hold: a nested `github` state spawned under an outer non-`github` declaring state inherits the sentinel and fails closed at spawn, rather than reaching the operator's Keychain.
- **Residual gap (macOS, Keychain-backed `gh`):** `gh auth token` reads the OAuth token from the login Keychain by a fixed per-hostname service name, independent of `GH_CONFIG_DIR`/`hosts.yml` — unlike `gh auth status`/`gh api`, which do consult `hosts.yml` and are correctly hidden by the redirect. The `GH_SCOPED_NO_TOKEN` sentinel closes this for every path `gh_scope_extra()` controls (`with_token=False` directly, and `with_token=True` via the env/probe rejection above); the only case it cannot cover is a fully *ambient* invocation with no scoping declared at all, which is outside this function's contract. Recorded in `.ll/learning-tests/gh.md`.
- **Observable behavior change:** a declaring-but-non-`github` child's `gh` commands that hit the network now fail with HTTP 401 "Bad credentials" instead of a local "run gh auth login" message, and any other `GH_TOKEN`-honoring tool loses its anonymous fallback inside such a child.

**Callers (`fsm/runners.py`'s `DefaultActionRunner` shell branch, and `runner_spec.py::_run_cmd()` since BUG-3400):** whenever `scopes is not None`, each opens a per-spawn `tempfile.TemporaryDirectory()` scoped (via `try`/`finally`) across the `Popen`+wait, and merges `gh_scope_extra(Path(tmpdir), with_token="github" in scopes)` into the `extra=` dict passed to `project_child_env`. A `RuntimeError` is caught and converted into a failed result (`ActionResult` on the FSM path, `RunnerResult(exit_code=2, ...)` on the CMD path), mirroring `resolve_scopes`'s `ValueError` catch just above it. The probe itself bounds its `gh auth token` call with `timeout=10` and normalizes `FileNotFoundError`/`TimeoutExpired`/`OSError` into the same `RuntimeError` contract (BUG-3400) — a missing `gh` binary or a hung probe (e.g. a headless macOS Keychain prompt) fails only the declaring dispatch, not the whole run.
...
from little_loops.queue_store import (
    ...
    remove_entry,
    update_entry_result,   # for the FEAT-2683 worker loop to record status/result; guarded WHERE status='running' (ENH-3416); also nulls claimed_at/owner_pid
    claim_entry,          # atomic pending->running acquisition write (BUG-2929); stamps claimed_at/owner_pid (FEAT-2930), increments attempt and honors next_attempt_at (ENH-3416)
    reset_to_pending,      # running->pending; shared by _reclaim_stale and `ll-queue requeue` (FEAT-2930); never touches attempt/next_attempt_at (ENH-3416)
