...
| `input_tokens` | `int \| null` | prompt only | Input tokens: sum of the observations that reported it, `null` when no observation reported it (ENH-3538) |
| `output_tokens` | `int \| null` | prompt only | Output tokens generated (same null rule) |
| `cache_read_tokens` | `int \| null` | prompt only | Cache-read tokens consumed (same null rule) |
...
| `harness_tokens` | `int \| null` | Tokens consumed by the harness arm; `null` when either arm's usage was incomplete |
| `baseline_tokens` | `int \| null` | Tokens consumed by the baseline arm; `null` when either arm's usage was incomplete |
...
| `failure_terminal` | `bool` | Emitted unconditionally (ENH-2814). `true` when the reached terminal state declares `failure: true` — the single source of truth for "did this run fail?", keyed on the flag rather than the state's name. Holds regardless of whether `terminated_by` is `"terminal"`, `"max_steps"`, or `"max_iterations_reached"` (BUG-3499: a step/iteration cap handler routing to a `failure: true` terminal still reports `true`). Absent in run archives predating ENH-2814. |
| `terminated_by` | `str` | Reason for termination: `"signal"` (OS signal), `"error"` (ENH-3471: attempt-batch — the action itself crashed), `"no_route"` (ENH-3471: decision-step — no valid transition, a `before_route` veto, or an evaluator crash; the executor could not decide where to go next), `"timeout"` (wall-clock timeout elapsed), `"terminal"` (a terminal state was reached), `"stall_detected"` (FEAT-1637 circuit fired with `on_repeated_failure: "abort"`), `"cycle_detected"` (same edge traversed more than `max_edge_revisits` times), `"max_steps"` (step cap reached), `"max_iterations_reached"` (full-pass cap reached), `"user_stopped"`, `"system_signal"`, `"interrupted"`, `"host_pressure_abort"` (ENH-2452 memory pressure), `"host_budget_exceeded"` (ENH-2453 subprocess RSS budget), `"cost_ceiling_exceeded"` (BUG-3360 per-state cost ceiling), or `"handoff"` (ContextLimitHandoff handler) |
| `error` | `str` | only when `terminated_by="error"` or `terminated_by="no_route"` (ENH-3471) | Error message explaining why the loop crashed. Read this field directly from the JSONL stream to diagnose crash reasons without filesystem forensics. |
...
| `session_id` | `str \| None` | always | Current session identifier (from `_session_id_or_none()`); `null` when no session context |
...
| `captured_at` | `string \| null` | ISO 8601 timestamp from the issue's frontmatter; `null` for issues created before ENH-1839. |
| `session_id` | `str \| None` | always | Current session identifier; `null` when no session context |
...
- **`ll-harness`** uses `RunnerResult.exit_code` with a caller-supplied `--exit-code` threshold (default `2` for timeout/exception markers, `0` for success). `exit_code == 2` alone does not distinguish a timeout from a runner error (bad target, MCP config, scope resolution) — both branches store `2`. `RunnerResult.timed_out` is persisted to `harness_events`; `RunnerResult.error` has no column. ENH-3464 additionally persists `RunnerResult`'s five efficiency fields (`input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`, `tool_calls`), parsed post hoc from captured stdout — `NULL` on `cmd`/`mcp` runs and on any timed-out/errored row, since `RunnerResult.timed_out`/`.error` above already gate an early return before those fields are copied onto the outcome. `--retry-of`'s admissibility gate (ENH-3407) therefore reads the persisted `timed_out` column, not `exit_code` — a retry of a genuine timeout is admissible, a retry of any other `2` (or a graded `1`) is refused until a persisted runner-error signal exists. ENH-3415: a stochastic runner (`skill`/`prompt`) graded over N>1 samples writes one `harness_events` row **per sample** (`attempt_kind='repetition'`) instead of one row per invocation — `RunnerResult.exit_code`/`timed_out` above still describe a single sample's row; the invocation-level verdict (`PASS`/`FAIL`/`ABSTAIN`/`ERROR`/`INCONCLUSIVE` — `ERROR` covers both a runner infra error/timeout and a grader-internal judge error, BUG-3477) is derived by banding the tally of those per-sample rows and is not itself persisted as a distinct row or column. ENH-3435: baseline-arm repetitions (`--measure-baseline` / `--compare-baseline`) land in this same table under this same convention — a baseline *is* the set of authoritative `repetition` rows for the incumbent content under matching conditions. Baseline-path rows additionally carry the v50 condition columns (`timeout_s`, `host_cli`, `subject_model`, `input_hash`, `conditions_fp`); `input_hash` and `conditions_fp` are never NULL on those rows, so pre-baseline rows are excluded from baseline matching by construction. Baselines are matched on `(runner, target, input_hash, target_content_hash, conditions_fp)` — never on `head_sha` (provenance only: the incumbent and the candidate share a HEAD in the meta-loop flow) and never on `cell_key` (both arms share it). The candidate rows a `--compare-baseline` run writes become the baseline for that content once accepted — the measurement is paid once (bidirectional store). ENH-3476 additionally persists ENH-3462's widened evidence as `channels_json`: a JSON array of `{name, examined, chars, note, passed}` objects (stdout/stderr/declared-artifact/git-clean channels, with per-side-effect pass/fail carried by `passed`) — raw channel `content` is never persisted. `NULL` means no graded channel evidence: pre-migration rows, the same timed-out/errored early return that leaves the efficiency fields `NULL`, the DSL aggregate row, and the DSL malformed-task row.
