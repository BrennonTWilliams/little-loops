...
| `design_guidance_context` | `cmd_run` / `cmd_resume` | Prose body of a project's `DESIGN.md` (Do's/Don'ts, visual intent), gated by the same `use_design_tokens: false` opt-out; empty string when absent. Consumed only by `html-website-generator.yaml` (ENH-3267). |
...
**Per-loop design-token opt-out (ENH-3099):** set `use_design_tokens: false` in a loop's `context:` block (or pass `--context use_design_tokens=false`) to skip token injection for that loop even when `design_tokens.enabled` is true globally. Accepted falsy values (case-insensitive): `false`, `no`, `off`, `0`, or an empty string; anything else, including omitting the key, defaults to `true`. `context.design_tokens_context` and `context.design_guidance_context` are always set (to `""` when opted out) so prompts referencing them never fail to interpolate.
