---
description: Analyze user message history to suggest FSM loop configurations automatically. Uses ll-messages output to identify repeated workflows and generate ready-to-use loop YAML. Also supports --from-commands mode to suggest loops from the available command/skill catalog without requiring message history, and --from-sequences mode to suggest loops from ll-logs sequences n-gram output.
...
**Mode Selection**: If `--from-sequences` is present in `{{args}}`, skip to [From-Sequences Mode](#from-sequences-mode). If `--from-commands` is present in `{{args}}`, skip to [From-Commands Mode](#from-commands-mode). Otherwise, proceed with message history analysis below.

## Process

### Step 1: Load Messages

1. If `{{args}}` is provided, read the JSONL file at that path
...
When `--from-sequences` is present in `{{args}}`, perform n-gram analysis on `ll-logs sequences` output instead of raw message history. This mode uses the telemetry primitive shipped in ENH-1919 as a real input source.
...
When `--from-commands` is present in `{{args}}`, perform catalog analysis instead of message history analysis. This mode works on fresh installations with zero Claude Code message history.
