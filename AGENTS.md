...
# little-loops (ll) - Codex Plugin

Development workflow toolkit for Codex with issue management, code quality commands, and automated processing.
...
- **Codex plugin** — via the marketplace, providing `/ll:*` commands, skills, agents, and hooks

`ll-init` sets both up and records which one is in play as `install_source` in the
project's `.ll/ll-config.json` (`pypi`, `local-editable`, `global-Codex`,
`project-Codex`).
...
hooks/          # Lifecycle hooks (subdivided below)
  adapters/     # Host translation layer; one subdir per host (Codex/, opencode/, codex/) that envelopes the host event into LLHookEvent
...
- **Planning & Implementation**: `create-sprint`, `review-sprint`, `review-epic`^, `manage-issue`^, `iterate-plan`, `confidence-check`^, `go-no-go`^, `advise`^, `create-eval-from-issues`^, `spike`^
...
- **Meta-Analysis**: `audit-Codex-config`^, `analyze-workflows`, `analyze-history`^, `improve-Codex-md`^
...
commands, or `.Codex/AGENTS.md` itself) are **meta-loops** with three shape
...
All host CLI invocations (`Codex`, `codex`, `opencode`, `pi`) must go
through `resolve_host()` in `scripts/little_loops/host_runner.py`. Never
add new `"Codex"` literals to automation code — call
