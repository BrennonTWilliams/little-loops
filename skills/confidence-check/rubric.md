...
be completed — issue references files or code that cannot be located,
criteria are unparseable, evaluation context is genuinely unavailable.
Do not use `"cannot_judge"` to mask a poor readiness score; a
low-confidence `"fail"` is more honest. `confidence` is the READINESS
SCORE (0-100) or `null` when `cannot_judge`. `findings_count` and
`severity_counts.p1` are `null` when `cannot_judge` (no audit
completed) and otherwise equal the number of `### Gaps to Address`
bullets; readers MUST treat `null` distinctly from `0` (do not
coalesce `row['findings_count'] or 0`). `abstention_reason` is
required when `verdict` is `"cannot_judge"` and MUST be one of the
four closed tags:
...
Adding a fifth tag is a contract-change PR that updates this rubric,
the schema `CHECK` constraint, the aggregation bucket, and the docs
together — it is a contract change, not a config change.
