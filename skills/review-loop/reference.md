...
**Exit code note**: `scripts/little_loops/cli/loop/runner.py:EXIT_CODES` — exit code 1 covers `max_steps`, `timeout`, and `cycle_detected`. Do not use exit code alone to distinguish SIM-3; always parse stdout. Exit code **2** is distinct and unambiguous: the loop reached a terminal declared `failure: true` (ENH-2814). So `2` means "ran to completion and reported failure", `1` means "never reached a terminal", and `0` means success.
