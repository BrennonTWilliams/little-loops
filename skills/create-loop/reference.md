...
| `score_stall_gate` | `evaluate.type: score_stall` + `evaluate.max_stall: 2` — yes while the aggregate rubric score keeps improving by > epsilon, no on N consecutive no-improvement rounds (reads `${context.run_dir}/.score_history`). Score-plateau companion to `diff_stall_gate` (ENH-2428). | `action`, `action_type`, routing (`on_yes`, `on_no`); optionally `on_error`, `evaluate.history_file`, `evaluate.epsilon` |
