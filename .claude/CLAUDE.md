...
- `ll-session` - Query the unified SQLite session store (`search --fts` / `recent --kind` / `recent --issue <ID>` / `backfill` / `path <session_id>` / `grep <pattern>` / `expand <id>` / `describe <id>` subcommands; default DB `.ll/history.db`)
