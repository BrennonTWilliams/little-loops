...
- `ll-doctor` - Check host CLI capability support and little-loops install surface (default checks: entry points, skills/commands discoverability, decisions store, history DB, loop validity; `--full` aggregates the `ll-verify-*`/`ll-check-links` family)
