#!/usr/bin/env bash
#
# Gemini CLI adapter for the SessionEnd hook intent.
#
...
# intent). Gemini fires SessionEnd natively but best-effort — the CLI
# won't wait on it, so failures here must stay silent.
#
