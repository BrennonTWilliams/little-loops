#!/usr/bin/env bash
#
# Gemini CLI adapter for the BeforeTool hook intent.
#
# Reads the host's stdin payload (set by Gemini) and pipes it through
# the host-agnostic Python dispatcher, which routes to
# ``little_loops.hooks.pre_tool_use.handle`` (learning-test discoverability
# gate, FEAT-1742). BeforeTool is blocking on Gemini — exit 2 denies the
# call, stderr is the reason shown to the agent.
#
