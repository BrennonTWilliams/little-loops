#!/usr/bin/env bash
#
# Gemini CLI adapter for the BeforeAgent hook intent.
#
# Reads the host's stdin payload (set by Gemini) and pipes it through
# the host-agnostic Python dispatcher, which routes to
# ``little_loops.hooks.user_prompt_submit.handle``. BeforeAgent is
# blocking on Gemini — exit 2 denies the turn, stderr is the reason shown
# to the agent.
#
...
echo "$INPUT" | "$PY" -m little_loops.hooks user_prompt_submit
exit $?
