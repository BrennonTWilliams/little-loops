#!/usr/bin/env bash
#
# Qwen Code adapter for the SubagentStop hook intent.
#
# Reads the host's stdin payload (set by Qwen Code) and pipes it through
# the host-agnostic Python dispatcher, which routes to
# ``little_loops.hooks.subagent_stop.handle``. Advisory; payload carries
# ``agent_id``/``agent_type`` for subagent runs.
#
