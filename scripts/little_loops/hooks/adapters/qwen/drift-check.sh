#!/usr/bin/env bash
#
# Qwen Code adapter for the SessionStart hook intent.
#
# Reads the host's stdin payload (set by Qwen Code) and pipes it through
# the host-agnostic Python dispatcher, which routes to
# ``little_loops.hooks.drift_check.handle``. Qwen fires SessionStart with a
# ``source`` payload field (startup|resume|clear|compact); the handler is
# advisory — exit code and stderr do not block session start.
#
