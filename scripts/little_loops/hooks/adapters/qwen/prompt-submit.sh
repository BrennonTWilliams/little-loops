#!/usr/bin/env bash
#
# Qwen Code adapter for the UserPromptSubmit hook intent.
#
# Reads the host's stdin payload (set by Qwen Code) and pipes it through
# the host-agnostic Python dispatcher, which routes to
# ``little_loops.hooks.user_prompt_submit.handle``. Exit 2 blocks the prompt;
# stderr is the reason shown to the user.
#
