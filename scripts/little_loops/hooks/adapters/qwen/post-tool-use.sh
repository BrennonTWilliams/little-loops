...
# ``little_loops.hooks.post_tool_use.handle``. Fire-and-forget per the
# FEAT-1489 hot-path decision; the handler persists a single ``tool_events``
# row per tool call when ``analytics.enabled`` is set.
#
