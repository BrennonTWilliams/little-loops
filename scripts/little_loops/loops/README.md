...
| `auto-refine-and-implement` | For each issue in priority order: refine to ready, then implement; skips issues that fail refinement. Optional `scope` param (sprint name or EPIC-NNN) limits to that set via SprintManager; omit for full backlog poll |
