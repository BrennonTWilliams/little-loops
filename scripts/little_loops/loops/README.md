...
> `ll-loop run loop-router 'goal="<what you want done>"'`.
...
| Loop | Description |
|---|---|
| `issue-discovery-triage` | Scan codebase for new issues, triage and create issue files. Uses `scope: [".issues/", "${context.run_dir}"]` (BUG-3087) so it no longer locks the repo root against unrelated concurrent loops. |
| `issue-refinement` | Progressively refine all active issues to ready-state by delegating each one to the `refine-to-ready-issue` sub-loop; uses `ll-issues next-action` to pick the highest-value action each cycle. Uses `scope: [".issues/", "${context.run_dir}"]` (BUG-3087) so it no longer locks the repo root against unrelated concurrent loops. |
| `issue-staleness-review` | Find old issues, review relevance, and close or reprioritize stale ones. Uses `scope: [".issues/", "${context.run_dir}"]` (BUG-3087) so it no longer locks the repo root against unrelated concurrent loops. |
| `refine-to-ready-issue` | Pick the next issue and run format → refine → wire → confidence-check until ready. Uses `scope: [".issues/", "${context.run_dir}"]` (BUG-3087) — it reads/writes issue files outside its own `run_dir`, so `run_dir`-only scoping (the `autodev` pattern) wouldn't cover it; this was the loop most likely to false-conflict with a concurrent `ll-auto` run. |
| `recursive-refine` | Refine one or more issues to readiness recursively; when size-review decomposes an issue into children, each child is enqueued and refined before the next sibling. Uses `scope: [".issues/", "${context.run_dir}"]` (BUG-3087) so it no longer locks the repo root against unrelated concurrent loops. |
| `auto-refine-and-implement` | For each issue in priority order: refine to ready, then implement; skips issues that fail refinement. Optional `scope` param (sprint name or EPIC-NNN) limits to that set via SprintManager; omit for full backlog poll. When `scope` is an EPIC-NNN id and `parallel.epic_branches.enabled` is true, ensures the `epic/<EPIC-ID>-<slug>` integration branch exists (create-without-switch) before delegating, and runs a post-implementation `test_cmd`/`lint_cmd` verify pass folded into `summary.json` (ENH-2601). Note: the `context.scope` variable above is unrelated to the FSM lock-scope key — the loop's top-level lock `scope:` is `[".issues/", "${context.run_dir}"]` (BUG-3087) so it no longer locks the repo root against unrelated concurrent loops. |
...
| `rn-refine` | Recursive-N refinement loop — treats an existing plan as the root of a decomposition tree and refines it recursively to adaptive depth: refine each node to rubric convergence, decide leaf-vs-decompose (ADaPT-style), split coarse nodes into child sub-plans enqueued depth-first, then roll the refined leaves bottom-up into a reassembled plan written back over the source. Per-node refinement + the decompose decision are delegated to `oracles/plan-node-refine`; depth is bounded by `max_depth`/`max_nodes`, and a wall-clock soft-deadline drain (ENH-2707) reserves `synth_reserve` seconds for bottom-up synthesis so a `timeout` kill still yields a written-back partial plan instead of a total loss. Terminal paths write machine-readable run-status artifacts to the run dir (ENH-2709): `summary.json` (node counters, `source_overwritten`, `terminated_by: success/timeout/aborted`) and `writeback.json` (`written`, `timestamp`, `source_path`, `backup_path`) | `plan_file` (path to `.md`); optional `max_depth` (default 3), `max_node_iters` (default 2), `max_nodes` (default 40), `synth_reserve` (default 3600) |
...
| `rn-implement` | Queue orchestrator for recursive plan-and-implement — delegates per-issue remediation to rn-remediate and decomposition to rn-decompose | `<issue-id>` (single or comma-separated list), or `--context epic=EPIC-NNN` to seed the queue with the EPIC's `parent:`-linked children (ENH-2660) |
...
| `spike-gate` | Opt-in wrapper that gates any implementation loop on `/ll:spike --check` — the internal-mechanism analogue of `proof-first-task`. When the issue carries `spike_needed` (set by `/ll:confidence-check` Phase 4.10) and not yet `spike_completed`, runs the read-only spike check; on failure runs `/ll:spike --auto` once and re-checks before blocking. Skips the gate (runs the impl loop directly) when no `issue_id` is given, `spike_needed` is unset, or `spike_completed` is already true. Delegates to a caller-specified impl loop (default `general-task`). |
...
| `flux-image-generator` | Generator-evaluator harness for raster text-to-image generation against a self-hosted FLUX endpoint — expands a description into a visual brief, then iterates prompt authoring → `POST ${IMAGE_BASE_URL}/generate` → base64-decode PNG → vision rubric score → prompt refinement until every criterion passes. The generator is a diffusion HTTP endpoint rather than an LLM emitting code; each iteration keeps a distinct `image-iter-N.png` with its seed recorded in `seeds.txt`. Requires `IMAGE_BASE_URL` (hard dependency); `VISION_*` degrades to a logged skip. `max_steps: 24` (wrapper) / `60` (delegated oracle); budget exhaustion routes to a distinct `partial_done` terminal instead of discarding the last generated image (BUG-2822). |
...
| `interactive-component-generator` | Fan-out generator-evaluator harness for self-contained INTERACTIVE HTML — profiles a natural-language brief or referenced data file, ideates many candidate interactive components (data-viz idioms + widgets), ranks them, builds the best 3–5 each in isolation via `oracles/generator-evaluator`, smoke-tests that each works as advertised, then selects the best 1–3 and composes them into one self-contained `index.html`. Configurable shadow / scope-css / scoped isolation; offline (no-CDN) by default. |
| `fleet-loop-improve` | Meta-loop that automates `docs/runbooks/FLEET_LOOP_REVIEW.md` — harvests every other project's run history with `ll-logs fleet-review`, picks the worst flagged built-in loop, diagnoses it inside the projects that ran it, has the `loop-specialist` agent write a diagnosis artifact and apply the smallest fix, gates it deterministically (`ll-loop validate` + built-in loop tests), and commits. One cycle per run with a durable ledger (`.loops/diagnostics/fleet-loop-improve-ledger.jsonl`): each run first re-measures earlier fixes against the fresh harvest (the measure-externally step), then records its own fix as `pending`. Run only from the little-loops source checkout. |
| `workflow-generator` | Meta-loop that lowers a prose brief into a reusable, validated FSM-loop YAML artifact via six sequential compiler-lowering passes (intent capture -> state-graph sketch -> evaluator attachment -> routing-table resolution -> artifact emission -> optional adversarial minimum-coupling shrink), each LLM pass paired with a non-LLM `exit_code` gate. HITL-gated promotion (`auto_promote`, default off) copies the validated artifact into the project loops dir with collision-safe naming. |
...
| `adversarial-redesign` | Generator-vs-critic figure refinement demo using AutoFigure — a generator produces an SVG from a text concept, a critic returns structured complaints, the loop regenerates addressing each complaint and exits on score-improvement stall or SVG-diff convergence. Every round is persisted for demo playback. Requires: `pip install -e ./AutoFigure && playwright install chromium` + `OPENROUTER_API_KEY`. Example: `ll-loop run adversarial-redesign 'concept="how a transformer attends"'` |
...
| `oracles/generator-evaluator-flux` | Diffusion-API oracle variant of generator-evaluator (`from: generator-evaluator`) — the LLM `generate` state authors/refines a text-to-image prompt, a new `synthesize` shell state POSTs it to `${IMAGE_BASE_URL}/generate` and base64-decodes the response into a per-iteration PNG with its seed (failing loudly via `IMAGE_FAIL` if the prompt is stale/unchanged since the prior iteration), and `evaluate` skips Playwright entirely (the response is already a raster, so it is copied straight to `screenshot.png`). All snapshot/score/stall machinery inherited unchanged. Overrides `max_steps: 60` (~7 scored cycles at 8 states/cycle) and adds an `on_max_steps: max_steps_summary` handler that names the best-scoring iteration and emits `SUMMARY_EMITTED` instead of silently discarding a generated-but-unscored image on budget exhaustion (BUG-2822). Used by flux-image-generator. |
