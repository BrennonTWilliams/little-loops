...
**Using Kimi Code CLI?** Run `ll-init --hosts kimi-code` — it installs the hook adapter and mirrors skills/agents into `.kimi-code/` automatically, then install the plugin for the `/ll:*` slash-command surface and lifecycle hooks (`/plugins install https://github.com/BrennonTWilliams/little-loops` in the kimi TUI — installs are per-user, not per-project). Set `LL_HOST_CLI=kimi-code` to drive `ll-auto`/`ll-loop` with kimi. See [docs/kimi/getting-started.md](docs/kimi/getting-started.md).

**Using Qwen Code?** Run `ll-init --hosts qwen` (installs managed hook entries into project `.qwen/settings.json` — fires in interactive *and* `qwen -p` headless sessions) — it also mirrors skills, `/ll:*` commands, and agents into `.qwen/` automatically. Optional native extension: `qwen extensions link .` (repo-root `qwen-extension.json`) or the marketplace path `qwen extensions install BrennonTWilliams/little-loops:ll`. Set `LL_HOST_CLI=qwen` to drive `ll-auto`/`ll-loop` with qwen — the second host after Claude Code with structured-output FSM evaluator verdicts. See [docs/qwen/getting-started.md](docs/qwen/getting-started.md).
...
- **~106 FSM loops** — recurring automation workflows (backlog triage, sprint building, eval harnesses)
...
| [Evaluation](docs/guides/EVALUATION_GUIDE.md) | You want to measure whether a feature, skill, or agent actually works |
