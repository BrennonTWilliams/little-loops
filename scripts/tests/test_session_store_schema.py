...
class TestSchemaV49HarnessRunModel:
    """v49 migration adds run-model columns to harness_events plus the
    harness_admissions audit table (ENH-3406)."""

    def test_harness_events_gains_run_model_columns(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = connect(db)
        try:
    ...
    def test_attempt_kind_check_rejects_invalid_value(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
    ...
    def test_harness_admissions_columns(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = connect(db)
        try:
            cols = {r[1] for r in conn.execute("PRAGMA table_info(harness_admissions)")}
        finally:
            conn.close()
        assert cols == {
            "id",
    ...
    def test_harness_admissions_reason_check_rejects_invalid_value(self, tmp_path: Path) -> None:
        ...
        try:
            with pytest.raises(sqlite3.IntegrityError):
                conn.execute(
                    "INSERT INTO harness_admissions (ts, attempt_id, superseded_id, reason) "
                    "VALUES ('t', 1, 2, 'bogus')"
                )
        finally:
            conn.close()

    def test_harness_admissions_superseded_id_not_null(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
    ...
    def test_indexes_exist(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            conn.close()
        assert {
            "idx_harness_cell_key",
            "idx_harness_cell_repetition",
            "idx_harness_admissions_attempt",
        } <= names

    def test_partial_unique_index_rejects_duplicate_repetition(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
            conn.execute(
                "INSERT INTO harness_events (ts, cell_key, repetition, attempt_kind) "
                "VALUES ('t', 'c1', 1, 'repetition')"
            )
            with pytest.raises(sqlite3.IntegrityError):
                conn.execute(
                    "INSERT INTO harness_events (ts, cell_key, repetition, attempt_kind) "
    ...
    def test_partial_unique_index_allows_infra_retry_duplicate(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
            ...
            conn.execute(
                "INSERT INTO harness_events (ts, cell_key, repetition, attempt_kind) "
                "VALUES ('t', 'c1', 1, 'infra_retry')"
            )
        finally:
            conn.close()

    def test_partial_unique_index_allows_null_cell_key_duplicates(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
    ...
    def test_v48_db_upgrades_gains_harness_admissions(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            conn.close()
        assert "harness_admissions" in names
        assert {"cell_key", "repetition", "attempt_kind", "continuations", "superseded_by"} <= cols

    def test_kind_registration(self) -> None:
        assert "harness_admission" in VALID_KINDS
        assert _KIND_TABLE["harness_admission"] == "harness_admissions"

    def test_excluded_from_rebuild(self) -> None:
        from little_loops.session_store import _REBUILD_TABLES

        assert "harness_admissions" not in _REBUILD_TABLES

    def test_not_kindless(self) -> None:
        from little_loops.session_store import _KINDLESS_TABLES

        assert "harness_admissions" not in _KINDLESS_TABLES
...
class TestSchemaV45AdvisorConsults:
    """v45 migration adds the advisor_consults table (FEAT-3300)."""

    def test_advisor_consults_columns(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = connect(db)
        try:
            cols = {r[1] for r in conn.execute("PRAGMA table_info(advisor_consults)")}
        finally:
            conn.close()
        assert cols == {
            "id",
    ...
    def test_advisor_consults_indexes_exist(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
            ...
            conn.close()
        assert {
            "idx_advisor_consults_session",
            "idx_advisor_consults_ts",
            "idx_advisor_consults_signal",
            "idx_advisor_consults_outcome",
        } <= names
    ...
    def test_v44_db_upgrade_gains_advisor_consults(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            conn.close()
        assert "advisor_consults" in names

    def test_kind_registration(self) -> None:
        assert "advisor_consult" in VALID_KINDS
        assert _KIND_TABLE["advisor_consult"] == "advisor_consults"

    def test_excluded_from_rebuild(self) -> None:
        from little_loops.session_store import _REBUILD_SEARCH_KINDS, _REBUILD_TABLES

        assert "advisor_consults" not in _REBUILD_TABLES
        assert "advisor_consult" not in _REBUILD_SEARCH_KINDS

    def test_not_kindless(self) -> None:
        from little_loops.session_store import _KINDLESS_TABLES

        assert "advisor_consults" not in _KINDLESS_TABLES


class TestSchemaV46ResearchTriageEvents:
    """v46 migration adds the research_triage_events table (ENH-2990)."""

    def test_research_triage_events_columns(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = connect(db)
        try:
            cols = {r[1] for r in conn.execute("PRAGMA table_info(research_triage_events)")}
        finally:
            conn.close()
        assert cols == {
            "id",
    ...
    def test_research_triage_events_indexes_exist(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
            ...
            conn.close()
        assert {
            "idx_research_triage_events_ts",
            "idx_research_triage_events_issue",
            "idx_research_triage_events_axis",
            "idx_research_triage_events_reason",
        } <= names

    def test_axis_check_constraint_rejects_bad_value(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
            with pytest.raises(sqlite3.IntegrityError):
                conn.execute(
                    ...
                    "(ts, issue_id, axis, covered, reason, evidence)"
                    " VALUES(?, ?, ?, ?, ?, ?)",
                    ("2026-09-02T00:00:00Z", "ENH-1", "not_an_axis", 0, None, ""),
                )
        finally:
            conn.close()

    def test_reason_check_constraint_rejects_bad_value(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
            with pytest.raises(sqlite3.IntegrityError):
                conn.execute(
                    ...
                    "(ts, issue_id, axis, covered, reason, evidence)"
                    " VALUES(?, ?, ?, ?, ?, ?)",
                    ("2026-09-02T00:00:00Z", "ENH-1", "locator", 0, "not_a_reason", ""),
                )
        finally:
            conn.close()
    ...
    def test_v45_db_upgrade_gains_research_triage_events(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            conn.close()
        assert "research_triage_events" in names

    def test_kind_registration(self) -> None:
        assert "research_triage" in VALID_KINDS
        assert _KIND_TABLE["research_triage"] == "research_triage_events"

    def test_excluded_from_rebuild(self) -> None:
        from little_loops.session_store import _REBUILD_SEARCH_KINDS, _REBUILD_TABLES

        assert "research_triage_events" not in _REBUILD_TABLES
        assert "research_triage" not in _REBUILD_SEARCH_KINDS

    def test_not_kindless(self) -> None:
        from little_loops.session_store import _KINDLESS_TABLES

        assert "research_triage_events" not in _KINDLESS_TABLES


class TestSchemaV47CredentialScopeEvents:
    """v47 migration adds the credential_scope_events table (ENH-3204)."""

    def test_credential_scope_events_columns(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = connect(db)
        try:
            cols = {r[1] for r in conn.execute("PRAGMA table_info(credential_scope_events)")}
        finally:
            conn.close()
        assert cols == {
            "id",
    ...
    def test_credential_scope_events_indexes_exist(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
    ...
    def test_v46_db_upgrade_gains_credential_scope_events(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            conn.close()
        assert "credential_scope_events" in names

    def test_kind_registration(self) -> None:
        assert "credential_scope" in VALID_KINDS
        assert _KIND_TABLE["credential_scope"] == "credential_scope_events"

    def test_excluded_from_rebuild(self) -> None:
        from little_loops.session_store import _REBUILD_SEARCH_KINDS, _REBUILD_TABLES

        assert "credential_scope_events" not in _REBUILD_TABLES
        assert "credential_scope" not in _REBUILD_SEARCH_KINDS

    def test_not_kindless(self) -> None:
        from little_loops.session_store import _KINDLESS_TABLES

        assert "credential_scope_events" not in _KINDLESS_TABLES


class TestSchemaV48LlVersionColumns:
    """v48 migration: ll_version column on orchestration_runs/loop_runs (FEAT-3404)."""

    def test_orchestration_runs_has_ll_version_column(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
            ...
            conn.close()
        assert "ll_version" in cols

    def test_loop_runs_has_ll_version_column(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
    ...
    def test_v47_db_upgrades_preserving_unstamped_rows(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            conn.execute(
                "INSERT INTO loop_runs(run_id, loop_name, ended_at) "
                "VALUES('old-loop-run', 'some-loop', '2026-06-01T00:00:00Z')"
            )
            conn.commit()
        finally:
            conn.close()
        ...
        try:
            ...
            orch_row = conn.execute(
                "SELECT * FROM orchestration_runs WHERE run_id='old-run'"
            ).fetchone()
            loop_row = conn.execute(
                "SELECT * FROM loop_runs WHERE run_id='old-loop-run'"
            ).fetchone()
        finally:
            conn.close()
        assert int(version[0]) == SCHEMA_VERSION
        assert orch_row is not None
        assert orch_row["ll_version"] is None
        assert loop_row is not None
        assert loop_row["ll_version"] is None

    def test_excluded_from_rebuild(self) -> None:
        from little_loops.session_store import _REBUILD_TABLES

        assert "orchestration_runs" not in _REBUILD_TABLES
        assert "loop_runs" not in _REBUILD_TABLES
...
class TestSchemaV50BaselineConditions:
    """v50 migration (ENH-3435): baseline condition columns on harness_events."""

    def test_harness_events_gains_baseline_condition_columns(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
    ...
    def test_v49_db_upgrades_gains_baseline_condition_columns(self, tmp_path: Path) -> None:
        ...
        try:
            conn.execute(
                "INSERT INTO harness_events(ts, runner, target) VALUES('2026-09-10T00:00:00Z', "
                "'skill', 'check-code')"
            )
            conn.commit()
        finally:
        ...
        try:
            ...
            cols = {r[1] for r in conn.execute("PRAGMA table_info(harness_events)")}
            row = conn.execute(
                "SELECT timeout_s, host_cli, subject_model, input_hash, conditions_fp "
                "FROM harness_events WHERE target='check-code'"
            ).fetchone()
        finally:
            conn.close()
        assert int(version[0]) == SCHEMA_VERSION
        assert {"timeout_s", "host_cli", "subject_model", "input_hash", "conditions_fp"} <= cols
        # Fix-forward only: pre-migration rows keep NULL on every new column.
        assert row == (None, None, None, None, None)

    def test_baseline_match_index_exists(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
            ...
            conn.close()
        assert "idx_harness_baseline" in indexes


class TestSchemaV51EfficiencyColumns:
    """v51 migration (ENH-3464): efficiency-vector columns on harness_events."""
    ...
    def test_harness_events_gains_efficiency_columns(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
    ...
    def test_v50_db_upgrades_gains_efficiency_columns(self, tmp_path: Path) -> None:
        ...
        try:
            conn.execute(
                "INSERT INTO harness_events(ts, runner, target) VALUES('2026-09-14T00:00:00Z', "
                "'skill', 'check-code')"
            )
            conn.commit()
        finally:
        ...
        try:
            ...
            row = conn.execute(
                ...
                "tool_calls FROM harness_events WHERE target='check-code'"
            ).fetchone()
        finally:
            conn.close()
        assert int(version[0]) == SCHEMA_VERSION
...
class TestSchemaV52ChannelsColumn:
    """v52 migration (ENH-3476): persist ll-harness's widened evidence
    (channels + per-side-effect pass/fail, D1) as a single nullable
    `channels_json` column on `harness_events`."""

    def test_harness_events_gains_channels_column(self, tmp_path: Path) -> None:
        db = tmp_path / "history.db"
        ensure_db(db)
        conn = sqlite3.connect(str(db))
        try:
    ...
    def test_v51_db_upgrades_gains_channels_column(self, tmp_path: Path) -> None:
        ...
        try:
            conn.execute(
                "INSERT INTO harness_events(ts, runner, target) VALUES('2026-09-15T00:00:00Z', "
                "'skill', 'check-code')"
            )
            conn.commit()
        finally:
        ...
        try:
            ...
            cols = {r[1] for r in conn.execute("PRAGMA table_info(harness_events)")}
            row = conn.execute(
                "SELECT channels_json FROM harness_events WHERE target='check-code'"
            ).fetchone()
        finally:
            conn.close()
        assert int(version[0]) == SCHEMA_VERSION
        assert "channels_json" in cols
        # Fix-forward only: pre-migration rows keep NULL on the new column.
        assert row == (None,)
