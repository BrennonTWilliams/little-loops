...
def _run_init(argv: list[str]) -> int:
    ...
    with (
        patch(_NO_INSTALL[0], return_value=(None, None, None)),
...
class TestInitHeadlessIntrospection:
    ...
    def test_yes_reinit_warns_on_divergent_declared_test_cmd(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        """Re-init warns (stderr) on declared/stored divergence but keeps the stored value."""
        project = tmp_path / "drift_reinit"
        project.mkdir()
    ...
    def test_plan_reflects_divergent_declared_test_cmd_against_existing_config(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        """--plan against an existing config surfaces the same drift warning as --yes."""
        import io
        from contextlib import redirect_stdout

        project = tmp_path / "drift_plan"
        project.mkdir()
        (project / "pyproject.toml").write_text(
            '[tool.pytest.ini_options]\ntestpaths = ["scripts/tests"]\n'
        )
        ...
        with redirect_stdout(buf):
            assert _run_init(["--plan", "--root", str(project)]) == 0
        plan = json.loads(buf.getvalue())  # --plan stdout stays pure JSON
        assert "provenance" in plan
