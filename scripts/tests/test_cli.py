...
class TestMainLoopAdditionalCoverage:
    ...
    def test_argv_preprocessing_inserts_run(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "little_loops.fsm.validation.load_and_validate", return_value=(mock_fsm, [])
            ):
...
class TestMainLogsIntegration:
    ...
    def test_scan_failures_returns_0(self) -> None:
        """ll-logs scan-failures --all returns 0 on success (empty corpus)."""
        from little_loops.cli import main_logs
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch.object(sys, "argv", ["ll-logs", "scan-failures", "--all"]),
                patch("pathlib.Path.home", return_value=home),
            ):
                result = main_logs()
