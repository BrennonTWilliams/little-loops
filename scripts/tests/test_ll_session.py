...
class TestBackfillSinceFlag:
    ...
    def test_backfill_host_codex_discovers_via_detect_sessions(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        """ENH-3422 D4: codex has no project folder to glob, so a full
        backfill discovers via detect_sessions() and passes handles= to
        backfill() instead of jsonl_files=; the old ENH-3420 notice is gone."""
        db = tmp_path / "session.db"
        fake_handles = ["fake-handle"]
        with patch("sys.argv", ["ll-session", "--db", str(db), "backfill", "--host", "codex"]):
            with patch(
                "little_loops.cli.session.detect_sessions", return_value=fake_handles
            ) as mock_detect:
                with patch("little_loops.cli.session.backfill") as mock_backfill:
                    mock_backfill.return_value = {
                        "issues": 0,
                    ...
                    result = main_session()
        assert result == 0
        assert mock_detect.call_args.args[1] == "codex"
        assert mock_backfill.call_args.kwargs["handles"] == fake_handles
        assert mock_backfill.call_args.kwargs.get("jsonl_files") is None
