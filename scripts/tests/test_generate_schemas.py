...
class TestSchemaDefinitions:
    ...
    def test_all_58_event_types_defined(self) -> None:
        """All 61 LLEvent types must be defined (doc audit 2026-08-31: added messages_append,
        ...
        FEAT-1794 added human_approval_requested/human_approval_resolved)."""
        assert len(SCHEMA_DEFINITIONS) == 61

    def test_expected_event_types_present(self) -> None:
        """Each of the 61 known event types must appear in catalog."""
        expected = {
            "loop_start",
            "state_enter",
            "route",
...
class TestGenerateSchemas:
    ...
    def test_creates_58_files(self, tmp_path: Path) -> None:
        """Generates exactly 61 schema files (FEAT-1794 added 2 new event types)."""
    ...
    def test_creates_output_dir_if_missing(self, tmp_path: Path) -> None:
        ...
        assert len(list(output_dir.glob("*.json"))) == 61
...
class TestGenerateSchemasCLI:
    ...
    def test_cli_creates_files(self, tmp_path: Path) -> None:
        """CLI generates 61 schema files in the specified output directory."""
        from little_loops.cli.schemas import main_generate_schemas

        with patch("sys.argv", ["ll-generate-schemas", "--output", str(tmp_path)]):
            main_generate_schemas()
        assert len(list(tmp_path.glob("*.json"))) == 61
