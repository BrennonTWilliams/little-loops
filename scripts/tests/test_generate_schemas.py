...
class TestSchemaDefinitions:
    ...
    def test_all_19_event_types_defined(self) -> None:
        """All 19 LLEvent types must be defined."""
        assert len(SCHEMA_DEFINITIONS) == 19

    def test_expected_event_types_present(self) -> None:
        """Each of the 19 known event types must appear in catalog."""
        expected = {
            "loop_start",
            "state_enter",
            "route",
...
class TestGenerateSchemas:
    ...
    def test_creates_19_files(self, tmp_path: Path) -> None:
        """Generates exactly 19 schema files."""
...
class TestGenerateSchemasCLI:
    ...
    def test_cli_creates_files(self, tmp_path: Path) -> None:
        """CLI generates 19 schema files in the specified output directory."""
        from little_loops.cli.schemas import main_generate_schemas
