...
Asserts that the /ll:debug-loop-run section of docs/reference/COMMANDS.md
...
class TestDebugLoopRunCommandsWiring:
    """docs/reference/COMMANDS.md must document the Execution Summary output block."""

    def _debug_loop_run_section(self) -> str:
        content = COMMANDS_REF.read_text()
        start = content.index("### `/ll:debug-loop-run`")
    ...
    def test_execution_summary_present(self) -> None:
        section = self._debug_loop_run_section()
        assert "Execution Summary" in section, (
            "docs/reference/COMMANDS.md /ll:debug-loop-run entry must document "
    ...
    def test_loop_goal_field_present(self) -> None:
        ...
        assert "**Loop goal**" in section, (
            "docs/reference/COMMANDS.md /ll:debug-loop-run entry must include "
...
class TestAuditLoopRunCommandsWiring:
    """docs/reference/COMMANDS.md must document the Goal-vs-Outcome Scorecard output block."""

    def _audit_loop_run_section(self) -> str:
        content = COMMANDS_REF.read_text()
        start = content.index("### `/ll:audit-loop-run`")
        # Use backtick pattern to skip ### headings inside fenced code blocks
        next_heading = content.find("\n### `", start + 1)
        return content[start:next_heading]

    def test_scorecard_present(self) -> None:
        section = self._audit_loop_run_section()
        assert "Goal-vs-Outcome Scorecard" in section, (
            "docs/reference/COMMANDS.md /ll:audit-loop-run entry must document "
    ...
    def test_verdict_field_present(self) -> None:
        ...
        assert "**Verdict**" in section, (
            "docs/reference/COMMANDS.md /ll:audit-loop-run entry must include "
...
class TestDebugLoopRunHeadlessFlagsWiring:
    """docs/reference/COMMANDS.md must document --skip-issue-creation and --auto for /ll:debug-loop-run."""
