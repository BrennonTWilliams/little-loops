...
# =============================================================================
# Policy Router Wizard YAML Structural Tests (ENH-2299)
# =============================================================================

POLICY_ROUTER_WIZARD_YAML = """
name: policy-review
initial: score
max_steps: 10
...
context:
  subject: "artifact.md"
  rubric_dimensions: "quality|feasibility|security"
  threshold_high: "85"
  threshold_medium: "65"
  policy_rules: |
    security:<65 -> escalate
    quality:>=85 & feasibility:>=85 -> done
    * -> repair
...
class TestPolicyRouterWizardYAML:
    """Structural validation for policy-router YAML generated by /ll:create-loop wizard (ENH-2299)."""

    @pytest.fixture
    def loop(self) -> dict:
        return yaml.safe_load(POLICY_ROUTER_WIZARD_YAML)

    def test_import_contains_both_fragment_libs(self, loop: dict) -> None:
        """Policy-router loops must import both rubric-router and policy-router."""
        imports = loop.get("import", [])
        assert "lib/rubric-router.yaml" in imports, "import must include lib/rubric-router.yaml"
        assert "lib/policy-router.yaml" in imports, "import must include lib/policy-router.yaml"

    def test_import_order_rubric_before_policy(self, loop: dict) -> None:
        """lib/rubric-router.yaml must appear before lib/policy-router.yaml."""
        imports = loop.get("import", [])
        rubric_idx = imports.index("lib/rubric-router.yaml")
        policy_idx = imports.index("lib/policy-router.yaml")
        assert rubric_idx < policy_idx, "rubric-router must precede policy-router in import list"

    def test_context_policy_rules_present(self, loop: dict) -> None:
        """context.policy_rules must be present for the decision table."""
        context = loop.get("context", {})
        assert "policy_rules" in context, "context must include policy_rules"

    def test_policy_dispatch_route_has_catch_all(self, loop: dict) -> None:
        """policy_dispatch.route must contain a _: catch-all entry."""
        route = loop["states"]["policy_dispatch"]["route"]
        assert "_" in route, "policy_dispatch.route must contain _: catch-all"

    def test_policy_dispatch_route_has_error_catch_all(self, loop: dict) -> None:
        """policy_dispatch.route must contain a _error: catch-all entry."""
        route = loop["states"]["policy_dispatch"]["route"]
        assert "_error" in route, "policy_dispatch.route must contain _error: catch-all"

    def test_required_states_exist(self, loop: dict) -> None:
        """score, parse_scores, policy_dispatch, and done must all be present."""
        states = set(loop["states"].keys())
        required = {"score", "parse_scores", "policy_dispatch", "done"}
        assert required.issubset(states), f"Missing states: {required - states}"

    def test_initial_state_is_score(self, loop: dict) -> None:
        """Policy-router loops must start at the score state."""
        assert loop.get("initial") == "score"

    def test_done_is_terminal(self, loop: dict) -> None:
        """done must be a terminal state."""
        assert loop["states"]["done"].get("terminal") is True

    def test_threshold_fields_present_for_llm_rubric(self, loop: dict) -> None:
        """LLM-rubric scorer path must include threshold_high and threshold_medium."""
        context = loop.get("context", {})
        assert "threshold_high" in context, "context must include threshold_high for LLM rubric"
        assert "threshold_medium" in context, "context must include threshold_medium for LLM rubric"
