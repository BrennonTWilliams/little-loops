...
class TestWorkerPoolSetupWorktree:
    ...
    def test_setup_worktree_with_base_branch_appends_commit_ish(
        self, mock_logger: MagicMock
    ) -> None:
        """Worktree setup with base_branch appends commit-ish to git worktree add args."""
        import tempfile
        from pathlib import Path
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            worktree_path = worktree_base / "feature-feat-001"
            branch_name = "feature/feat-001-thing"

            with patch("subprocess.run", side_effect=mock_run):
                with patch("shutil.copy2"):
                    pool._setup_worktree(worktree_path, branch_name, base_branch="main")
