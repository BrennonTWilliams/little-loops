...
class TestDisplayProgressEvents:
    ...
    def test_tall_fsm_falls_back_to_neighborhood(self, capsys: pytest.CaptureFixture[str]) -> None:
        """On a short terminal (80x12) a tall layered FSM falls back to neighborhood.

        The fallback ladder is full → neighborhood → single.  At 80×12 the full
        diagram does not fit (20 states) but the 3-node neighborhood box does, so
        the display stops there rather than degrading all the way to single-line.
        """
        import os
        import shutil
        ...
        out = capsys.readouterr().out
        # Neighborhood fallback: immediate neighbours s4/s5/s6 shown; far states must NOT appear.
        assert "s4" in out
        assert "s5" in out
        assert "s6" in out
        assert "fsm:" not in out
...
class TestWindowedDiagramStubTerminators:
    ...
    def test_top_stub_back_edge_gets_dome_terminator(self) -> None:
        ...
        # First row after the "▲ N layers above" banner.
    ...
    def test_full_span_pipe_still_blanked_no_terminator(self) -> None:
        from little_loops.cli.loop.layout import _render_windowed_diagram

        # Back-edge from s15 (layer 15, cropped below) UP to s2 (layer 2,
        # cropped above) with active=s8 — both endpoints outside the window,
        # so the pipe is Case 3 (full pass-through). It must remain blanked,
        # not terminated, because it does not terminate at the cut: it
        # passes through.
        fsm = _long_back_edge_fsm(20, back_src="s15", back_dst="s2")
        out = _render_windowed_diagram(fsm, "s8", budget=14, mode="full")
        lines = _strip(out)
