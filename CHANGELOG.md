...
- **FEAT-2186**: Hook adapter — `hooks/adapters/gemini/` translating gemini-cli events to `LLHookEvent`
- **FEAT-2190**: `GEMINI.md` project context file generated by `ll-init --hosts gemini`
- **FEAT-2261**: omp hook adapter — `hooks/adapters/omp/`
- **FEAT-2379**: Fleet loop-review runbook + `ll-logs fleet-review`
- **FEAT-3182**: Deterministic verification-evidence bundle from verify-loop runs
...
- **FEAT-3372**: `ll-init` auto-installs the `ll@little-loops` plugin from the marketplace
- **FEAT-3384**: EventBus HITL adapter
- **FEAT-3386**: `fleet-loop-improve` built-in meta-loop — automates the fleet loop-review runbook
...
- **BUG-3368**: Epic verify gate produces environment-only false failures (third recurrence)
...
- **BUG-3375**: autodev cascades Errno 2 into clean terminal exit and silently abandons the queue when a worktree vanishes
- **BUG-3380**: AGENTS.md/CLAUDE.md install line hardcoded to editable install for all install_source
- **BUG-3387**: Inbound interaction POST body overwrites `_emit()` envelope (event name spoofing)
- **BUG-3390**: autodev dead `outcome_gate_waived` path, stale guard-2 capture provenance, and unwired go-no-go escalation
- **BUG-3392**: `ll-issues show`: derived 'Superseded by' row hidden when superseding issue is deferred/done/cancelled
- fix: fall back to `stderr_preview` for the printed loop failure reason (d2d955bef)
- fix(issues): stop bullet-tier option regex matching prose that cites an option (5d7f822b0)
...
- **ENH-2191**: HOST_COMPATIBILITY.md Gemini column — cells populated as children landed
...
- **ENH-3000**: References into untracked-by-design directories always report `stale`
- **ENH-3298**: Baseline verdicts gated on a paired sign test, not raw delta sign
- **ENH-3365**: FSM shell actions invoke the executor's own Python interpreter, not bare `python3`
- **ENH-3366**: `delegate`(autodev) verdict no longer laundered through `recheck_set`
- **ENH-3374**: Worktree liveness marker made resilient so active worktrees cannot be classified orphaned
...
- **ENH-3383**: Batch general-task plan steps into context-bounded passes; add stepwise-task thin entry
- **ENH-3389**: `ll-init` runs `ll-adapt` for adapter hosts after wiring hooks, so skills/commands are mirrored at init
...
- **ENH-3393**: history.db backfill host coverage for gemini and omp (oh-my-pi)
- **ENH-3394**: history.db backfill host coverage for omp (oh-my-pi)

### Maintenance

- fix: resolve lint and format issues across codebase (e2e8c2ee6)
- style: apply ruff formatting to generate_schemas.py and subprocess_utils.py (a9a8391a8)
- test(tests): quarantine test_tsc_noemit_passes under epic verify gate (64fed3c29)
- chore(learning-tests): update threading assertions and results (0449754b7)

[1.161.0]: https://github.com/BrennonTWilliams/little-loops/compare/v1.160.0...v1.161.0
